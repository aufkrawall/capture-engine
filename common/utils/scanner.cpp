...
namespace Scanner {
    ...
    uintptr_t ScanForStringRef(HMODULE module, const char* string) {
        ...
        for (size_t i = 0; string[i]; i++) stringBytes.push_back(string[i]);
        ...
        // We will bruteforce scan for any 4-byte displacement that resolves to stringAddr
        ...
        for (; p < end; p++) {
            ...
            // Let's just check the displacement logic for typical instruction sizes (7 bytes)
    ...
    std::vector<uintptr_t> ScanForPointer(HMODULE module, uintptr_t target) {
        ...
        if (!module || !target) return results;
        ...
        for (uintptr_t* p = pStart; p <= pEnd; p++) {
            // Check alignment risk? standard scan is byte-by-byte but pointers usually aligned.
            // Let's do byte scan for safety but cast to uintptr_t
            // Actually, scanning byte-by-byte for a 64-bit value is safer to avoid alignment faults 
            // (though x64 handles unaligned loads fine mostly, but valid ptrs are aligned).
            // Let's stick to 8-byte alignment for speed and probability.
            if (*p == target) {
