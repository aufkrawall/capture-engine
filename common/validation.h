...
#define CE_LOG_DEBUG(module, fmt, ...) \
    do { if (g_DebugLoggingEnabled) CE_LogImpl("D", module, 0, fmt, ##__VA_ARGS__); } while(0)

#define CE_LOG_INFO(module, fmt, ...) \
    CE_LogImpl("I", module, 0, fmt, ##__VA_ARGS__)

#define CE_LOG_WARN(module, fmt, ...) \
    CE_LogImpl("W", module, 0, fmt, ##__VA_ARGS__)

#define CE_LOG_ERROR(module, fmt, ...) \
    CE_LogImpl("E", module, __LINE__, fmt, ##__VA_ARGS__)
...
#define CE_ENSURE_NOT_NULL(ptr) \
    CE_VALIDATE((ptr) != nullptr, #ptr " is null")

// CE_ENSURE_NOT_NULL_RET - Validates pointer, returns custom value
#define CE_ENSURE_NOT_NULL_RET(ptr, ret_val) \
    CE_VALIDATE_RET((ptr) != nullptr, #ptr " is null", ret_val)
...
// CE_DEBUG_ONLY - Code block that only executes in debug builds or when debug logging enabled
#ifdef _DEBUG
    #define CE_DEBUG_ONLY(code) do { code; } while(0)
#else
    #define CE_DEBUG_ONLY(code) do { if (g_DebugLoggingEnabled) { code; } } while(0)
#endif
...
inline Result Ok() { return Result(ErrorCode::Success); }
inline Result Err(ErrorCode c, const char* msg = nullptr) { return Result(c, msg); }
