...
// Provides automatic cleanup for Windows handles, COM objects, Vulkan resources, etc.
// All wrappers are move-only to prevent double-free issues.
...
public:
    ...
    MappingGuard(MappingGuard&& o) noexcept : ptr_(o.ptr_) { o.ptr_ = nullptr; }
...
public:
    ...
    ~VirtualAllocGuard() { reset(); }
    
    VirtualAllocGuard(VirtualAllocGuard&& o) noexcept 
        : process_(o.process_), ptr_(o.ptr_) {
        o.process_ = nullptr;
        o.ptr_ = nullptr;
    }
...
public:
    ...
    // Move-only
    ComGuard(ComGuard&& o) noexcept : ptr_(o.ptr_) { o.ptr_ = nullptr; }
...
public:
    ...
    VkHandleGuard(T handle, DestroyFunc destroy) 
        : handle_(handle), destroy_(std::move(destroy)), valid_(true) {}
    
    ~VkHandleGuard() { reset(); }
    
    VkHandleGuard(VkHandleGuard&& o) noexcept 
        : handle_(o.handle_), destroy_(std::move(o.destroy_)), valid_(o.valid_) {
        o.valid_ = false;
    }
...
// ScopedThreadPriority - temporarily changes thread priority, restores on destruction
class ScopedThreadPriority {
    HANDLE thread_;
    int oldPriority_;
    bool valid_ = false;
public:
    explicit ScopedThreadPriority(int newPriority, HANDLE thread = GetCurrentThread()) 
        : thread_(thread) {
...
#define CE_SCOPE_EXIT(code) \
    auto CE_CONCAT(_scope_guard_, __LINE__) = ::ce::make_scope_guard([&]() { code; })
