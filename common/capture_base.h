...
public:
  ...
  // Frame statistics
  std::atomic<uint32_t> droppedFrames{
      0}; // Count of frames dropped due to ring full
  ...
  bool HasPendingSpace() const {
    ...
    return (wIdx - pendingReadIdx.load(std::memory_order_relaxed)) <
           CAPTURE_RING_SIZE;
  }

  // Enqueue a frame for async capture
  bool EnqueueFrame(int64_t timestampQPC, uint64_t gpuFenceValue,
                    uint32_t backBufferIndex, void *apiData,
                    uint64_t syncObject = 0) {
    uint32_t wIdx = pendingWriteIdx.load(std::memory_order_acquire);
    if (wIdx - pendingReadIdx.load(std::memory_order_relaxed) >=
        CAPTURE_RING_SIZE) {
  ...
  template <typename ThreadFunc> void StartCaptureThread(ThreadFunc func) {
