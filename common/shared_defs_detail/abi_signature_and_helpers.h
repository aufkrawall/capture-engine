...
constexpr uint32_t ComputeSharedMemoryAbiSignature() {
    ...
    hash = MixSharedMemoryAbiValue(hash, offsetof(SharedMemoryLayout::SharedSystemMetrics, cpuTemperatureC));
    hash = MixSharedMemoryAbiValue(hash, offsetof(SharedMemoryLayout::SharedSystemMetrics, gpuTemperatureC));
    ...
    hash = MixSharedMemoryAbiValue(hash, sizeof(SharedMemoryLayout::SharedBenchmarkState));
    hash = MixSharedMemoryAbiValue(hash, offsetof(SharedMemoryLayout, benchmark));
    return hash;
}
#if defined(__clang__)
