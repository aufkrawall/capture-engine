...
struct QueuedFrame {
  ...
  ID3D11Texture2D *texture =
      nullptr; // AddRef'd by producer, Release'd by consumer
  ...
  uint32_t height = 0;
  bool isInjectMode =
      false;          // true = use inject fields, false = use framegrab fields
  ...
  bool Pop(QueuedFrame &frame, int timeoutMs) {
    ...
    if (buffer.empty()) {
      ...
      auto result = cv.wait_for(lock, std::chrono::milliseconds(timeoutMs),
                                [this] { return !buffer.empty() || !running; });
...
private:
  static constexpr std::chrono::seconds GRACE_PERIOD{
