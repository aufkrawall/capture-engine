...
ProcessMode ParseProcessMode(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++) {
        if (strncmp(argv[i], "--mode=", 7) == 0) {
            ...
            if (strcmp(mode, "inject") == 0) return ProcessMode::Inject;
            if (strcmp(mode, "media") == 0) return ProcessMode::Media;
            if (strcmp(mode, "limiter") == 0) return ProcessMode::Limiter;
            if (strcmp(mode, "logger") == 0) return ProcessMode::Logger;
            if (strcmp(mode, "sensors") == 0) return ProcessMode::Sensors;
        }
    }
...
{
    if (!lpCmdLine || lpCmdLine[0] == '\0') return ProcessMode::Controller;

    // Simple search for --mode=
    const char* modeStr = strstr(lpCmdLine, "--mode=");
    if (!modeStr) return ProcessMode::Controller;

    modeStr += 7;  // Skip "--mode="
    if (strncmp(modeStr, "inject", 6) == 0) return ProcessMode::Inject;
    if (strncmp(modeStr, "media", 5) == 0) return ProcessMode::Media;
    if (strncmp(modeStr, "limiter", 7) == 0) return ProcessMode::Limiter;
    if (strncmp(modeStr, "logger", 6) == 0) return ProcessMode::Logger;
    if (strncmp(modeStr, "sensors", 7) == 0) return ProcessMode::Sensors;
...
ProcessIPCServer::ProcessIPCServer(ProcessMode mode)
    : mode(mode), hPipe(INVALID_HANDLE_VALUE), connected(false), lastSequence(0)
{
}
...
bool ProcessIPCServer::PollCommand(ProcessCommand& outCmd, char* outPayload, size_t payloadSize)
{
    ...
    if (outPayload && payloadSize > 0) {
        ...
        if (copyLen > sizeof(msg.payload)) copyLen = sizeof(msg.payload);
