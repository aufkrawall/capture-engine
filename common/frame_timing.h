...
public:
  ...
  int64_t GetNextPtsMs(int64_t captureTimestampUs = -1) {
    ...
    if (captureTimestampUs >= 0) {
      ...
      int64_t captureTicksRelative = (captureTimestampUs * qpcFrequency) / 1000000;
  ...
  int64_t GetNextPtsInTimebase(int timebaseNum, int timebaseDen, int64_t captureTimestampUs = -1) {
    if (!initialized || timebaseDen == 0) return 0;
    
    int64_t ptsMs = GetNextPtsMs(captureTimestampUs);
    // Convert ms to timebase: pts = (ptsMs / 1000) * (timebaseDen / timebaseNum)
    // Rearranged to avoid overflow: pts = ptsMs * timebaseDen / (1000 * timebaseNum)
  ...
  int64_t GetElapsedMs() const {
    if (!initialized || startTimeTicks == 0) return 0;
    LARGE_INTEGER now;
    QueryPerformanceCounter(&now);
    int64_t elapsed = now.QuadPart - startTimeTicks;
  ...
  bool ShouldCaptureFrame() const {
    if (!initialized || targetIntervalTicks == 0) return true;
