...
// All checks are gated by _DEBUG or g_DebugLoggingEnabled to avoid production overhead.
...
#ifdef _DEBUG
    if (!g_DebugLoggingEnabled) return;
...
#ifdef _DEBUG
    if (!g_DebugLoggingEnabled || !shm) return;
