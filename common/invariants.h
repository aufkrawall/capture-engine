...
inline void
CheckRingBufferInvariants([[maybe_unused]] const FrameRingBuffer &rb) {
#ifdef _DEBUG
  ...
  CE_ASSERT_MSG((write % FRAME_RING_SIZE) < (uint32_t)FRAME_RING_SIZE,
                "write index out of bounds");
  CE_ASSERT_MSG((read % FRAME_RING_SIZE) < (uint32_t)FRAME_RING_SIZE,
...
inline void
CheckSharedMemoryInvariants([[maybe_unused]] const SharedMemoryLayout *shm) {
#ifdef _DEBUG
  if (!g_DebugLoggingEnabled || !shm)
    return;

  // Magic number check
  CE_ASSERT_MSG(shm->magic == SHARED_MEMORY_MAGIC,
                "shared memory magic mismatch");

  // Version check
  CE_ASSERT_MSG(shm->version >= SHARED_MEMORY_MIN_VERSION,
                "shared memory version too old");
  CE_ASSERT_MSG(shm->version <= SHARED_MEMORY_VERSION,
                "shared memory version too new");
...
inline void
CheckCaptureStateInvariants([[maybe_unused]] const CaptureState &state) {
#ifdef _DEBUG
  if (!g_DebugLoggingEnabled)
    return;
  ...
  if (state.isRecording.load(std::memory_order_relaxed)) {
    CE_ASSERT_MSG(state.recordingStartTime.load(std::memory_order_relaxed) > 0,
                  "recording but start time not set");
  }
#endif
}

// Periodic health check - call every N frames in main loop
inline void PeriodicHealthCheck([[maybe_unused]] const SharedMemoryLayout *shm,
