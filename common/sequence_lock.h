...
template <typename T> class SequenceLock {
public:
  static_assert(sizeof(T) <= 1024,
                "SequenceLock data should fit in cache line for efficiency");
   ...
   */
  template <typename CopyFunc> bool Read(T &out, CopyFunc copyFunc) const {
   ...
   * src)
   */
  template <typename CopyFunc> void Write(const T &source, CopyFunc copyFunc) {
 ...
 */
template <typename T> class VersionedConfig {
public:
  /**
   ...
   */
  bool Read(T &out) const { return lock_.Read(out); }

  /**
   * Write new config
   */
  void Write(const T &source) { lock_.Write(source); }

  /**
   * Get current version
   */
  uint32_t GetVersion() const { return lock_.GetSequence(); }
...
inline bool SequenceLock_ReadRetry(const SequenceLockHeader *header,
                                   uint32_t startSeq) {
