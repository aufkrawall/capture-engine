...
void LoadConfig(const std::string &path, AppConfig &config,
  ...
  if (!currentProcessName.empty()) {
    ...
    std::transform(procNameLower.begin(), procNameLower.end(),
                   procNameLower.begin(), ::tolower);
    ...
    for (int i = 1; i <= 8; ++i) {
      ...
      GetPrivateProfileStringA(appSec, "Process", "", buffer, 4096,
                               path.c_str());
      std::string configProc = Trim(buffer);
      if (configProc.empty()) {
        GetPrivateProfileStringA(appSec, "ProcessName", "", buffer, 4096,
                                 path.c_str());
      ...
      if (!configProc.empty()) {
        ...
        // target
        if (std::find(config.gameWhitelist.begin(), config.gameWhitelist.end(),
                      configProc) == config.gameWhitelist.end()) {
          config.gameWhitelist.push_back(configProc);
        }

        std::string configProcLower = configProc;
        std::transform(configProcLower.begin(), configProcLower.end(),
                       configProcLower.begin(), ::tolower);
  ...
  auto GetStr = [&](const char *section, const char *key, const char *def) {
    if (!overrideSection.empty()) {
      ...
      GetPrivateProfileStringA(overrideSection.c_str(), explicitKey.c_str(), "",
                               buffer, 4096, path.c_str());
      std::string val = Trim(buffer);
      if (!val.empty())
        return val;

      // 2. Try Override Simplified: [App.N] Key=Value
      GetPrivateProfileStringA(overrideSection.c_str(), key, "", buffer, 4096,
                               path.c_str());
      val = Trim(buffer);
      if (!val.empty())
        return val;
  ...
  config.graphics.anisotropicFiltering =
      GetStr("Graphics", "anisotropic_filtering", "default");
  ...
  config.graphics.cpuPrerenderLimit =
      GetFloat("Graphics", "cpu_prerender_limit", -1.0f);
  config.graphics.backbufferCount = GetInt("Graphics", "backbuffer_count", 0);
  config.graphics.sgssaa = GetBool("Graphics", "sgssaa", false);
  config.graphics.disableAutoMipBias =
      GetBool("Graphics", "disable_auto_mip_bias", false);
  ...
  config.graphics.dlssPresetDLAA =
      GetStr("Graphics", "dlss_preset_dlaa", "default");
  config.graphics.dlssPresetQuality =
      GetStr("Graphics", "dlss_preset_quality", "default");
  config.graphics.dlssPresetBalanced =
      GetStr("Graphics", "dlss_preset_balanced", "default");
  config.graphics.dlssPresetPerformance =
      GetStr("Graphics", "dlss_preset_performance", "default");
  config.graphics.dlssPresetUltraPerformance =
      GetStr("Graphics", "dlss_preset_ultra_performance", "default");
  config.graphics.dlssPresetUltraQuality =
      GetStr("Graphics", "dlss_preset_ultra_quality", "default");
  config.graphics.dlssSRPreset =
      GetStr("Graphics", "dlss_sr_preset", "default");

  // RR Presets
  config.graphics.dlssRRPresetDLAA =
      GetStr("Graphics", "dlss_rr_preset_dlaa", "default");
  config.graphics.dlssRRPresetQuality =
      GetStr("Graphics", "dlss_rr_preset_quality", "default");
  config.graphics.dlssRRPresetBalanced =
      GetStr("Graphics", "dlss_rr_preset_balanced", "default");
  config.graphics.dlssRRPresetPerformance =
      GetStr("Graphics", "dlss_rr_preset_performance", "default");
  config.graphics.dlssRRPresetUltraPerformance =
      GetStr("Graphics", "dlss_rr_preset_ultra_performance", "default");
  config.graphics.dlssRRPresetUltraQuality =
      GetStr("Graphics", "dlss_rr_preset_ultra_quality", "default");
  config.graphics.dlssRRPreset =
      GetStr("Graphics", "dlss_rr_preset", "default");
  config.graphics.dlssSharpening =
      GetStr("Graphics", "dlss_sharpening", "default");
  config.graphics.nvidiaSmoothMotionCompat =
      GetStr("Graphics", "nvidia_smooth_motion_compat", "auto");
  ...
  config.graphics.streamlineDllPath =
      GetStr("Graphics", "streamline_dll_path", "");

  config.graphics.dlssDebugOverlay =
      GetStr("Graphics", "dlss_debug_overlay", "default");

  // Fill parsed versions for efficiency
  config.graphics.parsed.presetDLAA =
      ParseDlssPreset(config.graphics.dlssPresetDLAA);
  config.graphics.parsed.presetQuality =
      ParseDlssPreset(config.graphics.dlssPresetQuality);
  config.graphics.parsed.presetBalanced =
      ParseDlssPreset(config.graphics.dlssPresetBalanced);
  config.graphics.parsed.presetPerformance =
      ParseDlssPreset(config.graphics.dlssPresetPerformance);
  config.graphics.parsed.presetUltraPerformance =
      ParseDlssPreset(config.graphics.dlssPresetUltraPerformance);
  config.graphics.parsed.presetUltraQuality =
      ParseDlssPreset(config.graphics.dlssPresetUltraQuality);
  config.graphics.parsed.srPreset =
      ParseDlssPreset(config.graphics.dlssSRPreset);

  config.graphics.parsed.rrPresetDLAA =
      ParseDlssRRPreset(config.graphics.dlssRRPresetDLAA);
  config.graphics.parsed.rrPresetQuality =
      ParseDlssRRPreset(config.graphics.dlssRRPresetQuality);
  config.graphics.parsed.rrPresetBalanced =
      ParseDlssRRPreset(config.graphics.dlssRRPresetBalanced);
  config.graphics.parsed.rrPresetPerformance =
      ParseDlssRRPreset(config.graphics.dlssRRPresetPerformance);
  config.graphics.parsed.rrPresetUltraPerformance =
      ParseDlssRRPreset(config.graphics.dlssRRPresetUltraPerformance);
  config.graphics.parsed.rrPresetUltraQuality =
      ParseDlssRRPreset(config.graphics.dlssRRPresetUltraQuality);
  config.graphics.parsed.rrPreset =
      ParseDlssRRPreset(config.graphics.dlssRRPreset);

  config.graphics.parsed.dlssSharpening =
      ParseDlssSharpening(config.graphics.dlssSharpening);

  // Parse NVIDIA Smooth Motion compatibility
  if (config.graphics.nvidiaSmoothMotionCompat == "on" ||
      config.graphics.nvidiaSmoothMotionCompat == "1" ||
      config.graphics.nvidiaSmoothMotionCompat == "true") {
    config.graphics.parsed.nvidiaSmoothMotionCompat = 1;
  } else if (config.graphics.nvidiaSmoothMotionCompat == "off" ||
             config.graphics.nvidiaSmoothMotionCompat == "2" ||
  ...
  if (config.debugLogging) {
    LogInfo("Config: Parsed dlss_sr_preset='%s' -> ID %u",
            config.graphics.dlssSRPreset.c_str(),
            config.graphics.parsed.srPreset);
    if (config.graphics.parsed.srPreset > 0) {
      LogInfo("Config: Global SR Preset Override Active: '%c'",
              (config.graphics.parsed.srPreset <= 13)
                  ? ('A' + config.graphics.parsed.srPreset - 1)
                  : '?');
    }
  }
  ...
  if (cfgFile.is_open()) {
    ...
    auto AddEntry = [&](std::string entry,
      ...
      if (!entry.empty()) {
        ...
        if (std::find(targetList.begin(), targetList.end(), entry) ==
            targetList.end()) {
    ...
    while (std::getline(cfgFile, line)) {
      ...
      if (inInjection) {
        ...
        } else if (trimmed.find("overlay_whitelist=") == 0 ||
                   trimmed.find("overlay-whitelist=") == 0) {
        ...
        } else if (trimmed.find("wgc-window-detection=") == 0 ||
                   trimmed.find("wgc_window_detection=") == 0) {
          size_t eqPos = trimmed.find('=');
          std::string rest = trimmed.substr(eqPos + 1);
          rest = Trim(rest);
          if (!rest.empty() && rest != "(" && rest != ")") {
  ...
  auto ParseColor = [&](const std::string &hexStr,
                        uint32_t defaultColor) -> uint32_t {
    if (hexStr.empty())
      return defaultColor;
  ...
  config.overlay.captureIncludeOverlay =
      GetBool("Overlay", "capture_include_overlay", true);
  ...
  config.overlay.roundedCorners =
      GetFloat("Overlay", "rounded_corners", 8.0f); // Default 8px rounding

  // Visual Styling - MangoHud Inspired Defaults
  // 0xAABBGGRR format

  // Background: Black with 0.5 Alpha
  config.overlay.bgColor =
      ParseColor(GetStr("Overlay", "bg_color", ""), 0xFF000000);
  ...
  config.overlay.fpsColor =
      ParseColor(GetStr("Overlay", "fps_color", ""), 0xFF05FAB8);
  config.overlay.cpuColor =
      ParseColor(GetStr("Overlay", "cpu_color", ""), 0xFF62972E);
  config.overlay.gpuColor =
      ParseColor(GetStr("Overlay", "gpu_color", ""), 0xFF62972E);
  config.overlay.ramColor =
      ParseColor(GetStr("Overlay", "ram_color", ""), 0xFF9366C2);
  config.overlay.vramColor =
      ParseColor(GetStr("Overlay", "vram_color", ""), 0xFF265FAD);
  config.overlay.frametimeColor =
      ParseColor(GetStr("Overlay", "frametime_color", ""), 0xFF00FF00);
  config.overlay.textColor =
      ParseColor(GetStr("Overlay", "text_color", ""), 0xFFFFFFFF);

  // Text Outline
  config.overlay.textOutline = GetBool("Overlay", "text_outline", true);
  config.overlay.textOutlineColor =
      ParseColor(GetStr("Overlay", "text_outline_color", ""), 0xFF000000);
  config.overlay.textOutlineThickness =
      GetFloat("Overlay", "text_outline_thickness", 1.5f);

  // Load Colors (Green -> Yellow -> Red) - ImGui uses ABGR format
  config.overlay.loadColorLow = ParseColor(
      GetStr("Overlay", "load_color_low", ""), 0xFF62972E); // Greenish
  config.overlay.loadColorMed = ParseColor(
      GetStr("Overlay", "load_color_med", ""), 0xFF00CFFF); // Amber/Yellow
  config.overlay.loadColorHigh = ParseColor(
      GetStr("Overlay", "load_color_high", ""), 0xFF0000FF); // Pure Red

  // Update Interval
  config.overlay.textUpdateInterval =
      GetInt("Overlay", "text_update_interval", 500);
  ...
  config.video.scaling.outputResolution =
      GetStr("Scaling", "output_resolution", "native");
  config.video.scaling.outputResolution =
      GetStr("Scaling", "output_resolution", "native");
  ...
  if (!legacyFilter.empty() && legacyFilter != "auto" &&
      config.video.scaling.sharpness == 0) {
    std::transform(legacyFilter.begin(), legacyFilter.end(),
                   legacyFilter.begin(), ::tolower);
  ...
  for (int appIdx = 1; appIdx <= 8; appIdx++) {
    ...
    if (appAudio.enabled &&
        (!appAudio.processName.empty() || appAudio.processId != 0)) {
...
AppConfig::HotkeyConfig ParseHotkey(const std::string &val) {
  ...
  if (upper.find("WIN+") != std::string::npos ||
      upper.find("WINDOWS+") != std::string::npos) {
    hk.win = true;
  }
  ...
  std::string key =
      (lastPlus != std::string::npos) ? upper.substr(lastPlus + 1) : upper;
