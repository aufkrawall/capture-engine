...
public:
  ...
  bool Init(const VideoConfig &config, int width, int height, int fps,
  ...
  bool EncodeFrame(HANDLE sharedHandle, HANDLE fenceHandle, uint64_t fenceValue,
                   int64_t timestamp, uint32_t sourcePid, int width, int height,
                   int format, bool isHDR, bool isShmem = false,
                   int shmemSlot = 0);

  // Encode a D3D11 texture directly (framegrab mode - zero copy)
  bool EncodeFrameD3D11(ID3D11Texture2D *texture, int64_t pts,
  ...
  int64_t GetLastFrameEncodeTimeUs()
      const; // Get duration of last frame encoding (excluding wait)
  int64_t
  GetLastFrameFenceWaitUs() const; // Get duration of last fence wait (GPU wait)

  int AddAudioStream(const AudioConfig &config,
  ...
  int AddAudioContext(const AudioConfig &config, AVCodecContext *audioCtx,
                      int track);
  ...
  void SetSharedMem(struct SharedMemoryLayout *shm,
                    struct ShmemBuffer *shmBuf = nullptr) {
    pSharedMem = shm;
    pShmem = shmBuf;
  }
...
private:
  ...
  bool currentIsHDR =
      false; // Track HDR state (thread-local to EncodeFrame mostly)
  ...
  std::atomic<bool> codecOpenFailed{
  ...
  std::unique_ptr<CursorRenderer>
      cursorRenderer; // GPU cursor compositing (for inject mode)
  ...
  bool ConvertBGRAtoNV12(ID3D11Texture2D *bgraTexture,
                         ...
                         bool cursorVisible = false, int cursorX = 0,
                         int cursorY = 0);
  ...
  bool InitCudaPath();
  bool EncodeFrameCuda(HANDLE sharedHandle, uint64_t fenceValue, int64_t pts,
                       uint32_t pid, uint32_t frameWidth, uint32_t frameHeight);
  void CleanupCuda();
#endif
