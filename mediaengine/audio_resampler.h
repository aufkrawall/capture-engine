...
public:
   ...
   * @return true on success
   */
  bool Process(const uint8_t *inputData, int inputBytes, uint8_t ***outputData,
               int *outputSamples);
   ...
   */
  bool IsReady() const { return swrCtx != nullptr; }

  /**
   * Get the current output format.
   */
  const OutputFormat &GetOutputFormat() const { return outFmt; }
...
private:
  ...
  bool Unpack24BitTo32Bit(const uint8_t *input, int inputBytes,
