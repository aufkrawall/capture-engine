...
void AudioResampler::AdjustForClockDrift(int64_t videoElapsedMs,
  ...
  if (lastCompensationTimeMs > 0 &&
      (videoElapsedMs - lastCompensationTimeMs) < 100) {
    return;
  }
  ...
  if (currentDelta != 0 || compensationActive) {
    ...
    if (ret >= 0) {
      ...
      if (!compensationActive ||
          std::abs(currentDelta - (int32_t)lastDriftSamples) >
              200) { // Log every ~20 samples/sec change
        DLL_Log("[AudioResampler] Drift Comp: smoothed=%.1f, current=%d/10s "
                "(%.3f%%)",
                smoothedDrift, currentDelta,
                (double)currentDelta * 100.0 /
                    (outFmt.sampleRate * COMPENSATION_PERIOD_SEC));
