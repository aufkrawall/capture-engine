...
AudioResampler::AudioResampler() : swrCtx(nullptr), unpackBuffer(nullptr), unpackBufferSize(0)
{
    inFmt = {};
    outFmt = {};
}
...
void AudioResampler::AdjustForClockDrift(int64_t videoElapsedMs, int64_t audioSamplesOutput)
{
    ...
    if (std::abs(driftSamples) > STEADY_STATE_THRESHOLD) {
        ...
        if (largeSkipCounter++ % 100 == 0) {
            DLL_Log("[AudioResampler] Huge drift (%lld samples) - skipping pitch correction (buffering/scheduling)",
                    driftSamples);
        }
    ...
    if (currentDelta != 0 || compensationActive) {
        ...
        if (ret >= 0) {
            ...
            if (!compensationActive ||
                ...
                DLL_Log("[AudioResampler] Drift Comp: smoothed=%.1f, current=%d/10s (%.3f%%)", smoothedDrift,
                        currentDelta, (double)currentDelta * 100.0 / (outFmt.sampleRate * COMPENSATION_PERIOD_SEC));
