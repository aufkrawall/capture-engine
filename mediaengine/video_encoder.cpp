...
VideoEncoder::VideoEncoder()
    : fmtCtx(nullptr), codecCtx(nullptr), stream(nullptr), hwDeviceCtx(nullptr), hwFramesCtx(nullptr),
      cudaDeviceCtx(nullptr), cudaFramesCtx(nullptr), d3d11DeviceCtx(nullptr), d3d11FramesCtx(nullptr),
      d3d11Device(nullptr), d3d11Context(nullptr), luidLow(0), luidHigh(0), initDone(false), currentIsHDR(false),
      fileOpened(false), recordingRequested(false), isStopping(false), flushRequested(false), codecOpenFailed(false),
      startPts(-1), width(0), height(0), cachedSourcePid(0), lastEncodeTimeUs(0), fenceEvent(nullptr),
      videoDevice(nullptr), videoContext(nullptr), videoProcessor(nullptr), videoProcessorEnum(nullptr),
      currentNV12Buffer(0), inputView(nullptr), videoProcessorInit(false)
{
}
    ...
    if (!isMF) {
        ...
        if (profileToUse == "auto" || profileToUse.empty()) {
            ...
            if (isH264)
                ...
                profileToUse = (codecCtx->color_primaries == AVCOL_PRI_BT2020) ? "main10" : "main";
        }
...
bool VideoEncoder::EnsureDevice()
{
    ...
    if (luidLow == 0 && luidHigh == 0) {
        DLL_Log("[VideoEncoder] WARNING: EnsureDevice called with default LUID (0:0). "
...
// createDeviceFlags |= D3D11_CREATE_DEVICE_DEBUG; // Optional
        ...
        if (!d3d11DeviceCtx) return false;
        ...
        if (av_hwdevice_ctx_init(d3d11DeviceCtx) < 0) return false;
...
bool VideoEncoder::Start()
{
    ...
    // Stop() will signal the async finalize if needed, then we wait for it to finish.
    Stop();
    if (writerThread.joinable()) {
                               ...
                               int shmemSlot)
{
    ...
    if (isShmem) {
        ...
        if (cacheSlot >= 0) {
            ...
            if (skipFence) {
                ...
                // Try NT handle path
                if (DuplicateHandle(hProcess.get(), fenceHandle, GetCurrentProcess(), dupFence.addressof(), 0, FALSE,
            ...
            if (sharedHandle == NULL) {
                ...
                if (!(formatSupport & D3D11_FORMAT_SUPPORT_SHAREABLE)) {
                    DLL_Log("[VideoEncoder] CRITICAL WARNING: DXGI_FORMAT_B8G8R8A8_UNORM (87) does NOT support "
                            "SHAREABLE! Flags=%x",
                            formatSupport);
                }
                ...
                if (DuplicateHandle(hProcess.get(), sharedHandle, GetCurrentProcess(), dupTex.addressof(), 0, FALSE,
                    ...
                    if (FAILED(hr)) {
                        DLL_Log(
                            "[VideoEncoder] Frame %d: OpenSharedResource1(dup=%p) failed HR=%x. Falling back to KMT...",
            ...
            for (int i = 0; i < 8; i++) {
                ...
                if (i == 7) targetSlot = 0;  // Fallback to 0 if all full
            }
    ...
    if (stats.totalMs > g_maxFrameTime) g_maxFrameTime = stats.totalMs;
    if (stats.totalMs > expectedFrameMs * 2) g_slowFrameCount++;
    ...
    if (stats.totalMs > expectedFrameMs * 2 || encodeFrameCounter <= 5 || encodeFrameCounter % 30 == 0) {
        std::string features = "";
        if (savedConfig.lookahead) features += "Lookahead ";
        if (savedConfig.aq) features += "AQ ";
        if (savedConfig.bFrames > 0) features += "B-Frames ";
        if (!savedConfig.multipass.empty() && savedConfig.multipass != "disabled") features += "Multipass ";
    ...
    if (encodeFrameCounter % 120 == 0) {
        ...
        DLL_Log("[PERF SUMMARY] Frames=%lld Avg: total=%.2fms fence=%.2f "
                ...
                g_framesEncoded, avgTotal, avgFence, avgConvert, avgEncode, g_maxFrameTime, g_slowFrameCount,
                bottleneck);
...
void VideoEncoder::Stop()
{
    ...
    if (wasRecording && writerRunning) {
        DLL_Log("[VideoEncoder] Async Stop: Signaling finalize (queueBytes=%zu)...", currentQueueBytes.load());
        isStopping = true;
        queueCV.notify_all();
    ...
    // Double-check if we still need to write trailer/close if thread wasn't running
    if (fileOpened) {
...
{
    if (videoProcessorInit) return true;
    ...
    // Create video processor enumerator with potentially different input/output dims
    ...
    for (int i = 0; i < nv12BufferCount; i++) {
        ...
        hr = videoDevice->CreateVideoProcessorOutputView(nv12StagingTextures[i], videoProcessorEnum, &outputViewDesc,
                                                         &outputViews[i]);
        if (FAILED(hr)) {
...
VideoEncoder::CursorCacheEntry* VideoEncoder::GetCursorCacheEntry(HCURSOR handle)
{
    ...
    // On first frame, this hasn't happened yet - return existing cached entries only
    ...
    if (!cursorRenderer) return nullptr;
...
{
    if (!codecCtx || codecCtx->framerate.num == 0) return 0;
...
int64_t VideoEncoder::GetLastFrameEncodeTimeUs() const { return lastEncodeTimeUs; }

int64_t VideoEncoder::GetLastFrameFenceWaitUs() const { return lastFenceWaitUs; }
            ...
            if (fileOpened && fmtCtx) {
                ...
                if (ret < 0) {
                    ...
                    if (writeErrorCount++ < 10) {
                        char errbuf[AV_ERROR_MAX_STRING_SIZE];
                        av_strerror(ret, errbuf, sizeof(errbuf));
                        DLL_Log("[VideoEncoder] ERROR: av_interleaved_write_frame failed: %d (%s) pts=%lld", ret,
                                errbuf, pkt->pts);
                    }
                }
            }
