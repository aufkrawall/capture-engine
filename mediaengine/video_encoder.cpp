...
bool VideoEncoder::CreateSharedCaptureTextures(uint32_t w, uint32_t h,
  ...
  if (sharedMem) {
    ...
    for (int i = 0; i < 4; i++) {
      sharedMem->encoderTextures.textureHandles[i] =
          (uint64_t)sharedCaptureHandles[i];
    }
...
// createDeviceFlags |= D3D11_CREATE_DEVICE_DEBUG; // Optional
  ...
  if (savedConfig.scaling.enabled && 
      savedConfig.scaling.outputWidth > 0 && 
...
bool VideoEncoder::EncodeFrame(HANDLE sharedHandle, HANDLE fenceHandle,
                               ...
                               int format, bool isHDR, bool isShmem, int shmemSlot) {
  ...
  if (cacheSlot >= 0) {
    ...
    if (encodeFrameCounter % kCacheLogIntervalFrames == 1) {
      DLL_Log("[VideoEncoder] Using cached handles (pid=%u, slot=%d, frame=%d)",
...
bool VideoEncoder::InitVideoProcessor() {
  ...
  if (scalingEnabled) {
    DLL_Log("[VideoProcessor] GPU SCALING ENABLED: %dx%d -> %dx%d",
  ...
  DLL_Log("[VideoProcessor] Created BGRA staging texture at %dx%d for DD "
          "compatibility", inputWidth, inputHeight);

  videoProcessorInit = true;
  
  if (scalingEnabled) {
    DLL_Log("[VideoProcessor] Initialized with SCALING: %dx%d -> %dx%d "
            "BGRA→NV12", inputWidth, inputHeight, outputWidth, outputHeight);
  } else {
    DLL_Log("[VideoProcessor] Initialized for %dx%d "
            "BGRA→NV12 (no scaling)", outputWidth, outputHeight);
  }
  return true;
}
        ...
        if (fileOpened && fmtCtx) {
             ...
             if (ret < 0) {
                 ...
                 if (writeErrorCount++ < 10) {
                   char errbuf[AV_ERROR_MAX_STRING_SIZE];
                   av_strerror(ret, errbuf, sizeof(errbuf));
                   DLL_Log("[VideoEncoder] ERROR: av_interleaved_write_frame failed: %d (%s) pts=%lld",
                           ret, errbuf, pkt->pts);
                 }
             }
        }
