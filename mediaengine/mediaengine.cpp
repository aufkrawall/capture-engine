...
public:
    MediaEngine()
        : recording(false), audioRunning(false), firstVideoFrameMs(0), lastVideoFrameMs(0), videoElapsedMs(0),
          recordingStartTime()
    {
    }
    ...
    bool Init(const AppConfig* config)
    {
        ...
        for (size_t i = 0; i < config->audioSources.size(); i++) {
            ...
            DLL_Log("MediaEngine::Init setting up audio source %zu (type=%d device=%s process=%s)", i,
                    (int)audioConfig.sourceType, audioConfig.device.empty() ? "default" : audioConfig.device.c_str(),
    ...
    bool StartRecording()
    {
        ...
        if (!videoEnc->Start()) return false;
        ...
        if (!audioSources.empty()) {
            ...
            if (startedCount > 0) {
                DLL_Log("MediaEngine: %d audio source(s) started (sync pending first video frame)", startedCount);
                audioRunning = true;
    ...
    void StopRecording()
    {
        ...
        if (videoEnc) {
            ...
            DLL_Log("MediaEngine: Setting audio end time. Start: %lld us, Dur: %lld us, End: %lld us", 0LL, durationUs,
                    endUs);
    ...
    void ProcessFrameD3D11(void* texture, int64_t timestampQPC, uint32_t width, uint32_t height)
    {
        ...
        if (this->firstVideoFrameMs == 0) {
            ...
            DLL_Log("MediaEngine: First D3D11 frame at %lld ms (QPC: %lld) (SystemQPC: %lld)", debugTimestamp,
                    timestampQPC, this->recordingStartSystemQPCMs.load());
    ...
    {
        if (!recording || audioSources.empty()) return;
        ...
        for (auto& kv : trackToSources) {
            ...
            if (samplesToEncode <= 0) continue;  // Already caught up
            ...
            for (size_t srcIdx : srcIndices) {
                ...
                if (src.syncResampler && src.syncResampler->IsReady()) {
                    ...
                    if (driftLogCounter++ % 1000 == 0 && abs(rbError) > 100) {
                        ...
                        DLL_Log("[PullAudio] Src %d Buffer Level: %zu (Err: %lld) -> Comp Drift: %lld", (int)srcIdx,
                                rbAvailable, rbError, rbError);
                    }
                }
            ...
            if (activeSources == 0) {
                ...
                */
                ...
                if (silenceLogCounter++ % 500 == 0) {
                    DLL_Log("[PullAudio] Track %d silent - generating %lld samples of silence to maintain sync", track,
                            samplesToEncode);
                }
            ...
            if (syncCheckCounter++ % 6000 == 0 && firstSrcIdx < encodedSamplesPerSource.size()) {
                ...
                DLL_Log("[A/V SYNC CHECK] Track %d: Video=%lld ms, Audio=%lld ms, Drift=%lld ms", track, videoMs,
                        audioMs, avDrift);

                if (std::abs(avDrift) > 100) {
                    DLL_Log("[A/V SYNC WARNING] Track %d drift exceeds 100ms! Investigate potential sync issues.",
                            track);
                }
            }
        }
...
private:
    void AudioLoop()
    {
        ...
        while (audioRunning) {
            ...
            for (size_t srcIdx = 0; srcIdx < audioSources.size(); srcIdx++) {
                auto& src = audioSources[srcIdx];
                if (!src.sharedEncoderPtr) continue;

                // Skip if no capture source available
                if (!src.capture && !src.appCapture) continue;
...
MEDIAENGINE_API bool MediaEngine_Init(const AppConfig* config)
{
    ...
    if (!g_Engine) g_Engine = std::make_unique<MediaEngine>();
...
{
    if (g_Engine) return g_Engine->StartRecording();
    return false;
}

MEDIAENGINE_API void MediaEngine_ReloadConfig(const AppConfig* config)
{
    if (g_Engine) g_Engine->ReloadConfig(config);
}
...
{
    if (g_Engine) g_Engine->ProcessFrameD3D11(texture, timestamp, width, height);
}
...
{
    if (g_SharedD3D11Device) return g_SharedD3D11Device;
