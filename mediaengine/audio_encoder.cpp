...
#pragma warning(disable : 4996)
  ...
  audioFifo = av_audio_fifo_alloc(
      codecCtx->sample_fmt, codecCtx->ch_layout.nb_channels, fifoCapacity);
  if (!audioFifo) {
...
void AudioEncoder::EncodeSamples(const uint8_t *data, int sizeBytes,
  ...
  if (needsReset) {
    ...
    lastInputTimestamp =
        -1; // CRITICAL: Reset to prevent false discontinuity detection
    if (audioFifo)
  ...
  if (samplesCount < FADE_SAMPLES) {
    ...
    int numPlanes =
        av_sample_fmt_is_planar(codecCtx->sample_fmt) ? channels : 1;
    int planeSamples =
        av_sample_fmt_is_planar(codecCtx->sample_fmt) ? 1 : channels;
      ...
      if (codecCtx->sample_fmt == AV_SAMPLE_FMT_FLT ||
        ...
        for (int i = 0; i < samplesToFade; i++) {
          ...
          if (numPlanes == 1) {
            for (int c = 0; c < channels; c++)
              sData[i * channels + c] =
                  (int16_t)(sData[i * channels + c] * gain);
          } else {
        ...
        for (int i = 0; i < samplesToFade; i++) {
          ...
          if (numPlanes == 1) {
            for (int c = 0; c < channels; c++)
              sData[i * channels + c] =
                  (int32_t)(sData[i * channels + c] * gain);
          } else {
  ...
  if (firstTimestamp < 0 && recordingStartUs > 0) {
    ...
    if (samplesCount != 0 || resampledSamplesTotal != 0) {
      DLL_Log("[AudioEnc] First packet safety reset: samplesCount=%lld -> 0, "
  ...
  if (logCounter++ % 5000 == 0) {
    ...
    if (currentSize > fifoCapacity / 2) {
      DLL_Log("[AudioEnc] WARN: Audio FIFO high: %d/%d samples", currentSize,
              fifoCapacity);
    } else {
      DLL_Log("[AudioEnc] FIFO size=%d, frame_size=%d", currentSize,
              frame_size);
    }
  }
  ...
  while (av_audio_fifo_size(audioFifo) >= frame_size) {
    ...
    if (frameLogCounter++ < 10) {
      DLL_Log("[AudioEnc] FRAME PTS DEBUG: pts=%lld (%.3f sec) "
              ...
              (long long)frame->pts, (double)frame->pts / codecCtx->sample_rate,
...
void AudioEncoder::Flush() {
  ...
  if (recordingEndUs > 0 && recordingStartUs >= 0 &&
    ...
    maxSamples =
        av_rescale_rnd(durationUs, codecCtx->sample_rate, 1000000, AV_ROUND_UP);

    DLL_Log("[AudioEncoder] Video duration: %lld us, max audio samples: %lld, "
            "current: %lld",
  ...
  if (fifoSize > 0 && frame) {
    ...
    while (fifoSize > 0) {
      ...
      if (remainingAllowed <= 0) {
        DLL_Log("[AudioEncoder] Already at sample limit, discarding %d "
                "buffered samples",
                fifoSize);
        av_audio_fifo_reset(audioFifo);
        break;
      ...
      int rret =
          av_audio_fifo_read(audioFifo, (void **)frame->data, samplesToRead);
      ...
      if (samplesToSend > samplesToRead) {
        ...
        for (int p = 0; p < numPlanes && frame->data[p]; p++) {
          uint8_t *dst =
              (uint8_t *)frame->data[p] + (samplesToRead * framePlaneStride);
