...
AudioEncoder::AudioEncoder()
    : codecCtx(nullptr), resampler(nullptr), frame(nullptr), samplesCount(0), streamIndex(-1), initDone(false),
      audioFifo(nullptr), savedCodecId(AV_CODEC_ID_NONE), firstTimestamp(-1), recordingStartUs(-1), recordingEndUs(0)
{
    currentInputFormat = {};
}
...
#pragma warning(disable : 4996)
    ...
    if (codec->id == AV_CODEC_ID_ALAC) {
        ...
        for (auto pref : preferences) {
            ...
            if (best != AV_SAMPLE_FMT_NONE) break;
        }
                                 ...
                                 int validBitsPerSample, int blockAlign, bool isFloat, int64_t timestamp)
{
    ...
    if (!initDone || !codecCtx || !data || sizeBytes <= 0) return;
...
// The videoTimeGetter callback is still available for monitoring if needed.
  ...
  if (videoTimeGetter && resampler) {
    ...
    int ret = av_audio_fifo_write(audioFifo, (void**)resampledData, convertedSamples);
    ...
    if (codecCtx && savedCodecId != AV_CODEC_ID_NONE) {
        ...
        if (codecCtx->ch_layout.nb_channels > 0) {
            ...
            if (av_channel_layout_copy(&chLayout, &codecCtx->ch_layout) < 0) {
                DLL_Log("[AudioEnc] Failed to copy channel layout, using default stereo");
        ...
        if (codec) {
            ...
            if (codecCtx) {
                ...
                DLL_Log("[AudioEnc] Reopening: rate=%d, fmt=%d, channels=%d, bitrate=%lld", sampleRate, sampleFmt,
                        chLayout.nb_channels, (long long)bitRate);
...
void AudioEncoder::Flush()
{
    ...
    if (recordingEndUs > 0 && recordingStartUs >= 0 && recordingEndUs >= recordingStartUs) {
        ...
        DLL_Log("[AudioEncoder] Video duration: %lld us, max audio samples: %lld, current: %lld", durationUs,
    ...
    if (recordingEndUs > 0 && maxSamples != INT64_MAX) {
        ...
        if (discardPaddingSamples > 0 && discardPaddingSamples < codecCtx->frame_size) {
            DLL_Log("[AudioEncoder] Setting trailing_padding for sample-accurate end: %lld samples (target=%lld, "
                    "actual=%lld)",
