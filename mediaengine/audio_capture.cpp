  ...
  while (isCapturing) {
    ...
    while (packetLength != 0) {
      ...
      if (!firstSet && devicePosition > 0) {
        ...
        firstSet = true;
        DLL_Log("[AudioCapture] Source Sync Start: DevPos=%llu QPC=%llu",
        ...
        DLL_Log("[AudioCapture] Source Sync: Duration Samples=%.4fs, "
                "QPC=%.4fs, Drift=%.2f ms (%.4f%%)",
                samplesDuration, qpcDuration, driftMs,
                (driftMs / (qpcDuration * 1000.0) * 100.0));
      }
