...
// These are normally in audioclientactivationparams.h but not available in MinGW
// ============================================================================
...
public:
  ...
  // IUnknown - must return S_OK for IAgileObject to prevent E_ILLEGAL_METHOD_CALL
  STDMETHODIMP QueryInterface(REFIID riid, void **ppvObject) override {
    if (riid == __uuidof(IUnknown) ||
        riid == __uuidof(IActivateAudioInterfaceCompletionHandler) ||
        riid == IID_IAgileObject) {
      *ppvObject = static_cast<IActivateAudioInterfaceCompletionHandler *>(
          this);
      AddRef();
      return S_OK;
    }
...
bool AppAudioCapture::InitializeCaptureForPID(DWORD pid) {
  ...
  // In that case, we use CD quality format (44.1kHz 16-bit stereo) per Microsoft sample
  // Force format to 48kHz Stereo Float 32-bit
  // We use AUDCLNT_STREAMFLAGS_AUTOCONVERTPCM, so Windows handles the conversion
  // from the app's actual format (e.g. 44.1k int16) to our requested format.
  ...
  // Initialize audio client - per Microsoft sample, use LOOPBACK + AUTOCONVERTPCM
  // AUTOCONVERTPCM tells Windows to convert the process audio to our format
  // Use 10ms buffer (100000 hns) to reduce latency and burstiness
  ...
  if (FAILED(hr)) {
    ...
    DLL_Log("[AppAudioCapture] Initialize with EVENTCALLBACK failed: 0x%x, trying without", hr);
    hr = pAudioClient->Initialize(AUDCLNT_SHAREMODE_SHARED,
    ...
    if (FAILED(hr)) {
      ...
      DLL_Log("[AppAudioCapture] Initialize with LOOPBACK failed: 0x%x, trying plain", hr);
      hr = pAudioClient->Initialize(AUDCLNT_SHAREMODE_SHARED,
                                    0,
                                    2000000,
                                    0, pwfx, nullptr);
...
void AppAudioCapture::CaptureLoop() {
  ...
  while (isCapturing.load() && !shouldStop.load()) {
    ...
    if (packetLength == 0) {
      ...
      // Otherwise, large gaps accumulate and trigger the "Gap Too Large" cap logic later.
      
      // HIGH-PRECISION TIMING: Calculate actual elapsed time since last real packet
      auto now = std::chrono::steady_clock::now();
      auto realElapsedMs = std::chrono::duration_cast<std::chrono::milliseconds>(
      ...
      while (synthesizedMs + 20 <= realElapsedMs) { 
           ...
           if (pwfx) {
               ...
               if (pwfx->wFormatTag == WAVE_FORMAT_IEEE_FLOAT) {
                   isFloat = true;
                    ...
                    if (IsIEEEFloat(wfex->SubFormat)) isFloat = true;
                    else isFloat = false;
               } else {
                   isFloat = false;
               }
           ...
           if (blockAlign == 0) blockAlign = channels * bytesPerSample;
           ...
           // Don't just sample "Now", calculate strictly from previous position to avoid jitter.
           LARGE_INTEGER freq;
           QueryPerformanceFrequency(&freq);
           
           // Valid packet updates lastQpcPosition. If we are pure synthesis (start), init it.
    ...
    while (packetLength != 0 && isCapturing.load()) {
      ...
      if (!firstSet && devicePosition > 0) {
        ...
        DLL_Log("[AppAudioCapture] Source Sync Start (%lu): DevPos=%llu QPC=%llu", 
         ...
         DLL_Log("[AppAudioCapture] Source Sync (%lu): Duration Samples=%.4fs, QPC=%.4fs, Drift=%.2f ms (%.4f%%)", 
