...
public:
    ActivationHandler(HANDLE completeEvent)
        : refCount(1), completeEvent(completeEvent), resultCode(E_FAIL), audioClient(nullptr)
    {
    }
...
// ============================================================================
AppAudioCapture::AppAudioCapture() { activationCompleteEvent = CreateEventW(nullptr, TRUE, FALSE, nullptr); }
...
bool AppAudioCapture::IsSupported()
{
    ...
    if (osvi.dwMajorVersion == 10 && osvi.dwBuildNumber >= 20348) return true;
    ...
    while (isCapturing.load() && !shouldStop.load()) {
        ...
        while (packetLength != 0 && isCapturing.load()) {
            ...
            if (!firstSet && devicePosition > 0) {
                ...
                DLL_Log(
                    "[AppAudioCapture] Source Sync (%lu): Duration Samples=%.4fs, QPC=%.4fs, Drift=%.2f ms (%.4f%%)",
