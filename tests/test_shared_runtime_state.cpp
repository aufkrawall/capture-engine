...
TEST(SharedDefsTest, NameGeneratorsIncludeExpectedPidFormatting) {
    ...
    EXPECT_EQ(std::wcscmp(sharedMemName, L"Local\\CE_SM_61_1234ABCD"), 0);
    EXPECT_EQ(std::wcscmp(SHARED_MEM_DISCOVERY, L"Local\\CE_Disc_61"), 0);
    ...
    EXPECT_EQ(std::wcscmp(hostStoppingEventName, L"Local\\CE_InjectHostStopping_61"), 0);
    EXPECT_EQ(std::wcscmp(injectReactivateEventName, L"Local\\CE_InjectReactivate_61_1234ABCD"), 0);
    EXPECT_EQ(std::wcscmp(vulkanReactivateEventName, L"Local\\CE_VulkanReactivate_61_1234ABCD"), 0);
    EXPECT_EQ(std::wcscmp(injectDormantEventName, L"Local\\CE_InjectDormant_61_1234ABCD"), 0);
    EXPECT_EQ(std::wcscmp(vulkanDormantEventName, L"Local\\CE_VulkanDormant_61_1234ABCD"), 0);
}
