...
TEST(SharedDefsTest, NameGeneratorsIncludeExpectedPidFormatting) {
    ...
    EXPECT_EQ(std::wcscmp(sharedMemName, L"Local\\CE_SM_64_1234ABCD"), 0);
    EXPECT_EQ(std::wcscmp(SHARED_MEM_DISCOVERY, L"Local\\CE_Disc_64"), 0);
    ...
    EXPECT_EQ(std::wcscmp(hostStoppingEventName, L"Local\\CE_InjectHostStopping_64"), 0);
    EXPECT_EQ(std::wcscmp(injectReactivateEventName, L"Local\\CE_InjectReactivate_64_1234ABCD"), 0);
    EXPECT_EQ(std::wcscmp(vulkanReactivateEventName, L"Local\\CE_VulkanReactivate_64_1234ABCD"), 0);
    EXPECT_EQ(std::wcscmp(injectDormantEventName, L"Local\\CE_InjectDormant_64_1234ABCD"), 0);
    EXPECT_EQ(std::wcscmp(vulkanDormantEventName, L"Local\\CE_VulkanDormant_64_1234ABCD"), 0);
}
