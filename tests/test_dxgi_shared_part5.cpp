...
TEST(DXGISharedTest, RuntimeOwnedOverlayRoutesUseCachedPresentationContractAndRefreshTransitions) {
    ...
    EXPECT_NE(dx12.find("dx12_hook_g_D3D11On12Adapter.SetHDR(isHdr, static_cast<int>(format))"),
              std::string::npos);
    size_t secondaryColorSyncCalls = 0;
    for (size_t position = 0;
         (position = dx12.find("SyncSecondaryDx12OverlayColorState(dx12_hook_g_State.format)", position)) !=
         std::string::npos;
         ++position) {
...
TEST(DXGISharedTest, InactiveFGOverlaySpaceIsNeverReserved) {
    EXPECT_FALSE(ce::dx12_overlay_policy::ShouldReserveInactiveFGOverlaySpaceForCurrentFrame(true, true, true));
