...
- **Stopping a recording could hang forever on a stuck disk or dead network share:** writes to local recording files were unbounded, so a hung write wedged the stop path and the recording was never finalized. Local outputs now have a write timeout (30 s) that turns a hung write into a reportable error and lets the stop complete.
...
- **Recording ran to its end as a frozen picture when the capture source died:** a failed capture replacement whose rollback also failed left a frozen frame plus live audio with no visible error. The recording now stops through the normal path and completes as saved (degraded).
...
- **Misspelled settings silently changed behavior:** an unknown `capture_method` became `auto` with no warning, and `auto` normally means injected capture — one wrong character in `capture_method=wgc` quietly enabled injection. Unknown capture methods, hotkeys, and limiter modes now log which key was invalid and what replaced it.
...
- **Foreign breakpoint traffic could stall the game writing crash dumps:** an `int3` from anti-cheat or another hooking tool each cost a full in-game dump stall. Only the first unowned breakpoint of a run dumps immediately (kept so an escaped assert can never die unrecorded); the rest are recorded and dump only if the process actually dies of one.
...
- **No more compressed audio after encoder faults:** a consumed chunk the encoder could not take (resampler/FIFO failure) was re-requested and refilled with newer samples, silently compressing the track while every length check stayed green — consumed ranges are never re-filled now; lost tails are placed as explicit silence holes at their exact timeline positions.
