...
- **Installations in a folder with non-Latin characters ran on default settings:** `config.ini` and the logs were looked up through a path the Windows language setting could not express, both in CaptureEngine and in the game. The folder's short (8.3) name is used when needed, and a warning is logged when the volume has none.
