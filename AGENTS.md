...
- **Then close with exactly ONE gate. The gates are nested, not cumulative.** `--verify` already performs a clean product build, the full native test suite, the Python tool self-tests, lint including the clang-tidy ratchet, and ASan/UBSan validation. When the change requires `--verify`, run only that: prefixing it with `--incremental`, `--no-build --run-tests`, or `--no-build --lint` repeats work already covered by `--verify` and wastes quota for no added signal.
...
- The complete gate is `python build.py --verify --skip-updates --concise`: clean product build, the full native test suite, the nineteen Python tool self-tests, lint including the clang-tidy and file-size ratchets, and ASan/UBSan regression coverage in one run. Require it before committing changes to `build.py`, toolchain/compiler/linker/hardening policy, shared ABI/layout, analyzer or test-gate policy, or the capture/CFR/FG/audio paths. Do not make it the routine per-change gate.
    - Verification starts with a read-only file-size/static-analysis preflight against the last compatible full compile database, so a known regression fails before the expensive build. The authoritative lint still runs after the new clean product database is generated.
    ...
    - The sanitizer child uses isolated object, binary, unit-test, temp, compile-database, and SDK-staging roots. On a miss it runs concurrently with the clean product build under a split worker budget. A prior sanitizer success is reused only when its manifest proves every discovered first-party input, compiler/dependency input, and required output byte-identical; changed/missing/ambiguous state runs the stage again. Explicit `--force-rebuild` bypasses this stage-success reuse.
...
- clang-tidy findings are ratcheted against `tools/clang_tidy_baseline.json`, which records accepted counts per check plus the translation units those counts were measured over. Lint fails when any check exceeds its count or a previously unseen check appears; counts below baseline are folded in automatically so a fixed warning cannot silently return.
    ...
    - **Prefer linting after a full product build.** Lint covers whatever `compile_commands.json` currently holds, and a `--tests-only` build regenerates it with a reduced entry count (218 of 528 translation units as of 2026-08-06). Such a run logs `clang-tidy lint scope reduced`, still fails on increases, and never folds its artificially low counts in, so it cannot corrupt the baseline — but it also proves nothing about the unlinted product sources.
