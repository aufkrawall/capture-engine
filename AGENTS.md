...
- The complete gate is `python build.py --verify --skip-updates --concise`: clean product build, the full native test suite, the eleven Python tool self-tests, lint including the clang-tidy and file-size ratchets, and the ASan/UBSan regression pass in one run. Require it before committing changes to `build.py`, toolchain/compiler/linker/hardening policy, shared ABI/layout, analyzer or test-gate policy, or the capture/CFR/FG/audio paths. It costs roughly 350 seconds against about 50 for the incremental build gate, so do not make it the routine per-change gate.
