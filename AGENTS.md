...
- **Always update CHANGELOG.md before committing changes:** Agents must record task-owned fixes, improvements, and features in `CHANGELOG.md` under `## Unreleased` prior to committing. Never let the changelog lag behind code changes! Entries must adhere strictly to `llm-wiki/changelog-guidelines.md`: it must be immediately transparent which application issue, bug, user-facing behavior, or game-specific defect the code change fixes or improves; concise, "ADHD friendly", highly scannable with bold lead-in anchors (`- **<Anchor>** <Details>`), clear standard categories (`### New`, `### Improved`, `### Fixed`, etc.), and structured to be continuously updated per change. Frame capabilities and settings natively without unnecessarily citing or claiming parity with third-party tools (e.g. NVIDIA Profile Inspector, RTSS, OBS) unless resolving direct runtime interoperability. These criteria apply identically to both the `CHANGELOG.md` document and GitHub release tag notes (automated via `tools/manage_changelog.py`). Verify formatting with `python tools/manage_changelog.py --validate`.
- **Always git commit completed and verified changes:** run relevant tests/unit tests first to ensure build and tests succeed, ensure `CHANGELOG.md` is updated and included in task-owned changes, then commit with plain git commands (`git status`, `git add -- <task-owned paths>`, `git commit -m "<message>"`). Use `git add -A` only after verifying every worktree change is task-owned and safe to commit! Do not push to cloud unless explicitly requested, generally just commit locally.
