...
- **Then close with exactly ONE gate. The gates are nested, not cumulative.** `--verify` already performs a clean product build, the full native test suite, the Python tool self-tests, lint including the clang-tidy ratchet, and the ASan/UBSan pass. When the change requires `--verify`, run only that: prefixing it with `--incremental`, `--no-build --run-tests`, or `--no-build --lint` repeats work that `--verify` redoes from scratch and wastes minutes for no added signal.
    ...
    - Change in one of the categories listed in the complete-gate bullet below -> `python build.py --verify --skip-updates --concise` (about 350 s), and nothing else.
- `python build.py --incremental --skip-updates --concise` reuses an object only when its source, compiler binary, flags, dependency file, and project-header content signatures remain valid; production outputs still relink for the new exact build identity, while validation-only unit-test/test-app links may be reused only when their compiler, linker, flags, object/library inputs, and output hashes remain valid. Packaging and the normal binary/PE verification stages still run.
...
- Use the clean `python build.py --skip-updates --concise` build gate instead of the incremental one when investigating possible stale artifacts, when explicitly requested, or when the task touches dependency/toolchain/FFmpeg configuration or generated-build machinery without also requiring `--verify`. It is likewise build-only, and it is redundant whenever `--verify` is required, because `--verify` already builds clean. `--force-rebuild` remains the stronger recovery that physically removes `build/obj` first.
- The complete gate is `python build.py --verify --skip-updates --concise`: clean product build, the full native test suite, the six Python tool self-tests, lint including the clang-tidy ratchet, and the ASan/UBSan regression pass in one run. Require it before committing changes to `build.py`, toolchain/compiler/linker/hardening policy, shared ABI/layout, analyzer or test-gate policy, or the capture/CFR/FG/audio paths. It costs roughly 350 seconds against about 50 for the incremental build gate, so do not make it the routine per-change gate.
...
- clang-tidy findings are ratcheted against `tools/clang_tidy_baseline.json`, which records accepted counts per check. Lint fails when any check exceeds its count or a previously unseen check appears; counts below baseline are folded in automatically so a fixed warning cannot silently return.
    - **Run lint explicitly, never routinely.** `--verify` already includes it, so a separate `python build.py --no-build --lint --skip-updates --concise` (about 68 seconds) is worth it only when closing with the incremental gate instead of `--verify`, or when specifically investigating a lint regression.
    - **Only lint after a full product build.** The ratchet folds improvements in automatically, so linting against a partial `compile_commands.json` — for example straight after a `--tests-only` build, which regenerates it with a reduced entry count — silently records artificially low counts and makes the next full run fail with phantom regressions across nearly every check. Recover with `git checkout -- tools/clang_tidy_baseline.json`.
    - Regenerate the baseline only deliberately, with `python build.py --no-build --lint --update-lint-baseline --skip-updates --concise`, when an increase is genuinely justified. The raw warning list stays advisory; only a regression is fatal.
