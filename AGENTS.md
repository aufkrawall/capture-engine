...
- The complete gate is `python build.py --verify --skip-updates --concise`: clean product build, the full native test suite, the twelve Python tool self-tests, lint including the clang-tidy and file-size ratchets, and the ASan/UBSan regression pass in one run. Require it before committing changes to `build.py`, toolchain/compiler/linker/hardening policy, shared ABI/layout, analyzer or test-gate policy, or the capture/CFR/FG/audio paths. It costs roughly 350 seconds against about 50 for the incremental build gate, so do not make it the routine per-change gate.
...
- clang-tidy findings are ratcheted against `tools/clang_tidy_baseline.json`, which records accepted counts per check plus the translation units those counts were measured over. Lint fails when any check exceeds its count or a previously unseen check appears; counts below baseline are folded in automatically so a fixed warning cannot silently return.
    ...
    - **Prefer linting after a full product build.** Lint covers whatever `compile_commands.json` currently holds, and a `--tests-only` build regenerates it with a reduced entry count (157 of 236 translation units). Such a run logs `clang-tidy lint scope reduced`, still fails on increases, and no longer folds its artificially low counts in, so it can no longer corrupt the baseline — but it also proves nothing about the unlinted product sources.
