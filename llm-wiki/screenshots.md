...
Last cross-checked: 2026-09-12 (asynchronous D3D12 readback off the present thread, actual-Present-aware PostSL ordering across DLSS-G suspend/cutscene intervals, exact swapchain-resource/queue device validation, stale injected-source identity after game exit, presentation-contract-aware inject/WGC source encoding, native HDR versus forced-SDR output policy, bounded parallel/realtime 10-bit 4:4:4 AVIF, placeholder-free atomic publication, explicit result notification, split-device WGC readback ownership, and shared ABI 38/request-specific completion)
...
- `screenshot_include_overlay` decides *where* in the Present the D3D12 copy is taken, not whether a copy is filtered afterwards. Whoever draws the overlay owns that ordering, and under DLSS/FSR frame generation that is the PostSL callback — which runs **earlier in the same Present than `ProcessFrame`** (`ExecuteStartupRouting` precedes `ExecutePresentCore`). So when `PostSLOwnsThisFramesOverlayDraw()` is true, both variants are taken inside the PostSL submit chunk: the overlay-free copy immediately ahead of the overlay list on the same queue, the overlay-included copy after it. `ProcessFrame` yields both to PostSL in that case; taking the overlay-free shot from `ProcessFrame` captured a backbuffer the overlay was already drawn into (session `20260818_205615`). The submit chunk has sixteen mutually exclusive submit branches, and `ScreenshotPresentThreadPolicyTest` asserts every one of them is immediately preceded by the capture call for the same queue.

DLSS-G suspension changes who owns a particular Present without immediately retiring the proven PostSL backend. The global PostSL active/confirmed latches therefore are not sufficient screenshot evidence: a cutscene cooldown or a contended render lock can make that callback return before it submits, and yielding solely from those latches strands the pending request. Inside a real presented-output callback PostSL still owns ordering; after it returns, ProcessFrame yields only when that same Present records a successful inactive PostSL draw. Otherwise ProcessFrame takes the still-pending request on its live route. The overlay-included tail rechecks that the request remains Pending after `ProcessFrame`, preventing an in-ProcessFrame fallback callback from submitting the same request twice.
...
D3D9, D3D10/11, D3D12, OpenGL, and Vulkan all route mapped pixels through the same request-ID worker boundary. **The D3D12 readback is asynchronous and must stay that way**: the render thread records the backbuffer copy, submits it on the queue it is already presenting on, signals a fence and hands the resources to the worker. Waiting there froze the game outright under DLSS frame generation (session `20260818_172155`) — the copy rides the game's own swapchain queue, which a frame-generation runtime only drains once its presenter thread makes progress, and that thread cannot progress while the present call it drives is blocked inside our hook. The producer therefore reserves the single worker slot *before* recording anything, so a submitted copy can never be stranded by a busy queue; the worker waits on the fence in one-second slices, checks `GetDeviceRemovedReason()` between them, and strands (never releases) resources a live GPU may still be reading. The D3D12 producer obtains the device from the exact current swapchain resource, retains the selected queue for the submission, and rejects a queue whose COM device identity differs; it never combines a transition-stale global device with a fresh proxy backbuffer. PostSL passes the live base swapchain into this helper rather than retaining an already-released `IDXGISwapChain3` interface. The other backends still copy their mapped pixels before the backend returns, and the mapped memory is never referenced after that. There is no file-size polling, stable-size heuristic, fixed sleep, unchecked BMP parsing, or replace-existing write to an unreserved destination.
