...
Last cross-checked: 2026-07-22 (DXGI source-limited cadence and UHD120 playback diagnosis, delayed cursor ownership and Windows-SDR-white cursor calibration, HDR/SDR file signaling and chroma, AMF/oneVPL/MF encoding, stable fail-closed monitor selection, and the existing current build, capture, graphics, audio, screenshot, and security state; see hardware-encoding.md, configuration.md, wgc-capture.md, cfr-capture-sync.md, current.md, and log/recent.md)
...
- `wgc-capture.md`
  - WGC and DXGI Desktop Duplication backends, including stable single-monitor selection/fail-closed retargeting, driver-independent FP16 scRGB-to-HDR10/P010 conversion, Rec.2100 top-left chroma, codec/container/static metadata, explicit Windows-SDR-white-calibrated HDR-to-SDR output and cursor composition, delayed source-pointer ownership, source/output HDR policy, max-rate CFR production, transactional handoff, the FP16/R10 reservoir, and crash-safe DPI cursor composition. Last verified 2026-07-22. Stale-risk: medium.
- `cfr-capture-sync.md`
  - Shared WGC/DXGI/inject CFR and A/V invariants, including atomic warm-up cancellation, capture-sync/CFR selection phase preservation with varying-cadence fallback, contiguous screen-grab packet PTS under overload, cursor-aware rendering from an uncomposited repeat cache, packet-only video validation, encoder-latency-independent inject audio targeting, max-rate variable-input WGC production, transactional startup, 60 ms audio-ingestion look-ahead, 300 ms WGC/DXGI look-ahead, nearest monotonic source selection, causal inject transport, convergent per-output overload recovery, and exact decoded stop/tail accounting. Last verified 2026-07-22. Stale-risk: medium.
- `nvenc-encoding.md`
