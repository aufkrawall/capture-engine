...
- `multi-audio-capture.md`
  - Multi system/microphone/app capture, including recording-sticky bootstrap settlement across ordered capture epochs, truthful bootstrap/rejoin diagnostics, timeline-recovery-safe route-local compensation, encoder-latency-independent inject CFR source-clock targets, rate-limited worker scheduling evidence, process-loopback protocol 4 transport/lifecycle integrity, polling-first activation, render-session recovery, private disposable worker isolation, the adaptive CFR ingestion reservoir, exact finalization, cross-pull track fades, and the steady-state placement drift diagnostic. Last verified 2026-09-25. Stale-risk: medium (fresh device/runtime validation remains necessary).
- `wgc-capture.md`
