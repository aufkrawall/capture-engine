...
Last cross-checked: 2026-06-11 (updated: A/V source-stall attribution, real-session triage, and matrix runner)
...
- `cfr-capture-sync.md`
  - Shared WGC/inject CFR timeline and audio sync invariants, packet-level and deterministic content-marker diagnostics, track startup/cursor rules, stop-drain rules, codec-padding finalization, WGC live scheduler rebase with bounded source-selection clamping/fresh-catchup rejection, inject timer-rebase debt handling, borderless `dx12_av_sync_test.exe` stimulus captures, planned source-stall classification, real-session stutter triage, and the `run_av_sync_matrix.py` workflow. Last verified 2026-06-11. Stale-risk: medium.
