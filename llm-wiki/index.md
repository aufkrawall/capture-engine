...
- `graphics-overrides-and-frame-pacing.md`
  - Cross-API sampler/config semantics including normalized mip filtering, CPU/present queue-depth enforcement, adaptive rational FPS limiting (incl. gate-every-present native-Vulkan grid pacing), phase-preserving capture-sync/FG rules, the DLSS on-screen indicator, typed persistent UE5 RR-quality/post-processing/sharpen CVar bundles plus the `t.MaxFPS`/`r.MaxAnisotropy`+`r.VT.MaxAnisotropy` internal engine overrides, authoritative NGX/Streamline RR lifecycle evidence, the DRS-sourced DLSS FG render preset, the process-local NVIDIA LOD-spread quality fix (`nv_lod_spread_fix`, the validated branch patch applied by the Vulkan layer before device creation), the DLSS/Streamline runtime DLL override loader including the no-duplicate-instance redirect invariant, the open `ShowFlag.*` force-bit investigation (measured bit maps for UE 5.4.4/5.6.1, what is proven, ordered next steps), the Vulkan swapchain image-count floor (`backbuffer_count` may raise `minImageCount`, never lower it), the rule that CE never writes diagnostics to the host process's standard streams, diagnostics, and runtime validation matrix. Last verified 2026-08-19.
