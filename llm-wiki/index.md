...
- `graphics-overrides-and-frame-pacing.md`
  - Cross-API sampler/config semantics including normalized mip filtering, CPU/present queue-depth enforcement, adaptive rational FPS limiting (incl. gate-every-present native-Vulkan grid pacing), phase-preserving capture-sync/FG rules, Vulkan compute-present overlay compositing, the DLSS on-screen indicator, authoritative NGX/Streamline RR lifecycle evidence, the DRS-sourced DLSS FG render preset, the process-local NVIDIA LOD-spread quality fix (`nv_lod_spread_fix`, the validated branch patch applied by the Vulkan layer before device creation), the DLSS/Streamline runtime DLL override loader including the no-duplicate-instance redirect invariant, the Vulkan swapchain image-count floor (`backbuffer_count` may raise `minImageCount`, never lower it), the rule that CE never writes diagnostics to the host process's standard streams, diagnostics, and runtime validation matrix. The `[UE5]` console-variable overrides moved to `ue5-cvar-overrides.md`. Last verified 2026-08-22.
