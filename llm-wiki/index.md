...
Last cross-checked: 2026-07-27 (measured WGC/DXGI overload repeat pacing, attainable startup smoothness reservoir, adaptive CFR audio ingestion reservoir, immutable-grid encoder-debt recovery, grid-matched historical recovery, timeline-recovery-safe app audio, transition-aware capture diagnostics, DXGI source-limited cadence, delayed cursor ownership, HDR/SDR signaling, hardware encoders, monitor selection, and the existing current build, graphics, audio, screenshot, and security state; see cfr-capture-sync.md, wgc-capture.md, multi-audio-capture.md, regression-testing-and-logging.md, current.md, and log/recent.md)
...
- `wgc-capture.md`
  - WGC and DXGI Desktop Duplication backends, including service-measured proactive overload repeat pacing, retained-cap-bounded pre-live startup history, grid-relative deep-debt sync protection, grid-matched historical-frame recovery with held-repeat fallback on the immutable CFR output grid, timeline-recovery-safe audio, loss-aware copy-pool and actual-backend phase-lock diagnostics, stable single-monitor selection/fail-closed retargeting, HDR/SDR conversion and metadata/chroma, Windows-SDR-white cursor composition, delayed source-pointer ownership, max-rate CFR production, transactional handoff, and the FP16/R10 reservoir. Last verified 2026-07-27. Stale-risk: medium.
- `cfr-capture-sync.md`
  - Shared WGC/DXGI/inject CFR and A/V invariants, including service-measured WGC/DXGI overload repeat pacing, an attainable retained-cap-bounded startup reservoir, the adaptive audio ingestion reservoir against consumer-overrun starvation, immutable-grid deep-debt holds plus grid-matched historical-frame recovery, timeline-recovery-safe audio, transition-aware diagnostics, atomic warm-up cancellation, phase-preserving nearest source selection, contiguous screen-grab packet PTS, cursor-aware repeat rendering, packet-only validation, transactional startup, bounded source reservoirs, convergent inject recovery, and exact decoded stop/tail accounting. Last verified 2026-07-27. Stale-risk: medium.
- `nvenc-encoding.md`
