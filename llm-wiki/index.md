...
Last cross-checked: 2026-06-30 (updated: WGC source-rate smoothness reservoir sizing, millisecond-capped smoothness diagnostics, and source-coverage best-effort logging)
...
- `wgc-capture.md`
  - Windows Graphics Capture device/copy path, same-device low-overhead default with dedicated-device fallback, explicit 10-bit high-precision policy, CFR startup sync barrier with reserve-aware render-delay/smoothness handoff and startup reservoir fill budget, WGC copy-pool slot lease/generation ownership, split WGC source/copy VRAM budgeting, 125% source-frame smoothness reservoir sizing with millisecond-capped startup delay and diagnostics, surplus copy-pool pressure trim, uniform-playout ownership of source-above-target surplus plus sync-safe repeat rescue, source-coverage best-effort logging, adaptive overcapture, WGC CFR bounded live scheduler/source-selection policy, encoder-limited near-live smoothness mode, sync-delay source/policy hold attribution, source-limited smoothness ceiling triage, overload policy-fault attribution, audio-continuity/exact-stop drain guard, callback locking model, and `[WGC Perf]` / `[WGC CFR CADENCE EVENT]` telemetry. Last verified 2026-06-30. Stale-risk: medium.
- `cfr-capture-sync.md`
