...
- `dx12-injection-bootstrap.md`
  - Startup and late injection, cooperative resident deject/reactivation, process-exit dormant latching and process-lifetime state that outlives static destruction, generation-safe IPC ownership, capture-method/injection independence, active inject-video publication gating, CFG-safe hook bootstrap, the evidence-based Vulkan renderer gate plus exact DXGI creation pass-through and pre-injection direct-child profile inheritance, the implicit-layer transparency invariant (loader dispatch-key object ownership, both physical-device enumeration routes, never failing a call on CE's own bookkeeping), and the D3D12 device-creation forensics report plus its terminal-failure retry budget. Last verified 2026-08-28.
