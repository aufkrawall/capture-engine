...
Last cross-checked: 2026-07-26 (adaptive CFR audio ingestion reservoir, encoder-overload CFR content sync, grid-matched historical WGC/DXGI recovery, timeline-recovery-safe app audio, transition-aware capture diagnostics, DXGI source-limited cadence, delayed cursor ownership, HDR/SDR signaling, hardware encoders, monitor selection, and the existing current build, graphics, audio, screenshot, and security state; see cfr-capture-sync.md, multi-audio-capture.md, regression-testing-and-logging.md, current.md, and log/recent.md)
