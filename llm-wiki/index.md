...
- `build.py.md`
  - Supported `build.py` flags (including `--verify-clean` and `--skip-package`), automatic clean CaptureEngine/test-app 7z artifacts, content-addressed clang-tidy and exact-input sanitizer reuse, `--verify` content-validated object reuse with the strict clean rebuild behind `--verify-clean`, isolated/concurrent verification stages, early preflight, Windows Clang/Linux MinGW GCC hardening and strict-FP selection, host-native PE/shader/debug verification, cross-host FG SDK preparation, fail-closed Vulkan stages, validated object/link caches, explicit per-architecture test-app task identity, the nested gate rule and default tests-only development loop, scope-aware ratchets, incremental/resume/no-build workflow, dependency closure (vendored PGP keys, toolchain CA bundle + bounded download retry, and the appended source-dependency build policy: `package_outputs`-only subpackages and path-independent documentation output for MAX_PATH), manual-only hardening CI and self-hosted native Windows stable-release automation (runner is this PC, started manually via `run.cmd`; delete-then-trigger procedure) with public-repo artifact attestation, privacy-sanitized release artifacts (prefix-mapped PDB paths, bare PDB names in images, redacted manifests, fail-closed finalize scan), operational notes, and MinGW pitfalls. Last verified 2026-08-07.
- `screenshots.md`
