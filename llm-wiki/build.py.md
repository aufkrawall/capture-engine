...
- **Required versus optional Vulkan coverage**: x64 Vulkan import libraries and successful hook/layer/test-app links are required on every supported host. Missing x64 inputs or a Vulkan-layer link/manifest failure must propagate immediately; do not log and continue into a misleading partial-success build. Linux x86 Vulkan coverage remains optional because the i686 import library may be absent. Every mixed compile queue must assign a unique normalized object output to each task so basename collisions cannot race or silently replace layer inputs. Source anchors: `build.py` and `tools/tests/test_build_flags.py`.
