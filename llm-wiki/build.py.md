...
- The post-link verifier scans every shipped first-party/source-built PE for expected architecture, ASLR, NX, x64 high-entropy VA, non-writable/executable sections, complete runtime imports, and first-party PDBs. It also requires effective Guard CF metadata and a non-empty target table for x64 and source-built runtime PEs. The current clang64-to-mingw32 linker emits empty x86 Guard CF target tables; at the user's direction only that x86 CFG-table requirement is deferred through `--allow-missing-x86-cfg`, while every other x86 check remains mandatory. Do not interpret this exception as completed x86 CFG work.
