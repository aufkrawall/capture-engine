...
- Cache and compile-database snapshot JSON publication uses one uniquely created staging file per writer. Each writer
  snapshots the destination file identity before staging; under the final in-process publication lock, only a writer
  that still sees that same generation performs the atomic replacement. Overlapping writers are therefore
  first-publisher-wins and discard stale staging files instead of repeatedly replacing a just-published Windows file,
  while a later sequential writer observes the new identity and can replace it normally. A PID-only staging name is
  unsafe because concurrent worker writes can overwrite or move the same temporary file; clang-tidy execution itself
  remains parallel.
...
- The sanitizer child sets `CE_ISOLATED_BUILD_ROOT=build/stages/sanitize`, separating objects, installed binaries, unit-test outputs, temp files, compile commands, and generated SDK headers from the product build. On cache misses, the two builds share the requested worker budget and run concurrently. Shared SDK archives are prepared before the split. The Vulkan layer follows `OBJ_DIR` too (fixed 2026-08-13): it previously hardcoded `PROJECT_ROOT/build/obj/<arch>/vulkan_layer`, so the concurrent sanitizer child's ASan-instrumented layer objects landed in the product cache and the product layer link failed with undefined `__asan*`/`__ubsan*` symbols, forcing a full `--verify --force-rebuild` rerun (sessions 20260813_180126, 20260813_180535). When the child finishes, the product build releases the reserved worker budget for the remaining stages (unit tests, captureengine, test apps, vulkan layer).
...
clang-format findings are advisory: the stage reports style differences without failing (92 files differed as of 2026-07-25) and `step.lint=warning` does not fail `--verify`. Wrap-column differences on newly added lines are not worth an edit or a rebuild, and `AGENTS.md` forbids running the formatter over existing files. Since 2026-08-13 clang-format checks only C++ sources changed in the worktree (`collect_changed_lintable_cpp_sources`: `git diff HEAD` plus untracked files, with a full-set fallback when git is unavailable), so the ~17 s scan over 575 unchanged files is gone; a run with no changed C++ sources logs `clang-format: no changed C++ sources to check`.
...
- **Lint before dispatching a stable release (added 2026-09-21):** the release job runs `--verify --verify-clean`, and its *preflight* is a read-only static gate that the routine per-change gate never executes. `0.1.6767` died there in 74 seconds on a file-size violation (`hook_common.cpp` at 813 lines), and two further gates were queued behind it: the source-line-length baseline (`dxgi_shared_internal.h` 228 -> 239 characters) and the clang-tidy ratchet (`clang-analyzer-core.CallAndMessage` 26 > 25, a re-analysis effect of a changed header, not a changed .cpp). Run `python build.py --no-build --lint --skip-updates --concise` after a full product build and before dispatching; a green build+test gate says nothing about any of these. Adding a translation unit also re-emits its headers' diagnostics into the clang-tidy totals, so a split file should carry only the includes it needs.
