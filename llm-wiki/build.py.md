...
- **Unverified on posix-threads MinGW hosts:** the clean Linux build cannot finish on Arch, whose `mingw-w64-gcc` 16.1.0 uses the posix thread model. `mediaengine/video_encoder_part_005.inl:447` and `video_encoder_part_009.inl:241` assign `writerThread.native_handle()` directly to `HANDLE`, which only compiles where `std::thread::native_handle_type` *is* `HANDLE` — MSYS2 clang64's Win32 thread API, and Ubuntu CI's `update-alternatives` default of the win32 model. A winpthreads `pthread_t` is not a Win32 thread handle, so this needs `pthread_getw32threadhandle_np()` behind an `if constexpr` model check or a thread that publishes its own duplicated handle, not a cast. Untouched so far: product code on the encoder finalize path, requiring `--verify` plus Windows runtime validation.
