...
Last cross-checked: 2026-08-07 (release artifacts are now privacy-sanitized: prefix-mapped PDB paths, bare PDB names in images, redacted manifests, fail-closed finalize scan; stable-release operations document the manual self-hosted runner start and the delete-then-trigger procedure; successful product builds emit clean, verified CaptureEngine and first-party test-app 7z archives; `--verify` reuses content-validated objects by default with `--verify-clean` for the strict clean rebuild, and `--skip-package` skips only the automatic 7z archives for dev iteration; previously 2026-08-06)
...
- **GitHub Actions release automation (2026-08-02):** `hardening-ci.yml` is manual-only (`workflow_dispatch`); push/PR CI is disabled. `release-stable.yml` builds a stable release on manual dispatch on the maintainer's self-hosted Windows runner (`runs-on: [self-hosted, Windows, X64]`) with the canonical native command `python build.py --skip-updates`, so release artifacts match a local Windows build: PDBs, effective x64 CFG, the custom-patched FFmpeg closure, and FG test-app runtime payloads. The runner workspace is a dedicated persistent clone; the workflow syncs it with plain `git fetch`/`git switch -C main origin/main` and deliberately does not use `actions/checkout`, because that action deletes the entire workspace when its expected repository URL differs from the configured fetch URL (even by the trailing `.git`), and its Windows `rmRF` follows directory junctions - this destroyed the dev toolchain during the first end-to-end test (see `log/recent.md`). A `Prepare persistent toolchain junctions` step creates junctions for `build/msys64`, `ffmpeg_build/`, and `external/` from repository variable `CE_TOOLCHAIN_ROOT` (the maintainer's dev checkout) and fails closed if those trees are missing or already exist without being junctions; do not run a concurrent dev build against shared junctions. Git sync and publishing use the runner's `gh` credentials (`gh auth setup-git`); on this runner the `GITHUB_TOKEN` environment variable arrives empty (broker flow) while the `github.token` context is populated, so the job sets `GITHUB_TOKEN: ${{ github.token }}` explicitly for steps/actions that read the env var. The workflow seeds `build/build_number.txt` with N-1 so the build mints exactly `0.1.N`, and fails closed when the built `CAPTURE_VERSION` or an already-existing tag contradicts the requested version. New `attest` dispatch input (`auto`/`always`/`never`, default `auto`) runs `actions/attest@v4` over the four release assets before publishing; `auto` queries the repository visibility via `gh api` and attests only when the repository is public, because GitHub Free/Pro/Team artifact attestation is restricted to public repositories (private requires Enterprise Cloud), and `always` fails closed on a private repo. GitHub CLI (`gh`) must be installed on the runner (the job preflights it). End-to-end test on 2026-08-02 published native `v0.1.5268` with all four release assets. Enabling immutable releases in repository settings additionally generates automatic release attestations verifiable with `gh release verify` / `gh release verify-asset`.
- **Stable release operations - runner start and trigger (2026-08-07):** the release runner is the maintainer's Windows PC itself, installed at `%USERPROFILE%\Programme\build\actions-runner` (work folder `%USERPROFILE%\Programme\build\runner-work`), registered to this repository as a self-hosted runner (broker V2 flow). It is **not** a Windows service and has no scheduled task (`runsvc.exe` is absent), so it is started manually and stops when its console closes or after the job finishes; GitHub reports it offline unless `Runner.Listener.exe` is running. To publish a stable release: (1) ensure the runner is up - `Start-Process -FilePath "$env:USERPROFILE\Programme\build\actions-runner\run.cmd" -WorkingDirectory "$env:USERPROFILE\Programme\build\actions-runner" -WindowStyle Hidden`, then confirm `gh api repos/aufkrawall/capture-engine/actions/runners --jq '.runners[] | {name, status}'` reports `online`; (2) if replacing the previous release, delete it first with `gh release delete v0.1.N --yes --cleanup-tag` (and local `git tag -d v0.1.N`); (3) trigger with `gh workflow run release-stable.yml --ref main --field version=0.1.N` where N = previous N + 1 - an explicit version is required once no `v0.1.*` tag remains, because the empty-input auto-derive needs an existing tag; (4) watch via `gh run view <id> --json status` / `gh run watch <id>`; a stable build takes tens of minutes on the self-hosted runner. The run builds from the `origin/main` head at dispatch time and publishes tag `v0.1.N` plus release "Stable release 0.1.N" with the four assets (`captureengine.7z`, `testapps.7z`, `latest_manifest.json`, `latest_summary.txt`). Release assets are privacy-sanitized (see the next bullet); after a privacy fix, delete the leaky release/tag and re-run the workflow with the same version to replace the assets.
- **Release asset privacy (2026-08-07):** release artifacts must never leak the developer's Windows user name. The 0.1.5290 release embedded `C:\Users\<developer>\...` paths in `latest_manifest.json`/`latest_summary.txt` (run dir, command, artifact paths) and - far worse - in the shipped PDBs (hundreds to thousands of occurrences per PDB from source paths, object paths, and the compiler/linker command-line records) plus the absolute PDB path in every PE's RSDS debug record. Fixes, enforced by `tools/tests/test_privacy_paths.py` and a fail-closed finalize scan: (1) every native compile flag list now gets `-ffile-prefix-map` for both profile spellings, so debug-info source paths become `C:\Users\<developer>\...`; (2) `append_windows_pdb_linker_flag` adds `-Wl,/pdbaltpath:<bare>.pdb`, so images embed only the PDB file name (debuggers still find the sidecar via module directory / symbol path); (3) the finalize stage (`scrub_and_verify_privacy_paths`) redacts the user-name path component in shipped PDBs and third-party FFmpeg-closure DLLs in place with a length-identical `redact` filler (PDB records are length-prefixed, so byte offsets must not change), covering every spelling the tools emit - plain and escaped (doubled) backslashes in compiler/linker command-line records, forward slashes, and MSYS drive paths like `/c/Users/<user>` - in UTF-8 and UTF-16LE, and fails if any shipped first-party PE/PDB still contains a user-name path component; (4) `latest_manifest.json`/`latest_summary.txt` are written with the profile root replaced by `C:\Users\<developer>`. GitHub Actions run logs are covered too: the release workflow sets `CE_PRIVACY_SANITIZE_LOGS=1` so `log()` redacts the profile from console output, `actions/setup-python` is not used (it dumps the toolcache path into run logs; the self-hosted runner needs Python 3.12+ on PATH, preflighted), and the junction step prints `%USERPROFILE%`-redacted roots. The automatic `Runner name`/`Machine name` lines GitHub adds to every self-hosted run still contain the runner/hostname - rename the runner registration and/or the Windows host to fully remove those. Sanitizer/isolated builds skip the binary scan because their ASan runtime is third-party and nothing ships from them. Symbolization is unaffected: module matching (RSDS GUID+age), types, and line numbers survive the length-preserving scrub, verified with `llvm-pdbutil` and a real `cdb -z` crash-dump session (private PDB symbols + `@ line` records resolve). To open actual source files from the debugger, set a source-path mapping (`_NT_SOURCE_PATH=C:\Users\<developer>\Programme\build\captureproject` or cdb `.srcpath`) because the embedded paths are redacted. Source anchors: `tools/build/build_privacy.py` (helpers + flag lists + PDB link flags + `privacy_sanitize_log_text`), `tools/build/build_bootstrap.py` (manifest/summary redaction), `tools/build/build_project_finalize.py` (scrub + scan), `.github/workflows/release-stable.yml` (sanitized logs, no setup-python), `tools/tests/test_privacy_paths.py`.
