...
- **A write into a flip chain's front buffer is not a presentation while the chain is being flipped.** The next flip replaces it, so compositing there writes the surface the display is scanning out for a frame nobody sees. `ScanoutWriteIsPresentation` judges this by structure, not by a clock: a flip resets the run of scanout writes, a write extends it, and only a run reaching `kScanoutWritesWithoutFlipThreshold` without a flip answering it counts. Gothic II gameplay (86 flips/s, 11 front-buffer writes/s) never reaches it; a loading screen, where nothing flips, reaches it on the second write.
