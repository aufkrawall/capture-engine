...
- All telemetry readers first validate shared-memory ABI 37's exact version, size, layout fingerprint, and discovery build identity. ABI 37 additionally lets media publish a separate seqlocked screen-grab target PID/capture-device LUID for WGC/DXGI sensor attribution without touching hook-owned source identity. Version/fingerprint isolation prevents an old reader from interpreting shifted fields; range/finite/validity checks remain a second line of defense.
