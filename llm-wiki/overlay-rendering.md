...
- `hook/common/overlay_cpu_raster.{h,cpp}`
- `hook/common/ddraw_native_overlay_damage.h`
- `hook/apis/ddraw_hook_overlay_composite.cpp`
- `hook/apis/ddraw_hook_overlay_route.cpp`
- `hook/apis/ddraw_hook_write_tracking.{h,cpp}`
- `hook/apis/ddraw_hook_capture_frame.cpp`
- `tests/test_ddraw_present_policy.cpp`
- `tests/test_legacy_d3d7_vtable_abi.cpp`
- `hook/common/legacy_d3d_texture_bindings.h`
- `hook/apis/ddraw_hook_texture_bindings.cpp`
...
- **CE never keeps an application's presentation chain alive past the application's own references.** DirectDraw allows one primary per DirectDraw object, and a primary is gone only with its last reference. CE takes references into the chain: the CPU-prerender queue holds the surface it last presented (for a Flip, the primary itself), `directDrawOwner` holds the DirectDraw object, and the tracked D3D7 device (`TrackLegacyD3D7Device`) plus the native sidecar (`D3D7Backend`) hold the device, which holds its render target - the chain's back buffer. The chain reset used to run only after a *successful* primary creation, which those references made impossible: Gothic II session `20260924_233030` re-created its primary after a focus loss four times, got `DDERR_PRIMARYSURFACEALREADYEXISTS` each time and quit through its own `Error-Message` box. `ReleaseDirectDrawChainBeforePrimaryCreation` now runs before every application primary creation is forwarded (legacy, DirectDraw4 and DirectDraw7 detours, gated by `ce::ddraw_chain_lifetime::ShouldReleaseChainBeforeCreation`), drops all of the above including the tracked device - released outside `ddraw_hook_g_DDrawIdentityMutex`, since it is normally the last reference and Direct3D tears its render target down through hooked DirectDraw interfaces - and clears the raw primary identities. The D3D7 `EndScene` detour re-tracks the device every frame, so the sidecar re-primes even in a title that stops calling `SetTextureStageState`. Diagnostics: `Released CE's references into the previous presentation chain ... (presentationRefs= nativeSidecar= d3d7Device=)` or `CE held no references into a previous chain`, and `Application <api> primary creation FAILED hr= (<DDERR name>)`. Stale-risk: the post-alt-tab `DDERR_UNSUPPORTEDMODE` of `20260916_014133` is likely the same class but unverified; an application `SetCooperativeLevel(DDSCL_EXCLUSIVE)` on a *new* DirectDraw object while CE still references the old one is not a release boundary yet (not hooked; not observed). `hook/common/ddraw_chain_lifetime_policy.h`, `hook/apis/ddraw_hook_overlay_route.cpp`, `tests/test_ddraw_chain_lifetime.cpp`. Last verified 2026-09-24 (unit + real-DirectDraw contract; in-game focus-loss run pending).
