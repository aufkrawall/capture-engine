...
- **No presentation detour may re-enter itself, and the way out is never its own saved original.** CE hooks the DirectDraw surface vtable's Flip/Blt/BltFast slots and keeps the pointer it replaced. A second overlay that hooks the same slots, in the wrong order, ends up holding CE's detour as *its* original while CE holds that overlay's - so "call the original" is an infinite mutual call. Gothic II session `20260916_011148` is that cycle with Steam's `gameoverlayrenderer`, three frames repeating down eight megabytes of the render thread's stack with a full 4K composite per level; 3,600 levels over thirteen seconds looked exactly like a game presenting at ~270/s, and not one flip completed. Validated in session `20260916_014133`, the first clean Gothic II run: sidecar primed, `route=native-d3d7`, `nativeFail=0` over 4,040 flips, and the guard logged nothing. **A nested presentation returns without calling anything** - the only answer that cannot recurse by construction, at the cost of one dropped presentation. Session `20260916_013230` is why it is stated that strongly: an earlier attempt called an entry point captured from the vtable before CE patched it and validated as ddraw-owned, and the cycle continued regardless - 32,768 levels in two milliseconds, the game dead five seconds later, because removing the composite from each level only made the stack run out faster. `NoteDirectDrawPresentCycle` instead names the module that re-entered CE, CE's saved original, and the recorded entry point, since the recursion alone cannot distinguish a co-resident overlay from DirectDraw from CE itself. `VTableHook::Create` supplies the other half before anything recurses: when the pointer CE keeps as "the original" is code from outside the module that owns the vtable, CE is joining another injector's chain - the precondition for the cycle - and logs which module, and whether it is a recognized overlay. CE cannot stop that injector reinstalling after it, so this stays diagnostic; no hooking behaviour depends on it. A nested blit that is not classified as a presentation is ordinary CE capture work and passes through. `hook/apis/ddraw_hook_present_reentry.cpp`.
