...
- All telemetry readers first validate shared-memory ABI 36's exact version, size, layout fingerprint, and discovery build identity. This is essential because adding the publication sequence shifted every old telemetry field: an old reader displayed the sequence as CPU, CPU as RAM, RAM as GPU, GPU as VRAM, and the low VRAM-capacity word as maximum-core load. Range/finite/validity checks now reject an impossible coherent publication as a second line of defense.
