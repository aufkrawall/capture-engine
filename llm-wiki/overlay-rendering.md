...
- **No presentation detour may re-enter itself, and the way out is never its own saved original.** CE hooks the DirectDraw surface vtable's Flip/Blt/BltFast slots and keeps the pointer it replaced. A second overlay that hooks the same slots, in the wrong order, ends up holding CE's detour as *its* original while CE holds that overlay's - so "call the original" is an infinite mutual call. Gothic II session `20260916_011148` is that cycle with Steam's `gameoverlayrenderer`, three frames repeating down eight megabytes of the render thread's stack with a full 4K composite per level; 3,600 levels over thirteen seconds looked exactly like a game presenting at ~270/s, and not one flip completed. `RecordDirectDrawPresentEntryPoints` snapshots those slots before CE patches them and keeps each only if it lies inside `ddraw.dll`; a re-entered detour calls that instead, so the cycle ends at its first bounce with the flip still performed. With no recorded entry point the nested call returns without presenting - a dropped frame is bounded, a hang is not. A nested blit that is not classified as a presentation is ordinary CE capture work and passes through. `hook/apis/ddraw_hook_present_reentry.cpp`.
