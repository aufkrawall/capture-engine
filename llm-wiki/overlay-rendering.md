...
- `hook/common/custom_overlay_d3d7.{h,cpp}`
...
- **DirectDraw/DX6/DX7 draw the overlay with the application's own Direct3D 7 device when it owns the surface being presented.** `CustomOverlay::D3D7Backend` converts the shared draw list to `D3DTLVERTEX`, uploads the font atlas once as a managed ARGB8888 texture surface, and issues the same handful of `DrawIndexedPrimitive` calls the DX9 backend issues. Nothing leaves the GPU: no readback, no second device, no staging surfaces. `TryDrawNativeLegacyD3DOverlay` takes this route only when the device's current render target *is* the surface this presentation publishes, and rebuilds the backend when the application recreates its device. Device state is saved and restored with a `D3DSBT_ALL` state block; without one the overlay suppresses itself rather than leaking its blend and stage setup into the application's next draw. `d3d.h`/`d3dtypes.h` redefine enumerators `d3d9.h` also defines, so they appear in exactly one translation unit, the backend's interface is `void*`, and the device methods the hook calls itself go by vtable index (pinned by `LegacyD3D7VTableAbiTest`). CE's own state calls run inside `LegacyD3DInternalScope` so the forced-filtering layer does not record them as the application's.
- **The backend the adapter holds must match the route actually executing, and nothing renders while they disagree.** `ce::ddraw_present_policy::SelectOverlayRoute` derives the route from the presentation - only a flip the device actually rendered into can use the native path - and `BackendCanRenderRoute` is the guard. `hook/apis/ddraw_hook_overlay_route.cpp` switches the adapter before anything renders and **confirms the loaded backend afterwards**, because `OverlayAdapter::Init*` returns true without doing anything when an adapter is already initialized. That silent success is what crashed Gothic II on startup (session `20260914_182411`): the composite route ran with the backend still bound to the application's Direct3D 7 device, so every frame issued `BeginScene`, state changes, a draw and `EndScene` on the game's own device between CE's own Lock and Unlock of the presented surface and outside `LegacyD3DInternalScope`, then read back a helper backbuffer the overlay had never been drawn into. Route switches are counted and bounded at eight, after which the route latches on the composite; `DDraw: Presentation mix` reports `route=`, `routeSwitches=` and `routeLatched=`.
- **The D3D9Ex composite remains the fallback for everything else** - a pure 2D DirectDraw title, a DX6 device, a presentation into a surface the D3D device is not rendering to. It costs a CPU round trip per present and ends in `GetRenderTargetData`, which blocks the render thread until the GPU catches up, so it is not the preferred path on any title that can avoid it. It moves only the overlay's own bounding rectangle (`Renderer::GetDrawBounds` aligned to 64 pixels: roughly 1 MB each way at 4K instead of 33 MB), and a frame whose overlay grows past the staged rectangle re-stages the union and re-submits the same geometry via `OverlayAdapter::ResubmitLastFrame` rather than being clipped. Its helper device is created lazily, and the adapter LUID the host's telemetry needs is published from a device-less `Direct3DCreate9Ex`. The helper swapchain `PresentEx` is a last-resort fallback only - it returns `S_PRESENT_OCCLUDED` for as long as the application holds the display.
- **The native Direct3D 7 route is opt-in (`[Graphics] legacy_d3d_native_overlay`, default off).** It has taken a co-resident Steam overlay down twice in Gothic II with an identical fault inside `gameoverlayrenderer.dll` (sessions `20260914_182411` and `20260914_183948`), and CE's crash dumps carry no 32-bit stack for a WoW64 target - the x64 external helper's `kRichCrashDumpType` records the native stacks only - so it cannot be attributed further. The composite draws the same overlay on every title.
- **The composite must never blend the overlay over its own previous output.** It is a read-modify-write, so the pixels it reads have to be the application's. A flip or a blit-present brings a newly produced image and is clean by construction; consecutive direct writes into the same scanout surface are not, because the application may have changed only part of it. A translucent overlay then darkens with every repetition and the next flip restores a clean one - Gothic II's intro logos (about ten front-buffer writes per flip) made that look like the overlay's values were flashing in and out. `CompositeBackdropIsReusable` governs it: the application's pixels for the region are read once per (surface, rectangle) and reused for every repeat composite into the same place, discarded as soon as something republishes that surface. Repeat composites are then idempotent, so front-buffer writes never have to be suppressed - an earlier attempt that suppressed them dropped the overlay for whole loading screens, where the game flips occasionally between its writes.
- **The overlay always goes into the image the presentation publishes, never into the one already on screen.** `ce::ddraw_present_policy` (`hook/common/ddraw_present_policy.h`) decides which that is: `Flip` publishes its target, a full-surface blit onto a single-buffered scanout surface publishes its source, and a blit onto a flip chain's *front* buffer or an `Unlock` of the primary is already visible so the overlay is restored on top. Only a **back** buffer is published later by Flip. Getting that distinction wrong in either direction has a measured signature: compositing into the visible surface after a Flip races scanout and the next flip discards it (Gothic II flicker, session `20260914_173658`); suppressing front-buffer blits because the primary heads a flip chain drops the overlay for the whole of a loading screen, where the 3D scene is not running and nothing flips (session `20260914_180020`). When the published image cannot be resolved CE composites nothing rather than falling back to the visible surface. A partial blit only restores the overlay when it intersects it. The per-ten-second `DDraw: Presentation mix` line reports the counts, because a route that composites nothing is otherwise indistinguishable from one that is never called.
