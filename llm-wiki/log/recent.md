...
### 2026-08-12 - CORRECTION + the real defect: the overlay went to a queue the FG runtime does not flush (0.1.5972)

- **The 0.1.5971 revert was based on a misread of CE's own diagnostic and is itself reverted.** The freeze line reads `[overlay-gpu-breadcrumb] … lastCompletedOp=0 (none (GPU never reached the overlay list this frame — **CE's list is NOT the stall**))`. The parenthetical is the verdict, and it exonerates CE. The frozen thread confirms it: `# 51 Id: 5a20.71e8 "RHIThread"` is in `ntdll!NtWaitForSingleObject` inside pure `Talos1_Win64_Shipping` frames — no CE, no DXGI, no D3D12, no present anywhere on that stack. The session also has exactly one 29.7 ms `TOTAL SLOW` present in total, and the overlay-completion wait is a 16 ms bounded wait (`kCompatWaitTimeoutMs`), not an unbounded one, so it cannot produce a 54 s freeze.
- What the same session DOES prove is a real defect in the 0.1.5970 attempt: **the below-the-chain overlay draw never landed once.** The FFX callback logged zero `YIELDS` edges for the whole session, which by construction means no other route ever recorded a draw. The reason is the submit queue: `SubmitOverlayCommandList` uses `gameQueue` whenever the dedicated queue is disabled (and it is disabled for runtime-owned FG), but while an FG runtime owns the swapchain the game's queue is not what gets flushed — the runtime presents from and drains the swapchain-owning queue. During a save load the game queue is idle outright: `origGame=0000021BFDCE3CD0` vs `scQueue=0000021B8FAAA6F0`.
- Fix: while `dx12_hook_g_FGRuntimeOwnsSwapchain` and a swapchain-owning queue exists, a non-dedicated overlay submit is routed to that queue. It is the queue the runtime's own present callback records into and the one the FG action plan already selects (`FGQueueRole::kFFXCallbackQueue` -> `snapshot.swapchainQueue`); only the submit site was not consulting it. Same shape as the 0.1.5922 DLSS-FG fix ("route late-inject DLSS-FG overlay submits to the swapchain-owning queue"). Log marker: `DX12: Routing … to the swapchain-owning queue while the FG runtime owns presentation (scQueue=… gameQueue=…)`.
- The fail-safe from 0.1.5970 is unchanged and now actually has something to arbitrate: the FFX callback yields only once another route's draw has been recorded, and resumes on the very next frame if that stops — so a submit that still does not complete costs at most one frame of the callback's own draw, never a missing overlay.
- Validation pending: Talos + Steam + RTSS with FSR FG on, INCLUDING loading a save into 3D rendering. Expect `Routing … to the swapchain-owning queue`, then a `YIELDS` edge, CE's overlay over Steam's, `[OVERLAY DOUBLE-DRAW]` absent. If the load freezes again, capture the freeze dump and read the RHIThread stack first — the previous one had no CE frames at all.
