...
Fix: `hWnd` is created with `WS_EX_TOPMOST`; before displaying the menu, `WS_EX_NOACTIVATE` is temporarily cleared, `hWnd` is brought to the foreground and topmost (`AttachThreadInput` if needed); `TrackPopupMenuEx` is used with `TPMPARAMS.rcExclude` bound to the detected taskbar rect and `TPM_BOTTOMALIGN | TPM_VERTICAL` to anchor the menu above bottom-docked taskbars. Windows popup menus (`#32768`) naturally inherit the topmost Z-order of their owner window (`hWnd`); modifying `#32768` creation parameters inside a `WH_CBT` hook caused User32 menu window creation to abort (`ERROR_FILE_NOT_FOUND`), so CBT hooking was removed and `WM_INITMENUPOPUP` safely reinforces `HWND_TOPMOST` post-creation. Unit coverage in `tests/test_tray_source.cpp`.
