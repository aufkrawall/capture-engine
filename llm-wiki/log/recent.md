...
- Validation pending: re-run the FG switch matrix (off <-> DLSS FG <-> FSR FG) from Steam with RTSS active.
...
- Validation pending: re-run `dx12_fg_switch_test` from Steam with RTSS active, FG off at start, then the full FG switch matrix (off <-> DLSS FG <-> FSR FG). Expect `CE intercepts BELOW the foreign Present chain via a deep body hook` and a visible CE overlay while Steam's and RTSS's overlays keep drawing.
