...
- `hook/common/fps_limiter.h` / `hook/common/reflex_limiter.h`
  ...
  - Explicit CE-owned Reflex local cadence should also log `queue=1` and pair with `ApplyPresentFrameLatencyOverrides: SetMaximumFrameLatency(1) OK (explicit_reflex_limiter, ...)` plus `DX11/DX12: Implicit prerender limit 1 for explicit Reflex FPS limiter` when no user prerender override is set. This closes the remaining queue-depth latency gap after the post-Present cadence fix.
  - Game-owned Reflex handoff remains intentionally stricter in auto/non-manual paths: require a fresh stable `slReflexSleep` streak and no recent present-gap churn before skipping CE's local cadence. Manual explicit Reflex mode may use CE-owned local cadence even while DLSS FG is active until that stable handoff exists; this preserves GTA Reflex/DLSS FG switching behavior because GTA's comparison path does not enable the manual CE Reflex limiter.
- `hook/apis/streamline_hook.cpp`
...
- If you touch explicit Reflex limiter mode, verify that `fps_limiter_trace.log` does not merely report `limiter=reflex` while timer fallback is active. The trace should show either native/game sleep handoff or CE-owned cadence plus NvAPI Sleep success, and fallback diagnostics should include `device=1`/`device=0` plus PushFpsLimit/Sleep status. For DXGI/DX12 Talos latency validation, prefer the post-present shape (`Apply: REFLEX post-present cadence ... queue=1`) over a pre-Present local wait, and verify the hook log shows the explicit Reflex frame-queue/prerender clamp. Manual Reflex + DLSS FG should stay in the manual/explicit path unless a stable game Sleep handoff is proven; GTA game-owned Reflex/DLSS FG switching should remain outside the manual clamp path when the CE manual limiter is not enabled.
