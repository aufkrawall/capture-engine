...
- Fresh application routing comes from named `[Profile.*]` sections. `video_capture=inherit|inject|wgc|dxgi_dup|none` selects the video source; `dll_injection=when_needed|always|never` separately controls DLL loading. `always` resolves to the injector's full route for injected video and overlay-only route otherwise, while `when_needed` injects only for injected video. Explicit WGC/DXGI video therefore preserves the injected overlay and graphics overrides only when requested. Legacy `injection_mode`, `injection`, `whitelist`, and `overlay_whitelist` remain parser inputs with their historical behavior.
