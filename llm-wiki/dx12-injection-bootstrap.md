...
Last cross-checked: 2026-09-23 (UTF-16 injection path; Vulkan layer negotiation membership and no-host target-list staging gap)
...
- **Vulkan layer membership is decided at negotiation (2026-09-23).** With a compatible host published, `vkNegotiateLoaderLayerInterfaceVersion` admits only processes allowed by `LayerIPC_IsProcessEligibleByCurrentHost`; other processes return `VK_ERROR_INITIALIZATION_FAILED` and the loader can create the instance without CE. DllMain no longer pins the module; negotiation pins it only after admission. This host-present gate keeps CE out of profiles marked `dll_injection=never` and other non-target applications. With no host published, the layer admits exactly the executables in the whitelist the inject child persisted as the REG_MULTI_SZ value `HKCU\Software\CaptureEngine\VulkanLayerTargets` (`PersistVulkanLayerTargetList` writes, `IsListedAsResidentTarget` reads, both through `common/vulkan_layer_target_list.h`). **The list must never be located relative to either module:** the layer runs from a versioned staging copy (`vulkan_layers\b<build>`, `common/vulkan_layer_registration.cpp`) that the injector does not know and that a resident older-build layer does not share with a newer host. The first version wrote `vulkan_layer_targets.txt` beside `captureengine.exe` and read it beside the staged DLL, so every Vulkan title started before CaptureEngine was declined and could not late-wake; `tests/test_vulkan_layer_target_list.cpp` guards the shared location. No-host decisions are not logged at all: layer logging requires a published host (its debug flag and session logs path), so the end-to-end check below is the diagnostic for that path. Stale-risk: an elevated host running under a *different* account than the game writes that account's HKCU. The participating-target dormant path still uses passthrough metadata and wakes on the next present after host reactivation. `DISABLE_CE_VULKAN_LAYER=1` and `layer_register.exe --unregister` (which also deletes the persisted list) remain the explicit controls.
- **The manifest names a negotiation gate, not the full layer (2026-09-23).** Declining at negotiation still let the loader *map* the library first: every Vulkan process on the machine loaded the 1.5 MB `VK_LAYER_CE_overlay.dll`, pulled in dxgi/user32/gdi32/winmm/version, ran its static initialization and held a file lock, only to be declined. The staged manifest now names `VK_LAYER_CE_gate[_x86].dll` (`hook/vulkan_layer/layer_gate.cpp`; imports kernel32/advapi32 and the CRT only). It decides with `layer_participation.cpp`, the same unit the full layer uses, and only for an admitted process loads the full layer from its own directory and returns the full layer's negotiation result, so the loader then calls the full layer directly. When the full layer declines anyway (a host appeared in between), the gate releases it with `FreeLibrary`. The full layer still decides again itself and stays correct when loaded directly. **The gate must export `vkNegotiateLoaderLayerInterfaceVersion` only:** current Vulkan-Loader skips a layer whose negotiation fails (`loader_create_instance_chain`), but a gate that also exported `vkGetInstanceProcAddr` could be chained by a loader that falls back to that export. `tools/verify_vulkan_layer_exports.py` fails the build on either a missing or a forbidden gate export, and the manifest's `functions` block names negotiation only. `LayerManifest::IsUsable` requires both images; a full layer without its gate is not registered. End-to-end: `testapp/run_vulkan_layer_participation.py` (run by `--verify-runtime`, x64 and x86) probes `vulkan_test.exe --probe-ce-layer`, which reports which layer images are mapped, under a whitelisted and an unlisted executable name, with CaptureEngine running and then killed. It also checks that the persisted list names the test app, and refuses to run the no-host cases while any process still holds the discovery mapping. Not yet run on hardware.
...
- `RepairOwnedRegistrations` prunes only *superseded* CE entries (a previous install directory, the wrong HKLM registry view, a manifest no longer on disk) and retains the entries this instance is about to rewrite, so the live registration is never momentarily absent for a title starting right now. `SelectStaleOwnedEntries` is the pure policy behind it and matches CE manifest file names only — a foreign implicit layer in the same key is never eligible for deletion. **`BuildRepairScopes` visits each physical key once:** WOW64 redirects `HKLM\Software` but *shares* `HKCU\Software`, so both HKCU install targets (x64 and x86) write the same key and its single scope retains both manifests; the two HKLM views stay separate scopes, each retaining only its own architecture. Until 2026-09-26 the repair treated HKCU/64 and HKCU/32 as two keys, so each pass deleted the other architecture's live entry as a "wrong-view" leftover and both stayed absent for ~18 ms, through artifact staging, on every CE start (`logs/20260926_044427`). `tests/test_vulkan_layer_registration.cpp` covers the scopes and checks the shared-view premise against the running OS (`HKCUSoftwareIsSharedBetweenRegistryViews`). The repair logs one `[VulkanReg] Owned-entry repair <scope>: retaining N ..., pruning M` line per scope.
- **Vulkan layer registration is decoupled from the installation tree via runtime staging.** Registering `installed/captureengine/` directly caused external non-whitelisted processes (Explorer, Chrome, Discord, `DataExchangeHost.exe`, etc.) to map `VK_LAYER_CE_overlay.dll` into their address space whenever any Vulkan enumeration occurred. This held file locks preventing replacing or rebuilding `installed/captureengine/` binaries even when CaptureEngine was closed. `BuildRegistrationPlan` and `ApplyRegistrationPlan` (`common/vulkan_layer_registration.{h,cpp}`) stage layer manifests and DLLs into `%LOCALAPPDATA%\CaptureEngine\vulkan_layers\b<build_number>\` (or `%PROGRAMDATA%` when elevated), registering the staged manifest paths in the registry. External processes lock only the staged copy in AppData, leaving `installed/captureengine/` completely unlocked. Since the negotiation gate (above), non-target processes map and lock only the staged gate; the staged full layer stays locked only by processes that were admitted. `CleanupStaleStagingDirectories` automatically prunes older `b*` staging folders during startup, gracefully retaining any directory whose DLL is still locked by a surviving external process.
