...
- Fatal-dump hook bootstrap is transactional with respect to callable originals: surviving termination/fail-fast inline trampolines are installed and atomically published before their IAT routes become reachable, while fallback calls use the hook DLL's own unpatched static Kernel32/ntdll imports. The bootstrap never publishes a raw dynamically resolved OS export as a callable original. Live exception-raising primitives (`RaiseException`, `RtlRaiseException`, `RtlRaiseStatus`, and `Nt`/`ZwRaiseException`) remain byte-identical to avoid a process-wide patch race; VEH plus IAT/dynamic interception retain diagnostic coverage. CDB sessions `20260716_023403`/`20260716_023432` proved the old ordering could route `OutputDebugStringA` through a suppressed exception export and fast-fail.
...
- Preserve static-import fallbacks and publish every successfully installed callable trampoline before patching process imports to its hook. Do not cache dynamically resolved Kernel32/ntdll exports for guarded indirect calls, and do not inline-patch live exception-dispatch primitives during process-wide bootstrap.
