...
- **Vulkan layer membership is decided at negotiation (2026-09-23).** With a compatible host published, `vkNegotiateLoaderLayerInterfaceVersion` admits only processes allowed by `LayerIPC_IsProcessEligibleByCurrentHost`; other processes return `VK_ERROR_INITIALIZATION_FAILED` and the loader can create the instance without CE. DllMain no longer pins the module; negotiation pins it only after admission. This host-present gate keeps CE out of profiles marked `dll_injection=never` and other non-target applications. The no-host path is currently incomplete: `PersistVulkanLayerTargetList` writes `vulkan_layer_targets.txt` beside `captureengine.exe`, but `IsListedAsResidentTarget` reads beside the layer DLL. Registration stages that DLL and manifest into a versioned `vulkan_layers\b<build>` directory and does not stage the list (`common/vulkan_layer_registration.cpp`). Consequently a Vulkan game launched before CaptureEngine cannot find the list and is declined too; its layer is unavailable for late wake. Test the no-host case after correcting the list location before treating it as supported. The participating-target dormant path still uses passthrough metadata and wakes on the next present after host reactivation. `DISABLE_CE_VULKAN_LAYER=1` and `layer_register.exe --unregister` remain the explicit controls.
