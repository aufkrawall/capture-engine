...
WGC CFR audio continuity policy separates live sync from stop/finalization. During live overload or source starvation, audio must not audibly chase WGC/encoder shortfall with broad speed-up. Visual catch-up must spend video debt, not let audio outlive the picture: WGC visual debt is bounded to about `250 ms` or `32` frames, whichever is tighter; stale buffered WGC history is dropped; and dropped visual timeline debt is also subtracted from the CFR/audio target. At stop, the encoder may drain real WGC frames that were captured at or before the stop QPC. Post-stop WGC frames are discarded. If no pre-stop captured WGC frame remains, cached last-frame repeats are not a valid WGC drain source and the remaining frozen-tail debt is removed from the final timeline. Final audio follows the resulting encoded video endpoint so audio and video end at the captured picture endpoint.
...
- `WGC CFR visual timeline debt drop`: WGC was too far behind live visual content, so the CFR/audio target was shortened to the bounded visual window.
- `WGC CFR stale visual debt drop`: buffered visual history older than the live window was discarded.
- `WGC CFR stop drain discarded frozen-tail debt`: stop drain had no pre-stop captured WGC frames left and refused a cached-repeat-only tail.
- `WGC CFR post-stop frame drop`: a WGC frame timestamped after the stop QPC was discarded.
- `WGC CFR slot repeat: buffered frame is too new for scheduled slot`: recovery path when the WGC buffer only contains future visual content for the effective selection target. It should pair with bounded debt-drop/repeat behavior, not multi-second audio/video drift.
