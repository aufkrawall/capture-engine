...
Last cross-checked: 2026-06-13 (WGC encoder-limited smoothness path and cadence attribution)
Stale-risk: medium
...
Manual validation is still required for fresh WGC CFR 4K120 10-bit AV1 capture under normal load, desktop capture, high CPU load, high GPU load, encoder-limited overload such as NVENC `p5`, `wgc_skip_split_device_flush=0/1`, and optional `wgc_same_device_capture=1`. Watch for corruption, device removal, source-starved duplicates, encoder starvation, unbounded queue growth, startup anchor deltas, stale startup frame age, final stream duration deltas, `WgcSelBias`/`LeadExcess` growth, `LiveClamp`, `WGC CFR visual timeline debt drop`, `WGC CFR live scheduler rebase`, unexpected large rebase chunks outside encoder-limited mode, `WGC CFR encoder-limited source drop`, `[WGC CFR CADENCE EVENT]`, `[WGC CFR SMOOTHNESS SUMMARY]`, post-stop frame drops that are discarded rather than encoded, WGC stop-drain repeats or nonzero drain duplicate summaries, WGC fresh catch-up, repeated `WGC CFR slot repeat` clusters, audio trim/drop warnings, audible pitch artifacts, phase sawtooth/backtracking in the synthetic dual-lane stimulus, and game-performance regression.
