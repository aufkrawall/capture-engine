...
Use these with existing `FreshMiss`, `BufAvg`, `BufMin`, `NoFresh`, `NoReserve`, `EncQ`, `Encode`, and encoder overload flags to separate encoder overload, mux backpressure, and WGC source starvation. Shared memory publishes WGC capture health flags; the overlay encoder-overload warning is suppressed while WGC is source-limited or scheduler-limited instead of showing a misleading hard-encoder warning or a separate source-limited overlay warning.
...
- `python build.py --skip-updates` passed on build `0.1.2914`.
- `python build.py --no-build --run-tests --skip-updates` passed 693/693 tests; the test-only command bumped displayed version metadata to `0.1.2915` without rebuilding binaries.
