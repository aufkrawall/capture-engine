...
WGC CFR audio continuity policy separates live sync from stop/finalization. During live overload or source starvation, audio must not audibly chase WGC/encoder shortfall: positive speed-up is suppressed while a live shortfall or encoder bottleneck is active, WGC micro-correction is limited to 0.05%, and the large audio ring is used as a continuity cushion with only an emergency near-capacity trim. Visual catch-up must spend video debt, not audio: each WGC CFR slot keeps its scheduled selection target, frames more than one output tick newer than that target are rejected for that old slot, and the last encoded frame is repeated until the slot timeline can accept fresh content again. At stop, already-accrued CFR debt may close with cached last-frame repeats when no captured WGC frame remains; this is allowed only to finish the existing video timeline, not to extend a new frozen tail. Final audio follows the encoded video timeline so packet durations match without audible speed or trim correction.
...
- `WGC CFR slot repeat: buffered frame is too new for scheduled slot`: the correct recovery path for an old CFR slot when the WGC buffer only contains future visual content. It should pair with repeated video frames, not audio trim or speed-up.
...
- `installed/captureengine/logs/20260507_035914` confirmed the first stop-drain trap: unbounded cached-repeat tail extension can produce equal stream durations while adding seconds of frozen video with continuing audio. Later validation refined the rule: cached repeats are valid only to close already-accrued CFR debt on the existing video timeline, not to create a new tail beyond it.
- `installed/captureengine/logs/20260605_015533` on build `0.1.3742` showed the newer content-clock failure: final stream durations were structurally aligned, but WGC CFR selection bias grew into multi-second territory (`WgcSelBias` about 1.9 s, `Shortfall` about 3.1 s, `LeadExcess` about 3.2 s), with fresh-frame catch-up and a stop-drain abort. The fix keeps WGC selection targets tied to scheduled CFR slots, rejects future frames for old slots, allows repeated video to close accrued debt, and leaves audio continuous.
