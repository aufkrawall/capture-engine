...
- `WGC CFR live scheduler rebase`: live scheduler skipped old WGC visual ticks after the live-window debt bound was exceeded. Current policy permits only one skipped tick per loop; larger per-event chunks indicate a stale build or regression that can create visible cadence holes. Inspect the total count for smoothness impact under overload.
...
`tools/analyze_capture_av.py` exposes `max_wgc_live_rebase_ticks` and `max_wgc_startup_frame_age_us` so strict log validation can catch the two cadence failures from `20260605_222205`: multi-tick scheduler holes and stale startup handoff frames.
