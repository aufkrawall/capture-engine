...
`tools/analyze_capture_av.py` exposes `max_wgc_live_rebase_ticks`, `max_wgc_startup_frame_age_us`, WGC cadence-event evidence, and `wgc_encoder_limited_judder` classification. Strict log validation should catch stale startup handoff, unbounded live rebase chunks, repeated too-new slot holds, encoder-limited phase sawtooth, and source-limited repeat clusters as separate failure families.
