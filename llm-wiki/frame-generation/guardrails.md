...
- Talos `installed/captureengine/logs/20260423_215138` and the immediate rerun `installed/captureengine/logs/20260423_220858` showed that generic FG switching correctness also depends on CE actually observing the last Streamline feature-call edge. When `slDLSSGSetOptions` export inline patching fails, CE still needs generic direct-import / dynamic-lookup interception for `slDLSSGSetOptions`, `slDLSSGGetState`, and `slReflexSetConstants`, and that fallback has to be armed on the actual owner module of the failing export rather than only on the core Streamline DLLs; otherwise menu-side `DLSS FG -> off` can leave the overlay on the last known mode even though the publication layer itself is behaving correctly.
...
- Re-check this page after changes to Streamline feature-export hook coverage. Talos `20260423_215138` and `20260423_220858` proved that export-inline failure on `slDLSSGSetOptions` is survivable only if CE also catches the same feature through direct imports / dynamic lookups on the real owner DLL of that export.
