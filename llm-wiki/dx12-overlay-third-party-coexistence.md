...
- **Focus loss must NOT hide the overlay (v8, supersedes v3-v7).** The rejected v3-v7 rule keyed the backbuffer-work hold on `!processHasForeground`, so the overlay disappeared whenever the window lost focus. The user's scenario: the borderless `dx12_test.exe`, when Alt+Tabbed, stays a VISIBLE background window that keeps presenting `S_OK`; RTSS keeps its overlay visible there and survives the refocus-to-fullscreen transition. Current invariant: CE holds new swapchain backbuffer overlay/capture submissions ONLY when the swapchain is genuinely not presentable — `DXGI_STATUS_OCCLUDED` (tracked from the wrapped Present HRESULT via `g_SwapchainPresentOccluded` in `DX12_NoteWrappedD3D12PresentResult`), minimized (iconic), or zero-sized. A merely-unfocused but visible window renders the overlay directly to the backbuffer, exactly like RTSS. Focus is not an input. Predicate: `ShouldHoldD3D12OverlayBackbufferWorkForNonPresentableSwapchain(...)`. The `present-passthrough-v7` frame-latency-waitable disablement and the device-lost immediate-dump-once trigger remain. IMPORTANT: the actual `0x887A0006` device-hung at the iflip<->composited mode switch was never root-caused (the v6 `20260602_213952` failure was always observed with the direct `PRESENT->RT` barrier path on refocus, never with v8's visibility gate, and no GPU-fault data was ever captured). DRED now captures it: see `llm-wiki/debug-tools.md` and look for `DX12 DRED: ===== device-removed extended data` with the hung command list + page-fault VA + recently-freed allocations. Healthy traces: `DX12 focus-loss sync policy=v8 visibility-gated-backbuffer-hold`, `Swapchain presentability changed -> PRESENTABLE/NOT-PRESENTABLE`, `Holding overlay/capture backbuffer work while swapchain is NOT presentable` (only when occluded/iconic/zero-size), `Resuming overlay/capture backbuffer work`, `DX12 DRED: armed auto-breadcrumbs`.
