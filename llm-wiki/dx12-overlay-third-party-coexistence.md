...
- **Focus loss must NOT hide the overlay, and the overlay draw must not hang the GPU (v9, supersedes v3-v8).** The rejected v3-v7 rule keyed the hold on `!processHasForeground`, hiding the overlay on focus loss. v8 fixed that (gate on not-presentable only) but then DRED proved why the device still hung. DRED root cause (`installed/captureengine/logs/20260603_020053`): `0x887A0006` is a PURE GPU hang (`pageFaultVA=0`) inside **CE's own overlay command list** — it completed the `PRESENT->RT` barrier + one `DRAWINDEXEDINSTANCED`, then hung on the next draw. CE's direct overlay DRAW on the LIVE backbuffer hangs while the window is unfocused / mid iflip<->composited mode switch (the backbuffer is transiently owned by DWM/the display; DWM loads its `d3d11.dll` composition device during the stall). Current invariant: (1) not-presentable (occluded/iconic/zero-size) -> hold, no backbuffer touch (`ShouldHoldD3D12OverlayBackbufferWorkForNonPresentableSwapchain`); (2) unfocused-but-visible OR within the post-refocus transition window -> render the overlay to CE's OFFSCREEN RT and composite via implicit-promotion copies, NOT a direct backbuffer draw (`ShouldUseD3D12UnfocusedOffscreenOverlayComposite`) — overlay stays visible, hang-prone draw moved off the live backbuffer; (3) steady focused -> fast direct draw. The `present-passthrough-v7` waitable disablement and device-lost immediate-dump-once trigger remain. Healthy traces: `DX12 focus-loss sync policy=v9 unfocused-offscreen-composite`, `DX12: Unfocused overlay via offscreen composite`, `DX12: Using offscreen compositing for focus-loss overlay`, `DX12 DRED: armed auto-breadcrumbs`. OPEN/STALE-RISK: the offscreen path still touches the backbuffer via `CopyTextureRegion` (read+write); DRED stays armed (`CE_DX12_DRED`) — if a hang recurs the breadcrumb shows whether a COPY (not a draw) hung, and the fallback is a brief transition-only skip of all backbuffer touches.
