...
Last cross-checked: 2026-06-02 (updated: build 0.1.3682 x86 DX12 focus-loss foreground-reacquire validation)
...
- Focus loss is not a reason to blank or rebuild a drawable DX12 overlay, but it is also not safe for CE to keep touching the swapchain backbuffer through every focus transition. `20260602_030350` proved the older v3 same-frame fence still failed when CE drew directly to the focus-lost backbuffer; `20260602_161416` and the later local v6 repro `20260602_213952` showed that even a background-only hold was insufficient because CE resumed direct backbuffer overlay/capture work immediately when focus returned and device removal followed shortly after. Current wrapped single-queue D3D12 rule: preserve the live backend/resources, hold new CE swapchain backbuffer overlay/capture submissions while the process is backgrounded, then continue holding them through a short foreground-reacquire proof window of successful Presents before resuming direct backbuffer work. This is intentionally a backbuffer-work hold, not backend teardown, pseudo-overlay replacement, or generic overlay disable. DXGI frame-latency waitable probing is disabled for this focus-loss family and logged as `present-passthrough-v7`, because the waitable repeatedly returned immediately signaled and did not gate correctness. Device-lost HRESULTs during a recent focus transition now request an immediate freeze dump once. Manual injected x86 DX12 stress in `20260602_215334` and `20260602_215620` exercised irregular short/long focus-loss intervals with no hang, dump, device removal, `0x887A`, or Present-stall markers.
...
- During Alt+Tab/focus changes, preserve the DX12 overlay backend but hold CE's risky swapchain backbuffer overlay/capture work while backgrounded and during the foreground-reacquire proof window. Healthy traces show `DX12 focus-loss sync policy=v7 background+foreground-reacquire-backbuffer-hold`, `DX12: Holding focus-loss overlay/capture backbuffer work while process is backgrounded`, `DX12: Holding focus-loss overlay/capture backbuffer work during foreground reacquire proof`, `DX12: Focus-loss foreground reacquire Present proof accepted`, and `DX12: Resuming focus-loss overlay/capture backbuffer work`; frame-latency lines should be skip/pass-through telemetry, not waits used as a correctness gate.
