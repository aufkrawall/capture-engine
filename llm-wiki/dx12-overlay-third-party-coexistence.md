...
Last cross-checked: 2026-06-02 (updated: build 0.1.3671 x86 DX12 focus-loss manual validation)
...
- Focus loss is not a reason to blank or rebuild a drawable DX12 overlay. `20260602_030350` proved the v3 immediate-fence path was present and direct D3D12Core ECL submit was available (`origECL=63154fa0` resolved to `D3D12Core!CCommandQueue<0>::ExecuteCommandLists`), but repeated unfocused direct swapchain backbuffer RTV/barrier overlay drawing still preceded device removal. For wrapped single-queue D3D12 focus-loss swapchains, CE now keeps the inject overlay visible through the existing offscreen copy/render/copy composite path, then immediately signals CE's overlay fence for that submitted command list, updates the current fence value, and lets the existing pre-Present wait prove completion before the app can enter the next frame. Direct focus-lost `PRESENT -> RENDER_TARGET -> PRESENT` fallback is suppressed; if the offscreen RT cannot be prepared, preserve backend/resources and skip that risky draw rather than reverting to the failing direct backbuffer path. Normal foreground frames still use the deferred post-Present signal path for NVIDIA stall avoidance. Frame-latency waitable probing is telemetry only because `20260601_212556` / `20260602_030350` showed the waitable can return immediately signaled during the failing focus-loss loop. Manual x86 DX12 stress in `20260602_160115` revalidated installed build `0.1.3671` with uncapped focus cycling: the v4 marker/offscreen breadcrumbs appeared, and no hang, dump, device removal, `0x887A`, or Present-stall markers appeared.
...
- During Alt+Tab/focus changes, prefer offscreen-composited overlay drawing plus same-frame overlay-fence synchronization over hiding/reinitializing the DX12 overlay. Healthy traces show `DX12 focus-loss sync policy=v4 offscreen-composite+immediate-fence`, `DX12: Focus-loss overlay using offscreen composite`, `DX12: Using offscreen compositing for focus-loss overlay`, `DX12: Focus-loss immediate overlay fence signal ... offscreen=1`, `DX12: Overlay completion wait ... focus-loss mode`, and frame-latency lines labeled `telemetry` rather than overlay suspension or direct focus-lost swapchain RTV fallback.
