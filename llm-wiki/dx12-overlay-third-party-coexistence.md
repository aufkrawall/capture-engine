...
Last cross-checked: 2026-06-06 (updated: third-party-overlay detection made loader-free on the Present hot path; x86 Alt+Tab freeze re-rooted to a loader stall — see log/recent.md 2026-06-06)
...
- **The dedicated DX12 overlay queue is used for FG AND for plain non-FG DX12 (2026-06-06).** It was formerly FG-only; it is now also used for non-FG so the overlay's per-frame `ExecuteCommandLists` does not pressure the application's shared command-queue command-buffer pool (that pressure made the app's own ECL allocate a command buffer during the Alt+Tab mode switch, a kernel GPU allocation that blocked ~2 s and tripped the GPU TDR — see log/recent.md 2026-06-06). It STAYS single-queue for: Streamline-FG / runtime-owned-FG (cross-queue access rejected under SL FG), startup-overlay compatibility (`processNeedsDelay`), and when a startup-blocking third-party overlay (Social Club / EOS) is loaded — those last two because cross-queue swapchain-backbuffer state transitions caused GTA5 Enhanced `ERR_GFX_STATE`. Cross-queue backbuffer access is otherwise legal because PRESENT==COMMON (overlay queue does COMMON->RT->COMMON on a backbuffer the app left in PRESENT). Decision: `ce::overlay_compat::ShouldUseDedicatedDX12OverlayQueue`.
