...
Last cross-checked: 2026-08-12 (multi-overlay Present-entry rule added: CE stays out of a `dxgi!Present` entry that two or more foreign overlays already share, and — since the same day — intercepts *below* that chain with a deep hook in the `dxgi!Present` body, because the swapchain wrapper alone cannot cover a swapchain created before injection. Generic hook-chain ownership, thread-quiesced patching, proxy-module exclusions, and resident pass-through lifecycle remain as audited on 2026-08-11; the prior DX12/FG provenance rules remain in force.)
...
- `ce::overlay_compat::CountLoadedTrackedOverlayModules(TrackedOverlaySubset::kOverlay)` counts the loaded overlay subset (loader-free, off the seed/notification cache). `ShouldLeavePresentEntryToForeignOverlayChain(foreignEntryJumpDetected, loadedOverlayCount, frameGenerationInterposerLoaded, hasNonEntryRuntimePresentView)` is the decision. The FG-interposer exception now only applies while no wrapped non-entry view of the runtime's presents exists yet (install time, or when the runtime swapchain cannot be wrapped); `ShouldLeavePresentEntryAfterRuntimeSwapchainWrap(entryPatchStillIntact, ...)` gates the ownership-checked un-prepend once the Streamline runtime swapchain has been wrapped.
...
- `InstallPresentBodyHooksBelowForeignChain` (`hook/common/dxgi_shared_hooks_present.cpp`) runs inside the leave-entry branch. A `Present1` entry a foreign overlay also owns gets the same deep treatment; an *unclaimed* `Present1` entry gets CE's ordinary prepend, because there is no other participant to drop there.
