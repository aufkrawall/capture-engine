...
### CONFIRMED ROOT-CAUSE MECHANISM (ground truth)
The **application's OWN `ExecuteCommandLists`** blocks for the whole stall inside a **kernel GPU virtual-address map**: `dx12_test!Render → capture_hook!DetourExecuteCommandLists → D3D12Core!CCommandQueue::ExecuteCommandLists → nvwgf2um (NV UMD) → NDXGI::CDevice::MapGpuVirtualAddressCB → win32u!NtGdiDdDDIMapGpuVirtualAddress` (blocked in VidMm). This is a **CPU-side kernel block** during the iflip<->composited mode switch; the 2 s GPU TDR then fires. Proven by a **mid-stall watchdog dump** (dump taken DURING the stall, not post-mortem) — `logs/20260606_211023` (also a crash variant where the NV UMD AVs in the same path: `logs/20260608_162931`). It is a **timing-sensitive RACE** (32-bit/WoW64 only; 64-bit does the same op in <20 ms).

### The trigger is CE NATIVELY TOUCHING THE BACKBUFFER during the transition
`observer_only=true` (CE injected, all hooks active — `DetourPresent`/`DetourExecuteCommandLists` still wrap the app — but NO overlay GPU resources created and NO overlay GPU work submitted) → **NEVER freezes** under extreme Alt+Tab (`logs/20260607_003611`, `logs/20260608_163139`). So it is specifically CE's overlay RTV-transition+draw+submit on the app's backbuffer, NOT CE's mere hook/device presence. The historical v8/v9 DRED data agrees: ANY CE backbuffer touch (v8 `DRAWINDEXEDINSTANCED`, v9 `COPYTEXTUREREGION`) hangs during the switch.
...
- **"iflip disabled by the debug layer"** — DISPROVEN. Enabling the D3D12 debug layer (`ce_dx12_debug_layer`=1) PREVENTS the freeze (13 Alt+Tab edges, no stall, `logs/20260608_171158`) but the trace still shows `MMIOFlipMultiPlaneOverlay` (iflip still ON) → the debug layer dodges the race purely via added CPU timing, confirming the timing-race nature (NOT a shippable fix; a pure timing/sleep workaround is also forbidden by AGENTS.md).
...
### Fix-space (all rejected by the user, hence the bug is hard)
D3D11On12 (rejected), DComp/composited separate-surface overlay (rejected — "evil"), hiding the overlay during the transition (rejected), pure timing/sleep bandaid (forbidden). Dedicated overlay queue for non-FG is also illegal (`ACCESS_DENIED`, see Facts). Remaining options: (1) targeted native fix-experiments with PROPER synchronization on CE's backbuffer-touch path (guess-and-check; v3–v12 already failed); (2) escalate/accept as a 32-bit/WoW64 NV-UMD limitation (64-bit unaffected); (3) revisit D3D11On12 scoped to non-FG only (the only technique proven to work).

### Diagnostic tools (committed, gated, OFF by default)
- `[Overlay] dx12_focus_analysis=true` (config) → in-process residency flight recorder + present-gap + **CPU VA-space probe** (`vaspace committedMB/freeMB/largestFreeBlockMB`, ~1/s and at the stall — to test the 32-bit VA/command-buffer-pool exhaustion hypothesis); dumps on stall/device-removal. Logs: `DX12 ANALYSIS:`. Code: `Dx12SampleVaSpace`/`DX12_UpdateFocusAnalysis`/`DX12_DumpFocusAnalysisRing` in `dx12_hook.cpp`. See `handoff-dx12-32bit-crash.md`.
- `ce_dx12_trace` flag file (or env `CE_DX12_TRACE=1`) + `tools/dx12_call_trace.py` → caller-attributed D3D12 call trace (CreateCommandQueue/Resource/DescriptorHeap, ExecuteCommandLists, Signal, CreateSwapChain). Logs: `DX12 TRACE:`. NOTE: CE's own overlay ECL/Signal use the raw `realECL` pointer so they are NOT captured (a known blind spot); it captures the app's and co-resident modules' calls.
- `tools/gpu_trace.py capture [--debug-layer N] [--open]` → automated GPUView kernel capture (wraps in-box `gpuview/log.cmd`; needs an ELEVATED shell; user triggers the Alt+Tab; auto-stops on the dump, merges to `Merged.etl`, coarse-parses).
...
- **[PARTIALLY SUPERSEDED — see the ACTIVE INVESTIGATION section above for the current 2026-06-08 understanding. The "never hide the overlay" rule still holds, but the v10 transition-HOLD was abandoned: current policy is v12 draw-every-frame (overlay is NEVER held on focus, even during the switch), and the freeze is now root-caused as the app's own ExecuteCommandLists stalling in a kernel GPU-VA map — NOT only CE's own command list hanging.]** Focus loss must NOT hide the overlay, and CE must not touch the backbuffer during the mode switch (v10, supersedes v3-v9). The rejected v3-v7 rule hid the overlay on focus loss; v8 (direct draw while unfocused-visible) and v9 (offscreen composite while unfocused) both still hung. DRED root-caused it across two repros: `0x887A0006` is a PURE GPU hang (`pageFaultVA=0`) inside **CE's own overlay command list** — v8 hung on `DRAWINDEXEDINSTANCED` (`logs/20260603_020053`); v9 hung on the `bb->offscreen` `COPYTEXTUREREGION` (`logs/20260603_150241`). So **ANY** CE backbuffer touch (draw OR copy) pure-hangs while the swapchain is mid iflip<->composited mode switch around a focus change (the backbuffer is transiently owned by DWM/the display; DWM loads its `d3d11.dll` composition device during the stall). The hangs were at the REFOCUS edge (≈3 presents after regaining foreground), i.e. the brief transition — not steady unfocused. Current invariant: (1) not-presentable (occluded/iconic/zero) -> hold (`ShouldHoldD3D12OverlayBackbufferWorkForNonPresentableSwapchain`); (2) **focus-change transition window** (a foreground-change edge arms `g_FocusTransitionHoldFrames`; either direction) -> hold ALL backbuffer overlay/capture work (`ShouldHoldD3D12OverlayBackbufferWorkDuringFocusTransition`) — no draw, no copy, like v7 which never hung; (3) steady states (focused AND unfocused-but-visible) -> fast direct draw, like RTSS. The overlay is only briefly absent during the actual mode switch (screen transitioning anyway), never during steady visible use. The `present-passthrough-v7` waitable disablement and device-lost immediate-dump-once trigger remain. Healthy traces: `DX12 focus-loss sync policy=v10 focus-transition-hold`, `Focus-change edge (regained/lost foreground) — holding ...`, `Holding overlay/capture backbuffer work during focus-change mode switch`, `Resuming overlay/capture backbuffer work — focus-change mode switch settled`, `DX12 DRED: armed auto-breadcrumbs`. OPEN/STALE-RISK: the hold is a present-COUNT (`kFocusTransitionHoldFrames=60`) so its real-time coverage scales with FPS; DRED stays armed — a hang DURING the hold means raise the count, a hang in STEADY unfocused (no recent edge) means steady-unfocused direct draw is unsafe and only a non-backbuffer overlay surface could keep it visible.
- **Third-party-overlay detection must be loader-free on the Present hot path (one fixed x86 Alt+Tab freeze contributor; the freeze itself is still open — see ACTIVE INVESTIGATION).** `GetLoadedThirdPartyOverlayModuleName()` is called from `DetourPresent` every frame and must NEVER call into the Windows loader. The earlier design walked `GetModuleHandleA` over the overlay list and invalidated its cache on *every* DLL load; the Alt+Tab iflip<->composited mode switch reloads `d3d11.dll` repeatedly, so each load re-forced a per-Present 16x `GetModuleHandleA` walk. On x86/WoW64 the not-found loader path (`LdrGetDllHandleEx` + ApiSet resolution) is slow enough that this stalled Present >2 s and tripped the GPU TDR (`DEVICE_HUNG`), killing the overlay (x64 stayed <10 ms). Current rule: the Present path does a single atomic read; the loaded-overlay set is maintained off the Present thread via a one-time seed scan (`SeedThirdPartyOverlayModuleCacheFromLoader`, HookThread), `LdrRegisterDllNotification` (all load/unload, loader-safe base-name compare + atomic store), and the existing `LoadLibrary`/`LdrLoadDll` -> `NotifyHookModuleLoaded` path — all reacting only to known overlay modules (so unrelated loads like `d3d11.dll` never disturb detection). Canonical-list order is the detection priority (Steam wins over RTSS/Discord/etc.). This is detection-only: it never changes or hides overlay rendering. Healthy logs: `Third-party overlay detection: seed scan ...`, `LdrRegisterDllNotification active ...`, `DllNotification: third-party overlay module loaded/unloaded: ...`.
- **Focus-transition/occlusion safety must be fed on BOTH present paths.** All the focus-loss/occlusion safety state (`g_SwapchainPresentOccluded`, the not-presentable backbuffer-work hold, v10 focus-edge telemetry, DRED dump-window widening) lives in `DX12_NoteWrappedD3D12PresentResult`. It was historically called ONLY from the CWrapDXGISwapChain wrapper, so **vtable-hooked DX12 apps (no swapchain wrapper) never engaged the occlusion hold** and drew to the backbuffer through the Alt+Tab iflip<->composited mode switch → GPU `DEVICE_HUNG`. This is why wrapped-path apps (Talos/GTA) didn't hang but the vtable-path `dx12_test` did. `DetourPresent` now also feeds the present result (guarded by `!IsInWrapperPresent()` for exactly-once), and `g_HaveD3D12PresentResultSignal` is OR'd into the hold predicate so the invisible-safe not-presentable hold engages for both paths. Invariant: any new focus-loss/occlusion gating must be reachable from the vtable `DetourPresent` path, not only the wrapper. (Present1/`DetourPresent1` wiring is still a follow-up.)
