...
Last cross-checked: 2026-06-06 (ACTIVE: x86 Alt+Tab freeze still open — multiple contributors fixed (loader stalls, DRED), now localized to a present-path stall during the mode switch; see the "ACTIVE INVESTIGATION" section below and log/recent.md 2026-06-06)
...
## ACTIVE INVESTIGATION: x86 Alt+Tab DX12 freeze (dx12_test) — state as of 2026-06-06
Single hand-off reference for continuing in a fresh session; chronology/details in `log/recent.md` (2026-06-06 entries, newest-first).

- **Symptom**: injected **32-bit** `dx12_test.exe` in borderless-fullscreen freezes on Alt+Tab in/out → GPU `DEVICE_HUNG (0x887A0006)`, overlay dies. **64-bit never freezes.** **Bare 32-bit (no CE) never freezes → CE is the trigger.** Both bitnesses use the same `testappconfig.ini` (vsync=1, borderless 4K, gpu_load=120) and flip at **144 fps** (RTSS-confirmed, fg+bg). RTSS injected does NOT freeze and does NOT hide its overlay.
- **HARD constraints**: overlay must NEVER be hidden, not even temporarily during the switch; native DX12 (no D3D11On12); must not regress FG (Talos/GTA) or other-overlay coexistence.
- **FIXED + committed (keep)**: (1) overlay/SL detection is loader-free on the Present hot path (no `GetModuleHandle*`/`Ldr*` ever) — `d7ae1eb`/`c30ccf1`/`1ca0f0b`; (2) DRED auto-breadcrumbs default-OFF (forced-on DRED's per-Reset kernel alloc was itself causing a freeze) — `94072bb`; (3) present-result/occlusion detection now also fed on the vtable `DetourPresent` path — `ede2886`. Each removed a real per-frame stall source but the freeze persisted.
- **RULED OUT (do not retry)**: **Dedicated overlay queue for non-FG** — DXGI `ACCESS_DENIED` (only the app/swapchain queue may render the backbuffer; reverted `6c8375b`). **DComp independent surface** — user ruled out for latency (loses Independent Flip unless MPO keeps it on a HW plane; MPO likely OK on the user's RTX but config-dependent) — NOT implemented. **Transition-hold (v10)** — documented to never hang but hides the overlay during the switch; user rejects any hide.
- **CURRENT understanding**: the freeze is a single **present-interval stall** (`heartbeat #1049 gap=2354ms`) during the iflip<->composited mode switch → TDR. It is **NOT** in `ExecuteCommandLists` (ECL timing diag max 9.5 ms) and **NOT** a frame-rate/footprint difference (both 144 fps pre-freeze, identical overlay footprint draws=4/vb=13760/ib=2064; the earlier "2150 fps" was the app spinning on the dead device AFTER the freeze). Leading hypothesis: CE's overlay backbuffer GPU work hangs the GPU during the switch and the CPU stalls in the **real `Present`** call waiting on it (original v8 class), 32-bit-specific via WoW64 timing. Needs confirmation.
- **NEXT STEP**: run 32-bit `dx12_test` with CE, Alt+Tab to freeze, read the present-phase diag (added `52c55fb`, see debug-tools.md "DX12 DIAG"): a slow `DetourPresent TOTAL` with NO slow `ProcessFrame (overlay)`/`overlay-completion wait` ⇒ stall is the real `Present` (GPU hung on CE's overlay draw) → fix the overlay draw/sync during the switch; a slow `wait` ⇒ CE overlay GPU work hung; a slow `ProcessFrame` ⇒ CE record/submit path. A matching 64-bit run should show all phases fast.
