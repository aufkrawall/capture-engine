...
- **Steam NULL-callback slots are patched proactively before any Steam transport (2026-08-09).** Session `installed/captureengine/logs/20260809_141705` proved the crash-time VEH recovery is not reliable in every process: the guard was installed, yet the handler never ran (Streamline installs its own exception handling - `C:\ProgramData\NVIDIA\Streamline\...\sl-sha-*.dmp` - and the AV repeated without a patch). The slot itself also moves between Steam builds (0x1621d8 legacy, 0x167340 on the 2026-08-03 build). `EnsureSteamNullCallbacksPatched` (`hook/common/dxgi_shared_steam.cpp`) therefore discovers the Present-shaped `mov (e)ax,[slot] ... call (e)ax` pattern by scanning the overlay module once per version, caches the slot RVAs, and patches any NULL/invalid slot to CE's DXGI bypass before Steam is invoked - in `TryInvokeGuardedExternalSteamOverlayPresent`, the SL fast-path, and the non-SL E9 path. The scan must cover the WHOLE module: session `20260809_143040` crashed again when an 8-slot cap truncated the scan before the faulting slot (steam+0x167340) was reached; the cap is 256 now and a fail-open log reports cap hits. The VEH remains a backstop for unknown builds. Only NULL/invalid slots are ever written, so working sessions (Talos: slot already valid) are no-ops, and the written value is identical to the crash-time recovery's. The source-thread provenance rule is unaffected. Pattern scanner + wiring coverage: `tests/test_dxgi_shared_part12.cpp`.
