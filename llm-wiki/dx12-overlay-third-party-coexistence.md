...
- **Startup compat pass crash with vtable path + no bypass (build 0.1.2901 fix)**: When the vtable path is chosen (inline hooks skipped due to external E9 JMP), `oPresentTrampoline` is null. The startup compatibility pass (`kPassThroughOriginal`) must NOT run in this state unless a bypass trampoline (`oPresentBypass`) is available. Without either, `CallOriginalPresent` falls through to `presentOriginal` (= `dxgi!Present` with Steam's E9 JMP), enters Steam overlay, and Steam can call back into CE through a null pointer (RIP=0). Fix: `ShouldAllowDX12StartupPresentPassForState` now requires `bypassAvailable=true` when `presentTrampolineInstalled=false`. `GetDX12StartupPresentMode` proactively calls `EnsurePresentBypassTrampoline()` before making the policy decision.
