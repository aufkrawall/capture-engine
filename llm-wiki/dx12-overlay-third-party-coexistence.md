...
- Focus loss is not a reason to blank or rebuild a drawable DX12 overlay. For wrapped single-queue D3D12 swapchains, focus-lost overlay work now uses a same-frame fence gate: CE draws the inject overlay, immediately signals CE's overlay fence for that submitted command list, updates the current fence value, and lets the existing pre-Present wait prove completion before the app can enter the next frame. Normal foreground frames still use the deferred post-Present signal path for NVIDIA stall avoidance. Frame-latency waitable probing is telemetry only because `20260601_212556` showed the waitable can return immediately signaled during the failing focus-loss loop.
...
- During Alt+Tab/focus changes, prefer same-frame overlay-fence synchronization over hiding/reinitializing the DX12 overlay. Healthy traces show `DX12 focus-loss sync policy=v3 immediate-fence`, `DX12: Focus-loss immediate overlay fence signal`, `DX12: Overlay completion wait ... focus-loss mode`, and the wrapper's focus-lost `no deferred DX12 overlay fence signal` breadcrumb rather than overlay suspension.
