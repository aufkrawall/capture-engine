...
- `DX12 DIAG: DetourPresent TOTAL SLOW Xms` / `ProcessFrame (overlay) SLOW Xms` / `overlay-completion wait SLOW Xms` — present-phase split. **Interpretation**: slow TOTAL with no slow ProcessFrame/wait localizes that sample to the real `Present`; slow `wait` implicates outstanding CE overlay work. A slow `ProcessFrame` now includes an always-available breakdown: `external` is routing/policy/swapchain work outside the inner renderer and screenshots; `innerOther` is inner work not accounted to capture or the measured overlay block; `capture` is inject texture publication; overlay `acquire/record/submit/post` divides backbuffer acquisition, command recording, command submission/synchronization, and immediate post-submit work; `queueLockWait` is the external command-queue mutex wait and is already part of `external`; `wrapperInitOverlap=1` proves that wrapper initialization/retry activity started, ended, or remained active across the slow call. `valid=0` means the overlay draw exited before reaching the measured block, so use `innerOther`/`external` rather than treating zero overlay time as proof. These timings localize a stall but do not by themselves identify driver versus CE ownership inside a timed graphics API call.
