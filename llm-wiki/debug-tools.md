...
Built-in, ALWAYS-ON (no env/flag/install), written via `HookLogImportant` to `hook_debug.log`. Added 2026-06-06 to localize the x86 Alt+Tab present-path stall (the freeze is a present-interval stall, NOT in the ECL — see `dx12-overlay-third-party-coexistence.md` "ACTIVE INVESTIGATION"). Source: `hook/apis/dx12_hook.cpp` (`DetourExecuteCommandLists` ~17622, DescFree `Render` ~702) and `hook/common/dxgi_shared.cpp` (`DetourPresent` ~1735/2629/2599).
