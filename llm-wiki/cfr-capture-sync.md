...
Last cross-checked: 2026-06-05 (WGC CFR exact-stop visual debt handling, build 0.1.3749)
Stale-risk: medium
...
Packet-level duration equality is necessary but not sufficient for real sync or visual smoothness. If the encoder thread discards accrued CFR timer debt during a stall, the visual timeline can jump forward while audio remains continuous. If WGC accepts stale visual debt for too long, video content can fall seconds behind live and then catch up too quickly. If WGC selects fresh/current frames for old CFR PTS slots, video content jumps ahead of the audio timeline. If WGC stop drain closes multi-second debt with cached repeats, the picture visibly freezes while audio continues. Final video/audio packet durations may still match in all of those cases, but content-level A/V sync is wrong. Therefore CFR timer-rebase debt must be preserved for both WGC and inject CFR; WGC visual debt must stay bounded to a small live window; when visual debt is dropped, the CFR/audio timeline debt is dropped with it; and inject CFR must keep the live candidate buffer bounded and spend fresh ready candidates during catch-up when the encoder is healthy.
...
- WGC CFR may drain already queued/buffered captured WGC frames at stop only if their source timestamp is at or before the stop QPC. Post-stop frames are not part of the recording. If no pre-stop captured WGC frame remains, any outstanding WGC frozen-tail debt is discarded from the final CFR timeline instead of being encoded with cached repeats.
- WGC CFR visual source selection is bounded to about `250 ms` or `32` frames behind live, whichever is tighter. When the scheduled CFR timeline falls further behind that live visual window, capture drops the visual timeline debt and subtracts the same ticks from the audio/video target so continuous audio cannot outlive the captured picture.
- WGC CFR frame selection may move up to the bounded visual-debt floor, but must not choose arbitrary future content for old PTS slots. A buffered WGC frame more than three output ticks newer than the effective selection target is suspicious and should be visible through slot-repeat/debt-drop diagnostics instead of shifting content seconds ahead of audio.
...
- `[Cadence Health]` WGC fields: `WgcSelBias`, `Shortfall`, `LeadExcess`, `Oldest`, `BufNow`, and `AgeMax` expose visual debt and selected-content drift. Large values are a content-sync bug even when stream durations match.
...
- `[EncoderThread] WGC CFR visual timeline debt drop`: WGC was too far behind live visual content and shortened the pending CFR target so audio and video still end at the actual captured visual endpoint.
...
- `[EncoderThread] WGC CFR stop drain discarded frozen-tail debt`: stop drain had no pre-stop captured WGC frames left, so the remaining repeat-only tail was removed from the final timeline.
- `[EncoderThread] WGC CFR post-stop frame drop`: a WGC frame timestamped after the stop QPC was discarded and must not extend the recording.
- `tools/analyze_capture_av.py` log gates now include `wgc_stop_drain_aborted`, `wgc_fresh_catchup`, `wgc_too_new_slot_repeat`, `wgc_stale_visual_debt_drop`, `wgc_stale_fresh_catchup_blocked`, `wgc_visual_timeline_debt_drop`, `wgc_stop_frozen_tail_drop`, `wgc_post_stop_frame_drop`, `audio_extreme_drift`, and cadence metrics `age_max_us`, `wgc_sel_bias_abs_us`, `wgc_shortfall_ms`, `wgc_lead_excess_ms`, `wgc_oldest_ms`, and `wgc_buffered_frames`.
...
- Build/test `0.1.3747`-`0.1.3749` fixed the follow-up WGC frozen-tail/exact-stop family reported from `installed/captureengine/logs/20260605_025329`: the old file parsed as `max_wgc_sel_bias_abs_us=3999919`, `max_wgc_shortfall_ms=5242`, `max_wgc_lead_excess_ms=5811`, `wgc_fresh_catchup=27`, and many audio extreme-drift warnings. WGC now keeps live visual debt bounded to `250 ms / 32 frames`, subtracts dropped visual debt from the CFR/audio target, delays WGC cleanup until the encoder has drained pre-stop frames, rejects post-stop WGC frames, refuses cached-repeat-only WGC stop drain, and logs all of those decisions. Focused `CapturePipelinePolicyTest.*` passed 71/71 on build `0.1.3747`; full test-only passed 901/901 at metadata `0.1.3748`; required plain build passed at `0.1.3749`.
...
- Fresh WGC and inject codec-matrix recordings should be checked with `tools/analyze_capture_av.py --full-scan --decode-check --waveform-tail-scan --max-audio-spread-ms 0 --max-video-audio-delta-ms 0 --max-audio-tail-marker-spread-ms 0 --max-log-event audio_forced_bootstrap=0 --max-log-event audio_bootstrap_trim=0 --max-log-event audio_underrun=0 --max-log-event audio_extreme_drift=0 --max-log-event wgc_stop_drain_aborted=0 --max-cadence-metric wgc_sel_bias_abs_us=20000 --max-cadence-metric wgc_shortfall_ms=250 --max-cadence-metric wgc_lead_excess_ms=250 --max-cadence-metric wgc_oldest_ms=250 --max-cadence-metric wgc_buffered_frames=32`. `wgc_visual_timeline_debt_drop` and `wgc_stop_frozen_tail_drop` are warnings to inspect, not automatic file failures, because they are the intended fail-closed behavior when WGC was already behind.
