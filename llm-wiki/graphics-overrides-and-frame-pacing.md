...
grid, the `PresentSite` call-site contract, front-loaded cadence placement, FG output-group admission, and the
Reflex handoff rules.
...
- Moved to [frame-generation/dlss-driver-settings.md](frame-generation/dlss-driver-settings.md): the five DLSS
  driver-settings (DRS) keys CE answers process-locally - the FG render preset plus NVIDIA Profile Inspector's
  forced mode, fixed and dynamic multi-frame counts and target frame rate - who reads them, the value encodings,
  why `vsync_mode` has to travel the same channel, and how to read the on-screen indicator.
