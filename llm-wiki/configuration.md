...
- `video_capture=inherit|inject|wgc|dxgi_dup|none` chooses the application's video source. `inherit` follows `[Capture] capture_method`; with global `auto`, a profile that permits DLL injection when needed uses injected capture and one that forbids it uses WGC. The older `global` and `default` values remain aliases for `inherit`. `inject` requires `dll_injection=when_needed|always`; an invalid combination is logged and receives no video route rather than silently injecting or switching backends.
- `dll_injection=when_needed|always|never` answers the separate question of when CaptureEngine may load its DLL. `when_needed` injects only when the resolved video source is `inject`. `always` also injects for WGC, DXGI Duplication, or a profile with no video so the injected overlay and graphics overrides remain available. `never` does not inject. A new profile that omits `video_capture` contributes no video target even with `dll_injection=always`; this makes overlay-only profiles genuinely independent from video routing.
...
- The profile reference first explains video source and DLL injection as two separate choices, then shows their three common combinations. Three commented examples cover DXGI Duplication without injection plus DesktopOverlay/app audio, injected video with `when_needed`, and DXGI Duplication with `always` for the injected overlay/graphics overrides. They use distinct executable names and do not create active default profiles.
...
- DLL injection can trigger anti-cheat protection. The fresh profile recommends `dll_injection=never` for multiplayer/anti-cheat software unless injection is known to be permitted.
...
- The focused config/profile/pseudo-overlay/injected-overlay gate passed, covering generated-template parity, new DLL-policy resolution, safe invalid-value fallback, exact compatibility-key precedence, independent overlay-only profiles, legacy behavior, and all three commented examples.
- Incremental product build `0.1.5096` passed x64/x86 hooks, MediaEngine/CaptureEngine, 149 unit-test objects, 30 test apps, both Vulkan layers, packaging, import closure, PE mitigations, architecture, and PDB checks.
- The exact `0.1.5096` no-build gate passed all 1,746 native tests in 135 suites and all five Python tool self-tests. An initial run hit transient Win32 error 1175 while atomically publishing the AVIF test output; the isolated test and complete rerun both passed without source changes.
