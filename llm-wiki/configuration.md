...
- The focused config/profile/pseudo-overlay/injected-overlay gate passed 99 tests across six suites, covering generated-template parity, injected capture without a second DLL setting, source-driven WGC/DXGI behavior, `always`/`never` overrides, safe invalid-value fallback, compatibility-key precedence, overlay-only profiles, legacy behavior, and all three commented examples.
- Incremental product build `0.1.5102` passed x64/x86 hooks, MediaEngine/CaptureEngine, 149 unit-test objects, 30 test apps, both Vulkan layers, packaging, import closure, PE mitigations, effective CFG, architecture, and PDB checks.
- The exact `0.1.5102` no-build gate passed all 1,747 native tests in 135 suites and all six Python tool self-tests.
