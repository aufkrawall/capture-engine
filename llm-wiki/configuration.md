...
- `[Profile.1]` through `[Profile.8]` replace `[App.N]` for new configs. A profile can keep injection, `audio_*` app-capture settings, and qualified per-app overrides under one executable. `injection_mode` accepts the self-describing values `capture|overlay|none` and defaults to `none`; the template explains each mode immediately above the key. The older profile key `injection` and its `normal`/`inject` values remain aliases, with `injection_mode` winning when both keys are present. Merely selecting a process no longer silently enables injection. Graphics overrides require the application to be injected through the profile or a manual `[Injection]` entry. Legacy `[App.N]` sections still imply capture injection for compatibility. Profile-generated injection targets are merged after the manually configured lists and de-duplicated; this also fixes the old loader ordering bug that cleared the automatic `[App.N]` target later in the same load.
...
- Focused `ConfigTest.*:ConfigOverrideTest.*` passed 65 tests, including exact template/resource/generated-file parity, canonical-first aliases, legacy layout compatibility, `injection_mode` precedence plus the `injection` alias, unified profile audio/override behavior, cross-profile isolation, explicit profile modes, legacy App/AppAudio behavior, source overrides, and malformed-value cases.
- Final incremental build `0.1.5080` passed x64/x86 hooks, MediaEngine/CaptureEngine, 145 unit-test objects, 30 test apps, both Vulkan layers, packaging, import closure, PE mitigations, and PDB checks.
- The exact `0.1.5080` no-build gate passed all 1,716 native tests and all five Python tool self-tests.
