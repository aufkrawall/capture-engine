...
Last cross-checked: 2026-09-22 (hotkey keyboard-hook responsiveness rules)
...
- Fresh configs group capture selection in `[Capture]`, ordinary WGC behavior in `[WGC]` (including `wgc_same_device_capture`; it lives here, not in `[Diagnostics]`), file locations/container in `[Output]`, scaling in `[VideoScaling]`, UE engine overrides in `[UE5]`, NVIDIA runtime overrides in `[DLSS]`, audio timing in `[AudioSync]`, optional LibreHardwareMonitor overlay telemetry in `[HardwareSensors]`, in-game benchmark settings in `[Benchmark]` (start delay, duration, output directory, with hotkey under `[Hotkeys] benchmark=CTRL+7`), logging in `[Logging]`, and troubleshooting switches in `[Diagnostics]`.
...
- `[Screenshot] color_space=both|auto|bt709` is independent of the video setting. `both` is the default: an HDR source is published twice from the one capture, as a 10-bit BT.2020/PQ AVIF and a tone-mapped BT.709/sRGB PNG under one shared name seed, so the pair differs only by extension. `auto` preserves HDR as AVIF only and saves SDR as PNG; `bt709` tone-maps any HDR source to an ordinary BT.709/sRGB PNG. A source that is not in HDR has no second variant under any policy and always yields exactly one PNG. Missing or invalid values fall back to `both`. The option is present in the first-run template, but existing configs are intentionally not auto-merged; add the section explicitly when migrating an installed config.
- `[HardwareSensors] enabled=off|auto|on` controls the optional LibreHardwareMonitor bridge. Since 2026-09-03 the bridge is native first-party code inside `captureengine.exe` (`--sensor-bridge` role) rather than a PowerShell script beside it, so the only files a user supplies are LibreHardwareMonitor's own; nothing else about the option surface changed. `auto` silently retains native CPU/GPU/RAM/VRAM usage telemetry when the separately supplied files are absent; `on` additionally warns with only the missing safe filenames. `poll_interval_ms` accepts 250-10000; an active bridge also lowers the dedicated sensor-service wake interval to that value below one second, so sub-second sampling is actually published rather than accumulating in its pipe. Each metric selector is `off`, `auto`, or a bounded exact LibreHardwareMonitor identifier containing only ASCII letters, digits, `/`, `_`, `-`, and `.`; malformed values fall back independently. All nine selectors default to `auto` (`cpu_temperature`, `gpu_temperature`, `cpu_package_power`, `gpu_package_power`, `gpu_fan`, `cpu_core_clock`, `gpu_core_clock`, `gpu_memory_clock`, `gpu_voltage`); the previously conservative power/fan defaults were only needed while an unreadable rail could surface as a real zero. Existing `[Overlay] show_cpu` and `show_gpu` remain the row/display toggles. A live change to any effective hardware-sensor option restarts the long-lived sensor service (and the logger sharing its shutdown event), so selectors do not remain latched until the next application launch.
...
- `[DLSS] ngx_ota=default|off|on` governs NVIDIA's NGX over-the-air updates for the injected process, and it exists
  ...
  - Both mechanisms share one hard boundary: they cover what happens after CE is injected. The
    ...
    game, created in that window. How many escape after the DllMain-time environment publish is **unmeasured**.
    Updaters spawned by a process CE is not in at all (an NVIDIA service, the NVIDIA app) are out of reach by
    construction - but the nine in that session were all children of the game, which is the case this covers.
...
- ABI 55 carries nine optional hardware-sensor values plus independent validity bits in `SharedSystemMetrics` (ABI 50 added the first five; ABI 55 appended CPU/GPU core clock, GPU memory clock and GPU core voltage). They are controller configuration, not `OverlayConfig`; no managed code or selector string crosses into an injected process.
