...
void Cleanup()
{
    if (g_Device) {
        ...
        for (auto fb : g_Framebuffers) pfn_vkDestroyFramebuffer(g_Device, fb, nullptr);
        pfn_vkDestroyRenderPass(g_Device, g_RenderPass, nullptr);
        for (auto iv : g_SwapchainImageViews) pfn_vkDestroyImageView(g_Device, iv, nullptr);
    ...
    if (g_VulkanLib) FreeLibrary(g_VulkanLib);
}
