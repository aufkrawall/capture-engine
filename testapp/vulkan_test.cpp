...
static VkExtent2D g_SwapchainExtent = {
    0, 0}; // Set in main() from command line args
...
bool LoadVulkanLibrary() {
  ...
  pfn_vkGetInstanceProcAddr = (PFN_vkGetInstanceProcAddr)GetProcAddress(
      g_VulkanLib, "vkGetInstanceProcAddr");
  ...
  pfn_vkCreateInstance = (PFN_vkCreateInstance)pfn_vkGetInstanceProcAddr(
...
bool InitVulkan(HWND hwnd) {
  ...
  pfn_vkGetPhysicalDeviceQueueFamilyProperties(g_PhysDevice, &queueFamilyCount,
                                               nullptr);
  ...
  if (pfn_vkCreateDevice(g_PhysDevice, &deviceCreateInfo, nullptr, &g_Device) !=
      VK_SUCCESS) {
  ...
  swapchainInfo.presentMode = g_VSync ? VK_PRESENT_MODE_FIFO_KHR : VK_PRESENT_MODE_IMMEDIATE_KHR;  // VSync controlled by config
  swapchainInfo.clipped = VK_TRUE;

  if (pfn_vkCreateSwapchainKHR(g_Device, &swapchainInfo, nullptr,
                               &g_Swapchain) != VK_SUCCESS) {
  ...
  pfn_vkGetSwapchainImagesKHR(g_Device, g_Swapchain, &imageCount,
  ...
  for (uint32_t i = 0; i < imageCount; i++) {
    ...
    pfn_vkCreateImageView(g_Device, &viewInfo, nullptr,
  ...
  for (uint32_t i = 0; i < imageCount; i++) {
    ...
    pfn_vkCreateFramebuffer(g_Device, &framebufferInfo, nullptr,
                            &g_Framebuffers[i]);
  }
  ...
  for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
    pfn_vkCreateSemaphore(g_Device, &semaphoreInfo, nullptr,
...
void Render() {
  pfn_vkWaitForFences(g_Device, 1, &g_InFlightFences[g_CurrentFrame], VK_TRUE,
                      UINT64_MAX);
  pfn_vkResetFences(g_Device, 1, &g_InFlightFences[g_CurrentFrame]);

  uint32_t imageIndex;
  pfn_vkAcquireNextImageKHR(g_Device, g_Swapchain, UINT64_MAX,
  ...
  pfn_vkQueueSubmit(g_GraphicsQueue, 1, &submitInfo,
                    g_InFlightFences[g_CurrentFrame]);
...
int main(int argc, char *argv[]) {
  ...
  DWORD style = (g_WindowWidth >= 2560 || g_WindowHeight >= 1440)
                    ? WS_POPUP
                    : WS_OVERLAPPEDWINDOW;
  ...
  wchar_t title[256];
  swprintf(title, 256, L"Vulkan Test - %dx%d - GPU Load: %d", g_WindowWidth,
           g_WindowHeight, g_GpuLoadPasses);

  HWND hwnd = CreateWindowW(WINDOW_CLASS, title, style, 0, 0,
