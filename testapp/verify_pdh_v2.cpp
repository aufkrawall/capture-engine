...
void VerifyPdH(const char* targetLuid)
{
    ...
    for (int s = 0; s < 5; s++) {
        ...
        if (PdhGetFormattedCounterArrayA(gpuCounter, PDH_FMT_DOUBLE, &gpuBufSize, &gpuItemCount, gpuItems) ==
            ERROR_SUCCESS) {
            for (DWORD i = 0; i < gpuItemCount; i++) {
                ...
                if (instance && (strstr(instance, targetLuid) || strstr(instance, "luid_") == nullptr)) {
                    ...
                    if (strstr(instance, "engtype_3D"))
                        ...
                        sumCompute += val;
                    else if (strstr(instance, "VideoDecode") || strstr(instance, "VideoEncode"))
                        sumVideo += val;
        ...
        double vramUsed = 0;
        if (PdhGetFormattedCounterArrayA(vramCounter, PDH_FMT_DOUBLE, &vramBufSize, &vramItemCount, vramItems) ==
            ERROR_SUCCESS) {
        ...
        printf("Sample %d: GPU Load = %.2f%% (Smoothed: %.2f%%), VRAM Used = %.2f MiB (Smoothed: %.2f MiB)\n", s + 1,
               totalGpuLoad, smoothedGpu, vramUsed / (1024 * 1024), smoothedVram / (1024 * 1024));
    }
