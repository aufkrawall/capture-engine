...
void VerifyPdH(const char *targetLuid) {
  ...
  PdhAddEnglishCounterA(gpuQuery, "\\GPU Engine(*)\\Utilization Percentage", 0,
                        &gpuCounter);
  PdhAddEnglishCounterA(vramQuery, "\\GPU Adapter Memory(*)\\Dedicated Usage",
                        0, &vramCounter);
  ...
  for (int s = 0; s < 5; s++) {
    ...
    PdhGetFormattedCounterArrayA(gpuCounter, PDH_FMT_DOUBLE, &gpuBufSize,
                                 &gpuItemCount, NULL);
    std::vector<char> gpuBuffer(gpuBufSize);
    PDH_FMT_COUNTERVALUE_ITEM_A *gpuItems =
        (PDH_FMT_COUNTERVALUE_ITEM_A *)gpuBuffer.data();

    double sum3D = 0, sumCompute = 0, sumVideo = 0, sumVR = 0;
    if (PdhGetFormattedCounterArrayA(gpuCounter, PDH_FMT_DOUBLE, &gpuBufSize,
                                     &gpuItemCount,
                                     gpuItems) == ERROR_SUCCESS) {
      for (DWORD i = 0; i < gpuItemCount; i++) {
        const char *instance = gpuItems[i].szName;
        if (instance && (strstr(instance, targetLuid) ||
                         strstr(instance, "luid_") == nullptr)) {
          double val = gpuItems[i].FmtValue.doubleValue;
          if (strstr(instance, "engtype_3D"))
            sum3D += val;
          else if (strstr(instance, "engtype_Compute"))
            sumCompute += val;
          else if (strstr(instance, "VideoDecode") ||
                   strstr(instance, "VideoEncode"))
            sumVideo += val;
          else if (strstr(instance, "engtype_VR"))
            sumVR += val;
        }
      }
    }
    double totalGpuLoad = std::max({sum3D, sumCompute, sumVideo, sumVR});
    if (totalGpuLoad > 100.0)
      totalGpuLoad = 100.0;

    // VRAM
    DWORD vramBufSize = 0, vramItemCount = 0;
    PdhGetFormattedCounterArrayA(vramCounter, PDH_FMT_DOUBLE, &vramBufSize,
                                 &vramItemCount, NULL);
    std::vector<char> vramBuffer(vramBufSize);
    PDH_FMT_COUNTERVALUE_ITEM_A *vramItems =
        (PDH_FMT_COUNTERVALUE_ITEM_A *)vramBuffer.data();

    double vramUsed = 0;
    if (PdhGetFormattedCounterArrayA(vramCounter, PDH_FMT_DOUBLE, &vramBufSize,
                                     &vramItemCount,
                                     vramItems) == ERROR_SUCCESS) {
    ...
    printf("Sample %d: GPU Load = %.2f%% (Smoothed: %.2f%%), VRAM Used = %.2f "
           "MiB (Smoothed: %.2f MiB)\n",
           s + 1, totalGpuLoad, smoothedGpu, vramUsed / (1024 * 1024),
           smoothedVram / (1024 * 1024));
  }
