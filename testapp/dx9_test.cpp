...
bool InitDX9(HWND hwnd) {
    ...
    if (FAILED(g_pD3D->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, hwnd, D3DCREATE_HARDWARE_VERTEXPROCESSING, &d3dpp, &g_pd3dDevice))) return false;
    return true;
}
...
void Render() {
    ...
    if (SUCCEEDED(g_pd3dDevice->BeginScene())) {
        ...
        // Draw Bar
        D3DRECT rect = { (LONG)(g_BarPosition * (g_WindowWidth - 100)), g_WindowHeight / 2 - 50, (LONG)(g_BarPosition * (g_WindowWidth - 100) + 100), g_WindowHeight / 2 + 50 };
...
int main(int argc, char* argv[]) {
    ...
    WNDCLASSEXW wc = { sizeof(WNDCLASSEXW), CS_HREDRAW | CS_VREDRAW, WndProc, 0, 0, GetModuleHandle(nullptr), nullptr, LoadCursor(nullptr, IDC_ARROW), nullptr, nullptr, L"DX9Test", nullptr };
    RegisterClassExW(&wc);
    HWND hwnd = CreateWindowW(L"DX9Test", L"DX9 Test", WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, g_WindowWidth, g_WindowHeight, nullptr, nullptr, wc.hInstance, nullptr);
    if (!InitDX9(hwnd)) return 1;
