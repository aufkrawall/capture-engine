...
bool InitDX12(HWND hwnd) {
  ...
  factory->CreateSwapChainForHwnd(g_CommandQueue.Get(), hwnd, &swapChainDesc,
  ...
  g_Device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT,
                              g_CommandAllocators[g_FrameIndex].Get(), nullptr,
...
void Render() {
  ...
  // Draw bar
  D3D12_RECT scissor = {(LONG)(g_BarPosition * (g_WindowWidth - 100)),
                        g_WindowHeight / 2 - 50,
...
int main(int argc, char *argv[]) {
  ...
  wchar_t title[256];
  swprintf(title, 256, L"DX12 Test - %dx%d - GPU Load: %d", g_WindowWidth,
