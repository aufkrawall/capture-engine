...
static bool InitDX10(HWND hwnd) {
  ...
  HRESULT hr = D3D10CreateDeviceAndSwapChain(
      nullptr, D3D10_DRIVER_TYPE_HARDWARE, nullptr, flags, D3D10_SDK_VERSION,
  ...
  const char *vsSrc =
      ...
      "VSOut main(VSIn i){ VSOut o; o.pos=float4(i.pos,0,1); o.col=i.col; "
      "return o; }";
  ...
  hr = D3DCompile(vsSrc, strlen(vsSrc), nullptr, nullptr, nullptr, "main",
                  "vs_4_0", 0, 0, &vsBlob, &err);
  if (FAILED(hr))
    return false;

  hr = D3DCompile(psSrc, strlen(psSrc), nullptr, nullptr, nullptr, "main",
                  "ps_4_0", 0, 0, &psBlob, &err);
  if (FAILED(hr))
    return false;

  hr = g_Device->CreateVertexShader(vsBlob->GetBufferPointer(),
                                    vsBlob->GetBufferSize(), &g_VS);
  if (FAILED(hr))
    return false;

  hr = g_Device->CreatePixelShader(psBlob->GetBufferPointer(),
                                   psBlob->GetBufferSize(), &g_PS);
  if (FAILED(hr))
    return false;
  ...
  hr = g_Device->CreateInputLayout(layout, 2, vsBlob->GetBufferPointer(),
...
static void UpdateGeometry() {
  ...
  Vertex v[6] = {
      ...
      {{left, top}, {1, 1, 1, 1}},     {{right, bottom}, {1, 1, 1, 1}},
      {{left, bottom}, {1, 1, 1, 1}},
  };

  void *mappedData = nullptr;
  if (SUCCEEDED(g_VB->Map(D3D10_MAP_WRITE_DISCARD, 0, &mappedData)) &&
      mappedData) {
...
int main(int argc, char *argv[]) {
  ...
  HWND hwnd =
      CreateWindowW(WINDOW_CLASS, L"DX10 Test", WS_OVERLAPPEDWINDOW,
