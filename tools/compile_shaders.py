...
def main():
    ...
    for source, target, name in shaders:
        ...
        part_file = name[2:].lower() + ".h" if name.startswith("g_") else name.lower() + ".h"
        part_files.append(part_file)
        part = list(banner)
        part.append("#pragma once")
        part.append("#include <cstdint>")
        part.append("")
    ...
    umbrella.append("")
    umbrella.extend(f'#include "overlay_shader_bytecode/{part_file}"' for part_file in part_files)
    umbrella.append("")
