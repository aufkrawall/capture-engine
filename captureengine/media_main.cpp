    ...
    while (!g_InjectCaptureShutdown && g_Recording) {
        ...
        if (now - lastLog >= 2000) {
            if (pushedCount > 0 || droppedCount > 0 || pacingDroppedCount > 0) {
                LogInfo("[Inject Thread] Pushed: %u, Dropped(Full): %u, Dropped(Pacing): %u", pushedCount, droppedCount,
                        pacingDroppedCount);
                pushedCount = 0;
                droppedCount = 0;
...
int MediaProcessMain(const AppConfig& config)
{
        ...
        if (g_pSharedMem) {
            ...
            if (!config.wgcWindowTitles.empty() && (now - lastWindowScanTime > 1000)) {
                ...
                if (foundWindow && foundWindow != currentCapturedWindow) {
                    LogInfo("[Media] WGC Trigger: Found window (0x%p) matching config. Switching capture...",
                            foundWindow);
                    ...
                    if (!IsWindow(currentCapturedWindow)) {
                        LogInfo("[Media] Captured window 0x%p no longer valid. Reverting to monitor/inject.",
            ...
            if (currentSourcePid != 0 && currentSourcePid != lastSourcePid) {
                ...
                if (forceWGC) {
                    ...
                    if (hGameWindow) {
                        LogInfo("[Media] Whitelist Optimization: Found main window 0x%p. Switching WGC to Window Mode.",
                                hGameWindow);
                        ...
                        LogInfo("[Media] Whitelist: No main window found for PID %u. Using Monitor Capture.",
                                currentSourcePid);
                    }
    ...
    if (g_UseScreenGrab) {
        if (g_WgcCap) g_WgcCap->StopCapture();
        if (d3dContext) d3dContext->Release();
        if (d3dDevice) d3dDevice->Release();
    }

    if (g_pShmem) UnmapViewOfFile(g_pShmem);
    if (g_hMapShmem) CloseHandle(g_hMapShmem);

    if (g_pSharedMem) UnmapViewOfFile(g_pSharedMem);
