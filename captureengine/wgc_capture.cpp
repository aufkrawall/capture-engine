...
public:
  ...
  void OnFrameArrived(winrt::Direct3D11CaptureFramePool const &sender,
    ...
    if (SUCCEEDED(surface.as<IUnknown>()->QueryInterface(
            IID_IDirect3DDxgiInterfaceAccess, (void **)&access)) && access) {
      ...
      if (SUCCEEDED(access->GetInterface(__uuidof(ID3D11Texture2D),
        ...
        if (!cachedTexture_ || cachedWidth_ != (int32_t)desc.Width ||
          ...
          if (cachedTexture_) cachedTexture_->Release();
  ...
  bool StartCapture(uint32_t &width, uint32_t &height, bool captureCursor) {
    ...
    // CRITICAL: Must use CreateFreeThreaded (not Create) because we have no message pump!
    // Create() requires a DispatcherQueue pumping messages for callbacks to fire.
    // CreateFreeThreaded() uses an internal worker thread for callbacks.
    // Using 4 buffers (instead of 2) to reduce frame loss when draining is slow
...
void WGCCapture::SetDirectFrameCallback(
    std::function<void(ID3D11Texture2D*, uint32_t, uint32_t, int64_t)> callback) {
#if HAS_WGC
  if (impl_) {
