...
InjectionManager::InjectionManager(const AppConfig& config) : config(config)
{
    ...
    try {
        if (fs::exists(hookDllPathX64)) hookDllPathX64 = fs::absolute(hookDllPathX64).string();
        if (fs::exists(hookDllPathX86)) hookDllPathX86 = fs::absolute(hookDllPathX86).string();
    } catch (const fs::filesystem_error& e) {
        LogError("Filesystem error resolving absolute paths: %s", e.what());
    }

    // Check if DLLs exist
    if (!fs::exists(hookDllPathX64)) LogError("Capture Hook X64 DLL not found: %s", hookDllPathX64.c_str());
    if (!fs::exists(hookDllPathX86)) LogError("Capture Hook X86 DLL not found: %s", hookDllPathX86.c_str());
...
void InjectionManager::Update()
{
    ...
    injectedProcesses.erase(std::remove_if(injectedProcesses.begin(), injectedProcesses.end(),
...
ULONG STDMETHODCALLTYPE InjectionManager::ProcessEventSink::AddRef() { return InterlockedIncrement(&m_lRef); }
...
bool InjectionManager::Inject(DWORD pid, const std::string& processName)
{
    ...
    ce::HandleGuard hProcess(OpenProcess(PROCESS_QUERY_INFORMATION | PROCESS_VM_READ | PROCESS_VM_WRITE |
  ...
  LogInfo("DEBUG BUILD: Skipping DLL signature verification");
    ...
    auto GetRemoteProcAddress = [&](HANDLE hProc, HMODULE hModule, const char* funcName) -> LPVOID {
        ...
        if (!ReadProcessMemory(hProc, hModule, &dosHeader, sizeof(dosHeader), NULL)) return nullptr;
        ...
        if (!ReadProcessMemory(hProc, pNtHeaders, &ntHeaders, sizeof(ntHeaders), NULL)) return nullptr;
...
bool InjectionManager::HasActiveInjections() const { return !injectedProcesses.empty(); }
...
void InjectionManager::Eject(DWORD pid)
{
    ...
    if (it == injectedProcesses.end()) CloseHandle(hProcess);
}
