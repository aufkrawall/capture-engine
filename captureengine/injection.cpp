...
void InjectionManager::SetOnInjectCallback(
...
bool InjectionManager::IsWhitelisted(const std::string &processName) {
  ...
  for (const auto &item : config.gameWhitelist) {
    ...
    if (lowerName == lowerItem) {
      LogInfo("[WMI] Whitelist match (Exact): %s matches %s",
              processName.c_str(), item.c_str());
      return true;
    }
    if (lowerName.find(lowerItem) != std::string::npos) {
      LogInfo("[WMI] Whitelist match (Partial): %s matches %s",
              processName.c_str(), item.c_str());
      return true;
    }
  }
  ...
  for (const auto &item : config.overlayWhitelist) {
    ...
    std::transform(lowerItem.begin(), lowerItem.end(), lowerItem.begin(),
                   ::tolower);
    if (lowerName == lowerItem) {
      LogInfo("[WMI] Overlay target match (Exact): %s matches %s",
              processName.c_str(), item.c_str());
      return true;
    }
    if (lowerName.find(lowerItem) != std::string::npos) {
      LogInfo("[WMI] Overlay target match (Partial): %s matches %s",
              processName.c_str(), item.c_str());
      return true;
    }
  }
...
void InjectionManager::Update() {
  ...
  failedInjections.erase(std::remove_if(failedInjections.begin(),
...
InjectionManager::ProcessEventSink::QueryInterface(REFIID riid, void **ppv) {
...
HRESULT STDMETHODCALLTYPE InjectionManager::ProcessEventSink::Indicate(
  ...
  try {
    ...
    for (int i = 0; i < lObjectCount; i++) {
      ...
      if (FAILED(pUnk->QueryInterface(IID_IWbemClassObject,
                                      (void **)&pTargetCase)))
        continue;
      ...
      if (vName.vt == VT_BSTR && vPid.vt == VT_I4) {
        ...
        if (pManager->IsWhitelisted(name)) {
          // DELAYED INJECTION FOR D3D12 GAMES: Wait for window/D3D12
          // initialization This prevents crashes during early D3D12 setup in
          ...
          std::thread([managerShared, pid, name]() {
            ...
            for (int i = 0; i < 300 && !ready; i++) {
              ...
              if (!hProcess) {
                LogInfo("[WMI] %s (PID: %d) - Process exited before injection",
                        name.c_str(), pid);
                return;
              }
              ...
              if (!d3d12Loaded) {
                ...
                if (EnumProcessModules(hProcess, hMods, sizeof(hMods),
                                       &cbNeeded)) {
                  for (unsigned int j = 0; j < (cbNeeded / sizeof(HMODULE));
                       j++) {
                    char szModName[MAX_PATH];
                    if (GetModuleFileNameExA(hProcess, hMods[j], szModName,
                      ...
                      if (strstr(szModName, "d3d12.dll")) {
                        d3d12Loaded = true;
                        LogInfo("[WMI] %s (PID: %d) - D3D12.dll detected, "
                                "waiting for init...",
                                name.c_str(), pid);
                        break;
                      }
    ...
    LogError("WMI Indicate: COM exception 0x%X: %s", e.Error(),
             e.ErrorMessage());
...
HRESULT STDMETHODCALLTYPE InjectionManager::ProcessEventSink::SetStatus(
...
bool InjectionManager::Inject(DWORD pid, const std::string &processName) {
  ...
  if (onInjectCallback) {
    LogInfo("[Inject] Executing pre-injection callback for %s",
            processName.c_str());
  ...
  if (!VerifyDLLSignature(dllPath)) {
    LogError("[SECURITY] DLL signature verification failed for %s - refusing "
             "to inject",
             dllPath.c_str());
    LogError("[SECURITY] In production builds, only properly signed DLLs can "
             "be injected");
    return false;
  }
  ...
  auto GetRemoteProcAddress = [&](HANDLE hProc, HMODULE hModule,
                                  const char *funcName) -> LPVOID {
    // Read DOS Header
    ...
    if (!ReadProcessMemory(hProc, pNtHeaders, &ntHeaders, sizeof(ntHeaders),
                           NULL))
      return nullptr;
    ...
    if (!ReadProcessMemory(hProc, (BYTE *)hModule + exportDirRVA, &exportDir,
                           sizeof(exportDir), NULL))
      return nullptr;

    // Read Name Table
    std::vector<DWORD> nameRVAs(exportDir.NumberOfNames);
    if (!ReadProcessMemory(hProc, (BYTE *)hModule + exportDir.AddressOfNames,
                           nameRVAs.data(), nameRVAs.size() * sizeof(DWORD),
                           NULL))
      return nullptr;
    ...
    for (DWORD i = 0; i < exportDir.NumberOfNames; i++) {
      char buffer[256];
      if (ReadProcessMemory(hProc, (BYTE *)hModule + nameRVAs[i], buffer,
                            sizeof(buffer), NULL)) {
        ...
        for (unsigned int i = 0; i < (cbNeeded / sizeof(HMODULE)); i++) {
          ...
          if (GetModuleFileNameExA(hProcess.get(), hMods[i], szModName,
            ...
            if (modName.find("kernel32.dll") != std::string::npos) {
              // Found kernel32!
              pLoadLibrary = GetRemoteProcAddress(hProcess.get(), hMods[i],
                                                  "LoadLibraryA");

              if (pLoadLibrary)
                LogInfo(
                    "Resolved LoadLibraryA in x86 process at 0x%p (Base: 0x%p)",
                    pLoadLibrary, hMods[i]);
              else
                LogError("Failed to resolve LoadLibraryA in x86 process via PE "
                         "parsing");
              goto found_kernel32;
            }
          }
  ...
  ce::VirtualAllocGuard pRemotePath(
      hProcess.get(), VirtualAllocEx(hProcess.get(), NULL, dllPath.size() + 1,
  ...
  ce::HandleGuard hThread(CreateRemoteThread(
      hProcess.get(), NULL, 0, (LPTHREAD_START_ROUTINE)pLoadLibrary,
  ...
  if (GetExitCodeThread(hThread.get(), &exitCode)) {
    if (exitCode == 0) {
      ...
      LogError("LoadLibraryA failed in remote process (Exit Code: 0). DLL "
               "failed to load.");
      return false;
    } else {
...
static std::string ComputeFileHash(const std::string &path) {
  ...
  if (BCryptCreateHash(hAlg, &hHash, pbHashObject.data(), cbHashObject, NULL, 0,
                       0) < 0) {
...
bool InjectionManager::ValidateDllSecurity(const std::string &dllPath) {
  ...
  if (checkPath.string().find(exePath.string()) != 0) {
    LogError("[Security] DLL path is outside application directory: %s",
             checkPath.string().c_str());
    return false;
  }
  ...
  if (GetNamedSecurityInfoA(dllPath.c_str(), SE_FILE_OBJECT,
    ...
    AllocateAndInitializeSid(&SIDAuth, 1, SECURITY_WORLD_RID, 0, 0, 0, 0, 0, 0,
                             0, &pEveryoneSid);
    ...
    if (access &
      ...
      LogError("[Security] DLL is writable by Everyone! Access Mask: 0x%X",
               access);
...
bool InjectionManager::VerifyDLLSignature(const std::string &dllPath) {
  ...
  trustData.fdwRevocationChecks =
      WTD_REVOKE_NONE; // Skip revocation check for performance
...
bool InjectionManager::VerifyDLLHash(const std::string &dllPath) {
  ...
  if (!fs::exists(hashFilePath)) {
    ...
    LogDebug("[Security] No hash file found for %s, skipping hash verification",
             dllPath.c_str());
    return true;
  }
...
std::string InjectionManager::ComputeFileHash(const std::string &filePath) {
  ...
  if (hFile == INVALID_HANDLE_VALUE) {
    LogError("[Security] Failed to open file for hashing: %s",
             filePath.c_str());
    return "";
  }
  ...
  if (BCryptOpenAlgorithmProvider(&hAlg, BCRYPT_SHA256_ALGORITHM, NULL, 0) ==
      STATUS_SUCCESS) {
    DWORD hashLen = 0;
    DWORD dataLen = 0;

    if (BCryptGetProperty(hAlg, BCRYPT_HASH_LENGTH, (PUCHAR)&hashLen,
                          sizeof(hashLen), &dataLen, 0) == STATUS_SUCCESS) {
      std::vector<BYTE> hashBytes(hashLen);

      if (BCryptCreateHash(hAlg, &hHash, NULL, 0, NULL, 0, 0) ==
          STATUS_SUCCESS) {
        if (BCryptHashData(hHash, (PUCHAR)pData, (ULONG)fileSize.QuadPart, 0) ==
            STATUS_SUCCESS) {
          if (BCryptFinishHash(hHash, hashBytes.data(), hashLen, 0) ==
              STATUS_SUCCESS) {
  ...
  if (Process32First(hSnapshot, &pe32)) {
    do {
      ...
      if (IsWhitelisted(name)) {
        if (!IsAlreadyInjected(pe32.th32ProcessID) &&
            !IsRecentlyFailed(pe32.th32ProcessID)) {
          LogInfo("[Scan] Found existing whitelisted process: %s (PID: %d)",
                  name.c_str(), pe32.th32ProcessID);
