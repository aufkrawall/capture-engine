  ...
  bool InitializeDup(ID3D11Device *device) {
    ...
    if (FAILED(hr)) {
      if (hr == DXGI_ERROR_NOT_CURRENTLY_AVAILABLE) {
        LogError("[DXGI] Desktop Duplication not available (limit reached or "
...
DxgiCapture::~DxgiCapture() { StopCapture(); }

bool DxgiCapture::Init(ID3D11Device *device, int monitorIndex,
                       std::mutex * /*d3dMutex*/) {
  device_ = device;
  device_->GetImmediateContext(&context_);
  impl_->monitorIdx_ = monitorIndex;
  // NOTE: d3dMutex parameter is deprecated. We use MediaEngine_LockD3D11()
  // instead.
  ...
  initialized_ = true;
  LogInfo("[DXGI] Initialized for monitor %d (%dx%d)", monitorIndex, width_,
          height_);
  return true;
}
...
void DxgiCapture::ResetStats() { droppedFrames_ = 0; }

bool DxgiCapture::GetNextFrame(WGCCapturedFrame &frame, bool skipCopy) {
  if (!capturing_ || !impl_->dup_)
    return false;

  IDXGIResource *desktopResource = nullptr;
  // Use 20ms timeout to avoid busy loop and give DDAPI time to lock surface
  HRESULT hr =
      impl_->dup_->AcquireNextFrame(20, &impl_->frameInfo_, &desktopResource);

  if (hr == DXGI_ERROR_WAIT_TIMEOUT) {
    // Log verbose only? No, duplicate output wait timeout is normal if screen
    // is static. But if game is running, it should update.
    return false;
  }
  ...
  if (desktopResource) {
    ...
    if (SUCCEEDED(hr)) {
      ...
      if (!impl_->mappedTexture_ || desc.Width != width_ ||
          desc.Height != height_) {
        if (impl_->mappedTexture_)
          impl_->mappedTexture_->Release();
