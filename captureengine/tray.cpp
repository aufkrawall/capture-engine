...
void TrayIcon::InitIcon() {
  hIconIdle = (HICON)LoadImageA(hInstance, MAKEINTRESOURCEA(IDI_ICON_IDLE),
                                IMAGE_ICON, 0, 0, LR_DEFAULTSIZE | LR_SHARED);
  hIconRecording =
      (HICON)LoadImageA(hInstance, MAKEINTRESOURCEA(IDI_ICON_RECORDING),
                        IMAGE_ICON, 0, 0, LR_DEFAULTSIZE | LR_SHARED);
  hIconShutdown =
      (HICON)LoadImageA(hInstance, MAKEINTRESOURCEA(IDI_ICON_SHUTDOWN),
                        IMAGE_ICON, 0, 0, LR_DEFAULTSIZE | LR_SHARED);
...
void TrayIcon::SetRecordingState(bool recording) {
  ...
  strcpy_s(nid.szTip,
           recording ? "Capture Engine (Recording...)" : "Capture Engine");
