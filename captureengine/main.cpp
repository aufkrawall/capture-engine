...
bool ConnectToChildProcesses(DWORD timeoutMs)
{
    ...
    while (GetTickCount() - startTime < timeoutMs) {
        ...
        if (g_hInjectProcess && !g_InjectClient->IsConnected()) {
            if (!g_InjectClient->Connect(100)) allConnected = false;
        }
        if (g_hMediaProcess && !g_MediaClient->IsConnected()) {
            if (!g_MediaClient->Connect(100)) allConnected = false;
        }
        if (g_hLimiterProcess && !g_LimiterClient->IsConnected()) {
            if (!g_LimiterClient->Connect(100)) allConnected = false;
        }
...
void ToggleRecording()
{
    ...
    if (g_Tray) g_Tray->SetRecordingState(g_Recording);
}
...
void ShutdownChildProcesses()
{
    ...
    if (g_hMediaProcess) handles[handleCount++] = g_hMediaProcess;
    if (g_hInjectProcess) handles[handleCount++] = g_hInjectProcess;
    if (g_hLoggerProcess) handles[handleCount++] = g_hLoggerProcess;
    if (g_hSensorProcess) handles[handleCount++] = g_hSensorProcess;
    ...
    // Cleanup handles
    if (g_hLimiterProcess) CloseHandle(g_hLimiterProcess);
    if (g_hMediaProcess) CloseHandle(g_hMediaProcess);
    if (g_hInjectProcess) CloseHandle(g_hInjectProcess);
    if (g_hLoggerProcess) CloseHandle(g_hLoggerProcess);
    if (g_hSensorProcess) CloseHandle(g_hSensorProcess);
...
void CheckChildProcessHealth()
{
    ...
    if (GetTickCount() - lastCheck < 1000) return;  // Check once per second
...
static void Registry_ManageImplicitLayer(bool install)
{
    ...
    if (RegCreateKeyExA(HKEY_CURRENT_USER, "Software\\Khronos\\Vulkan\\ImplicitLayers", 0, NULL,
