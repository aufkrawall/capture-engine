...
void LaunchGameSuspended(const std::string &path) {
  ...
  if (cleanPath.length() >= 2 && cleanPath.front() == '"' &&
      cleanPath.back() == '"') {
  ...
  if (looksLikeLauncher) {
    ...
    if (CreateProcessA(cleanPath.c_str(), NULL, NULL, NULL, FALSE, 0, NULL,
                       dir.c_str(), &si, &pi)) {
      LogInfo("[Launcher] Launcher started (PID: %d). WMI will catch the game.",
              pi.dwProcessId);
    ...
    LogInfo("[Launcher] Detected game: %s - Launching Suspended",
            cleanPath.c_str());
...
void ShutdownChildProcesses() {
  ...
  const char *handleNames[5] = {"Limiter", "Media", "Inject", "Logger",
                                "Sensor"};
  int handleCount = 0;
  ...
  if (handleCount > 0) {
    ...
    if (!allExited) {
      ...
      LogInfo("[Controller] Some processes didn't exit cleanly within timeout, "
              "terminating...");
      for (int i = 0; i < handleCount; i++) {
        DWORD exitCode;
        if (GetExitCodeProcess(handles[i], &exitCode) &&
            exitCode == STILL_ACTIVE) {
          LogInfo("[Controller] %s process did not exit cleanly, forcing "
                  "termination",
                  handleNames[i]);
        }
      }
...
int ControllerMain(HINSTANCE hInstance) {
  ...
  g_hLoggerProcess =
      SpawnChildProcess(ProcessMode::Logger, g_ConfigPath.c_str());
  g_hSensorProcess =
      SpawnChildProcess(ProcessMode::Sensors, g_ConfigPath.c_str());
  ...
  if (!g_DeferredLaunchPath.empty()) {
    LogInfo("[Controller] Launching deferred game: %s",
            g_DeferredLaunchPath.c_str());
  ...
  RegisterHotKey(NULL, HOTKEY_ID_RECORD,
                 g_Config.hotkeyStartStop.GetModifiers(),
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  if (launchPos != std::string::npos) {
    ...
    while (valueStart < cmdLine.length() &&
           (cmdLine[valueStart] == '=' || cmdLine[valueStart] == ' ')) {
      valueStart++;
    }
    ...
    if (valueStart < cmdLine.length()) {
      ...
      LogInfo("[Launcher] Deferred launch path: %s",
              g_DeferredLaunchPath.c_str());
