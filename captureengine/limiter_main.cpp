 ...
 * =============================================
 * 
 * This FPS limiter uses Windows high-resolution timers to achieve precise frame pacing.
 * 
 * KEY BEHAVIORS:
 * 
 * 1. timeBeginPeriod(1):
...
void ApplyFramePacing(SharedMemoryLayout *shm) {
  ...
  if (g_LastIntervalTicks > 0 && intervalTicks != g_LastIntervalTicks) {
    LogInfo("[Limiter] Rate change: %lld -> %lld ticks (%.2f fps). Resetting.",
  ...
  shm->fpsLimiter.targetTimeTicks.store(g_TargetTime.QuadPart,
...
int LimiterProcessMain(const AppConfig &config) {
  ...
  if (hDiscovery) {
    ...
    if (pDiscovery && pDiscovery->magic == DISCOVERY_MAGIC && pDiscovery->injectPid != 0) {
      ...
      if (hMapFile) {
        shm = (SharedMemoryLayout *)MapViewOfFile(
  ...
  swprintf_s(releaseName, L"Local\\CaptureLimiterRelease_%d",
             GetCurrentProcessId());
  swprintf_s(requestName, L"Local\\CaptureLimiterRequest_%d",
             GetCurrentProcessId());

  HANDLE hReleaseEvent =
      CreateEventW(NULL, FALSE, FALSE, releaseName); // Auto-reset
  HANDLE hRequestEvent =
      CreateEventW(NULL, FALSE, FALSE, requestName); // Auto-reset
  ...
  if (shm)
    lastSessionId =
        shm->fpsLimiter.hookSessionId.load(std::memory_order_relaxed);
  ...
  while (g_Running) {
    ...
    uint32_t currentSessionId =
        shm->fpsLimiter.hookSessionId.load(std::memory_order_acquire);
    if (currentSessionId != lastSessionId) {
      LogInfo("[Limiter] Hook session changed (%u -> %u). Resetting state.",
    ...
    DWORD waitResult =
        WaitForSingleObject(hRequestEvent, 100); // 100ms timeout for IPC check
    ...
    if (waitResult == WAIT_OBJECT_0) {
      ...
      // Check request count
      uint32_t requested =
          shm->fpsLimiter.requestCount.load(std::memory_order_acquire);
