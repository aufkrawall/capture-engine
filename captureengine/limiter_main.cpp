...
void ApplyFramePacing(SharedMemoryLayout* shm)
{
    ...
    if (isRecording && shm->fpsLimiter.captureSyncEnabled) {
        ...
        if (captureFps > 0 && multiplier >= 1 && multiplier <= 8) {
            ...
            if (!loggedOnce) {
                LogInfo("[Limiter] Capture sync: captureFps=%d, mult=%d, targetFps=%d, intervalTicks=%lld", captureFps,
                        multiplier, captureFps * multiplier, intervalTicks);
                loggedOnce = true;
            }
        }
    ...
    if (ticksUntilTarget > 0) {
        ...
        while (true) {
            QueryPerformanceCounter(&now);
            if (now.QuadPart >= g_TargetTime.QuadPart) break;
...
int LimiterProcessMain(const AppConfig& config)
{
    ...
    if (shm) lastSessionId = shm->fpsLimiter.hookSessionId.load(std::memory_order_relaxed);
