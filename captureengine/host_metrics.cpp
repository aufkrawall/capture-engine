...
void HostMetricsState::Initialize() {
  ...
  if (!gpuPdhInitialized) {
    if (PdhOpenQueryA(NULL, 0, &gpuQuery) == ERROR_SUCCESS) {
      ...
      if (status != ERROR_SUCCESS) {
        status =
            PdhAddCounterA(gpuQuery, "\\GPU Engine(*)\\Utilization Percentage",
                           0, &gpuCounter);
      }
  ...
  if (!vramPdhInitialized) {
    if (PdhOpenQueryA(NULL, 0, &vramQuery) == ERROR_SUCCESS) {
      ...
      PDH_STATUS status = PdhAddEnglishCounterA(
          vramQuery, "\\GPU Adapter Memory(*)\\Dedicated Usage", 0,
          &vramCounter);
      if (status != ERROR_SUCCESS) {
        status = PdhAddCounterA(vramQuery,
      ...
      if (status == ERROR_SUCCESS) {
        ...
        LogInfo("[Metrics] VRAM Usage PDH init failed with status: 0x%X",
                status);
...
uint64_t HostMetricsState::QueryVRAMTotalFromDXGI(int32_t luidLow,
                                                  int32_t luidHigh) {
  if (!dxgiFactory)
    return 0;
  ...
  for (UINT i = 0;
       dxgiFactory->EnumAdapters1(i, &adapter) != DXGI_ERROR_NOT_FOUND; ++i) {
    DXGI_ADAPTER_DESC1 desc;
    if (SUCCEEDED(adapter->GetDesc1(&desc))) {
      if (desc.AdapterLuid.LowPart == (DWORD)luidLow &&
        ...
        LogInfo("[Metrics] DXGI: Found VRAM Total %llu MB for LUID %08x:%08x",
                cachedVRAMTotal / (1024 * 1024), desc.AdapterLuid.HighPart,
  ...
  LogInfo("[Metrics] DXGI: No adapter found matching LUID %08x:%08x", luidHigh,
          luidLow);
  return 0;
}

void UpdateSystemMetrics(SharedMemoryLayout *shm, uint32_t targetPid,
                         int64_t luid) {
  if (!shm)
    return;
  ...
  if (g_HostMetrics.pdhInitialized) {
    if (PdhCollectQueryData(g_HostMetrics.cpuQuery) == ERROR_SUCCESS) {
      ...
      if (PdhGetFormattedCounterValue(g_HostMetrics.cpuCounter, PDH_FMT_DOUBLE,
                                      NULL, &value) == ERROR_SUCCESS) {
        shm->systemMetrics.cpuUsage.store((float)value.doubleValue,
  ...
  if (GlobalMemoryStatusEx(&memInfo)) {
    double usedGB = (double)(memInfo.ullTotalPhys - memInfo.ullAvailPhys) /
  ...
  if (g_HostMetrics.gpuPdhInitialized && g_HostMetrics.gpuCounter) {
    ...
    PdhGetFormattedCounterArrayA(g_HostMetrics.gpuCounter, PDH_FMT_DOUBLE,
    ...
    if (bufSize > 0) {
      ...
      if (g_HostMetrics.pdhBuffer) {
        PDH_FMT_COUNTERVALUE_ITEM_A *items =
            (PDH_FMT_COUNTERVALUE_ITEM_A *)g_HostMetrics.pdhBuffer;
        if (PdhGetFormattedCounterArrayA(g_HostMetrics.gpuCounter,
                                         PDH_FMT_DOUBLE,
                                         &g_HostMetrics.pdhBufferSize,
                                         &itemCount, items) == ERROR_SUCCESS) {
          if (debugLogging) {
            LogInfo("[Metrics] GPU Poll: Found %d engine items. Searching for "
                    "%s or %s",
          ...
          for (DWORD i = 0; i < itemCount; i++) {
            if (strstr(items[i].szName, luidStrUpper) ||
                strstr(items[i].szName, luidStrLower)) {
              foundAny = true;
              // Simplified logic: Just capture basic 3D/Compute loads if
              // possible, or SUM ALL? For now, mirroring hook logic: Sum all
              // non-video
              bool isVideo = strstr(items[i].szName, "VideoDecode") ||
                             strstr(items[i].szName, "VideoEncode");
              if (!isVideo) {
          ...
          if (debugLogging && !foundAny && luid != 0) {
            ...
            // running yet.
            LogInfo("[Metrics] Warning: No GPU engine found matching LUID %s",
                    luidStrUpper);
          }
          if (totalLoad > 100.0)
            totalLoad = 100.0;
          shm->systemMetrics.gpuUsage.store((float)totalLoad,
                                            std::memory_order_relaxed);
        }
      }
    }
  ...
  if (NtQuerySystemInformation && numProcs > 0) {
    ...
    ULONG len = 0;
    if (NtQuerySystemInformation(
            SystemProcessorPerformanceInformation, currCpuInfo.data(),
            sizeof(SYSTEM_PROCESSOR_PERFORMANCE_INFORMATION) * numProcs,
            &len) >= 0) {
      float maxCore = 0.0f;
      if (!prevCpuInfo.empty() && prevCpuInfo[0].IdleTime.QuadPart != 0) {
        for (int i = 0; i < numProcs; i++) {
          uint64_t idle = currCpuInfo[i].IdleTime.QuadPart -
                          prevCpuInfo[i].IdleTime.QuadPart;
          uint64_t kernel = currCpuInfo[i].KernelTime.QuadPart -
                            prevCpuInfo[i].KernelTime.QuadPart;
          uint64_t user = currCpuInfo[i].UserTime.QuadPart -
                          prevCpuInfo[i].UserTime.QuadPart;
      ...
      shm->systemMetrics.maxCoreLoad.store((uint32_t)maxCore,
                                           std::memory_order_relaxed);
    }
  }
  ...
  if (g_HostMetrics.vramPdhInitialized && g_HostMetrics.vramCounter) {
    ...
    DWORD bufSize = 0, itemCount = 0;
    PdhGetFormattedCounterArrayA(g_HostMetrics.vramCounter, PDH_FMT_LARGE,
                                 &bufSize, &itemCount, NULL);
    if (bufSize > 0) {
      std::vector<BYTE> tempBuf(bufSize);
      PDH_FMT_COUNTERVALUE_ITEM_A *items =
          (PDH_FMT_COUNTERVALUE_ITEM_A *)tempBuf.data();
      if (PdhGetFormattedCounterArrayA(g_HostMetrics.vramCounter, PDH_FMT_LARGE,
                                       &bufSize, &itemCount,
                                       items) == ERROR_SUCCESS) {
        if (debugLogging) {
          LogInfo("[Metrics] VRAM Poll: Found %d adapter items.",
                  (int)itemCount);
        }
        int64_t totalVRAM = 0;
        bool foundAny = false;
        ...
        if (debugLogging && !foundAny && luid != 0) {
          // Only warn if LUID is valid (non-zero). Zero LUID means no game is
          // running yet.
          LogInfo("[Metrics] Warning: No VRAM adapter found matching LUID %s",
                  luidStrUpper);
        }
        double vramMB = (double)totalVRAM / (1024.0 * 1024.0);
        shm->systemMetrics.vramUsage.store((float)vramMB,
                                           std::memory_order_relaxed);
      }
    }
  }
  ...
  if (luid != 0) {
    uint64_t vramTotal =
        g_HostMetrics.QueryVRAMTotalFromDXGI((int32_t)low, (int32_t)high);
    if (vramTotal > 0) {
