...
typedef void(STDMETHODCALLTYPE *CreateSamplerPtr)(ID3D12Device *,
...
DetourCreateSwapChainForHwnd(IDXGIFactory2 *pThis, IUnknown *pDevice, HWND hWnd,
...
void DX12_InvalidateSwapchain() {
  DXGIShared::g_SharedState.swapchainInvalid.store(true,
  ...
  // CRITICAL FIX: Defer overlay cleanup to next frame to avoid interfering with
  // FSR swapchain creation FSR FG is sensitive to D3D12 operations during its
  // swapchain setup
  if (g_State.imGuiInit) {
    HookLog("DX12: Deferring overlay shutdown to next frame (avoiding FSR "
            "swapchain conflict)");
...
void DX12_SignalFSR4SwapchainRecreated() {
  DXGIShared::g_SharedState.fsr4RecreationPending.store(
...
void DX12_NotifyCommandLists(UINT numCommandLists) {
  g_CommandListsExecutedThisFrame.fetch_add(numCommandLists,
...
static HRESULT STDMETHODCALLTYPE DetourCreateSwapChainGlobal(
  ...
  // The pDevice passed to CreateSwapChain is actually the command queue for
  // D3D12
  if (pDevice) {
...
static HRESULT STDMETHODCALLTYPE DetourCreateSwapChainForHwndGlobal(
  ...
  if (pDevice) {
    ...
    if (SUCCEEDED(pDevice->QueryInterface(IID_PPV_ARGS(&q)))) {
      DX12_HookQueueVTable(q);
      HookLog(
          "DX12: Global CreateSwapChainForHwnd - Command queue vtable hooked");
      q->Release();
    }
  }
  ...
  if (SUCCEEDED(hr) && ppSC && *ppSC) {
    ...
    if (SUCCEEDED((*ppSC)->QueryInterface(IID_PPV_ARGS(&sc3)))) {
      ...
      sc3->Release();
      HookLog(
          "DX12: Global CreateSwapChainForHwnd - Hooks installed on swapchain");
    }
  }
...
static void InstallGlobalVTableHooks() {
  ...
  if (VTableHook::Create(&vtable[10], (LPVOID)DetourCreateSwapChainGlobal,
                         (LPVOID *)&oCreateSwapChainGlobal)) {
    HookLog("DX12: Hooked global CreateSwapChain at vtable[10]");
  }

  if (VTableHook::Create(&vtable[15],
                         (LPVOID)DetourCreateSwapChainForHwndGlobal,
...
static void HookSwapchainVTableViaTempSwapchain() {
  ...
  PFN_CreateDXGIFactory1 pCreateFactory =
      (PFN_CreateDXGIFactory1)GetProcAddress(hDXGI, "CreateDXGIFactory1");
  PFN_D3D12CreateDevice pD3D12CreateDevice =
      (PFN_D3D12CreateDevice)GetProcAddress(hD3D12, "D3D12CreateDevice");
  ...
  if (FAILED(pD3D12CreateDevice(nullptr, D3D_FEATURE_LEVEL_11_0,
  ...
  HWND hwnd = CreateWindowExW(0, L"CE_Temp", L"", WS_POPUP, 0, 0, 2, 2, nullptr,
...
bool InitImGui(ID3D12Device *device, int buffers, DXGI_FORMAT format,
  ...
  if (g_OverlayAdapter.IsInitialized()) {
    HookLog("InitImGui: OverlayAdapter already initialized, shutting down for "
            "re-init");
    g_OverlayAdapter.Shutdown();
  }
  ...
  HookLog("InitImGui: Proceeding with initialization - buffers=%d, format=%d, "
          "hwnd=%p",
          buffers, format, hwnd);
  ...
  if (g_UseCachedRenderer && !g_CachedOverlayRenderer) {
    ...
    if (!g_CachedOverlayRenderer->Initialize(device, g_OverlayQueue, buffers)) {
      HookLog("DX12: Failed to initialize cached overlay renderer, falling "
              "back to standard");
      delete g_CachedOverlayRenderer;
      g_CachedOverlayRenderer = nullptr;
      g_UseCachedRenderer = false;
    } else {
      HookLog("DX12: Cached overlay renderer initialized for %d buffers",
              buffers);
    }
  }
...
void DrawOverlay(ID3D12GraphicsCommandList *cmdList, bool isRealFrame,
                 UINT bufferIdx) {
...
void CreateRTVs(ID3D12Device *device, IDXGISwapChain3 *swapChain,
  ...
  D3D12_DESCRIPTOR_HEAP_DESC rtvHeapDesc = {D3D12_DESCRIPTOR_HEAP_TYPE_RTV,
                                            (UINT)bufferCount,
...
void InitOverlaySync(ID3D12Device *device, int bufferCount) {
  ...
  // We use CPU-side WaitForSingleObject (not GPU-side Queue->Wait) to avoid
  // deadlocks.
  if (!g_OverlayQueue) {
    ...
    HookLog("InitOverlaySync: Created dedicated overlay queue %p",
            g_OverlayQueue);
  }

  if (FAILED(device->CreateFence(0, D3D12_FENCE_FLAG_NONE,
                                 IID_PPV_ARGS(&g_State.fence))))
    return;
...
void DX12_OnSwapchainResizeBegin() {
  ...
  HookLog("DX12: DX12_OnSwapchainResizeBegin called, wasAlreadySet=%d",
          wasAlreadySet);

  // Prevent recursion - if already in resize, return immediately
  if (wasAlreadySet) {
    HookLog("DX12: DX12_OnSwapchainResizeBegin - already in resize, returning "
            "early");
    return;
  }

  DXGIShared::g_SharedState.lastSwapchainCreation =
...
static void ApplyPrerenderLimitDX12(float limit) {
  ...
  if (g_PrerenderFences.empty()) {
    for (int i = 0; i < 16; i++) {
      ID3D12Fence *fence = nullptr;
      HANDLE event = CreateEvent(nullptr, FALSE, FALSE, nullptr);
      if (SUCCEEDED(ctx.device->CreateFence(0, D3D12_FENCE_FLAG_NONE,
                                            IID_PPV_ARGS(&fence)))) {
    ...
    HookLog("DX12: Created prerender limit fence ring buffer (size: %d)",
            (int)g_PrerenderFences.size());
  }
  ...
  if (limit == 0.0f) {
    ...
    if (g_PrerenderFrameIndex >= (uint64_t)lookback) {
      size_t waitIdx =
          (g_PrerenderFrameIndex - lookback) % g_PrerenderFences.size();
...
void ProcessFrame(IDXGISwapChain *pSwapChain, bool processCapture) {
  ...
  if (!pSwapChain || inResize) {
    HookLog("DX12: ProcessFrame - early return (null=%d, inResize=%d)",
            !pSwapChain, inResize);
    return;
  }
  ...
  if (!g_OverlayQueue && g_Device) {
    ...
    qd.NodeMask = 0;
    HRESULT hr =
        g_Device->CreateCommandQueue(&qd, IID_PPV_ARGS(&g_OverlayQueue));
    if (SUCCEEDED(hr)) {
      g_OverlayQueue->SetName(L"CE_OverlayQueue");
      HookLog("DX12: ProcessFrame - Created overlay queue %p (early creation)",
              g_OverlayQueue);
    } else {
      HookLog("DX12: ProcessFrame - Failed to create overlay queue (early) "
              "hr=0x%08X",
              hr);
    }
  }
  ...
  if (!g_Device || pSwapChain != g_LastSwapChain) {
    ...
    if (!s_metricsInitialized && pSwapChain) {
      ...
      if (SUCCEEDED(pSwapChain->GetDesc(&swapDesc))) {
        ...
        if (SUCCEEDED(pSwapChain->GetDevice(IID_PPV_ARGS(&pDeviceUnk)))) {
          ...
          if (SUCCEEDED(
                  pDeviceUnk->QueryInterface(IID_PPV_ARGS(&dxgiDevice)))) {
            ...
            if (SUCCEEDED(dxgiDevice->GetAdapter(&adapter))) {
              ...
              if (SUCCEEDED(adapter->GetDesc(&desc))) {
                ...
                HookLog("DX12: SystemMetricsCollector initialized with LUID "
                        "%08X:%08X, VRAM: %llu MB",
            ...
            if (SUCCEEDED(CreateDXGIFactory1(IID_PPV_ARGS(&factory)))) {
              ...
              if (SUCCEEDED(factory->EnumAdapters(0, &adapter))) {
                ...
                if (SUCCEEDED(adapter->GetDesc(&desc))) {
                  ...
                  SystemMetricsCollector::Get().SetVRAMTotal(
                      desc.DedicatedVideoMemory);
                  HookLog("DX12: SystemMetricsCollector FALLBACK init with "
    ...
    if (!s_metricsInitLogged) {
      ...
      HookLog("DX12: DIAGNOSTIC - SystemMetricsCollector GPU usage: %.1f%%, "
              "VRAM: %.1f MB",
    ...
    if (g_Device == nullptr || activeDevice != g_Device ||
      ...
      if (!g_OverlayQueue && g_Device) {
        ...
        if (SUCCEEDED(hr)) {
          g_OverlayQueue->SetName(L"CE_OverlayQueue");
          HookLog("DX12: ProcessFrame - Created overlay queue %p (immediate "
                  "after device set)",
                  g_OverlayQueue);
        } else {
          HookLog("DX12: ProcessFrame - Failed to create overlay queue "
                  "(immediate) hr=0x%08X",
                  hr);
        }
      }
  ...
  if (!g_State.imGuiInit) {
    ...
    if (frames < 1) {
      ...
      HookLog("DX12: ProcessFrame - Proceeding with ImGui init (frame %d)",
              frames);
    }
    ...
    if (g_OverlayAdapter.IsInitialized()) {
      ...
      HookLog(
          "DX12: ProcessFrame - cleaning up stale OverlayAdapter (mutex held)");
      g_OverlayAdapter.Shutdown();
      CleanupOverlay();
      CleanupRTVs();
    ...
    if (SUCCEEDED(pSwapChain->GetDesc(&desc))) {
      ...
      if (SUCCEEDED(pSwapChain->QueryInterface(IID_PPV_ARGS(&sc3)))) {
        ...
        if (validBuffers < imguiBufferCount) {
          HookLog("DX12: ProcessFrame - only %d/%d buffers valid, skipping "
        ...
        if (InitImGui(g_Device, imguiBufferCount, desc.BufferDesc.Format,
          ...
          if (actualBufferCount > 8) {
            HookLog("DX12: Swapchain has %d buffers, limiting RTVs to 8",
                    actualBufferCount);
            actualBufferCount = 8;
          }
  ...
  if (++s_preOverlayCheckCount <= 20) {
    HookLog(
        "DX12: ProcessFrame #%d - imGuiInit=%d, syncInit=%d, overlayQueue=%p",
        s_preOverlayCheckCount, g_State.imGuiInit, g_State.syncInit,
        g_OverlayQueue);
  }
  ...
  if (g_State.imGuiInit && g_State.syncInit) {
    ...
    if (++s_overlayBlockEntryCount <= 10) {
      HookLog("DX12: ProcessFrame - entered overlay block #%d (fence=%p, "
              "cmdList=%p, allocCount=%zu)",
              s_overlayBlockEntryCount, g_State.fence, g_State.cmdList,
              g_State.allocators.size());
    }
      ...
      if (comp < target) {
        ...
        if (SUCCEEDED(hr)) {
          ...
          if (waitResult != WAIT_OBJECT_0) {
            HookLog("DX12: Fence wait failed/timeout, result=%lu, skipping "
                    "overlay this frame",
                    waitResult);
            safeToProceed = false;
          }
    ...
    if (safeToProceed) {
      ...
      if (list && alloc) {
          ...
          if (SUCCEEDED(listResetHr)) {
            ...
            if (SUCCEEDED(pSwapChain->QueryInterface(IID_PPV_ARGS(&sc3)))) {
              ...
              if (bufferIdx >= (UINT)g_State.bufferCount) {
                HookLog("DX12: Buffer index %u exceeds allocated count %d, "
                        "clamping",
              ...
              if (SUCCEEDED(pSwapChain->GetBuffer(swapchainBufferIdx,
                ...
                if (++s_bufferLogCount <= 10) {
                  HookLog(
                      "DX12: Render to buffer %u/%u (swapchain says %u), bb=%p",
                ...
                // Re-enabled transitions after debugging confirmed barriers are
                // NOT the issue
                {
                  D3D12_RESOURCE_BARRIER b = {
                      D3D12_RESOURCE_BARRIER_TYPE_TRANSITION,
                ...
                if (++s_closeCount <= 10) {
                  HookLog("DX12: cmdList->Close hr=0x%08X, overlayQueue=%p, "
                          "fence=%p",
                ...
                if (SUCCEEDED(closeHr) && targetQueue) {
                  ...
                  if (!useGameQueue && g_State.fence && g_State.fenceEvent) {
                    ...
                    if (s_execCount <= 10) {
                      HookLog("DX12: Overlay fence %llu synced (CPU wait)",
                              waitValue);
                    }
                  }
                } else if (!targetQueue) {
        ...
        if (++s_listAllocNullCount <= 3) {
          HookLog("DX12: ProcessFrame - null list or alloc (list=%p, alloc=%p, "
                  "idx=%d, allocSize=%zu)",
                  list, alloc, idx, g_State.allocators.size());
        }
      }
    } else {
      ...
      if (g_SharedCaptureD3D12.IsActive()) {
        ...
        if (captureQueue &&
          ...
          if (g_SharedCaptureD3D12.GetCurrentFrame(&desc)) {
            shm->SetSharedHandle(
                0, (uint64_t)g_SharedCaptureD3D12.GetSharedHandle(0));
            shm->SetSharedHandle(
                1, (uint64_t)g_SharedCaptureD3D12.GetSharedHandle(1));
            shm->SetFenceShareHandle(
                (uint64_t)g_SharedCaptureD3D12.GetFenceShareHandle());
            ...
            // updates
            uint32_t wIdx =
                shm->frameRing.writeIndex.load(std::memory_order_acquire);
            uint32_t rIdx =
                shm->frameRing.readIndex.load(std::memory_order_acquire);
            if ((uint32_t)(wIdx - rIdx) < (uint32_t)FRAME_RING_SIZE) {
              ...
              shm->frameRing.droppedFrames.fetch_add(1,
                                                     std::memory_order_relaxed);
          }
        }
      }
...
static std::atomic<int> s_framesBeforeInit{
    0}; // Defined earlier in ProcessFrame
...
void DX12_ProcessFrameExternal(IDXGISwapChain *pSwapChain) {
  ...
  if (g_State.imGuiInit && !s_initDelayComplete.load()) {
    ...
    if (frames < 1) {
      ...
      HookLog("DX12: ProcessFrameExternal - Overlay rendering enabled (frame "
              "%d after init)",
              frames);
    }
  }
  ...
  if (!s_checkedForVulkan) {
    ...
    if (s_vulkanLayerActive) {
      HookLog("DX12: Vulkan layer detected, will skip DXGI overlay for Vulkan "
              "WSI swapchains");
    }
...
static const GUID SKID_D3D12SwapChainBufferBitmap = {
    ...
    0x956f,
    0x47db,
    {0xa6, 0x53, 0x5, 0xd7, 0xb8, 0x71, 0x53, 0x38}};
...
DetourExecuteCommandLists(ID3D12CommandQueue *pThis, UINT NumCommandLists,
  ...
  // The game command queue identification is NOT needed - we just exclude our
  // own overlay queue
  if (pThis != g_OverlayQueue) {
    ...
    // Now that we use game's queue instead of separate overlay queue, we need
    // to capture it
    DX12_SetCommandQueue(pThis);
  }

  // NOTE: Removed complex swapchain-based queue detection to prevent crashes
  // The isReal detection works without needing to identify the game's command
  // queue We simply count command lists from all queues except our overlay
  // queue
...
void DX12_HookQueueVTable(ID3D12CommandQueue *queue) {
  ...
  if (SUCCEEDED(
          queue->QueryInterface(IID_CWrapD3D12CommandQueue, &unwrapped))) {
...
HRESULT STDMETHODCALLTYPE DetourCreateCommittedResource(
  ...
  if (oCreateCommittedResource)
    return oCreateCommittedResource(device, pHeapProperties, HeapFlags, pDesc,
...
bool DX12Hook::IsRealFrame() const { return g_FGCompat.IsCurrentFrameReal(); }
