...
static bool ShouldAllowNormalOverlayFallbackForCurrentFFXPresentCallbackStall(bool ffxPresentCallbackStalled) {
    ...
    return ce::dx12_overlay_policy::ShouldAllowNormalOverlayFallbackForStalledFFXPresentCallback(
        ...
        currentFFXPresentCallbackProof, progressProof.proof, stallDurationMs);
}
        ...
        if (ffxStallAllowsNormalOverlay) {
            ...
            if (s_stallFallbackLogCount.fetch_add(1, std::memory_order_relaxed) < 10) {
                ...
                HookLogImportant(
                    ...
                    "progressAssumedFor=%llums proofSince=%llu directFFX=%d currentCallbackProof=%d stableProof=%d "
                    "stableFor=%llums sameQueue=%d deviceHr=0x%08X) "
            ...
            if (!timeoutOverrideAllowed) {
                ...
                if (logCount < 10 || (logCount % 600) == 0) {
                    HookLogImportant(
                        ...
                        "callbackEver=%d lastCallbackAge=%llums ffxStalled=%d ffxStallAllows=%d log=%d)",
    ...
    if (runtimeOwnedTeardownStillActive) {
        ...
        if (logCount < 20 || (logCount % 300) == 0) {
            HookLogImportant(
                ...
                g_SwapchainQueue, g_OriginalGameQueue, g_CommandQueue.load(std::memory_order_acquire), logCount + 1);
        }
    }
...
static HRESULT STDMETHODCALLTYPE DetourCreateSwapChainGlobal(IDXGIFactory* pThis, IUnknown* pDevice,
    ...
    if (SUCCEEDED(hr) && ppSwapChain && *ppSwapChain) {
        ...
        if (HandleProtectedOfficialFFXStartupSwapchainCreate(captureEvidence, *ppSwapChain, "CreateSwapChain")) {
            return hr;
        }
