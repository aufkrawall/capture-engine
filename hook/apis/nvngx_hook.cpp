...
typedef NVSDK_NGX_Result(
    STDMETHODCALLTYPE *PFN_NVSDK_NGX_GetFeatureRequirements)(
    void *InAdapter, const NVSDK_NGX_FeatureDiscoveryInfo *InDiscoveryInfo,
    NVSDK_NGX_FeatureRequirement *OutSupported);

static PFN_NVSDK_NGX_GetFeatureRequirements oGetFeatureRequirements_D3D11 =
    nullptr;
static PFN_NVSDK_NGX_GetFeatureRequirements oGetFeatureRequirements_D3D12 =
    nullptr;
static PFN_NVSDK_NGX_GetFeatureRequirements oGetFeatureRequirements_VULKAN =
    nullptr;
...
// DLSS Preset Hints
#define NVSDK_NGX_Parameter_DLSS_Hint_Render_Preset_DLAA                       \
  "DLSS.Hint.Render.Preset.DLAA"
#define NVSDK_NGX_Parameter_DLSS_Hint_Render_Preset_Quality                    \
...
#define NVSDK_NGX_Parameter_DLSS_Hint_Render_Preset_Performance                \
  "DLSS.Hint.Render.Preset.Performance"
#define NVSDK_NGX_Parameter_DLSS_Hint_Render_Preset_UltraPerformance           \
  "DLSS.Hint.Render.Preset.UltraPerformance"
#define NVSDK_NGX_Parameter_DLSS_Hint_Render_Preset_UltraQuality               \
...
#define NVSDK_NGX_Parameter_RayReconstruction_Available                        \
  "RayReconstruction.Available"
#define NVSDK_NGX_Parameter_RayReconstruction_FeatureInitResult                \
  "RayReconstruction.FeatureInitResult"

// DLSS Multi-Frame Generation (MFG) parameter
#define NVSDK_NGX_Parameter_FrameGenerationMultiplier                          \
  "FrameGenerationMultiplier" // 2=2x, 3=3x
...
const int NVSDK_NGX_Feature_FrameGeneration_11 =
    0xB; // Frame Generation (alternate ID)
const int NVSDK_NGX_Feature_RayReconstruction = 13; // Ray Reconstruction
const int NVSDK_NGX_Feature_MultiFrameGeneration =
    18; // Multi-Frame Generation (MFG) - generates 2x or 3x frames
...
// --- Typed VTable Hooks ---
typedef void(STDMETHODCALLTYPE *PFN_SetI)(NVSDK_NGX_Parameter *pThis,
                                          const char *InName, int InValue);
typedef void(STDMETHODCALLTYPE *PFN_SetUI)(NVSDK_NGX_Parameter *pThis,
                                           const char *InName,
                                           unsigned int InValue);
typedef void(STDMETHODCALLTYPE *PFN_SetF)(NVSDK_NGX_Parameter *pThis,
                                          const char *InName, float InValue);
typedef NVSDK_NGX_Result(STDMETHODCALLTYPE *PFN_GetI)(
    NVSDK_NGX_Parameter *pThis, const char *InName, int *OutValue);
typedef NVSDK_NGX_Result(STDMETHODCALLTYPE *PFN_GetUI)(
    NVSDK_NGX_Parameter *pThis, const char *InName, unsigned int *OutValue);
...
static void UpdatePresetHint(const char *paramName, uint32_t presetVal,
                             const char *source) {
  int idx = -1;
  if (strcmp(paramName,
             NVSDK_NGX_Parameter_DLSS_Hint_Render_Preset_Performance) == 0)
    idx = 0;
  else if (strcmp(paramName,
                  NVSDK_NGX_Parameter_DLSS_Hint_Render_Preset_Balanced) == 0)
    idx = 1;
  else if (strcmp(paramName,
                  NVSDK_NGX_Parameter_DLSS_Hint_Render_Preset_Quality) == 0)
    idx = 2;
  else if (strcmp(
               paramName,
    ...
    idx = 4;
                  ...
                  NVSDK_NGX_Parameter_DLSS_Hint_Render_Preset_DLAA) == 0)
    idx = 5;
  ...
  if (idx >= 0) {
    ...
    if (g_IPC && g_IPC->GetSharedMem() &&
      ...
      NVNGXLog("NVNGX: UpdatePresetHint [%s]: %s -> %u ('%c')", source,
               paramName, presetVal, c);
    }
  } else if (strstr(paramName, "RayReconstruction")) {
    // Log RR presets too for visibility
    if (g_IPC && g_IPC->GetSharedMem() &&
        g_IPC->GetSharedMem()->GetDebugLogging()) {
      NVNGXLog("NVNGX: UpdatePresetHint RR [%s]: %s -> %u", source, paramName,
               presetVal);
    }
  }
}
...
static bool IsSafeString(const char *s) { return IsSafePtr(s); }
...
void STDMETHODCALLTYPE Hooked_SetI(NVSDK_NGX_Parameter *pThis,
                                   const char *InName, int InValue) {
  if (IsSafeString(InName)) {
    ...
    if (strcmp(InName, NVSDK_NGX_Parameter_CreateFlags) == 0) {
      ...
      if (mode == "on") {
        if (!(InValue & NVSDK_NGX_DLSS_Feature_Flags_AutoExposure)) {
          if (g_IPC && g_IPC->GetSharedMem() &&
            ...
            LogOncePerParam(InName,
                            "NVNGX: Forcing AutoExposure flag ON (was 0x%X)",
                            InValue);
        ...
        if (InValue & NVSDK_NGX_DLSS_Feature_Flags_AutoExposure) {
          if (g_IPC && g_IPC->GetSharedMem() &&
              g_IPC->GetSharedMem()->GetDebugLogging())
            LogOncePerParam(InName,
                            "NVNGX: Forcing AutoExposure flag OFF (was 0x%X)",
                            InValue);
          InValue &= ~NVSDK_NGX_DLSS_Feature_Flags_AutoExposure;
        }
      }
      ...
      if (cfg.parsed.dlssSharpening > -1.5f) {
        ...
        const bool forceDisableSharpening =
            (cfg.parsed.dlssSharpening < 0.0001f);
        if (forceDisableSharpening) {
          if (InValue & NVSDK_NGX_DLSS_Feature_Flags_DoSharpening) {
            if (g_IPC && g_IPC->GetSharedMem() &&
                g_IPC->GetSharedMem()->GetDebugLogging())
              LogOncePerParam(InName,
                              "NVNGX: Forcing DoSharpening flag OFF (was 0x%X)",
                              InValue);
          ...
          if (!(InValue & NVSDK_NGX_DLSS_Feature_Flags_DoSharpening)) {
            if (g_IPC && g_IPC->GetSharedMem() &&
                g_IPC->GetSharedMem()->GetDebugLogging())
              LogOncePerParam(InName,
                              "NVNGX: Forcing DoSharpening flag ON (was 0x%X)",
                              InValue);
            InValue |= NVSDK_NGX_DLSS_Feature_Flags_DoSharpening;
          }
        }
      ...
      if (isDlssPreset) {
        uint32_t val =
            (specificOverride > 0) ? specificOverride : cfg.parsed.srPreset;
        if (val > 0) {
          if (g_IPC && g_IPC->GetSharedMem() &&
              g_IPC->GetSharedMem()->GetDebugLogging())
            NVNGXLog("NVNGX: SetI: Overriding %s: %d -> %u", InName, InValue,
                     val);
          InValue = (int)val;
        ...
        if (isRRPreset) {
          uint32_t finalRRPreset =
              specificRROverride > 0 ? specificRROverride : cfg.parsed.rrPreset;
          if (finalRRPreset > 0) {
            if (g_IPC && g_IPC->GetSharedMem() &&
                g_IPC->GetSharedMem()->GetDebugLogging())
              LogOncePerParam(
                  InName, "NVNGX: Overriding RR preset (SetI) %s = %u (was %d)",
                  InName, finalRRPreset, InValue);
...
void STDMETHODCALLTYPE Hooked_SetUI(NVSDK_NGX_Parameter *pThis,
                                    const char *InName, unsigned int InValue) {
  if (IsSafeString(InName)) {
    ...
    if (strcmp(InName, NVSDK_NGX_Parameter_AutoExposure) == 0) {
      ...
      if (isDlssPreset) {
        ...
        if (setUILogCount < 10 && g_IPC && g_IPC->GetSharedMem() &&
            g_IPC->GetSharedMem()->GetDebugLogging()) {
          NVNGXLog(
              "NVNGX: SetUI(%s): game=%u, specific=%u, global=%u, final=%u",
        ...
        if (val > 0) {
          if (g_IPC && g_IPC->GetSharedMem() &&
              g_IPC->GetSharedMem()->GetDebugLogging())
            LogOncePerParam(InName, "NVNGX: Overriding %s via SetUI -> %u",
                            InName, val);
          InValue = val;
        ...
        if (isRRPreset) {
          ...
          if (finalRRPreset > 0) {
            if (g_IPC && g_IPC->GetSharedMem() &&
                g_IPC->GetSharedMem()->GetDebugLogging())
              LogOncePerParam(InName,
                              "NVNGX: Overriding RR preset %s = %u (was %u)",
                              InName, finalRRPreset, InValue);
...
void STDMETHODCALLTYPE Hooked_SetF(NVSDK_NGX_Parameter *pThis,
                                   const char *InName, float InValue) {
  if (IsSafeString(InName)) {
    const auto &cfg = GetActiveGraphicsConfig();
    ...
    if (cfg.dlssExposureNormalization == "on") {
      if (strcmp(InName, NVSDK_NGX_Parameter_ExposureScale) == 0 ||
        ...
        if (g_IPC && g_IPC->GetSharedMem() &&
            g_IPC->GetSharedMem()->GetDebugLogging())
          LogOncePerParam(InName, "NVNGX: Normalizing exposure %s = 1.0",
                          InName);
        InValue = 1.0f;
      }
    ...
    if (strcmp(InName, NVSDK_NGX_Parameter_Sharpness) == 0 ||
      ...
      if (cfg.parsed.dlssSharpening > -1.5f) {
        ...
        if (g_IPC && g_IPC->GetSharedMem() &&
            g_IPC->GetSharedMem()->GetDebugLogging())
          LogOncePerParam(InName,
                          "NVNGX: Overriding sharpness = %.2f (was %.2f)",
                          overrideVal, InValue);
        InValue = overrideVal;
      }
    }
...
NVSDK_NGX_Result STDMETHODCALLTYPE Hooked_GetI(NVSDK_NGX_Parameter *pThis,
                                               const char *InName,
                                               int *OutValue) {
...
NVSDK_NGX_Result STDMETHODCALLTYPE Hooked_GetUI(NVSDK_NGX_Parameter *pThis,
                                                const char *InName,
...
void EnsureVTableHooks(NVSDK_NGX_Parameter *pParams) {
  ...
  if (mode == "on" || mode == "off") {
    ...
    if (g_IPC && g_IPC->GetSharedMem() &&
        g_IPC->GetSharedMem()->GetDebugLogging())
      LogOncePerParam(NVSDK_NGX_Parameter_AutoExposure,
                      "NVNGX: Injecting initial %s = %d",
                      NVSDK_NGX_Parameter_AutoExposure, val);
    if (vtable[3])
      ((PFN_SetI)vtable[3])(pParams, NVSDK_NGX_Parameter_AutoExposure, val);
    if (vtable[4])
      ((PFN_SetUI)vtable[4])(pParams, NVSDK_NGX_Parameter_AutoExposure,
                             (unsigned int)val);
  }
  ...
  InjectPreset(NVSDK_NGX_Parameter_DLSS_Hint_Render_Preset_DLAA,
               cfg.parsed.presetDLAA ? cfg.parsed.presetDLAA
                                     : cfg.parsed.srPreset);
  InjectPreset(NVSDK_NGX_Parameter_DLSS_Hint_Render_Preset_Quality,
               cfg.parsed.presetQuality ? cfg.parsed.presetQuality
                                        : cfg.parsed.srPreset);
  InjectPreset(NVSDK_NGX_Parameter_DLSS_Hint_Render_Preset_Balanced,
               cfg.parsed.presetBalanced ? cfg.parsed.presetBalanced
                                         : cfg.parsed.srPreset);
  InjectPreset(NVSDK_NGX_Parameter_DLSS_Hint_Render_Preset_Performance,
               cfg.parsed.presetPerformance ? cfg.parsed.presetPerformance
                                            : cfg.parsed.srPreset);
  InjectPreset(NVSDK_NGX_Parameter_DLSS_Hint_Render_Preset_UltraPerformance,
               cfg.parsed.presetUltraPerformance
                   ? cfg.parsed.presetUltraPerformance
                   : cfg.parsed.srPreset);
  InjectPreset(NVSDK_NGX_Parameter_DLSS_Hint_Render_Preset_UltraQuality,
               cfg.parsed.presetUltraQuality ? cfg.parsed.presetUltraQuality
                                             : cfg.parsed.srPreset);

  InjectPreset(NVSDK_NGX_Parameter_RayReconstruction_Hint_Render_Preset_DLAA,
               cfg.parsed.rrPresetDLAA ? cfg.parsed.rrPresetDLAA
                                       : cfg.parsed.rrPreset);
  InjectPreset(NVSDK_NGX_Parameter_RayReconstruction_Hint_Render_Preset_Quality,
               cfg.parsed.rrPresetQuality ? cfg.parsed.rrPresetQuality
                                          : cfg.parsed.rrPreset);
  InjectPreset(
      NVSDK_NGX_Parameter_RayReconstruction_Hint_Render_Preset_Balanced,
      cfg.parsed.rrPresetBalanced ? cfg.parsed.rrPresetBalanced
                                  : cfg.parsed.rrPreset);
  InjectPreset(
      NVSDK_NGX_Parameter_RayReconstruction_Hint_Render_Preset_Performance,
      cfg.parsed.rrPresetPerformance ? cfg.parsed.rrPresetPerformance
                                     : cfg.parsed.rrPreset);
  InjectPreset(
      NVSDK_NGX_Parameter_RayReconstruction_Hint_Render_Preset_UltraPerformance,
      cfg.parsed.rrPresetUltraPerformance ? cfg.parsed.rrPresetUltraPerformance
                                          : cfg.parsed.rrPreset);
  InjectPreset(
      NVSDK_NGX_Parameter_RayReconstruction_Hint_Render_Preset_UltraQuality,
      cfg.parsed.rrPresetUltraQuality ? cfg.parsed.rrPresetUltraQuality
                                      : cfg.parsed.rrPreset);
  ...
  if (cfg.parsed.dlssSharpening > -1.5f && vtable[6]) {
    ...
    if (g_IPC && g_IPC->GetSharedMem() &&
        g_IPC->GetSharedMem()->GetDebugLogging()) {
      LogOncePerParam(NVSDK_NGX_Parameter_Sharpness,
                      "NVNGX: Injecting initial sharpness = %.2f", overrideVal);
    }

    // Try both names for compatibility
    ((PFN_SetF)vtable[6])(pParams, NVSDK_NGX_Parameter_Sharpness, overrideVal);
    ((PFN_SetF)vtable[6])(pParams, NVSDK_NGX_Parameter_Sharpness_Alt,
                          overrideVal);
  }
  ...
  if (cfg.forceRayReconstruction) {
    ...
    if (vtable[3])
      ((PFN_SetI)vtable[3])(pParams,
                            NVSDK_NGX_Parameter_RayReconstruction_Available, 1);
    if (vtable[4])
      ((PFN_SetUI)vtable[4])(
          pParams, NVSDK_NGX_Parameter_RayReconstruction_Available, 1);
    ...
    if (vtable[3])
      ((PFN_SetI)vtable[3])(
          pParams, NVSDK_NGX_Parameter_RayReconstruction_FeatureInitResult, 1);
    if (vtable[4])
      ((PFN_SetUI)vtable[4])(
          pParams, NVSDK_NGX_Parameter_RayReconstruction_FeatureInitResult, 1);
  }
...
// --- Factory Hooks ---
typedef NVSDK_NGX_Result(STDMETHODCALLTYPE *PFN_NVSDK_NGX_GetParameters)(
...
NVSDK_NGX_Result STDMETHODCALLTYPE Hooked_ProcessParameters(
    PFN_NVSDK_NGX_GetParameters original, NVSDK_NGX_Parameter **OutParameters,
    const char *source) {
  if (!original)
  ...
  if (res == NVSDK_NGX_Result_Success && OutParameters && *OutParameters) {
    if (g_IPC && g_IPC->GetSharedMem() &&
        g_IPC->GetSharedMem()->GetDebugLogging())
      NVNGXLog("Hooked_ProcessParameters: Success, Params=%p, *Params=%p",
               OutParameters, *OutParameters);
    EnsureVTableHooks(*OutParameters);
  } else {
    if (g_IPC && g_IPC->GetSharedMem() &&
        g_IPC->GetSharedMem()->GetDebugLogging())
      NVNGXLog("Hooked_ProcessParameters: Result=%X, Params=%p", res,
               OutParameters);
  }
  return res;
}
...
Hooked_GetParams_D3D11(NVSDK_NGX_Parameter **p) {
  return Hooked_ProcessParameters(oGetParameters_D3D11, p, "GetParams_D3D11");
}
NVSDK_NGX_Result STDMETHODCALLTYPE
Hooked_AllocParams_D3D11(NVSDK_NGX_Parameter **p) {
  return Hooked_ProcessParameters(oAllocateParameters_D3D11, p,
                                  "AllocParams_D3D11");
}
NVSDK_NGX_Result STDMETHODCALLTYPE
Hooked_GetCaps_D3D11(NVSDK_NGX_Parameter **p) {
  return Hooked_ProcessParameters(oGetCapabilityParameters_D3D11, p,
                                  "GetCaps_D3D11");
}

NVSDK_NGX_Result STDMETHODCALLTYPE
Hooked_GetParams_D3D12(NVSDK_NGX_Parameter **p) {
  return Hooked_ProcessParameters(oGetParameters_D3D12, p, "GetParams_D3D12");
}
NVSDK_NGX_Result STDMETHODCALLTYPE
Hooked_AllocParams_D3D12(NVSDK_NGX_Parameter **p) {
  return Hooked_ProcessParameters(oAllocateParameters_D3D12, p,
                                  "AllocParams_D3D12");
}
NVSDK_NGX_Result STDMETHODCALLTYPE
Hooked_GetCaps_D3D12(NVSDK_NGX_Parameter **p) {
  return Hooked_ProcessParameters(oGetCapabilityParameters_D3D12, p,
                                  "GetCaps_D3D12");
}

NVSDK_NGX_Result STDMETHODCALLTYPE
Hooked_GetParams_VULKAN(NVSDK_NGX_Parameter **p) {
  return Hooked_ProcessParameters(oGetParameters_VULKAN, p, "GetParams_VULKAN");
}
NVSDK_NGX_Result STDMETHODCALLTYPE
Hooked_AllocParams_VULKAN(NVSDK_NGX_Parameter **p) {
  return Hooked_ProcessParameters(oAllocateParameters_VULKAN, p,
                                  "AllocParams_VULKAN");
}
NVSDK_NGX_Result STDMETHODCALLTYPE
Hooked_GetCaps_VULKAN(NVSDK_NGX_Parameter **p) {
  return Hooked_ProcessParameters(oGetCapabilityParameters_VULKAN, p,
                                  "GetCaps_VULKAN");
}
...
NVSDK_NGX_Result STDMETHODCALLTYPE Hooked_ProcessFeatureRequirements(
    ...
    const NVSDK_NGX_FeatureDiscoveryInfo *InDiscoveryInfo,
    NVSDK_NGX_FeatureRequirement *OutSupported) {
    ...
    if (InDiscoveryInfo->FeatureID == 13) {
      ...
      if (cfg.forceRayReconstruction) {
        ...
        OutSupported->FeatureSupported =
            0; // NVSDK_NGX_FeatureSupportResult_Supported
...
NVSDK_NGX_Result STDMETHODCALLTYPE Hooked_GetFeatureRequirements_D3D11(
    void *InAdapter, const NVSDK_NGX_FeatureDiscoveryInfo *InDiscoveryInfo,
    NVSDK_NGX_FeatureRequirement *OutSupported) {
  return Hooked_ProcessFeatureRequirements(
      oGetFeatureRequirements_D3D11, InAdapter, InDiscoveryInfo, OutSupported);
}

NVSDK_NGX_Result STDMETHODCALLTYPE Hooked_GetFeatureRequirements_D3D12(
    void *InAdapter, const NVSDK_NGX_FeatureDiscoveryInfo *InDiscoveryInfo,
    NVSDK_NGX_FeatureRequirement *OutSupported) {
  return Hooked_ProcessFeatureRequirements(
      oGetFeatureRequirements_D3D12, InAdapter, InDiscoveryInfo, OutSupported);
}

NVSDK_NGX_Result STDMETHODCALLTYPE Hooked_GetFeatureRequirements_VULKAN(
    void *InAdapter, const NVSDK_NGX_FeatureDiscoveryInfo *InDiscoveryInfo,
    NVSDK_NGX_FeatureRequirement *OutSupported) {
  return Hooked_ProcessFeatureRequirements(
      oGetFeatureRequirements_VULKAN, InAdapter, InDiscoveryInfo, OutSupported);
}

// --- CreateFeature Hooks ---
// CreateFeature signature is __cdecl (standard C export)
// CreateFeature signature (4 args based on NGX docs/reversing)
typedef NVSDK_NGX_Result(__cdecl *PFN_NVSDK_NGX_CreateFeature)(
    void *InCmdList, int InFeatureID, NVSDK_NGX_Parameter *InParameters,
    void **OutHandle);
      ...
      if (VerQueryValueW(data.data(), L"\\", (void **)&fileInfo, &len)) {
        if (g_IPC && g_IPC->GetSharedMem()) {
          ...
          if (g_IPC->GetSharedMem()->GetDebugLogging()) {
            NVNGXLog("NVNGX: Detected Version from %S: v%d.%d.%d", fileName,
                     major, minor, patch);
          }
        }
      }
...
static char GetPresetChar(int qualityValue) {
  ...
  if (g_IPC && g_IPC->GetSharedMem() &&
    ...
    if (logCount < 5) {
      NVNGXLog("NVNGX: GetPresetChar quality=%d -> presetVal=%u (GlobalSR=%u)",
               qualityValue, presetVal, cfg.parsed.srPreset);
      logCount++;
    }
  }
...
static void UpdateDLSSStatus(const NVSDK_NGX_FeatureDiscoveryInfo *info) {
  ...
  if (info->FeatureID == 1) { // Super Resolution
    ...
    if (info->FeatureInfo) {
      const auto *params =
          (const NVSDK_NGX_DLSS_Create_Params *)info->FeatureInfo;
      ...
      if (g_IPC->GetSharedMem()->GetDebugLogging()) {
        LogOncePerParam(
            "DLSS_SR_Create",
            "NVNGX: Created DLSS SR. Scale=%.2f, Quality=%d, Preset=%c",
            state.renderScale.load(), params->InPerfQualityValue,
            state.srPreset.load());
      }
    }
...
NVSDK_NGX_Result __cdecl
Hooked_CreateFeature_Process(PFN_NVSDK_NGX_CreateFeature original, void *ctx,
                             int featureID, NVSDK_NGX_Parameter *params,
                             void **handle) {
  if (g_IPC && g_IPC->GetSharedMem() &&
      g_IPC->GetSharedMem()->GetDebugLogging())
    NVNGXLog("Hooked_CreateFeature_Process: Entry ctx=%p, ID=%d, params=%p",
             ctx, featureID, params);
  ...
  if ((int)res >= 0) {
    ...
    if (g_IPC && g_IPC->GetSharedMem()) {
      ...
      if (featureID == 1) {
        ...
        if (!probeOnce && oGetUI && g_IPC && g_IPC->GetSharedMem() &&
          ...
          for (int i = 0; probeParams[i]; i++) {
            ...
            NVSDK_NGX_Result probeRes =
                oGetUI(params, probeParams[i], &probedVal);
            if (probeRes == NVSDK_NGX_Result_Success && probedVal > 0) {
              NVNGXLog("NVNGX: PROBE SUCCESS! '%s' = %u ('%c')", probeParams[i],
                       probedVal, PresetIDToChar(probedVal));
            }
          }
        ...
        // Retrieve captured Quality Mode and Dimensions
        {
          ...
          if (g_ParameterQualityMap.count(params)) {
            ...
            if (g_IPC->GetSharedMem()->GetDebugLogging())
              NVNGXLog("Hooked_CreateFeature: Quality=%d -> srPreset='%c'", q,
                       state.srPreset.load());
        ...
        if (featureID == NVSDK_NGX_Feature_RayReconstruction) {
          ...
          if (params && oGetI) {
            ...
            NVSDK_NGX_Result multRes =
                oGetI(params, NVSDK_NGX_Parameter_FrameGenerationMultiplier,
                      &multValue);
            if (multRes == NVSDK_NGX_Result_Success && multValue >= 2 &&
                multValue <= 3) {
          ...
          if (g_IPC->GetSharedMem()->GetDebugLogging())
            NVNGXLog("Hooked_CreateFeature: DLSS Multi-Frame Generation "
          ...
          if (g_IPC->GetSharedMem()->GetDebugLogging())
            NVNGXLog("Hooked_CreateFeature: DLSS FG ACTIVATED (ID 0x%X)",
                     featureID);
      ...
      if (cfg.parsed.srPreset > 0) {
        ...
        if (g_IPC->GetSharedMem()->GetDebugLogging()) {
          NVNGXLog("NV_NGX: Final Force SR Preset: '%c' -> '%c' (Config ID %u, "
                   "From String: %s)",
                   oldP, state.srPreset.load(), cfg.parsed.srPreset,
                   cfg.dlssSRPreset.c_str());
        }
      }
      if (cfg.parsed.rrPreset > 0 && featureID == 13) {
        state.rrPreset = PresetIDToChar(cfg.parsed.rrPreset);
        if (g_IPC->GetSharedMem()->GetDebugLogging())
          NVNGXLog("NV_NGX: Final Force RR Preset to '%c'",
                   state.rrPreset.load());
      }
    }
  }
...
NVSDK_NGX_Result __cdecl Hooked_CreateFeature_D3D11(void *ctx, int featureID,
                                                    NVSDK_NGX_Parameter *params,
                                                    void **handle) {
  return Hooked_CreateFeature_Process(oCreateFeature_D3D11, ctx, featureID,
                                      params, handle);
}
NVSDK_NGX_Result __cdecl Hooked_CreateFeature_D3D12(void *ctx, int featureID,
                                                    NVSDK_NGX_Parameter *params,
                                                    void **handle) {
  return Hooked_CreateFeature_Process(oCreateFeature_D3D12, ctx, featureID,
                                      params, handle);
}
NVSDK_NGX_Result __cdecl
Hooked_CreateFeature_VULKAN(void *ctx, int featureID,
                            NVSDK_NGX_Parameter *params, void **handle) {
  return Hooked_CreateFeature_Process(oCreateFeature_VULKAN, ctx, featureID,
                                      params, handle);
}
...
void NVNGXHook::Install() {
  ...
  if (presetVal > 0) {
    ...
    if (g_IPC && g_IPC->GetSharedMem()) {
      ...
      NVNGXLog("NVNGX: Config forced SR Preset to '%c' (via Install)",
               PresetIDToChar(presetVal));
    } else {
  ...
  InstallFactory("NVSDK_NGX_D3D11_GetParameters",
                 (LPVOID)&Hooked_GetParams_D3D11,
                 (LPVOID *)&oGetParameters_D3D11);
  InstallFactory("NVSDK_NGX_D3D11_AllocateParameters",
                 (LPVOID)&Hooked_AllocParams_D3D11,
                 (LPVOID *)&oAllocateParameters_D3D11);
  InstallFactory("NVSDK_NGX_D3D11_GetCapabilityParameters",
                 (LPVOID)&Hooked_GetCaps_D3D11,
                 (LPVOID *)&oGetCapabilityParameters_D3D11);

  InstallFactory("NVSDK_NGX_D3D12_GetParameters",
                 (LPVOID)&Hooked_GetParams_D3D12,
                 (LPVOID *)&oGetParameters_D3D12);
  InstallFactory("NVSDK_NGX_D3D12_AllocateParameters",
                 (LPVOID)&Hooked_AllocParams_D3D12,
                 (LPVOID *)&oAllocateParameters_D3D12);
  InstallFactory("NVSDK_NGX_D3D12_GetCapabilityParameters",
                 (LPVOID)&Hooked_GetCaps_D3D12,
                 (LPVOID *)&oGetCapabilityParameters_D3D12);

  InstallFactory("NVSDK_NGX_VULKAN_GetParameters",
                 (LPVOID)&Hooked_GetParams_VULKAN,
                 (LPVOID *)&oGetParameters_VULKAN);
  InstallFactory("NVSDK_NGX_VULKAN_AllocateParameters",
                 (LPVOID)&Hooked_AllocParams_VULKAN,
                 (LPVOID *)&oAllocateParameters_VULKAN);
  InstallFactory("NVSDK_NGX_VULKAN_GetCapabilityParameters",
                 (LPVOID)&Hooked_GetCaps_VULKAN,
                 (LPVOID *)&oGetCapabilityParameters_VULKAN);

  InstallFactory("NVSDK_NGX_D3D11_GetFeatureRequirements",
                 (LPVOID)&Hooked_GetFeatureRequirements_D3D11,
                 (LPVOID *)&oGetFeatureRequirements_D3D11);
  InstallFactory("NVSDK_NGX_D3D12_GetFeatureRequirements",
                 (LPVOID)&Hooked_GetFeatureRequirements_D3D12,
                 (LPVOID *)&oGetFeatureRequirements_D3D12);
  InstallFactory("NVSDK_NGX_VULKAN_GetFeatureRequirements",
                 (LPVOID)&Hooked_GetFeatureRequirements_VULKAN,
                 (LPVOID *)&oGetFeatureRequirements_VULKAN);

  InstallFactory("NVSDK_NGX_D3D11_CreateFeature",
                 (LPVOID)&Hooked_CreateFeature_D3D11,
                 (LPVOID *)&oCreateFeature_D3D11);
  InstallFactory("NVSDK_NGX_D3D12_CreateFeature",
                 (LPVOID)&Hooked_CreateFeature_D3D12,
                 (LPVOID *)&oCreateFeature_D3D12);
  InstallFactory("NVSDK_NGX_VULKAN_CreateFeature",
                 (LPVOID)&Hooked_CreateFeature_VULKAN,
