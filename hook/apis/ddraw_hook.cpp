...
struct IDirect3D7 : public IUnknown {
  ...
  virtual HRESULT STDMETHODCALLTYPE CreateDevice(REFCLSID,
                                                 IDirectDrawSurface7 *,
                                                 IDirect3DDevice7 **) = 0;
};
...
typedef HRESULT(STDMETHODCALLTYPE *SetTextureStageState7_t)(
    IDirect3DDevice7 *device, DWORD Stage, DWORD Type, DWORD Value);

typedef HRESULT(STDMETHODCALLTYPE *SetRenderState7_t)(IDirect3DDevice7 *device,
...
typedef HRESULT(STDMETHODCALLTYPE *DDSurface7Flip_t)(
    IDirectDrawSurface7 *surface, IDirectDrawSurface7 *destOverride,
    DWORD flags);

typedef HRESULT(STDMETHODCALLTYPE *DDSurface7Blt_t)(
    IDirectDrawSurface7 *surface, LPRECT destRect,
    IDirectDrawSurface7 *srcSurface, LPRECT srcRect, DWORD flags, void *bltFx);

typedef HRESULT(STDMETHODCALLTYPE *DDSurface7Unlock_t)(
    IDirectDrawSurface7 *surface, LPRECT rect);

typedef HRESULT(STDMETHODCALLTYPE *DDSurface7Lock_t)(
    IDirectDrawSurface7 *surface, LPRECT destRect, void *surfaceDesc,
    DWORD flags, HANDLE event);

typedef HRESULT(STDMETHODCALLTYPE *DDSurface7GetDC_t)(
    IDirectDrawSurface7 *surface, HDC *hdc);

typedef HRESULT(STDMETHODCALLTYPE *DDSurface7ReleaseDC_t)(
    IDirectDrawSurface7 *surface, HDC hdc);
...
static void ApplyPrerenderLimitDDraw(IDirectDrawSurface7 *surface,
                                     float limit) {
  if (limit < 0.0f)
    return;
  ...
  if (limit == 0.0f) {
    ...
    typedef HRESULT(STDMETHODCALLTYPE * GetFlipStatus_t)(IDirectDrawSurface7 *,
                                                         DWORD);
    void **vtable = *(void ***)surface;
    GetFlipStatus_t pGetFlipStatus =
        (GetFlipStatus_t)vtable[13]; // GetFlipStatus is index 13

    while (pGetFlipStatus(surface, 1 /* DDGFS_ISFLIPDONE */) ==
           0x887600FA /* DDERR_WASSTILLDRAWING */) {
    ...
    if (g_PrerenderSurfaces[waitIdx]) {
      ...
      typedef HRESULT(STDMETHODCALLTYPE *
                      GetFlipStatus_t)(IDirectDrawSurface7 *, DWORD);
...
public:
  ...
  bool CreateD3D9ExWrapper(HWND hwnd) {
    ...
    PFN_Direct3DCreate9Ex pDirect3DCreate9Ex =
        (PFN_Direct3DCreate9Ex)GetProcAddress(d3d9, "Direct3DCreate9Ex");
  ...
  void CaptureFrame(void *bits, int pitch) {
    ...
    HRESULT hr = d3d11Context->Map(stagingTexture, 0, D3D11_MAP_WRITE_DISCARD,
                                   0, &mapped);
    if (SUCCEEDED(hr)) {
  ...
  void CaptureFrameViaGDI(IDirectDrawSurface7 *surface) {
    ...
    // Release surface DC
    typedef HRESULT(STDMETHODCALLTYPE * ReleaseDC_t)(IDirectDrawSurface7 *,
                                                     HDC);
...
static void DrawDDrawOverlay() {
  ...
  g_OverlayAdapter.SetDroppedFrames(
      g_DDrawCapture.droppedFrames.load(std::memory_order_relaxed));
  g_OverlayAdapter.SetGraphicsAPI("DDraw");

  if (g_OverlayAdapter.IsInitialized() && g_DDrawCapture.width > 0 &&
      g_DDrawCapture.height > 0) {
    g_OverlayAdapter.RenderOverlay(g_DDrawCapture.width, g_DDrawCapture.height);
  }
}

// Get surface dimensions from DDSURFACEDESC2
static bool GetSurfaceSize(IDirectDrawSurface7 *surface, uint32_t &w,
                           uint32_t &h) {
  // DDSURFACEDESC2 is 124 bytes
  uint8_t desc[128] = {};
  *(DWORD *)desc = 124; // dwSize

  typedef HRESULT(STDMETHODCALLTYPE * GetSurfaceDesc_t)(IDirectDrawSurface7 *,
                                                        void *);
  void **vtable = *(void ***)surface;
  GetSurfaceDesc_t pGetSurfaceDesc =
      (GetSurfaceDesc_t)vtable[18]; // GetSurfaceDesc
...
typedef HRESULT(STDMETHODCALLTYPE *DDraw7CreateSurface_t)(
    IDirectDraw7 *pThis, void *pDesc, IDirectDrawSurface7 **ppSurface,
    IUnknown *pUnkOuter);
static DDraw7CreateSurface_t oDDraw7CreateSurface = nullptr;

static HRESULT STDMETHODCALLTYPE DetourDirectDraw7CreateSurface(
    IDirectDraw7 *pThis, void *pDesc, IDirectDrawSurface7 **ppSurface,
    IUnknown *pUnkOuter) {
  if (pDesc && g_IPC) {
    ...
    if (count >= 2 && count <= 6) {
      ...
      if (ddsd->dwFlags & 0x00000001) { // DDSD_CAPS
        ...
        HookLog("DDraw: CreateSurface: Overriding BackBufferCount to %d",
                count);
      }
    }
  }
...
DetourDDSurface7Flip(IDirectDrawSurface7 *surface,
                     IDirectDrawSurface7 *destOverride, DWORD flags) {
  ...
  if (g_IPC && g_IPC->GetSharedMem()->graphicsConfig.prerenderLimit > 0.0f) {
    ApplyPrerenderLimitDDraw(
        surface, g_IPC->GetSharedMem()->graphicsConfig.prerenderLimit);
  }
...
static HRESULT STDMETHODCALLTYPE DetourDDSurface7Blt(
    IDirectDrawSurface7 *surface, LPRECT destRect,
    IDirectDrawSurface7 *srcSurface, LPRECT srcRect, DWORD flags, void *bltFx) {
  HRESULT hr =
      oDDSurface7Blt(surface, destRect, srcSurface, srcRect, flags, bltFx);
...
static HRESULT STDMETHODCALLTYPE DetourSetRenderState7(IDirect3DDevice7 *device,
                                                       DWORD Type,
                                                       DWORD Value) {
  if (g_IPC) {
...
static HRESULT STDMETHODCALLTYPE DetourSetTextureStageState7(
    IDirect3DDevice7 *device, DWORD Stage, DWORD Type, DWORD Value) {
  if (g_IPC) {
...
void DDrawHook::Init() {
  ...
  typedef HRESULT(WINAPI * DirectDrawCreateEx_t)(GUID *, LPVOID *, const IID *,
                                                 IUnknown *);
  DirectDrawCreateEx_t pDirectDrawCreateEx =
      (DirectDrawCreateEx_t)GetProcAddress(ddrawModule, "DirectDrawCreateEx");
  ...
  HRESULT hr =
      pDirectDrawCreateEx(NULL, (LPVOID *)&ddraw7, &IID_IDirectDraw7, NULL);
  if (FAILED(hr) || !ddraw7) {
  ...
  HWND dummyHwnd =
      CreateWindowExA(0, wc.lpszClassName, "DDrawDummy", WS_OVERLAPPEDWINDOW, 0,
                      0, 100, 100, NULL, NULL, wc.hInstance, NULL);

  // Set cooperative level
  typedef HRESULT(STDMETHODCALLTYPE * SetCooperativeLevel_t)(IDirectDraw7 *,
                                                             HWND, DWORD);
  void **ddraw7VTable = *(void ***)ddraw7;
  SetCooperativeLevel_t pSetCooperativeLevel =
      (SetCooperativeLevel_t)ddraw7VTable[20];
  ...
  *(DWORD *)&ddsd.padding[0] = 0x200; // DDSCAPS_PRIMARYSURFACE
  ...
  typedef HRESULT(STDMETHODCALLTYPE * CreateSurface_t)(
      IDirectDraw7 *, void *, IDirectDrawSurface7 **, void *);

  // Hook CreateSurface on IDirectDraw7 (index 6)
  if (VTableHook::Create(
          &ddraw7VTable[6], (LPVOID)&DetourDirectDraw7CreateSurface,
          (LPVOID *)&oDDraw7CreateSurface) == VTableHook::Success) {
  ...
  if (SUCCEEDED(hr) && dummySurface) {
    ...
    // Hook Flip
    if (VTableHook::Create(&surfaceVTable[DDSURFACE7_VTABLE_FLIP],
    ...
    // Hook Blt
    if (VTableHook::Create(&surfaceVTable[DDSURFACE7_VTABLE_BLT],
    ...
    typedef HRESULT(STDMETHODCALLTYPE * QueryInterface_t)(IUnknown *, REFIID,
                                                          void **);
    ...
    if (SUCCEEDED(pQueryInterface((IUnknown *)ddraw7, IID_IDirect3D7,
                                  (void **)&d3d7))) {
      ...
      typedef HRESULT(STDMETHODCALLTYPE * D3D7CreateDevice_t)(
      ...
      if (SUCCEEDED(pD3D7CreateDevice(d3d7, IID_IDirect3DHALDevice,
                                      dummySurface, &d3d7Device))) {
        ...
        // SetTextureStageState is index 35
        if (VTableHook::Create(
                &d3d7DeviceVTable[35], (LPVOID)&DetourSetTextureStageState7,
        ...
        if (VTableHook::Create(
                &d3d7DeviceVTable[13], (LPVOID)&DetourSetRenderState7,
