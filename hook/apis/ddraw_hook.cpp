...
typedef HRESULT (STDMETHODCALLTYPE *SetRenderState7_t)(
    IDirect3DDevice7 *device,
    DWORD Type,
    DWORD Value
);
...
static HRESULT STDMETHODCALLTYPE DetourSetRenderState7(
    IDirect3DDevice7 *device,
    DWORD Type,
    DWORD Value
) {
    if (g_IPC) {
        ...
        if (msaa[0] != 'd') {
            if (Type == 2 /* D3DRENDERSTATE_ANTIALIAS */) {
                if (strcmp(msaa, "off") == 0) Value = 0; // D3DANTIALIAS_NONE
                else Value = 2; // D3DANTIALIAS_SORTINDEPENDENT
            }
        }
    }
...
static HRESULT STDMETHODCALLTYPE DetourSetTextureStageState7(
    ...
    if (g_IPC) {
        ...
        if (mip != "default") {
             // D3DTSS_MIPFILTER = 18
             if (Type == 18) {
                  if (mip == "trilinear") Value = 3; // D3DTFP_LINEAR
                  else if (mip == "bilinear") Value = 2; // D3DTFP_POINT
             }
        }
...
void DDrawHook::Init() {
    ...
    HWND dummyHwnd = CreateWindowExA(0, wc.lpszClassName, "DDrawDummy",
