...
// Note: Vulkan hook removed - using VK_LAYER_CE_overlay (ICD layer approach) instead
...
#define LOAD_GL(name)                                 \
    ...
    if (!p##name) p##name = (name##_t)GetProcAddress(gl, #name)
...
static void SwapBegin(HDC hdc)
{
    ...
    if (g_SwapRecurse == 0) {
        ...
        if (g_FunctionsLoaded) {
            ...
            if (diagSwap)
                HookLog("OpenGL: SwapBegin(HDC=0x%p) - shm=%p, shouldDraw=%d, isRecording=%d", hdc, shm,
                        (int)shouldDraw, (int)(g_IPC && g_IPC->IsRecording()));
