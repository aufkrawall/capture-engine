...
static bool IsVulkanPrimary() {
  ...
  if (g_IPC && g_IPC->GetSharedMem() &&
      g_IPC->GetSharedMem()->runtimeState.vulkanLayerActive)
    return true;
  return false;
}
...
typedef HANDLE(WINAPI *wglDXRegisterObjectNV_t)(HANDLE, void *, GLuint, GLenum,
                                                GLenum);
...
typedef void(WINAPI *glFramebufferTexture2D_t)(GLenum, GLenum, GLenum, GLuint,
                                               GLint);
typedef GLenum(WINAPI *glCheckFramebufferStatus_t)(GLenum);
typedef void(WINAPI *glBlitFramebuffer_t)(GLint, GLint, GLint, GLint, GLint,
...
typedef void(WINAPI *glReadPixels_t)(GLint, GLint, GLsizei, GLsizei, GLenum,
                                     GLenum, GLvoid *);
...
typedef void(WINAPI *glRenderbufferStorageMultisample_t)(GLenum, GLsizei,
                                                         GLenum, GLsizei,
                                                         GLsizei);
typedef void(WINAPI *glTexImage2DMultisample_t)(GLenum, GLsizei, GLenum,
...
static glRenderbufferStorageMultisample_t pglRenderbufferStorageMultisample =
    nullptr;
...
static void ApplyPrerenderLimitGL(float limit) {
  ...
  if (limit == 0.0f) {
    ...
    if (g_PrerenderFrameIndex >= (uint64_t)lookback) {
      GLsync waitSync = g_PrerenderSyncs[(g_PrerenderFrameIndex - lookback) %
                                         g_PrerenderSyncs.size()];
      if (waitSync) {
        pglClientWaitSync(waitSync, GL_SYNC_FLUSH_COMMANDS_BIT,
                          GL_TIMEOUT_IGNORED);
        pglDeleteSync(waitSync);
        g_PrerenderSyncs[(g_PrerenderFrameIndex - lookback) %
                         g_PrerenderSyncs.size()] = nullptr;
      }
    }

    // Create new sync for current frame
    GLsync currentSync = pglFenceSync(GL_SYNC_GPU_COMMANDS_COMPLETE, 0);
    g_PrerenderSyncs[g_PrerenderFrameIndex % g_PrerenderSyncs.size()] =
        currentSync;
...
public:
  ...
  void Cleanup() override { CleanupGL(); }
  ...
  bool CreateD3D11Device() {
    ...
    HRESULT hr = pD3D11CreateDevice(nullptr, D3D_DRIVER_TYPE_HARDWARE, NULL, 0,
  ...
  bool InitNVInterop() {
    ...
    for (int i = 0; i < CAPTURE_TEXTURE_COUNT; i++) {
      HRESULT hr =
          d3d11Device->CreateTexture2D(&texDesc, NULL, &sharedTextures[i]);
      if (FAILED(hr)) {
      ...
      nvTextureHandles[i] = wglDXRegisterObjectNV(
          nvDevice, sharedTextures[i], glTextures[i], GL_TEXTURE_2D,
  ...
  bool InitPBOFallback() {
    ...
    for (int i = 0; i < CAPTURE_TEXTURE_COUNT; i++) {
      HRESULT hr =
          d3d11Device->CreateTexture2D(&texDesc, NULL, &sharedTextures[i]);
      if (FAILED(hr)) {
        HookLog("OpenGL: Failed to create D3D11 texture %d for PBO fallback",
                i);
        return false;
      }
  ...
  void Init(HDC hDC) {
    ...
    if (!pglGenFramebuffers || !pglBindFramebuffer ||
      ...
      HookLog(
          "OpenGLCapture: FBO extensions not available. FBO capture disabled.");
      return;
    }
    ...
    pglTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, width, height, 0, GL_RGBA,
  ...
  void CaptureFrame() {
    ...
    pglBlitFramebuffer(0, 0, width, height, 0, 0, width, height,
    ...
    if (usingNVInterop) {
      ...
      if (data) {
        ...
        d3d11Context->UpdateSubresource(sharedTextures[idx], 0, NULL, data,
                                        width * 4, 0);
...
static void WINAPI DetourGlRenderbufferStorageMultisample(GLenum target,
                                                          GLsizei samples,
...
static void WINAPI DetourGlTexImage2DMultisample(
  ...
  if (pglTexImage2DMultisample)
    pglTexImage2DMultisample(target, samples, internalformat, width, height,
...
static bool LoadGLFunctions() {
  ...
  typedef PROC(WINAPI * wglGetProcAddress_t)(LPCSTR);
  wglGetProcAddress_t wglGetProcAddress_ptr =
      (wglGetProcAddress_t)GetProcAddress(gl, "wglGetProcAddress");
...
#define LOAD_GL(name)                                                          \
  ...
  wglDXOpenDeviceNV =
      (wglDXOpenDeviceNV_t)wglGetProcAddress_ptr("wglDXOpenDeviceNV");
  wglDXCloseDeviceNV =
      (wglDXCloseDeviceNV_t)wglGetProcAddress_ptr("wglDXCloseDeviceNV");
  wglDXRegisterObjectNV =
      (wglDXRegisterObjectNV_t)wglGetProcAddress_ptr("wglDXRegisterObjectNV");
  wglDXUnregisterObjectNV = (wglDXUnregisterObjectNV_t)wglGetProcAddress_ptr(
      "wglDXUnregisterObjectNV");
  wglDXLockObjectsNV =
      (wglDXLockObjectsNV_t)wglGetProcAddress_ptr("wglDXLockObjectsNV");
  wglDXUnlockObjectsNV =
      (wglDXUnlockObjectsNV_t)wglGetProcAddress_ptr("wglDXUnlockObjectsNV");
  ...
  HookLog("OpenGL: Functions loaded (NV Interop: %s)",
          g_NVInteropAvailable ? "Available" : "Not Available");
  return true;
}
...
static void LoadOpenGLExtensions() {
  ...
  if (oWglGetProcAddress)
    ...
    wglGetProcAddress_ptr = (wglGetProcAddress_t)GetProcAddress(
  ...
  pglMinSampleShading =
      (glMinSampleShading_t)wglGetProcAddress_ptr("glMinSampleShading");
  if (!pglMinSampleShading)
    pglMinSampleShading =
        (glMinSampleShading_t)wglGetProcAddress_ptr("glMinSampleShadingARB");

  pglEnable =
      (glEnable_t)GetProcAddress(GetModuleHandleA("opengl32.dll"), "glEnable");
}
...
static void DetectGPU(HDC hdc) {
  ...
  HRESULT hr = pD3D11CreateDevice(nullptr, D3D_DRIVER_TYPE_HARDWARE, NULL, 0,
                                  featureLevels, 2, D3D11_SDK_VERSION, &device,
...
static void SwapBegin(HDC hdc) {
  if (g_SwapRecurse == 0) {
    ...
    if (g_FunctionsLoaded && !g_VersionChecked) {
      ...
      glGetString_t pglGetString = (glGetString_t)GetProcAddress(
          GetModuleHandleA("opengl32.dll"), "glGetString");
      if (pglGetString) {
        ...
        if (verStr) {
          ...
          if (major < 3) {
            ...
            HookLog("OpenGL: Legacy Context detected (%s). Switching to GL2 "
                    "backend.",
                    (const char *)verStr);
          } else {
            HookLog("OpenGL: Modern Context detected (%s). Using GL3 backend.",
                    (const char *)verStr);
          }
  ...
  if (g_SwapRecurse == 0) {
    ...
    if (g_FunctionsLoaded) {
      ...
      if (diagSwap)
        HookLog("OpenGL: SwapBegin(HDC=0x%p) - shm=%p, shouldDraw=%d, "
                "isRecording=%d",
                hdc, shm, (int)shouldDraw,
                (int)(g_IPC && g_IPC->IsRecording()));
...
static BOOL WINAPI DetourWglSwapIntervalEXT(int interval) {
  ...
  if (!oWglSwapIntervalEXT) {
    ...
    if (oWglGetProcAddress) {
      // ... existing hooks
      oWglSwapIntervalEXT =
          (wglSwapIntervalEXT_t)oWglGetProcAddress("wglSwapIntervalEXT");
    }
  }
  ...
  if (strcmp(lpszProc, "glRenderbufferStorageMultisample") == 0) {
    ...
    if (proc)
      pglRenderbufferStorageMultisample =
          (glRenderbufferStorageMultisample_t)proc;
...
void OpenGLHook::Init() {
  ...
  IATHook::RegisterDynamicHook("SwapBuffers", (LPVOID)&DetourSwapBuffers,
                               (LPVOID *)&oSwapBuffers);
  // Patch explicit imports
  IATHook::PatchIATAllModules("gdi32.dll", "SwapBuffers",
                              (LPVOID)&DetourSwapBuffers,
                              (LPVOID *)&oSwapBuffers);

  // Hook wglSwapBuffers
  IATHook::RegisterDynamicHook("wglSwapBuffers", (LPVOID)&DetourWglSwapBuffers,
                               (LPVOID *)&oWglSwapBuffers);
  IATHook::PatchIATAllModules("opengl32.dll", "wglSwapBuffers",
                              (LPVOID)&DetourWglSwapBuffers,
                              (LPVOID *)&oWglSwapBuffers);

  // Hook wglSwapLayerBuffers
  IATHook::RegisterDynamicHook("wglSwapLayerBuffers",
                               (LPVOID)&DetourWglSwapLayerBuffers,
                               (LPVOID *)&oWglSwapLayerBuffers);
  IATHook::PatchIATAllModules("opengl32.dll", "wglSwapLayerBuffers",
                              (LPVOID)&DetourWglSwapLayerBuffers,
                              (LPVOID *)&oWglSwapLayerBuffers);

  // Hook wglDeleteContext
  IATHook::RegisterDynamicHook("wglDeleteContext",
                               (LPVOID)&DetourWglDeleteContext,
                               (LPVOID *)&oWglDeleteContext);
  IATHook::PatchIATAllModules("opengl32.dll", "wglDeleteContext",
                              (LPVOID)&DetourWglDeleteContext,
                              (LPVOID *)&oWglDeleteContext);

  // Hook wglGetProcAddress
  // Critical for intercepting extensions
  IATHook::RegisterDynamicHook("wglGetProcAddress",
                               (LPVOID)&DetourWglGetProcAddress,
                               (LPVOID *)&oWglGetProcAddress);
  IATHook::PatchIATAllModules("opengl32.dll", "wglGetProcAddress",
                              (LPVOID)&DetourWglGetProcAddress,
                              (LPVOID *)&oWglGetProcAddress);

  // Hook Core GL functions (glTexParameter)
  IATHook::RegisterDynamicHook("glTexParameteri",
                               (LPVOID)&DetourGlTexParameteri,
                               (LPVOID *)&pglTexParameteri);
  IATHook::PatchIATAllModules("opengl32.dll", "glTexParameteri",
                              (LPVOID)&DetourGlTexParameteri,
                              (LPVOID *)&pglTexParameteri);

  IATHook::RegisterDynamicHook("glTexParameterf",
                               (LPVOID)&DetourGlTexParameterf,
                               (LPVOID *)&pglTexParameterf);
  IATHook::PatchIATAllModules("opengl32.dll", "glTexParameterf",
                              (LPVOID)&DetourGlTexParameterf,
                              (LPVOID *)&pglTexParameterf);

  // Hook wglMakeCurrent
  IATHook::RegisterDynamicHook("wglMakeCurrent", (LPVOID)&DetourWglMakeCurrent,
                               (LPVOID *)&oWglMakeCurrent);
  IATHook::PatchIATAllModules("opengl32.dll", "wglMakeCurrent",
                              (LPVOID)&DetourWglMakeCurrent,
                              (LPVOID *)&oWglMakeCurrent);
