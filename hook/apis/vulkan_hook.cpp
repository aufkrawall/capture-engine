...
static VkSemaphore g_CrossDeviceSems_CaptureOwned[CAPTURE_TEXTURE_COUNT] = {
    VK_NULL_HANDLE}; // On Capture Device
static VkSemaphore g_CrossDeviceSems_GameImported[CAPTURE_TEXTURE_COUNT] = {
    VK_NULL_HANDLE}; // On Game Device
...
static VkImage g_InternalStagingImages[CAPTURE_TEXTURE_COUNT] = {
    VK_NULL_HANDLE};
static VkDeviceMemory g_InternalStagingMemories[CAPTURE_TEXTURE_COUNT] = {
    VK_NULL_HANDLE};
static VkSemaphore g_GraphicsReadySems[CAPTURE_TEXTURE_COUNT] = {
    VK_NULL_HANDLE};
...
static bool separateDeviceActive =
    false; // Global tracking for Separate Device Mode
static bool g_SampleRateShadingSupported =
    false; // Track if device supports/has enabled sample shading
static bool g_SamplerAnisotropySupported =
    false; // Track if device supports/has enabled anisotropy
...
static VkDevice g_PrerenderDevice =
    VK_NULL_HANDLE; // Device required for fence creation
...
static VkQueue g_TrackedGraphicsQueues[MAX_TRACKED_GRAPHICS_QUEUES] = {
    VK_NULL_HANDLE};
...
static bool IsTrackedGraphicsQueue(VkQueue queue) {
  ...
  for (int i = 0; i < count; i++) {
    if (g_TrackedGraphicsQueues[i] == queue)
      return true;
  }
  return false;
}
...
// Mask: 'x' = match, '?' = wild
static uint8_t *ScanPattern(uint8_t *start, size_t size, const char *pattern,
                            const char *mask) {
...
static int ApplyWhitelistedPattern(uint8_t *start, size_t size,
...
static int ApplyShotgunWhitelists(uint8_t *start, size_t size) {
  ...
  for (uint8_t reg : targetRegs) {
    ...
    const char *regName =
        (reg == 6 ? "ESI" : (reg == 7 ? "EDI" : (reg == 0 ? "EAX" : "GPR")));

    // Quad Zero (04, 08, 0C, 10) - 28 bytes
    uint8_t pQ4[] = {0xC7, mod,  0x04, 0x00, 0x00, 0x00, 0x00, 0xC7, mod,  0x08,
                     0x00, 0x00, 0x00, 0x00, 0xC7, mod,  0x0C, 0x00, 0x00, 0x00,
                     0x00, 0xC7, mod,  0x10, 0x00, 0x00, 0x00, 0x00};
    uint8_t hQ4[] = {0xC7, mod,  0x04, 0x00, 0x00, 0x00, 0x00, 0xC7, mod,  0x08,
                     0x10, 0x00, 0x00, 0x00, 0xC7, mod,  0x0C, 0x10, 0x00, 0x00,
                     0x00, 0xC7, mod,  0x10, 0x10, 0x00, 0x00, 0x00};
    totalHits +=
        ApplyWhitelistedPattern(start, size, pQ4, 28, hQ4, 28, regName);
    ...
    totalHits +=
        ApplyWhitelistedPattern(start, size, pZ3, 21, hZ3, 21, regName);

    // Handle + Double Zero (89 4X 08, C7 4X 0C, C7 4X 10) - 17 bytes
    uint8_t pHZ2[] = {0x89, mod,  0x08, 0xC7, mod,  0x0C, 0x00, 0x00, 0x00,
                      0x00, 0xC7, mod,  0x10, 0x00, 0x00, 0x00, 0x00};
    uint8_t hHZ2[] = {0x89, mod,  0x08, 0xC7, mod,  0x0C, 0x10, 0x00, 0x00,
                      0x00, 0xC7, mod,  0x10, 0x10, 0x00, 0x00, 0x00};
    totalHits +=
        ApplyWhitelistedPattern(start, size, pHZ2, 17, hHZ2, 17, regName);
  }

  // B: SHRD Variant (Aged version)
  uint8_t pEDI_S1[] = {0x0F, 0xAC, 0xC1, 0x02, 0x8B, 0x45, 0x08,
                       0xC7, 0x47, 0x08, 0x00, 0x00, 0x00, 0x00,
                       0xC7, 0x47, 0x0C, 0x00, 0x00, 0x00, 0x00};
  uint8_t hEDI_S1[] = {0x0F, 0xAC, 0xC1, 0x02, 0x8B, 0x45, 0x08,
                       0xC7, 0x47, 0x08, 0x10, 0x00, 0x00, 0x00,
                       0xC7, 0x47, 0x0C, 0x10, 0x00, 0x00, 0x00};
  totalHits +=
      ApplyWhitelistedPattern(start, size, pEDI_S1, 21, hEDI_S1, 21, "SHRD");

  // C: REP MOVSD Context (F3 A5 C7 4x 0C...)
  for (uint8_t reg : targetRegs) {
    uint8_t mod = 0x40 + reg;
    uint8_t pREP[] = {0xF3, 0xA5, 0xC7, mod,  0x0C, 0x00, 0x00, 0x00,
                      0x00, 0xC7, mod,  0x10, 0x00, 0x00, 0x00, 0x00};
    uint8_t hREP[] = {0xF3, 0xA5, 0xC7, mod,  0x0C, 0x10, 0x00, 0x00,
                      0x00, 0xC7, mod,  0x10, 0x10, 0x00, 0x00, 0x00};
    totalHits +=
        ApplyWhitelistedPattern(start, size, pREP, 16, hREP, 16, "REP_MOVSD");
  }
...
static void ScanForAllResets(uint8_t *start, size_t size) {
  const char *regNames[] = {"EAX", "ECX", "EDX", "EBX",
                            "ESP", "EBP", "ESI", "EDI"};
...
static bool CreateCaptureDevice() {
  ...
  vkGetPhysicalDeviceQueueFamilyProperties(g_PhysDevice, &qFamCount,
                                           qProps.data());
  ...
  // volkLoadDevice(g_CaptureDevice); // REMOVED to prevent global pointer
  // hijacking in multi-device scenarios
  ...
  HookLog("Vulkan: CaptureDevice created (Family %d, Queue %p)", transferFamily,
          g_CaptureQueue);
...
static bool CreateCrossDeviceImages(uint32_t width, uint32_t height,
                                    VkFormat format) {
  ...
  auto getMemHandle = (PFN_vkGetMemoryWin32HandleKHR)vkGetDeviceProcAddr(
  ...
  for (int i = 0; i < CAPTURE_TEXTURE_COUNT; i++) {
    ...
    // Adding COLOR_ATTACHMENT_BIT as it's often required for D3D11
    // OpenSharedResource success
    imageCI.usage =
        VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT |
        VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT;

    // Use EXCLUSIVE for shared export images to avoid complexity with D3D11
    // interop
    ...
    // Let's try to update struct if possible, but I can't see the struct def
    // here easily. I'll stick to variable names matching the device they are
    // on. Re-using the struct members but assuming: captureDeviceImage -> Image
    // on Creating Device (Now Game) gameDeviceImported -> Image on Importing
    // Device (Now Capture)

    VkResult res = vkCreateImage(g_Device, &imageCI, nullptr,
                                 &g_CrossDeviceImages[i].mainImage);
    if (res != VK_SUCCESS) {
    ...
    vkGetImageMemoryRequirements(g_Device, g_CrossDeviceImages[i].mainImage,
                                 &memReqs);

    VkExportMemoryWin32HandleInfoKHR win32ExportInfo = {
    ...
    allocInfo.memoryTypeIndex =
        FindMemoryType(g_PhysDevice, memReqs.memoryTypeBits,
                       VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

    res = vkAllocateMemory(g_Device, &allocInfo, nullptr,
                           &g_CrossDeviceImages[i].mainMemory);
    if (res != VK_SUCCESS) {
    ...
    res = vkBindImageMemory(g_Device, g_CrossDeviceImages[i].mainImage,
                            g_CrossDeviceImages[i].mainMemory, 0);
    if (res != VK_SUCCESS) {
    ...
    if (res != VK_SUCCESS) {
      HookLog("Vulkan: Error - vkGetMemoryWin32HandleKHR[%d] failed with %d", i,
              res);
      vkFreeMemory(g_Device, g_CrossDeviceImages[i].mainMemory, nullptr);
      vkDestroyImage(g_Device, g_CrossDeviceImages[i].mainImage, nullptr);
      g_CrossDeviceImages[i].mainImage = VK_NULL_HANDLE;
    ...
    if (g_CrossDeviceImages[i].ntHandle == INVALID_HANDLE_VALUE ||
        g_CrossDeviceImages[i].ntHandle == nullptr) {
      HookLog(
          "Vulkan: Error - CrossDeviceImage[%d] export returned invalid handle",
          i);
      vkFreeMemory(g_Device, g_CrossDeviceImages[i].mainMemory, nullptr);
      vkDestroyImage(g_Device, g_CrossDeviceImages[i].mainImage, nullptr);
      g_CrossDeviceImages[i].mainImage = VK_NULL_HANDLE;
    ...
    HookLog("Vulkan: CrossDeviceImage[%d] exported successfully, handle=%p", i,
            g_CrossDeviceImages[i].ntHandle);
  }
  return true;
}

// Unused function removed
static bool ImportCrossDeviceImages(uint32_t width, uint32_t height,
                                    VkFormat format) {
  return true; // No-op
}

static bool CreateCrossDeviceSemaphore() {
  VkExportSemaphoreWin32HandleInfoKHR exportSemWin32Info = {
  ...
  VkSemaphoreCreateInfo importedSemCI = {
  ...
  auto getSemHandle = (PFN_vkGetSemaphoreWin32HandleKHR)vkGetDeviceProcAddr(
      g_Device, "vkGetSemaphoreWin32HandleKHR");
  auto importSem = (PFN_vkImportSemaphoreWin32HandleKHR)vkGetDeviceProcAddr(
      g_CaptureDevice, "vkImportSemaphoreWin32HandleKHR");

  if (!getSemHandle || !importSem) {
    HookLog("Vulkan: Error - Missing cross-device semaphore extensions "
            "(Get=%p, Import=%p)",
            getSemHandle, importSem);
    return false;
  }

  for (int i = 0; i < CAPTURE_TEXTURE_COUNT; i++) {
    // 1. Create exportable binary semaphore on GAME Device (Primary)
    // We reuse 'g_CrossDeviceSems_GameImported' to store the Game Device
    // semaphore (even though it's now exported, preventing variable rename
    // churn).
    if (vkCreateSemaphore(g_Device, &semCI, nullptr,
                          &g_CrossDeviceSems_GameImported[i]) != VK_SUCCESS) {
      HookLog("Vulkan: Error - Failed to create semaphore %d on Game Device",
              i);
      return false;
    }
    ...
    // We reuse 'g_CrossDeviceSems_CaptureOwned' to store the Capture Device
    // semaphore Previously created? No, we need to create it! Wait,
    // ImportSemaphoreWin32Handle imports into EXISTING semaphore or creates? It
    // imports payload into EXISTING semaphore.

    // So we must Create semaphore on Capture Device first.
    if (vkCreateSemaphore(g_CaptureDevice, &importedSemCI, nullptr,
                          &g_CrossDeviceSems_CaptureOwned[i]) != VK_SUCCESS) {
      HookLog("Vulkan: Error - Failed to create semaphore %d on Capture Device",
              i);
      return false;
    }
    ...
    if (importSem(g_CaptureDevice, &importInfo) != VK_SUCCESS) {
      HookLog("Vulkan: Error - Failed to import semaphore %d to Capture Device",
              i);
      return false;
    }
...
public:
  ...
  // Without these, we may reset a command buffer while GPU is still executing
  // it
  ...
  void CleanupVulkan(VkDevice device) {
    ...
    if (g_CaptureDevice != VK_NULL_HANDLE) {
      ...
      for (int i = 0; i < CAPTURE_TEXTURE_COUNT; i++) {
        if (g_CrossDeviceSems_GameImported[i] != VK_NULL_HANDLE) {
          vkDestroySemaphore(g_Device, g_CrossDeviceSems_GameImported[i],
                             nullptr);
          g_CrossDeviceSems_GameImported[i] = VK_NULL_HANDLE;
        }
        if (g_CrossDeviceSems_CaptureOwned[i] != VK_NULL_HANDLE) {
          vkDestroySemaphore(g_CaptureDevice, g_CrossDeviceSems_CaptureOwned[i],
                             nullptr);
          g_CrossDeviceSems_CaptureOwned[i] = VK_NULL_HANDLE;
        }
        // Handles are closed by Import (consumed), so no CloseHandle needed for
        // imported ones. If export failed or import not done, we might need to
        // close, but we assume success if created.
        g_CrossDeviceSemHandles[i] = nullptr;
      }

      for (int i = 0; i < CAPTURE_TEXTURE_COUNT; i++) {
        if (g_CrossDeviceImages[i].importedImage) {
          vkDestroyImage(g_CaptureDevice, g_CrossDeviceImages[i].importedImage,
                         nullptr);
          g_CrossDeviceImages[i].importedImage = VK_NULL_HANDLE;
        }
        if (g_CrossDeviceImages[i].importedMemory) {
          vkFreeMemory(g_CaptureDevice, g_CrossDeviceImages[i].importedMemory,
                       nullptr);
    ...
    // Standard cleanup loop handles sharedImages (which alias
    // mainImage/mainMemory in separate device mode) No extra cleanup needed for
    // CrossDeviceImages on Game Device here.
    ...
    for (int i = 0; i < CAPTURE_TEXTURE_COUNT; i++) {
      ...
      // CRITICAL: If these aliased g_CrossDeviceImages, NULL out the globals
      // too!
      for (int j = 0; j < CAPTURE_TEXTURE_COUNT; j++) {
        if (sharedImages[i] == g_CrossDeviceImages[j].mainImage)
          g_CrossDeviceImages[j].mainImage = VK_NULL_HANDLE;
        if (sharedMem[i] == g_CrossDeviceImages[j].mainMemory)
          g_CrossDeviceImages[j].mainMemory = VK_NULL_HANDLE;
      }
    ...
    if (asyncCommandPool) {
      if (asyncCommandBuffers[0]) {
        vkFreeCommandBuffers(device, asyncCommandPool, CAPTURE_TEXTURE_COUNT,
                             asyncCommandBuffers);
        for (auto &cb : asyncCommandBuffers)
          cb = VK_NULL_HANDLE;
      }
  ...
  bool CreateD3D11SharedResources(uint32_t w, uint32_t h, uint32_t dxgiFormat) {
    ...
    if (res != VK_SUCCESS) {
      HookLog("Vulkan: Failed to create exportable timeline semaphore: %d",
              res);
      CleanupD3D11();
      return false;
    }
    ...
    if (GetSemaphoreWin32Handle) {
      ...
      getSemHandleInfo.handleType =
          VK_EXTERNAL_SEMAPHORE_HANDLE_TYPE_D3D12_FENCE_BIT;
      VkResult expRes = GetSemaphoreWin32Handle(g_Device, &getSemHandleInfo,
                                                &sharedFenceHandle);
      if (expRes == VK_SUCCESS) {
        HookLog("Vulkan: Exported timeline semaphore as D3D12 fence, handle=%p",
                sharedFenceHandle);
      } else {
    ...
    for (int i = 0; i < CAPTURE_TEXTURE_COUNT; i++) {
      ...
      vkCreateSemaphore(g_Device, &binarySemInfo, nullptr,
                        &presentTriggerSems[i]);
  ...
  bool ImportEncoderTextures(SharedMemoryLayout *ipcMem) {
    ...
    // Use the encoder's fence handle directly for synchronization
  ...
  void CreateSharedResources(uint32_t w, uint32_t h, uint32_t fmt) override {
    ...
    for (int i = 0; i < CAPTURE_TEXTURE_COUNT; i++) {
      ...
      imageInfo.queueFamilyIndexCount = (g_AsyncQueueFamily != UINT32_MAX &&
                                         g_AsyncQueueFamily != g_QueueFamily)
                                            ? 2
                                            : 1;
      imageInfo.pQueueFamilyIndices = families;

      if (vkCreateImage(g_Device, &imageInfo, nullptr,
                        &g_InternalStagingImages[i]) != VK_SUCCESS) {
        HookLog("Vulkan: Error - Failed to create internal staging image %d",
                i);
        continue;
      }

      VkMemoryRequirements memReqs;
      vkGetImageMemoryRequirements(g_Device, g_InternalStagingImages[i],
                                   &memReqs);
      ...
      if (vkAllocateMemory(g_Device, &allocInfo, nullptr,
                           &g_InternalStagingMemories[i]) != VK_SUCCESS) {
        HookLog("Vulkan: Error - Failed to allocate internal staging memory %d",
                i);
        continue;
      }
      vkBindImageMemory(g_Device, g_InternalStagingImages[i],
                        g_InternalStagingMemories[i], 0);
    }

    // --- OPTIMIZED PATH: Separate Capture Device (Zero Copy) ---
    // Try to create a separate device for capture to minimize game queue
    // impact.
    if (CreateCaptureDevice()) {
      separateDeviceActive = true;
      if (CreateCrossDeviceImages(width, height, vkFormat) &&
          CreateCrossDeviceSemaphore()) {
    ...
    if (!separateDeviceActive) {
      ...
      for (int i = 0; i < CAPTURE_TEXTURE_COUNT; i++) {
        ...
        if (allocRes != VK_SUCCESS) {
          HookLog("Vulkan: Error - vkAllocateMemory %d failed (%d)", i,
                  allocRes);
          continue;
        }
    ...
    // Create fences for command buffer synchronization - CRITICAL for safe
    // reuse Each fence tracks whether the corresponding command buffer has
    // finished execution
    VkFenceCreateInfo fenceInfo = {VK_STRUCTURE_TYPE_FENCE_CREATE_INFO};
    fenceInfo.flags = VK_FENCE_CREATE_SIGNALED_BIT; // Start signaled so first
                                                    // use doesn't wait
    for (int i = 0; i < CAPTURE_TEXTURE_COUNT; i++) {
      VkResult fenceRes = vkCreateFence(g_Device, &fenceInfo, nullptr,
                                        &captureCommandFences[i]);
    ...
    if (g_AsyncQueue != VK_NULL_HANDLE && g_AsyncQueueFamily != UINT32_MAX) {
      ...
      HookLog("Vulkan: Creating Async Command Pool (Family %d)...",
              asyncQueueFamily);
      if (vkCreateCommandPool(g_Device, &asyncPoolInfo, nullptr,
                              &asyncCommandPool) == VK_SUCCESS) {
        VkCommandBufferAllocateInfo asyncAlloc = {
            VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO};
        ...
        if (vkAllocateCommandBuffers(g_Device, &asyncAlloc,
                                     asyncCommandBuffers) == VK_SUCCESS) {
    ...
    // --- Timeline Semaphore (Encoder Sync) ---
    {
      ...
      HookLog("Vulkan: Creating timeline semaphore on %s device...",
              deviceName);
      VkResult semRes = vkCreateSemaphore(targetDevice, &timelineSemInfo,
                                          nullptr, &timelineSemaphore);
      if (semRes != VK_SUCCESS || timelineSemaphore == VK_NULL_HANDLE) {
        HookLog("Vulkan: ERROR - vkCreateSemaphore (timeline) failed on %s "
                "device (%d)",
                deviceName, semRes);
        if (separateDeviceActive) {
          separateDeviceActive = false; // Fallback? Might be too late if other
                                        // resources are already cross-device
        }
        return;
      }
      ...
      auto getSemHandle = (PFN_vkGetSemaphoreWin32HandleKHR)vkGetDeviceProcAddr(
          targetDevice, "vkGetSemaphoreWin32HandleKHR");
      if (getSemHandle) {
        if (getSemHandle(targetDevice, &getSemHandleInfo, &sharedFenceHandle) ==
            VK_SUCCESS) {
          HookLog("Vulkan: Exported shared fence handle from %s device: %p",
                  deviceName, sharedFenceHandle);
        } else {
          HookLog("Vulkan: ERROR - Failed to export timeline semaphore from %s "
                  "device",
                  deviceName);
        }
      } else {
        HookLog("Vulkan: ERROR - vkGetSemaphoreWin32HandleKHR not found on %s "
                "device",
                deviceName);
      }
    }
  ...
  VkSemaphore CopyFrame(VkQueue queue, VkImage srcImage, uint32_t srcWidth,
    ...
    int idx = writeIndex;
    VkImage dstImage =
        separateDeviceActive ? g_InternalStagingImages[idx] : sharedImages[idx];
    ...
    if (separateDeviceActive) {
      ...
      // Signal signalSem (Present trigger) AND g_GraphicsReadySems[idx]
      // (Transfer Bridge trigger)
...

      {
        ...
        if (gameSubmitRes != VK_SUCCESS) {
          HookLog("Vulkan: ERROR - Game queue submit failed: %d (Queue=%p, "
                  "Families=[%d,%d])",
      ...
      // Signal timeline semaphore (for IPC) AND copyCompleteSemaphore (for
      // Present) directly
      fenceValue++;
      ...
      uint64_t signalValues[] = {signalValue,
                                 0}; // 0 for binary semaphore ignored
...
struct VulkanSwapchain {
  ...
  uint32_t currentImageIndex =
      0; // Track current image index for overlay rendering
...
static void AsyncCaptureThreadProc() {
  ...
  if (g_AsyncQueue == VK_NULL_HANDLE && g_CaptureQueue == VK_NULL_HANDLE) {
    HookLog("Vulkan: ERROR - AsyncCaptureThread started without any valid "
            "queue. Exiting thread.");
  ...
  while (!g_VulkanCapture.captureThreadShutdown) {
    ...
    while (true) {
      uint32_t wIdxGlobal =
          g_VulkanCapture.pendingWriteIdx.load(std::memory_order_acquire);
      uint32_t rIdx =
          g_VulkanCapture.pendingReadIdx.load(std::memory_order_acquire);
      if (rIdx >= wIdxGlobal)
        break;

      PendingCaptureFrame &frame =
          g_VulkanCapture.pendingRing[rIdx % CAPTURE_RING_SIZE];
      int writeIdx =
          frame.backBufferIndex; // USE THE INDEX PASSED FROM THE GAME THREAD
      bool separateDeviceActive = (g_CaptureDevice != VK_NULL_HANDLE);

      HookLog("Vulkan: Async - Processing Frame (RIdx=%d, WriteIdx=%d, "
              "Separate=%d)",
              rIdx, writeIdx, separateDeviceActive);

      if (separateDeviceActive) {
        // --- STABLE THREE-STAGE PIPE ---
        VkCommandBuffer transCmd =
            g_VulkanCapture.asyncCommandBuffers[writeIdx];
        ...
        if (transCmd && semGraphicsReady && semGameImported && srcStage &&
            dstExport && g_AsyncQueue) {
          HookLog("Vulkan: Async - Stage 2: Bridge Copy (Index %d)...",
                  writeIdx);
          ...
          // Since we are in EXCLUSIVE mode, we need an ACQUIRE barrier here on
          // the ASYNC queue.
          ...
          vkCmdCopyImage(transCmd, srcStage,
                         VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, dstExport,
          ...
          HookLog("Vulkan: Async - ERROR: Missing resources for Stage 2 "
                  "(Cmd=%p, SemG=%p, SemI=%p, Src=%p, "
                  "Dst=%p, Q=%p)",
                  transCmd, semGraphicsReady, semGameImported, srcStage,
                  dstExport, g_AsyncQueue);
        }

        VkSemaphore semCaptureOwned = g_CrossDeviceSems_CaptureOwned[writeIdx];
        if (g_CaptureQueue && semCaptureOwned && cpuWaitFence) {
          HookLog("Vulkan: Async - Stage 3: Separate Sync (Index %d)...",
                  writeIdx);
          vkResetFences(g_CaptureDevice, 1, &cpuWaitFence);
          VkPipelineStageFlags syncWaitStage =
...

          {
            std::lock_guard<std::recursive_mutex> lock(g_VulkanMutex);
            VkResult res =
                vkQueueSubmit(g_CaptureQueue, 1, &syncSubmit, cpuWaitFence);
            if (res != VK_SUCCESS) {
              HookLog("Vulkan: Async - ERROR: Sync Submit failed: %d", res);
            }
          }
          vkWaitForFences(g_CaptureDevice, 1, &cpuWaitFence, VK_TRUE,
                          1000000000);
          HookLog("Vulkan: Async - Stage 3 Sync Complete");
        } else {
          HookLog("Vulkan: Async - ERROR: Missing resources for Stage 3 (Q=%p, "
                  "SemC=%p, Fence=%p)",
...

        {
          std::lock_guard<std::recursive_mutex> lock(g_VulkanMutex);
          VkResult res =
              vkQueueSubmit(g_CaptureQueue, 1, &finalSubmit, VK_NULL_HANDLE);
          if (res != VK_SUCCESS) {
            HookLog("Vulkan: Async - ERROR: Timeline Submit failed: %d", res);
          } else {
            HookLog("Vulkan: Async - Frame %d Ready (Fence=%llu)", writeIdx,
                    signalValue);
          }
        }
        // PASS RAW QPC: MediaEngine converts to MS using trusted frequency
        g_VulkanCapture.SignalFrameReady(g_IPC, writeIdx, frame.timestampQPC,
                                         signalValue);
        ...
        if (cmd && srcImage && dstImage && g_AsyncQueue) {
          ...
          barriers[0].subresourceRange = {VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0,
                                          1};
          ...
          barriers[1].subresourceRange = {VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0,
                                          1};
          ...
          VkSemaphore signalSems[] = {
              g_VulkanCapture.timelineSemaphore,
              g_VulkanCapture.presentTriggerSems[writeIdx]};
  ...
  if (cpuWaitFence)
    vkDestroyFence(g_CaptureDevice, cpuWaitFence, nullptr);
  g_VulkanCapture.captureThreadRunning = false;
...
static bool TryRecoverPhysicalDevice() {
  ...
  if (g_Device == VK_NULL_HANDLE) {
    ...
    vkGetPhysicalDeviceQueueFamilyProperties(g_PhysDevice, &queueFamilyCount,
                                             nullptr);
...
static PFN_vkCreateDescriptorUpdateTemplate o_vkCreateDescriptorUpdateTemplate =
    nullptr;
static PFN_vkCreateDescriptorUpdateTemplate
    o_vkCreateDescriptorUpdateTemplateKHR = nullptr;
static PFN_vkDestroyDescriptorUpdateTemplate
    o_vkDestroyDescriptorUpdateTemplate = nullptr;
static PFN_vkDestroyDescriptorUpdateTemplate
    o_vkDestroyDescriptorUpdateTemplateKHR = nullptr;
static PFN_vkUpdateDescriptorSetWithTemplate
    o_vkUpdateDescriptorSetWithTemplate = nullptr;
static PFN_vkUpdateDescriptorSetWithTemplate
    o_vkUpdateDescriptorSetWithTemplateKHR = nullptr;
...
static std::unordered_map<VkDescriptorUpdateTemplate, DescriptorTemplateRecord>
    g_TemplateRecords;

static VkSampler GetOrCreateForcedSampler(VkSampler originalSampler,
  ...
  if (rec.forcedSampler != VK_NULL_HANDLE)
    return rec.forcedSampler;
  if (rec.device == VK_NULL_HANDLE)
    return VK_NULL_HANDLE;
  ...
  if (anisotropyLevel > 1 && g_SamplerAnisotropySupported &&
  ...
  if (allowMipOverrides && biasStr && biasStr[0] != '\0' &&
      strncmp(biasStr, "default", 7) != 0) {
    float userBias = 0.0f;
  ...
  if (allowMipOverrides && mmStr && mmStr[0] != '\0' &&
      strncmp(mmStr, "default", 7) != 0) {
...
VkResult VKAPI_CALL Detour_vkCreateInstance(
  ...
  // This catches any samplers/instances created during driver initialization
  // triggered by o_vkCreateInstance. ApplyNvidiaLodBiasFix removed
...
VkResult VKAPI_CALL
Detour_vkCreateImage(VkDevice device, const VkImageCreateInfo *pCreateInfo,
...
VkResult VKAPI_CALL Detour_vkCreateRenderPass(
  ...
  if (pCreateInfo && g_IPC && g_IPC->GetSharedMem()) {
    ...
    if (msaa[0] != 'd') {
      ...
      if (samples > VK_SAMPLE_COUNT_1_BIT || strcmp(msaa, "off") == 0) {
        ...
        return o_vkCreateRenderPass(device, &modifiedCI, pAllocator,
                                    pRenderPass);
      }
    }
  }
...
VkResult VKAPI_CALL Detour_vkCreateRenderPass2(
  ...
  if (pCreateInfo && g_IPC && g_IPC->GetSharedMem()) {
    ...
    if (msaa[0] != 'd') {
      ...
      if (samples > VK_SAMPLE_COUNT_1_BIT || strcmp(msaa, "off") == 0) {
        ...
        modifiedCI.pAttachments = attachments.data();
        return o_vkCreateRenderPass2(device, &modifiedCI, pAllocator,
                                     pRenderPass);
      }
    }
  }
...
static VkResult VKAPI_PTR Detour_vkCreateGraphicsPipelines(
  ...
  if (g_IPC && g_IPC->GetSharedMem() &&
      g_IPC->GetSharedMem()->graphicsConfig.sgssaa) {
    std::vector<VkGraphicsPipelineCreateInfo> modifiedInfos(createInfoCount);
    std::vector<VkPipelineMultisampleStateCreateInfo> modifiedMsStates(
        createInfoCount);
    ...
    for (uint32_t i = 0; i < createInfoCount; i++) {
      ...
      if (pCreateInfos[i].pMultisampleState) {
        ...
        // Since forced MSAA happens at Image/RenderPass creation, the pipeline
        // state MUST match. However, the game might still be asking for 1x here
        // if it doesn't know about our override. But blindly changing this
        // might break things if the render pass is incompatible. For SGSSAA, we
        // primarily want to enable Sample Shading IF MSAA is active.
        ...
        VkSampleCountFlagBits forcedSamples =
            ParseVulkanMSAA(g_IPC->GetSharedMem()->graphicsConfig.msaaSamples);
        ...
        if (samples > VK_SAMPLE_COUNT_1_BIT) {
          ...
          if (g_SampleRateShadingSupported) {
            ...
            modifiedMsStates[i].minSampleShading =
                1.0f; // Force full per-sample shading
          }
          ...
          modified = true;
          // HookLog("Vulkan: SGSSAA enabled for pipeline %d (samples=%d,
          // shading=1.0)", i, samples);
        }
      }
    }

    if (modified) {
      return o_vkCreateGraphicsPipelines(device, pipelineCache, createInfoCount,
                                         modifiedInfos.data(), pAllocator,
                                         pPipelines);
    }
  }
...
static VkResult VKAPI_PTR Detour_vkCreateSampler(
  ...
  if (s_createSamplerCallCount <= 5) {
    HookLog("Vulkan: Detour_vkCreateSampler called (#%d) o_vkCreateSampler=%p "
            "IPC=%p SharedMem=%p",
            s_createSamplerCallCount, (void *)o_vkCreateSampler, (void *)g_IPC,
            g_IPC ? (void *)g_IPC->GetSharedMem() : nullptr);
  }
  ...
  if (g_IPC && g_IPC->GetSharedMem()) {
    ...
    const bool anyOverrides =
        ...
        (gfx.mipBias != "default" && !gfx.mipBias.empty()) || (gfx.sgssaa);
    ...
    if (allowMipOverrides &&
        GetSGSSAABias(gfx.sgssaa, gfx.msaaSamples.c_str(), sgssaaBias)) {
    ...
    if (s_samplerOverrideLogCount < 10) {
      ...
      HookLog("Vulkan: CreateSampler override (af=%s maxLod=%.2f allowMip=%d "
              "unnorm=%d) "
              "Aniso: %d/%.1f -> %d/%.1f  Filter: %d/%d/%d  Bias: %.3f -> %.3f",
              gfx.anisotropicFiltering.c_str(), pCreateInfo->maxLod,
              allowMipOverrides ? 1 : 0,
              (int)pCreateInfo->unnormalizedCoordinates,
              (int)pCreateInfo->anisotropyEnable,
              (double)pCreateInfo->maxAnisotropy,
              (int)modifiedInfo.anisotropyEnable,
              (double)modifiedInfo.maxAnisotropy, (int)modifiedInfo.minFilter,
              (int)modifiedInfo.magFilter, (int)modifiedInfo.mipmapMode,
...
Detour_vkDestroySampler(VkDevice device, VkSampler sampler,
                        const VkAllocationCallbacks *pAllocator) {
  if (!o_vkDestroySampler)
    return;
...
static void VKAPI_PTR Detour_vkUpdateDescriptorSets(
    ...
    const VkWriteDescriptorSet *pDescriptorWrites, uint32_t descriptorCopyCount,
    const VkCopyDescriptorSet *pDescriptorCopies) {
  if (!o_vkUpdateDescriptorSets)
    return;

  if (!g_IPC || !g_IPC->GetSharedMem() || !pDescriptorWrites ||
      descriptorWriteCount == 0) {
    o_vkUpdateDescriptorSets(device, descriptorWriteCount, pDescriptorWrites,
  ...
  const bool afActive =
      (gfx.anisotropicFiltering != "default" &&
       !gfx.anisotropicFiltering.empty() && gfx.anisotropicFiltering != "off");
  const bool anyMip =
      (gfx.mipMapping != "default" && !gfx.mipMapping.empty()) ||
      (gfx.mipBias != "default" && !gfx.mipBias.empty()) || (gfx.sgssaa);
  ...
  for (uint32_t i = 0; i < descriptorWriteCount; i++) {
    ...
    imageInfos.insert(imageInfos.end(), w.pImageInfo,
                      w.pImageInfo + w.descriptorCount);
  ...
  o_vkUpdateDescriptorSets(device, descriptorWriteCount, writes.data(),
                           descriptorCopyCount, pDescriptorCopies);
}
...
static VkResult VKAPI_PTR Detour_vkCreateDescriptorUpdateTemplate(
  ...
  if (s_templateCreateCount <= 5) {
    HookLog("Vulkan: Detour_vkCreateDescriptorUpdateTemplate called (#%d) "
            "o_core=%p o_khr=%p entries=%u",
  ...
  PFN_vkCreateDescriptorUpdateTemplate origFn =
      o_vkCreateDescriptorUpdateTemplate;
  if (!origFn)
    origFn = o_vkCreateDescriptorUpdateTemplateKHR;
  if (!origFn) {
  ...
  VkResult res =
      origFn(device, pCreateInfo, pAllocator, pDescriptorUpdateTemplate);
  if (res != VK_SUCCESS || !pDescriptorUpdateTemplate || !pCreateInfo)
    return res;
  ...
  for (uint32_t i = 0; i < pCreateInfo->descriptorUpdateEntryCount; i++) {
    const VkDescriptorUpdateTemplateEntry &e =
...
static void VKAPI_PTR Detour_vkDestroyDescriptorUpdateTemplate(
  ...
  PFN_vkDestroyDescriptorUpdateTemplate origFn =
      o_vkDestroyDescriptorUpdateTemplate;
  if (!origFn)
    origFn = o_vkDestroyDescriptorUpdateTemplateKHR;
  if (!origFn)
    return;
...
static void VKAPI_PTR Detour_vkUpdateDescriptorSetWithTemplate(
  ...
  if (s_templateUpdateCount <= 5) {
    HookLog("Vulkan: Detour_vkUpdateDescriptorSetWithTemplate called (#%d) "
            "o_core=%p o_khr=%p",
            s_templateUpdateCount, (void *)o_vkUpdateDescriptorSetWithTemplate,
            (void *)o_vkUpdateDescriptorSetWithTemplateKHR);
  }

  PFN_vkUpdateDescriptorSetWithTemplate origFn =
      o_vkUpdateDescriptorSetWithTemplate;
  if (!origFn)
    origFn = o_vkUpdateDescriptorSetWithTemplateKHR;
  if (!origFn) {
    ...
    for (uint32_t j = 0; j < entry.descriptorCount; j++) {
      ...
      const VkDescriptorImageInfo *imgInfo =
          reinterpret_cast<const VkDescriptorImageInfo *>(
              static_cast<const uint8_t *>(pData) + itemOffset);
      ...
      if (forced != VK_NULL_HANDLE && forced != imgInfo->sampler) {
        ...
        VkDescriptorImageInfo *modImgInfo =
            reinterpret_cast<VkDescriptorImageInfo *>(modifiedData.data() +
                                                      itemOffset);
  ...
  if (anyChanged && !modifiedData.empty()) {
    origFn(device, descriptorSet, descriptorUpdateTemplate,
           modifiedData.data());
  } else {
    origFn(device, descriptorSet, descriptorUpdateTemplate, pData);
  }
...
VkResult VKAPI_CALL Detour_vkCreateDevice(
  ...
  std::vector<VkQueueFamilyProperties> qProps(qFamCount);
  vkGetPhysicalDeviceQueueFamilyProperties(physicalDevice, &qFamCount,
                                           qProps.data());
  ...
  for (uint32_t i = 0; i < pCreateInfo->queueCreateInfoCount; i++) {
    ...
    if (famIdx < qFamCount) {
      g_QueueFamilies[famIdx].requestedCount +=
  ...
  for (uint32_t i = 0; i < qFamCount; i++) {
    if (g_QueueFamilies[i].requestedCount < g_QueueFamilies[i].totalAvailable) {
      ...
      if ((g_QueueFamilies[i].flags & VK_QUEUE_TRANSFER_BIT) &&
          ...
          !(g_QueueFamilies[i].flags & VK_QUEUE_COMPUTE_BIT))
        score = 100;
      else if ((g_QueueFamilies[i].flags & VK_QUEUE_COMPUTE_BIT) &&
               !(g_QueueFamilies[i].flags & VK_QUEUE_GRAPHICS_BIT))
        score = 50;
      else if (g_QueueFamilies[i].flags & VK_QUEUE_TRANSFER_BIT)
        score = 30;
      else
        score = 10;
  ...
  if (bestInjectionFam != -1) {
    ...
    g_AsyncQueueIndex = g_QueueFamilies[bestInjectionFam]
                            .requestedCount; // Next available index
    familyInjected = true;
    HookLog("Vulkan: Injecting extra queue for capture in Family %d (Index %d, "
            "TypeScore %d)",
  ...
  for (uint32_t i = 0; i < pCreateInfo->queueCreateInfoCount; i++) {
    ...
    if (familyInjected && ci.queueFamilyIndex == (uint32_t)bestInjectionFam) {
      ...
      for (uint32_t j = 0; j < ci.queueCount; j++)
        priorities[j] = ci.pQueuePriorities[j];
  ...
  if (!hasExternalMemWin32 &&
      IsSupported(VK_KHR_EXTERNAL_MEMORY_WIN32_EXTENSION_NAME))
  ...
  for (auto ext : extensions) {
    ...
    if (strcmp(ext, VK_KHR_EXTERNAL_SEMAPHORE_WIN32_EXTENSION_NAME) == 0)
      hasExternalSemWin32 = true;
  }
  if (!hasExternalSem && IsSupported(VK_KHR_EXTERNAL_SEMAPHORE_EXTENSION_NAME))
    extensions.push_back(VK_KHR_EXTERNAL_SEMAPHORE_EXTENSION_NAME);
  if (!hasExternalSemWin32 &&
      IsSupported(VK_KHR_EXTERNAL_SEMAPHORE_WIN32_EXTENSION_NAME))
  ...
  // We MUST enable sampleRateShading to use it in pipelines, otherwise it's a
  // spec violation.
  ...
  if (supportedFeatures.sampleRateShading) {
    ...
    HookLog("Vulkan: sampleRateShading feature enabled for %s",
            appUsesFeatures2 ? "pNext (Features2)" : "pEnabledFeatures");
  } else {
    g_SampleRateShadingSupported = false;
    HookLog("Vulkan: WARNING - sampleRateShading NOT supported by hardware. "
            "SGSSAA will be disabled.");
  }
  ...
  if (supportedFeatures.samplerAnisotropy) {
    ...
    HookLog("Vulkan: WARNING - samplerAnisotropy NOT supported by hardware. AF "
            "override will fail.");
  }
  ...
  if (res == VK_SUCCESS) {
    ...
    HookLog(
        "Vulkan: Device Created (with injected queue & interop extensions)");
    ...
    if (vkGetPhysicalDeviceProperties2) {
      ...
      if (idProps.deviceLUIDValid) {
        ...
        HookLog(
            "Vulkan: SystemMetricsCollector initialized with LUID: %08X%08X",
            luidHigh, luidLow);
    ...
    if (bestInjectionFam != -1) {
      // CRITICAL FIX: o_vkGetDeviceQueue might be NULL if the app hasn't called
      // vkGetDeviceProcAddr yet. We use the original o_vkGetDeviceProcAddr to
      // retrieve it safely from the newly created device.
      PFN_vkGetDeviceQueue pfnGetQueue =
          (PFN_vkGetDeviceQueue)o_vkGetDeviceProcAddr(g_Device,
                                                      "vkGetDeviceQueue");
      if (pfnGetQueue) {
        pfnGetQueue(g_Device, g_AsyncQueueFamily, g_AsyncQueueIndex,
                    &g_AsyncQueue);
        HookLog("Vulkan: Injected Async Queue retrieved: %p", g_AsyncQueue);
      } else {
        HookLog("Vulkan: ERROR - Failed to retrieve vkGetDeviceQueue from "
                "device! Async capture will fail.");
      }
    }
  }
  return res;
...
void VKAPI_CALL Detour_vkGetDeviceQueue(VkDevice device,
  ...
  if (!o_vkGetDeviceQueue && o_vkGetDeviceProcAddr) {
    o_vkGetDeviceQueue =
        (PFN_vkGetDeviceQueue)o_vkGetDeviceProcAddr(device, "vkGetDeviceQueue");
  }
  if (o_vkGetDeviceQueue)
    o_vkGetDeviceQueue(device, queueFamilyIndex, queueIndex, pQueue);
  if (pQueue && *pQueue) {
...

    {
      ...
      if (!alreadyRegistered) {
        g_QueueInstances.push_back(
            {*pQueue, queueFamilyIndex, queueIndex, flags});
        ...
        HookLog("Vulkan: Queue %p registered (Family %d, Index %d, Type: %s, "
                "Flags: 0x%X)",
...
void VKAPI_CALL Detour_vkGetDeviceQueue2(VkDevice device,
  ...
  if (g_InVulkanHook) {
    if (o_vkGetDeviceQueue2)
      o_vkGetDeviceQueue2(device, pQueueInfo, pQueue);
    return;
  }
  if (!o_vkGetDeviceQueue2 && o_vkGetDeviceProcAddr) {
    o_vkGetDeviceQueue2 = (PFN_vkGetDeviceQueue2)o_vkGetDeviceProcAddr(
  ...
  if (pQueue && *pQueue) {
    ...
    // Usage Tracking
    if (g_Device == device &&
        pQueueInfo->queueFamilyIndex < g_QueueFamilies.size()) {
      if (pQueueInfo->queueIndex + 1 >
          g_QueueFamilies[pQueueInfo->queueFamilyIndex].retrievedCount) {
        g_QueueFamilies[pQueueInfo->queueFamilyIndex].retrievedCount =
            pQueueInfo->queueIndex + 1;
      }
    }
...

    {
      ...
      if (!alreadyRegistered) {
        g_QueueInstances.push_back({*pQueue, pQueueInfo->queueFamilyIndex,
                                    pQueueInfo->queueIndex, flags});
        const char *typeStr = "Unknown";
        if (flags & VK_QUEUE_GRAPHICS_BIT)
          typeStr = "Graphics";
        else if (flags & VK_QUEUE_COMPUTE_BIT)
          ...
          typeStr = "Transfer";
        HookLog("Vulkan: Queue %p registered (Queue2) (Family %d, Index %d, "
                "Type: %s, Flags: 0x%X)",
                *pQueue, pQueueInfo->queueFamilyIndex, pQueueInfo->queueIndex,
                typeStr, flags);
      }
    }
...
VkResult VKAPI_CALL Detour_vkGetSwapchainImagesKHR(
  ...
  if (!o_vkGetSwapchainImagesKHR && o_vkGetDeviceProcAddr) {
    o_vkGetSwapchainImagesKHR =
        (PFN_vkGetSwapchainImagesKHR)o_vkGetDeviceProcAddr(
...
VkResult VKAPI_CALL Detour_vkAcquireNextImageKHR(
  ...
  if (!o_vkAcquireNextImageKHR && o_vkGetDeviceProcAddr) {
    o_vkAcquireNextImageKHR = (PFN_vkAcquireNextImageKHR)o_vkGetDeviceProcAddr(
        device, "vkAcquireNextImageKHR");
  }
  if (!o_vkAcquireNextImageKHR)
    return VK_ERROR_INITIALIZATION_FAILED;

  VkResult result = o_vkAcquireNextImageKHR(device, swapchain, timeout,
...
Detour_vkDestroySwapchainKHR(VkDevice device, VkSwapchainKHR swapchain,
  ...
  if (!o_vkDestroySwapchainKHR && o_vkGetDeviceProcAddr) {
    o_vkDestroySwapchainKHR = (PFN_vkDestroySwapchainKHR)o_vkGetDeviceProcAddr(
...
static void TryDiscoverAsyncQueue() {
  ...
  std::lock_guard<std::recursive_mutex> lock(g_VulkanMutex);
  if (g_AsyncQueue != VK_NULL_HANDLE)
    return;
  ...
  HookLog("Vulkan: Queue Discovery Analysis (%zu families):",
          g_QueueFamilies.size());
  for (uint32_t i = 0; i < (uint32_t)g_QueueFamilies.size(); i++) {
    ...
    HookLog("  Family %d: %s | Requested: %d, Retrieved: %d, HW Total: %d", i,
            typeStr, info.requestedCount, info.retrievedCount,
            info.totalAvailable);
    ...
    // If game requested 1 queue from a family with 2 available, we can use
    // index 1 ONLY if retrievedCount < requestedCount (meaning game requested
    // but didn't fetch yet) OR if requestedCount < totalAvailable AND we can
    // actually get the extra queue.
    //
    // Key insight: vkGetDeviceQueue for an index >= requestedCount will fail
    // silently (return VK_NULL_HANDLE). So we can only use queues the game
    // actually requested.
    //
    // NEW STRATEGY: Check if game requested queues but didn't retrieve all of
    // them.
  ...
  if (bestFam != UINT32_MAX) {
    ...
    if (!pGetDeviceQueue && g_Device != VK_NULL_HANDLE) {
      if (o_vkGetDeviceProcAddr)
        pGetDeviceQueue = (PFN_vkGetDeviceQueue)o_vkGetDeviceProcAddr(
            g_Device, "vkGetDeviceQueue");
    }
    ...
    if (spareQueue != VK_NULL_HANDLE) {
      ...
      HookLog("Vulkan: Lazy Discovery found spare queue: Family %d, Index %d "
              "(Handle %p)",
              bestFam, queueIndex, spareQueue);
    } else {
      HookLog("Vulkan: vkGetDeviceQueue returned NULL for Family %d Index %d",
              bestFam, queueIndex);
    }
  } else {
    HookLog("Vulkan: No spare queues found - all requested queues are in use "
            "by game.");
  }
}
...
VkResult VKAPI_CALL Detour_vkCreateSwapchainKHR(
  ...
  // VSync Override
  {
    ...
    VkPresentModeKHR presentMode =
        pFinalParams->presentMode; // Start with original
    ...
    if (mode != "default") {
      if (mode == "off")
        ...
        presentMode =
            VK_PRESENT_MODE_FIFO_RELAXED_KHR; // Or MAILBOX_KHR if strict
                                              // adaptive is not avail
    ...
    if (presentMode != pFinalParams->presentMode) {
      ...
      HookLog("Vulkan: CreateSwapchainKHR: Overriding VSync to %s (mode %d)",
              mode.c_str(), presentMode);
    }
    ...
    if (count >= 2 && count != pFinalParams->minImageCount) {
      ...
      HookLog("Vulkan: CreateSwapchainKHR: Overriding minImageCount to %d",
              count);
    }
  }

  if (!o_vkCreateSwapchainKHR && o_vkGetDeviceProcAddr) {
    o_vkCreateSwapchainKHR = (PFN_vkCreateSwapchainKHR)o_vkGetDeviceProcAddr(
  ...
  if (res == VK_SUCCESS) {
    ...
    EarlyLog("Vulkan: Swapchain created (%ux%u, format=%d)", sc.width,
             sc.height, sc.format);
    ...
    if (!g_RenderPass) {
      ...
      // and leaves in COLOR_ATTACHMENT_OPTIMAL (our post-render barrier handles
      // -> PRESENT_SRC)
...
static void InitOverlayVulkan(VkQueue queue) {
  ...
  if (!g_OverlayAdapter.InitVulkan(g_Device, g_PhysDevice, queue,
                                   g_QueueFamily)) {
...
// NOTE: DrawOverlayToSwapchain helper function removed - overlay is drawn
// inline in vkQueuePresentKHR The vkQueueSubmit approach doesn't work because
// we don't know the image index until present time.

VkResult VKAPI_CALL Detour_vkQueueSubmit(VkQueue queue, uint32_t submitCount,
                                         const VkSubmitInfo *pSubmits,
                                         VkFence fence) {
  static int submitTotal = 0;
  if (++submitTotal % 100 == 0) {
    HookLog("Vulkan: Detour_vkQueueSubmit (Total %d, queue=%p)", submitTotal,
            queue);
  }
  ...
  if (!o_vkQueueSubmit && o_vkGetDeviceProcAddr) {
    // We don't have a device handle here easily, but we can try to use a global
    // one if we have it
    if (g_Device != VK_NULL_HANDLE) {
      o_vkQueueSubmit =
          (PFN_vkQueueSubmit)o_vkGetDeviceProcAddr(g_Device, "vkQueueSubmit");
    }
  }
  ...
  if (!countThisSubmit) {
    int knownGfxQueues =
        g_TrackedGraphicsQueueCount.load(std::memory_order_relaxed);
    if (knownGfxQueues == 0 &&
        (g_Queue == VK_NULL_HANDLE || queue == g_Queue)) {
      countThisSubmit = true;
    }
  }
  if (countThisSubmit) {
    g_QueueSubmitsThisFrame.fetch_add((int)submitCount,
...
VkResult VKAPI_CALL Detour_vkQueueSubmit2(VkQueue queue, uint32_t submitCount,
                                          const VkSubmitInfo2 *pSubmits,
                                          VkFence fence) {
  static int submit2Total = 0;
  if (++submit2Total % 100 == 0) {
    HookLog("Vulkan: Detour_vkQueueSubmit2 (Total %d, queue=%p)", submit2Total,
            queue);
  }
  ...
  if (!o_vkQueueSubmit2 && o_vkGetDeviceProcAddr &&
      g_Device != VK_NULL_HANDLE) {
    o_vkQueueSubmit2 =
        (PFN_vkQueueSubmit2)o_vkGetDeviceProcAddr(g_Device, "vkQueueSubmit2");
  }
...
VkResult VKAPI_CALL Detour_vkQueueSubmit2KHR(VkQueue queue,
                                             uint32_t submitCount,
                                             const VkSubmitInfo2 *pSubmits,
                                             VkFence fence) {
  static int submit2KHRTotal = 0;
  if (++submit2KHRTotal % 100 == 0) {
    HookLog("Vulkan: Detour_vkQueueSubmit2KHR (Total %d, queue=%p)",
            submit2KHRTotal, queue);
  }
  ...
  if (!o_vkQueueSubmit2KHR && o_vkGetDeviceProcAddr &&
      g_Device != VK_NULL_HANDLE) {
    o_vkQueueSubmit2KHR = (PFN_vkQueueSubmit2)o_vkGetDeviceProcAddr(
        g_Device, "vkQueueSubmit2KHR");
  }
...
Detour_vkQueuePresentKHR(VkQueue queue, const VkPresentInfoKHR *pPresentInfo) {
  ...
  if (++presentCount % 60 == 0) { // Log every 60 frames to avoid spamming
    HookLog("Vulkan: Detour_vkQueuePresentKHR (Frame %d, queue=%p)",
            presentCount, queue);
  }
  ...
  if (!o_vkQueuePresentKHR && o_vkGetDeviceProcAddr &&
      g_Device != VK_NULL_HANDLE) {
    o_vkQueuePresentKHR = (PFN_vkQueuePresentKHR)o_vkGetDeviceProcAddr(
        g_Device, "vkQueuePresentKHR");
  }
  ...
  if (!g_AsyncPresentDetected) {
    if (IsComputeOnlyQueue(queue)) {
      ...
      HookLog("Vulkan: Present queue %p is COMPUTE-ONLY (Family %d)", queue,
              g_PresentQueueFamily);

      if (g_GraphicsQueueForOverlay) {
        HookLog(
            "Vulkan: Found graphics queue %p (Family %d) for overlay rendering",
            g_GraphicsQueueForOverlay, g_OverlayQueueFamily);

        if (g_GraphicsQueueForOverlay != VK_NULL_HANDLE &&
            g_OverlayQueueFamily != UINT32_MAX) {
        ...
        if (g_PresentQueueFamily != g_OverlayQueueFamily) {
          HookLog("Vulkan: WARNING - Present and overlay queues are in "
                  "different families");
          HookLog("Vulkan: Queue family ownership transfers may be needed for "
                  "correctness");
        }
      } else {
        HookLog("Vulkan: WARNING - No graphics queue found! Overlay will be "
                "disabled.");
      }
    }
  }
  ...
  if (pPresentInfo && pPresentInfo->swapchainCount > 0) {
    ...
    if (!found && !lateInjectionSwapchainWarned) {
      ...
      HookLog("Vulkan: Late injection detected - swapchain %p not registered",
              swapchain);
      HookLog("Vulkan: Overlay unavailable (game's swapchain created before "
              "injection)");
      HookLog("Vulkan: Resize or restart game to enable overlay, OR use DX12 "
              "mode if available");
      // Note: We cannot render to a swapchain created by a different Vulkan
      // device. Our recovered device is separate from the game's device, so we
      // can't access the game's swapchain images or create compatible render
      // passes.
    }
  }
  ...
  {
    // Enable CSV logging for frame times (one-time init, only if debug logging
    // enabled)
    static bool csvLoggingInitialized = false;
    SharedMemoryLayout *csvShm = (g_IPC) ? g_IPC->GetSharedMem() : nullptr;
    TryEnableFrameTimeCSVLogging(
        csvShm, (const void *)&Detour_vkQueuePresentKHR, g_PerfMetrics,
        "Vulkan", csvLoggingInitialized);
  ...
  if (g_Device != VK_NULL_HANDLE) {
    ...
    if (limit >= 0.0f) {
      ...
      if (fabs(lastLimit - limit) > 0.001f) {
        if (limit < 1.0f) {
          if (limit == 0.0f)
            HookLog("Vulkan: Prerender Limit active: 0.0 (Strict Serial)");
          else
            HookLog("Vulkan: Prerender Limit active: %.2f (Hybrid Serial, "
                    "Timeout Scaled)",
                    limit);
        } else {
          HookLog("Vulkan: Prerender Limit active: %.2f frames (Buffered)",
                  limit);
        }
        lastLimit = limit;
      }
      ...
      if (g_PrerenderDevice != g_Device) {
        for (auto f : g_PrerenderFences)
          vkDestroyFence(g_PrerenderDevice, f, nullptr);
      ...
      // Case > 0: Buffered (Ring Buffer) + Optional Pacing
        ...
        // Ensure Ring Buffer supports at least "Limit 1" (Lookback 2) for any
        // non-zero limit. This guarantees queueing (Queue Size 1), allowing GPU
        // overlap. limit=0.5 -> effective 1 -> Lookback 2. limit=1 -> effective
        // 1 -> Lookback 2. limit=2 -> effective 2 -> Lookback 3. Lookback =
        // effective + 1.
        ...
        if (g_PrerenderFences.size() != needed) {
          for (auto f : g_PrerenderFences)
            vkDestroyFence(g_Device, f, nullptr);
          g_PrerenderFences.clear();
        ...
        VkFence oldFence =
            g_PrerenderFences[(g_PrerenderFrameIndex + size - lookback) % size];
        ...
        if (isFractional) {
          ...
          int64_t idleGapUs =
              (int64_t)(targetFrameTimeUs * (1.0 - limit) * 0.10);
          if (idleGapUs > 0) {
            if (idleGapUs > 10000)
  ...
  if (!logDeviceOnce) {
    HookLog("Vulkan: Device state - g_Device=%p, g_Queue=%p, g_PhysDevice=%p, "
            "recoveryAttempted=%d",
  ...
  if (!recoveryAttempted &&
      (g_Device == VK_NULL_HANDLE || g_PhysDevice == VK_NULL_HANDLE)) {
  ...
  if (g_Device != VK_NULL_HANDLE && !g_OverlayInitialized &&
      g_Queue != VK_NULL_HANDLE) {
    InitOverlayVulkan(g_Queue);
  }

  // Vulkan: Always draw overlay normally (capture_include_overlay=false not
  // supported due to perf impact)
  if (g_OverlayInitialized && g_Device != VK_NULL_HANDLE) {
      ...
      for (auto &sc : g_Swapchains) {
        if (sc.swapchain == pPresentInfo->pSwapchains[0]) {
          ...
          if (idx >= sc.images.size()) {
            HookLog("Vulkan: ERROR - image index %u out of range (size %zu)",
                    idx, sc.images.size());
            break;
          }
          ...
          if (!allowOverlayFrame) {
            ...
            if (dbgPresentNum - dbgLastLogFrame > 60) {
              HookLog("Vulkan: DEBUG - Overlay SKIPPED (FG/notReal) present#%d "
                      "isReal=%d fgSuspended=%d",
          ...
          // The pipeline barrier in the command buffer provides GPU-side
          // synchronization. Flickering with no wait semaphores is a limitation
          // of how Strange Brigade handles VSync - when in-game VSync is off,
          // the game doesn't provide semaphores.
          ...
          if (g_CommandPool == VK_NULL_HANDLE) {
            ...
            poolInfo.queueFamilyIndex = g_QueueFamily;
            poolInfo.flags = VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT;
            VkResult poolResult = vkCreateCommandPool(g_Device, &poolInfo,
                                                      nullptr, &g_CommandPool);
            if (poolResult != VK_SUCCESS || g_CommandPool == VK_NULL_HANDLE) {
              HookLog("Vulkan: ERROR - Failed to create command pool: %d",
                      poolResult);
              break;
            }
          }
          ...
          g_OverlayAdapter.SetDroppedFrames(
              g_VulkanCapture.droppedFrames.load(std::memory_order_relaxed));
          ...
          if (!fencesCreated) {
            ...
            for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
              vkCreateSemaphore(g_Device, &semInfo, nullptr,
                                &inFlightSemaphores[i]);
            }
            fencesCreated = true;
          }

          // Wait for this frame's previous submission
          vkWaitForFences(g_Device, 1, &inFlightFences[currentFrame], VK_TRUE,
                          UINT64_MAX);
          ...
          // This ensures the game's rendering is complete before we draw
          // overlay. ReShade uses semaphores, but when game provides none, we
          // need this barrier. This is stronger than just the image barrier
          // below.
          ...
          vkCmdPipelineBarrier(cb, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
                               ...
                               &globalBarrier, 0, nullptr, 0, nullptr);
          ...
          VkViewport viewport = {0.0f, 0.0f, (float)sc.width, (float)sc.height,
                                 0.0f, 1.0f};
          ...
          toPresent.srcAccessMask = VK_ACCESS_COLOR_ATTACHMENT_READ_BIT |
                                    VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT;
          ...
          // NOTE: When game provides NO semaphores (waitSemaphoreCount == 0),
          // overlay may flicker in some games like Strange Brigade with in-game
          // VSync OFF. This is a fundamental limitation - without game-provided
          // semaphores, we cannot synchronize without blocking CPU (which kills
          // FPS).
          //
          // SOLUTION: Enable in-game VSync for stable overlay in affected
          // games. When in-game VSync is ON, games typically provide proper
          // semaphores.
          ...
          if (dbgPresentNum - dbgLastLogFrame > 120) {
            HookLog("Vulkan: Submitting overlay (present#%d, drawn=%d, "
                    "skipped=%d, imgIdx=%u, waitSems=%u)",
  ...
  // Video capture BEFORE Present - must copy while we still own the swapchain
  // image Only capture REAL frames
  if (g_IPC && g_IPC->IsRecording() && pPresentInfo->swapchainCount > 0 &&
      isRealFrame) {
    VkSwapchainKHR swapchain = pPresentInfo->pSwapchains[0];
    ...
    for (auto &sc : g_Swapchains) {
      if (sc.swapchain == swapchain && imageIndex < sc.images.size()) {
        if (!g_VulkanCapture.initialized) {
          // Late injection recovery: if we don't have device/physdevice, try to
          // recover
          ...
          if (g_PhysDevice == VK_NULL_HANDLE) {
            if (!TryRecoverPhysicalDevice()) {
              HookLog(
                  "Vulkan: Late injection recovery failed, capture disabled");
              break;
            }
          }
          ...
          // Initialize capture resources (uses optimized D3D11 path with async
          // queue)
          g_VulkanCapture.CreateSharedResources(sc.width, sc.height,
                                                (uint32_t)sc.format);
        ...
        if (g_AsyncQueue != VK_NULL_HANDLE && g_VulkanCapture.initialized) {
          ...
          // --- LEGACY PATH: Copy on game queue (for separateDeviceActive or
          // fallback) ---
          ...
          VkSemaphore completionSem =
              g_VulkanCapture.CopyFrame(queue, sc.images[imageIndex], sc.width,
                                        sc.height, waitSemaphores);
        ...
        // NOTE: This path runs on the game's present queue which may cause
        // stuttering. Async capture (queue injection) is disabled due to
        // stability issues.
        static bool syncPathLogged = false;
        if (!syncPathLogged) {
          HookLog("Vulkan: Using synchronous capture path (may cause stutter "
                  "in some games)");
          syncPathLogged = true;
        }
        ...
        if (completionSem != VK_NULL_HANDLE) {
          int usedTextureIdx =
              (g_VulkanCapture.writeIndex + CAPTURE_TEXTURE_COUNT - 1) %
              CAPTURE_TEXTURE_COUNT;
          // PASS RAW QPC: MediaEngine converts to MS using trusted frequency
          g_VulkanCapture.SignalFrameReady(g_IPC, usedTextureIdx, qpc.QuadPart,
                                           g_VulkanCapture.fenceValue);
...
PFN_vkVoidFunction VKAPI_CALL Detour_vkGetInstanceProcAddr(VkInstance instance,
                                                           const char *pName) {
  HookLog("Vulkan: vkGetInstanceProcAddr(%p, %s)", instance,
          pName ? pName : "NULL");
...
PFN_vkVoidFunction VKAPI_CALL Detour_vkGetDeviceProcAddr(VkDevice device,
                                                         const char *pName) {
  HookLog("Vulkan: vkGetDeviceProcAddr(%p, %s)", device,
          pName ? pName : "NULL");
...
VK_DetourGetInstanceProcAddr(VkInstance instance, const char *pName) {
  return Detour_vkGetInstanceProcAddr(instance, pName);
}

extern "C" PFN_vkVoidFunction VKAPI_CALL
VK_DetourGetDeviceProcAddr(VkDevice device, const char *pName) {
...
void VulkanHook::Init() {
  ...
  // Some exports are forwarded/stubs and patching them can crash (observed for
  // vkAcquireNextImageKHR). We rely on
  // vkGetInstanceProcAddr/vkGetDeviceProcAddr to detour most functions.
  ...
  // Fallback: some games call vulkan-1.dll exports directly (no ProcAddr
  // lookup). These exports appear safe to hook and are required for
  // overlay/capture to trigger.
  ...
  // Critical for AF forcing - game may call these directly without ProcAddr
  // lookup
  CreateHook("vkCreateSampler", (void *)&Detour_vkCreateSampler,
             (void **)&o_vkCreateSampler);
  CreateHook("vkCreateDescriptorUpdateTemplate",
             (void *)&Detour_vkCreateDescriptorUpdateTemplate,
             (void **)&o_vkCreateDescriptorUpdateTemplate);
  CreateHook("vkCreateDescriptorUpdateTemplateKHR",
             (void *)&Detour_vkCreateDescriptorUpdateTemplate,
             (void **)&o_vkCreateDescriptorUpdateTemplateKHR);
  CreateHook("vkDestroyDescriptorUpdateTemplate",
             (void *)&Detour_vkDestroyDescriptorUpdateTemplate,
             (void **)&o_vkDestroyDescriptorUpdateTemplate);
  CreateHook("vkDestroyDescriptorUpdateTemplateKHR",
             (void *)&Detour_vkDestroyDescriptorUpdateTemplate,
             (void **)&o_vkDestroyDescriptorUpdateTemplateKHR);
  CreateHook("vkUpdateDescriptorSetWithTemplate",
             (void *)&Detour_vkUpdateDescriptorSetWithTemplate,
             (void **)&o_vkUpdateDescriptorSetWithTemplate);
  CreateHook("vkUpdateDescriptorSetWithTemplateKHR",
             (void *)&Detour_vkUpdateDescriptorSetWithTemplate,
             (void **)&o_vkUpdateDescriptorSetWithTemplateKHR);
}
