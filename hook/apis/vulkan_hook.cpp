...
static bool IsComputeOnlyQueue(VkQueue queue)
{
    ...
    for (const auto& qi : g_QueueInstances) {
        if (qi.queue == queue) {
            return (qi.flags & VK_QUEUE_COMPUTE_BIT) && !(qi.flags & VK_QUEUE_GRAPHICS_BIT);
        }
    }
    return false;
}
...
static int ApplyWhitelistedPattern(uint8_t* start, size_t size, const uint8_t* pattern, size_t patternLen,
                                   const uint8_t* patch, size_t patchLen, const char* name)
{
    int hits = 0;
    uint8_t* p = start;
    uint8_t* end = start + size - patternLen;
...
static int ApplyShotgunWhitelists(uint8_t* start, size_t size)
{
    ...
    for (uint8_t reg : targetRegs) {
        ...
        uint8_t pQ4[] = {0xC7, mod, 0x04, 0x00, 0x00, 0x00, 0x00, 0xC7, mod, 0x08, 0x00, 0x00, 0x00, 0x00,
                         0xC7, mod, 0x0C, 0x00, 0x00, 0x00, 0x00, 0xC7, mod, 0x10, 0x00, 0x00, 0x00, 0x00};
        uint8_t hQ4[] = {0xC7, mod, 0x04, 0x00, 0x00, 0x00, 0x00, 0xC7, mod, 0x08, 0x10, 0x00, 0x00, 0x00,
                         0xC7, mod, 0x0C, 0x10, 0x00, 0x00, 0x00, 0xC7, mod, 0x10, 0x10, 0x00, 0x00, 0x00};
        totalHits += ApplyWhitelistedPattern(start, size, pQ4, 28, hQ4, 28, regName);

        // Triple Zero (08, 0C, 10) - 21 bytes
        uint8_t pZ3[] = {0xC7, mod,  0x08, 0x00, 0x00, 0x00, 0x00, 0xC7, mod,  0x0C, 0x00,
                         0x00, 0x00, 0x00, 0xC7, mod,  0x10, 0x00, 0x00, 0x00, 0x00};
        uint8_t hZ3[] = {0xC7, mod,  0x08, 0x10, 0x00, 0x00, 0x00, 0xC7, mod,  0x0C, 0x10,
                         0x00, 0x00, 0x00, 0xC7, mod,  0x10, 0x10, 0x00, 0x00, 0x00};
    ...
    uint8_t pEDI_S1[] = {0x0F, 0xAC, 0xC1, 0x02, 0x8B, 0x45, 0x08, 0xC7, 0x47, 0x08, 0x00,
                         0x00, 0x00, 0x00, 0xC7, 0x47, 0x0C, 0x00, 0x00, 0x00, 0x00};
    uint8_t hEDI_S1[] = {0x0F, 0xAC, 0xC1, 0x02, 0x8B, 0x45, 0x08, 0xC7, 0x47, 0x08, 0x10,
                         0x00, 0x00, 0x00, 0xC7, 0x47, 0x0C, 0x10, 0x00, 0x00, 0x00};
...
static int ApplyAllWhitelistedPatches(uint8_t* start, size_t size) { return ApplyShotgunWhitelists(start, size); }
...
static bool CreateCaptureDevice()
{
    ...
    const char* extensions[] = {VK_KHR_EXTERNAL_MEMORY_EXTENSION_NAME, VK_KHR_EXTERNAL_MEMORY_WIN32_EXTENSION_NAME,
...
static bool CreateCrossDeviceImages(uint32_t width, uint32_t height, VkFormat format)
{
    ...
    for (int i = 0; i < CAPTURE_TEXTURE_COUNT; i++) {
        ...
        allocInfo.memoryTypeIndex =
            FindMemoryType(g_PhysDevice, memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
...
static bool CreateCrossDeviceSemaphore()
{
    ...
    auto importSem =
        (PFN_vkImportSemaphoreWin32HandleKHR)vkGetDeviceProcAddr(g_CaptureDevice, "vkImportSemaphoreWin32HandleKHR");

    if (!getSemHandle || !importSem) {
        HookLog("Vulkan: Error - Missing cross-device semaphore extensions (Get=%p, Import=%p)", getSemHandle,
                importSem);
        return false;
    }
    ...
    for (int i = 0; i < CAPTURE_TEXTURE_COUNT; i++) {
        ...
        if (vkCreateSemaphore(g_CaptureDevice, &importedSemCI, nullptr, &g_CrossDeviceSems_CaptureOwned[i]) !=
            VK_SUCCESS) {
...
public:
    ...
    void CleanupVulkan(VkDevice device)
    {
        ...
        for (int i = 0; i < CAPTURE_TEXTURE_COUNT; i++) {
            if (sharedImages[i]) vkDestroyImage(device, sharedImages[i], nullptr);
            if (sharedMem[i]) vkFreeMemory(device, sharedMem[i], nullptr);
            if (copyCompleteSemaphores[i]) vkDestroySemaphore(device, copyCompleteSemaphores[i], nullptr);
            if (presentTriggerSems[i]) vkDestroySemaphore(device, presentTriggerSems[i], nullptr);
            if (copyTriggerSems[i]) vkDestroySemaphore(device, copyTriggerSems[i], nullptr);
        ...
        if (captureCommandPool) {
            if (captureCommandBuffers[0]) {
                vkFreeCommandBuffers(device, captureCommandPool, CAPTURE_TEXTURE_COUNT, captureCommandBuffers);
                for (auto& cb : captureCommandBuffers) cb = VK_NULL_HANDLE;
            }
    ...
    {
        HookLog("Vulkan: CreateD3D11SharedResources %dx%d format=%d", w, h, dxgiFormat);
        ...
        for (UINT i = 0; factory->EnumAdapters(i, &adapter) != DXGI_ERROR_NOT_FOUND; i++) {
            ...
            if (desc.AdapterLuid.LowPart == luidLow && desc.AdapterLuid.HighPart == (LONG)luidHigh) {
        ...
        if (!targetAdapter) {
            HookLog("Vulkan: Could not find matching DXGI adapter for LUID %08x", luidLow);
            return false;
        }
        ...
        hr = pD3D11CreateDevice(targetAdapter, D3D_DRIVER_TYPE_UNKNOWN, nullptr, D3D11_CREATE_DEVICE_BGRA_SUPPORT,
                                nullptr, 0, D3D11_SDK_VERSION, &d3d11Device, &featureLevel, &d3d11Context);
        targetAdapter->Release();
        ...
        texDesc.MiscFlags = D3D11_RESOURCE_MISC_SHARED;  // KMT sharing - works cross-process!
        ...
        for (int i = 0; i < CAPTURE_TEXTURE_COUNT; i++) {
            ...
            HookLog("Vulkan: Created D3D11 texture %d, KMT handle=%p", i, d3d11TextureHandles[i]);
        ...
        for (int i = 0; i < CAPTURE_TEXTURE_COUNT; i++) {
            ...
            extImageInfo.handleTypes = VK_EXTERNAL_MEMORY_HANDLE_TYPE_D3D11_TEXTURE_KMT_BIT;
            ...
            VkResult res = vkCreateImage(g_Device, &imageInfo, nullptr, &sharedImages[i]);
            if (res != VK_SUCCESS) {
                HookLog("Vulkan: Failed to create VkImage for D3D11 import %d: %d", i, res);
                CleanupD3D11();
                return false;
            }
        ...
        VkResult res = vkCreateCommandPool(g_Device, &poolInfo, nullptr, &captureCommandPool);
        if (res != VK_SUCCESS) {
        ...
        res = vkAllocateCommandBuffers(g_Device, &cbAllocInfo, captureCommandBuffers);
        if (res != VK_SUCCESS) {
        ...
        auto GetSemaphoreWin32Handle =
            (PFN_vkGetSemaphoreWin32HandleKHR)vkGetDeviceProcAddr(g_Device, "vkGetSemaphoreWin32HandleKHR");
        if (GetSemaphoreWin32Handle) {
            VkSemaphoreGetWin32HandleInfoKHR getSemHandleInfo = {VK_STRUCTURE_TYPE_SEMAPHORE_GET_WIN32_HANDLE_INFO_KHR};
        ...
        VkSemaphoreCreateInfo binarySemInfo = {VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO};
        for (int i = 0; i < CAPTURE_TEXTURE_COUNT; i++) {
            res = vkCreateSemaphore(g_Device, &binarySemInfo, nullptr, &copyCompleteSemaphores[i]);
        ...
        usingD3D11Path = true;
        initialized = true;
        HookLog("Vulkan: D3D11 intermediate path initialized successfully (%dx%d)", w, h);
        return true;
    }
    ...
    {
        if (!ipcMem || !ipcMem->encoderTextures.ready.load(std::memory_order_acquire)) {
            return false;
        }

        auto& enc = ipcMem->encoderTextures;
        HookLog("Vulkan: ImportEncoderTextures %dx%d format=%d", enc.width, enc.height, enc.format);

        // Get memory properties function
        auto vkGetMemoryWin32HandlePropertiesKHR = (PFN_vkGetMemoryWin32HandlePropertiesKHR)vkGetDeviceProcAddr(
        ...
        for (int i = 0; i < CAPTURE_TEXTURE_COUNT; i++) {
            ...
            VkResult res = vkGetMemoryWin32HandlePropertiesKHR(
                g_Device, VK_EXTERNAL_MEMORY_HANDLE_TYPE_OPAQUE_WIN32_BIT, texHandle, &handleProps);
            if (res != VK_SUCCESS) {
                HookLog("Vulkan: ERROR - vkGetMemoryWin32HandlePropertiesKHR failed: %d", res);
                return false;
            }
            ...
            for (uint32_t j = 0; j < memProps.memoryTypeCount; j++) {
                if ((handleProps.memoryTypeBits & (1 << j)) && (memReq.memoryTypeBits & (1 << j))) {
                    memTypeIndex = j;
                    break;
                }
            }
        ...
        res = vkAllocateCommandBuffers(g_Device, &allocInfoCB, captureCommandBuffers);
        if (res != VK_SUCCESS) {
            HookLog("Vulkan: Failed to allocate command buffers: %d", res);
            return false;
        }
        ...
        initialized = true;
        HookLog("Vulkan: Successfully imported encoder textures (%dx%d)", width, height);
        return true;
    }

    void CreateSharedResources(uint32_t w, uint32_t h, uint32_t fmt) override
    {
        if (initialized && width == w && height == h && format == fmt) return;
        ...
        HookLog("Vulkan: CreateSharedResources %dx%d, vkFormat=%d -> dxgiFormat=%d", width, height, vkFormat, format);
        ...
        auto vkGetPhysicalDeviceProperties2 =
            (PFN_vkGetPhysicalDeviceProperties2)vkGetInstanceProcAddr(g_Instance, "vkGetPhysicalDeviceProperties2");
        if (!vkGetPhysicalDeviceProperties2)
            vkGetPhysicalDeviceProperties2 = (PFN_vkGetPhysicalDeviceProperties2)vkGetInstanceProcAddr(
        ...
        auto GetWin32Handle = (PFN_vkGetMemoryWin32HandleKHR)vkGetDeviceProcAddr(g_Device, "vkGetMemoryWin32HandleKHR");
        if (!GetWin32Handle) {
        ...
        for (int i = 0; i < CAPTURE_TEXTURE_COUNT; i++) {
            ...
            imageInfo.queueFamilyIndexCount =
                (g_AsyncQueueFamily != UINT32_MAX && g_AsyncQueueFamily != g_QueueFamily) ? 2 : 1;
        ...
        if (!separateDeviceActive) {
            ...
            for (int i = 0; i < CAPTURE_TEXTURE_COUNT; i++) {
                ...
                VkResult imageRes = vkCreateImage(g_Device, &imageInfo, nullptr, &sharedImages[i]);
                ...
                VkResult allocRes = vkAllocateMemory(g_Device, &allocInfo, nullptr, &sharedMem[i]);
                ...
                HookLog("Vulkan: Created shared image %d, handle: %p", i, sharedTextureHandles[i]);
            }
        }
        ...
        VkResult poolRes = vkCreateCommandPool(g_Device, &poolInfo, nullptr, &captureCommandPool);
        ...
        VkResult cmdRes = vkAllocateCommandBuffers(g_Device, &allocCmdInfo, captureCommandBuffers);
        ...
        VkSemaphoreCreateInfo semInfo = {VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO};
        for (int i = 0; i < CAPTURE_TEXTURE_COUNT; i++) {
            vkCreateSemaphore(g_Device, &semInfo, nullptr, &copyCompleteSemaphores[i]);
        ...
        // --- Timeline Semaphore (Encoder Sync) ---
        {
            ...
            if (semRes != VK_SUCCESS || timelineSemaphore == VK_NULL_HANDLE) {
                ...
                if (separateDeviceActive) {
                    separateDeviceActive =
                        false;  // Fallback? Might be too late if other resources are already cross-device
                }
                return;
            }
            ...
            auto getSemHandle =
                (PFN_vkGetSemaphoreWin32HandleKHR)vkGetDeviceProcAddr(targetDevice, "vkGetSemaphoreWin32HandleKHR");
            if (getSemHandle) {
    ...
    // signal completion
    VkSemaphore CopyFrame(VkQueue queue, VkImage srcImage, uint32_t srcWidth, uint32_t srcHeight,
                          const std::vector<VkSemaphore>& waitSemaphores)
    {
        if (!initialized || !captureCommandBuffers[0]) return VK_NULL_HANDLE;
        ...
        if (!dstImage || !cmd || !signalSem) return VK_NULL_HANDLE;
        ...
        if (separateDeviceActive) {
...

            {
                ...
                if (gameSubmitRes != VK_SUCCESS) {
                    HookLog("Vulkan: ERROR - Game queue submit failed: %d (Queue=%p, Families=[%d,%d])", gameSubmitRes,
                            queue, g_QueueFamily, g_AsyncQueueFamily);
                }
            }
        } else {
...
static void AsyncCaptureThreadProc()
{
    ...
    while (!g_VulkanCapture.captureThreadShutdown) {
        ...
        while (true) {
            ...
            HookLog("Vulkan: Async - Processing Frame (RIdx=%d, WriteIdx=%d, Separate=%d)", rIdx, writeIdx,
                    separateDeviceActive);
            ...
            if (separateDeviceActive) {
                ...
                if (transCmd && semGraphicsReady && semGameImported && srcStage && dstExport && g_AsyncQueue) {
                    ...
                    barrier.oldLayout =
                        VK_IMAGE_LAYOUT_UNDEFINED;  // We don't care about previous layout if we just acquired it
                    ...
                    vkCmdPipelineBarrier(transCmd, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0,
                    ...
                    vkCmdCopyImage(transCmd, srcStage, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, dstExport,
                                   VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);
                    ...
                    vkCmdPipelineBarrier(transCmd, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT,
                    ...
                    HookLog("Vulkan: Async - ERROR: Missing resources for Stage 2 (Cmd=%p, SemG=%p, SemI=%p, Src=%p, "
                            "Dst=%p, Q=%p)",
...
{
    if (g_PhysDevice != VK_NULL_HANDLE && g_Device != VK_NULL_HANDLE) return true;  // Already have everything
    ...
    const char* instExtensions[] = {VK_KHR_GET_PHYSICAL_DEVICE_PROPERTIES_2_EXTENSION_NAME,
...
typedef VkResult(VKAPI_PTR* PFN_vkAcquireNextImageKHR)(VkDevice, VkSwapchainKHR, uint64_t, VkSemaphore, VkFence,
                                                       uint32_t*);
...
typedef VkResult(VKAPI_PTR* PFN_vkQueueSubmit)(VkQueue queue, uint32_t submitCount, const VkSubmitInfo* pSubmits,
                                               VkFence fence);
typedef VkResult(VKAPI_PTR* PFN_vkQueueSubmit2)(VkQueue queue, uint32_t submitCount, const VkSubmitInfo2* pSubmits,
                                                VkFence fence);
...
VkResult VKAPI_CALL Detour_vkCreateInstance(const VkInstanceCreateInfo* pCreateInfo,
...
VkResult VKAPI_CALL Detour_vkCreateImage(VkDevice device, const VkImageCreateInfo* pCreateInfo,
...
VkResult VKAPI_CALL Detour_vkCreateRenderPass(VkDevice device, const VkRenderPassCreateInfo* pCreateInfo,
...
VkResult VKAPI_CALL Detour_vkCreateRenderPass2(VkDevice device, const VkRenderPassCreateInfo2* pCreateInfo,
                                               const VkAllocationCallbacks* pAllocator, VkRenderPass* pRenderPass)
{
    if (pCreateInfo && g_IPC && g_IPC->GetSharedMem()) {
        const char* msaa = g_IPC->GetSharedMem()->graphicsConfig.msaaSamples;
...
static VkResult VKAPI_PTR Detour_vkCreateGraphicsPipelines(VkDevice device, VkPipelineCache pipelineCache,
                                                           ...
                                                           VkPipeline* pPipelines)
{
    if (g_IPC && g_IPC->GetSharedMem() && g_IPC->GetSharedMem()->graphicsConfig.sgssaa) {
        ...
        if (modified) {
            return o_vkCreateGraphicsPipelines(device, pipelineCache, createInfoCount, modifiedInfos.data(), pAllocator,
                                               pPipelines);
        }
    }
...
static VkResult VKAPI_PTR Detour_vkCreateSampler(VkDevice device, const VkSamplerCreateInfo* pCreateInfo,
                                                 const VkAllocationCallbacks* pAllocator, VkSampler* pSampler)
{
    ...
    if (g_IPC && g_IPC->GetSharedMem()) {
        ...
        const bool anyOverrides = (gfx.anisotropicFiltering != "default" && !gfx.anisotropicFiltering.empty()) ||
        ...
        if (s_samplerOverrideLogCount < 10) {
            ...
            HookLog("Vulkan: CreateSampler override (af=%s maxLod=%.2f allowMip=%d unnorm=%d) "
                    "Aniso: %d/%.1f -> %d/%.1f  Filter: %d/%d/%d  Bias: %.3f -> %.3f",
                    gfx.anisotropicFiltering.c_str(), pCreateInfo->maxLod, allowMipOverrides ? 1 : 0,
                    (int)pCreateInfo->unnormalizedCoordinates, (int)pCreateInfo->anisotropyEnable,
                    (double)pCreateInfo->maxAnisotropy, (int)modifiedInfo.anisotropyEnable,
                    (double)modifiedInfo.maxAnisotropy, (int)modifiedInfo.minFilter, (int)modifiedInfo.magFilter,
                    (int)modifiedInfo.mipmapMode, (double)pCreateInfo->mipLodBias, (double)modifiedInfo.mipLodBias);
        }
    }
...
static void VKAPI_PTR Detour_vkUpdateDescriptorSets(VkDevice device, uint32_t descriptorWriteCount,
                                                    const VkWriteDescriptorSet* pDescriptorWrites,
                                                    uint32_t descriptorCopyCount,
                                                    const VkCopyDescriptorSet* pDescriptorCopies)
{
    ...
    if (!g_IPC || !g_IPC->GetSharedMem() || !pDescriptorWrites || descriptorWriteCount == 0) {
        o_vkUpdateDescriptorSets(device, descriptorWriteCount, pDescriptorWrites, descriptorCopyCount,
                                 pDescriptorCopies);
        return;
    }

    const auto& gfx = GetActiveGraphicsConfig();
    const bool afActive = (gfx.anisotropicFiltering != "default" && !gfx.anisotropicFiltering.empty() &&
...
static void VKAPI_PTR Detour_vkDestroyDescriptorUpdateTemplate(VkDevice device,
                                                               VkDescriptorUpdateTemplate descriptorUpdateTemplate,
                                                               const VkAllocationCallbacks* pAllocator)
{
...
static void VKAPI_PTR Detour_vkUpdateDescriptorSetWithTemplate(VkDevice device, VkDescriptorSet descriptorSet,
                                                               VkDescriptorUpdateTemplate descriptorUpdateTemplate,
                                                               const void* pData)
{
...
VkResult VKAPI_CALL Detour_vkCreateWin32SurfaceKHR(VkInstance instance, const VkWin32SurfaceCreateInfoKHR* pCreateInfo,
...
VkResult VKAPI_CALL Detour_vkCreateDevice(VkPhysicalDevice physicalDevice, const VkDeviceCreateInfo* pCreateInfo,
                                          const VkAllocationCallbacks* pAllocator, VkDevice* pDevice)
{
    ...
    if (bestInjectionFam != -1) {
        ...
        familyInjected = true;
        HookLog("Vulkan: Injecting extra queue for capture in Family %d (Index %d, TypeScore %d)", bestInjectionFam,
                g_AsyncQueueIndex, bestInjectionScore);
    }
    ...
    if (res == VK_SUCCESS) {
        ...
        if (bestInjectionFam != -1) {
            ...
            PFN_vkGetDeviceQueue pfnGetQueue =
                (PFN_vkGetDeviceQueue)o_vkGetDeviceProcAddr(g_Device, "vkGetDeviceQueue");
            if (pfnGetQueue) {
...
void VKAPI_CALL Detour_vkGetDeviceQueue(VkDevice device, uint32_t queueFamilyIndex, uint32_t queueIndex,
                                        VkQueue* pQueue)
{
    if (g_InVulkanHook) {
    ...
    if (pQueue && *pQueue) {
...

        {
            ...
            if (!alreadyRegistered) {
                ...
                HookLog("Vulkan: Queue %p registered (Family %d, Index %d, Type: %s, Flags: 0x%X)", *pQueue,
                        queueFamilyIndex, queueIndex, typeStr, flags);
            }
        }
        ...
        if (g_Queue == VK_NULL_HANDLE) {
            ...
            HookLog("Vulkan: Using Queue: %p (Family: %d) for overlay", g_Queue, g_QueueFamily);
        }
    }
}

void VKAPI_CALL Detour_vkGetDeviceQueue2(VkDevice device, const VkDeviceQueueInfo2* pQueueInfo, VkQueue* pQueue)
{
    if (g_InVulkanHook) {
    ...
    if (pQueue && *pQueue) {
...

        {
            ...
            if (!alreadyRegistered) {
                ...
                if (flags & VK_QUEUE_GRAPHICS_BIT)
                    ...
                    typeStr = "Transfer";
                HookLog("Vulkan: Queue %p registered (Queue2) (Family %d, Index %d, Type: %s, Flags: 0x%X)", *pQueue,
                        pQueueInfo->queueFamilyIndex, pQueueInfo->queueIndex, typeStr, flags);
            }
        }
        ...
        if (g_Queue == VK_NULL_HANDLE) {
            ...
            HookLog("Vulkan: Using Queue (Queue2): %p (Family: %d) for overlay", g_Queue, g_QueueFamily);
        }
    }
}
...
VkResult VKAPI_CALL Detour_vkGetSwapchainImagesKHR(VkDevice device, VkSwapchainKHR swapchain,
                                                   uint32_t* pSwapchainImageCount, VkImage* pSwapchainImages)
{
    if (!o_vkGetSwapchainImagesKHR && o_vkGetDeviceProcAddr) {
        o_vkGetSwapchainImagesKHR =
            (PFN_vkGetSwapchainImagesKHR)o_vkGetDeviceProcAddr(device, "vkGetSwapchainImagesKHR");
    }
    if (!o_vkGetSwapchainImagesKHR) return VK_ERROR_INITIALIZATION_FAILED;
    return o_vkGetSwapchainImagesKHR(device, swapchain, pSwapchainImageCount, pSwapchainImages);
}

VkResult VKAPI_CALL Detour_vkAcquireNextImageKHR(VkDevice device, VkSwapchainKHR swapchain, uint64_t timeout,
                                                 VkSemaphore semaphore, VkFence fence, uint32_t* pImageIndex)
{
    ...
    VkResult result = o_vkAcquireNextImageKHR(device, swapchain, timeout, semaphore, fence, pImageIndex);
...
PFN_vkVoidFunction VKAPI_CALL Detour_vkGetDeviceProcAddr(VkDevice device, const char* pName);

void VKAPI_CALL Detour_vkDestroySwapchainKHR(VkDevice device, VkSwapchainKHR swapchain,
                                             const VkAllocationCallbacks* pAllocator)
{
    {
        std::lock_guard<std::recursive_mutex> lock(g_VulkanMutex);
        auto it = std::remove_if(g_Swapchains.begin(), g_Swapchains.end(), [&](const VulkanSwapchain& sc) {
            if (sc.swapchain == swapchain) {
...
static void TryDiscoverAsyncQueue()
{
    ...
    if (g_AsyncQueue != VK_NULL_HANDLE || g_QueueFamilies.empty()) return;
    ...
    for (uint32_t i = 0; i < (uint32_t)g_QueueFamilies.size(); i++) {
        ...
        HookLog("  Family %d: %s | Requested: %d, Retrieved: %d, HW Total: %d", i, typeStr, info.requestedCount,
                info.retrievedCount, info.totalAvailable);
    ...
    if (bestFam != UINT32_MAX) {
        ...
        if (spareQueue != VK_NULL_HANDLE) {
            ...
            HookLog("Vulkan: Lazy Discovery found spare queue: Family %d, Index %d (Handle %p)", bestFam, queueIndex,
                    spareQueue);
        } else {
...
VkResult VKAPI_CALL Detour_vkCreateSwapchainKHR(VkDevice device, const VkSwapchainCreateInfoKHR* pCreateInfo,
                                                const VkAllocationCallbacks* pAllocator, VkSwapchainKHR* pSwapchain)
{
    ...
    // VSync Override
    {
        ...
        if (mode != "default") {
            if (mode == "off")
                ...
                presentMode = VK_PRESENT_MODE_FIFO_RELAXED_KHR;  // Or MAILBOX_KHR if strict adaptive is not avail
    ...
    VkResult res = o_vkCreateSwapchainKHR(device, &modifiedCI, pAllocator, pSwapchain);
    if (res == VK_SUCCESS) {
        std::lock_guard<std::recursive_mutex> lock(g_VulkanMutex);
        VulkanSwapchain sc;
        ...
        if (!g_RenderPass) {
            ...
            VkResult rpRes = vkCreateRenderPass(device, &rp_info, nullptr, &g_RenderPass);
            HookLog("Vulkan: RenderPass creation result: %d (device=%p, g_Device=%p)", rpRes, device, g_Device);
        ...
        HookLog("Vulkan: Swapchain Created (%dx%d), framebuffers=%zu", sc.width, sc.height, sc.framebuffers.size());
    }
    return res;
}
...
VkResult VKAPI_CALL Detour_vkQueueSubmit(VkQueue queue, uint32_t submitCount, const VkSubmitInfo* pSubmits,
                                         VkFence fence)
{
...
VkResult VKAPI_CALL Detour_vkQueueSubmit2(VkQueue queue, uint32_t submitCount, const VkSubmitInfo2* pSubmits,
                                          VkFence fence)
{
...
VkResult VKAPI_CALL Detour_vkQueueSubmit2KHR(VkQueue queue, uint32_t submitCount, const VkSubmitInfo2* pSubmits,
                                             VkFence fence)
{
...
VkResult VKAPI_CALL Detour_vkQueuePresentKHR(VkQueue queue, const VkPresentInfoKHR* pPresentInfo)
{
    ...
    if (!g_AsyncPresentDetected) {
        if (IsComputeOnlyQueue(queue)) {
            ...
            if (g_GraphicsQueueForOverlay) {
                HookLog("Vulkan: Found graphics queue %p (Family %d) for overlay rendering", g_GraphicsQueueForOverlay,
                        g_OverlayQueueFamily);
    ...
    if (isFirstHook) {
        ...
        TryEnableFrameTimeCSVLogging(csvShm, (const void*)&Detour_vkQueuePresentKHR, g_PerfMetrics, "Vulkan",
                                     csvLoggingInitialized);
            ...
            if (fabs(lastLimit - limit) > 0.001f) {
                if (limit < 1.0f) {
                    if (limit == 0.0f)
                        ...
                        HookLog("Vulkan: Prerender Limit active: %.2f (Hybrid Serial, Timeout Scaled)", limit);
                } else {
    ...
    if (!logDeviceOnce) {
        HookLog("Vulkan: Device state - g_Device=%p, g_Queue=%p, g_PhysDevice=%p, recoveryAttempted=%d", g_Device,
                g_Queue, g_PhysDevice, recoveryAttempted);
        logDeviceOnce = true;
    }

    if (!recoveryAttempted && (g_Device == VK_NULL_HANDLE || g_PhysDevice == VK_NULL_HANDLE)) {
        recoveryAttempted = true;
        HookLog("Vulkan: Triggering late injection recovery (device=%p, phys=%p)", g_Device, g_PhysDevice);
            ...
            for (auto& sc : g_Swapchains) {
                if (sc.swapchain == pPresentInfo->pSwapchains[0]) {
                    ...
                    if (idx >= sc.framebuffers.size()) {
                        HookLog("Vulkan: ERROR - framebuffer index %u out of range (size %zu)", idx,
                                sc.framebuffers.size());
                        break;
                    }
                    ...
                    vkCmdPipelineBarrier(cb, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0,
                                         1, &globalBarrier, 0, nullptr, 0, nullptr);
    ...
    if (internalOverheadUs > 2000 && diagLogCount < 50) {
        EarlyLog("Vulkan SLOW: Total=%lldus, Limiter=%lldus, Overlay=%lldus", totalOverheadUs, limiterTimeUs,
                 diagT2 - diagT1);
        diagLogCount++;
    }
            ...
            if (sc.swapchain == swapchain && imageIndex < sc.images.size()) {
                ...
                if (g_AsyncQueue != VK_NULL_HANDLE && g_VulkanCapture.initialized) {
                    ...
                    VkSemaphore completionSem =
                        g_VulkanCapture.CopyFrame(queue, sc.images[imageIndex], sc.width, sc.height, waitSemaphores);

                    if (completionSem != VK_NULL_HANDLE) {
                        g_VulkanCapture.EnqueueFrame(us, 0, textureIdx, (void*)sc.images[imageIndex],
                ...
                VkSemaphore completionSem =
                    g_VulkanCapture.CopyFrame(queue, srcImage, sc.width, sc.height, waitSemaphores);

                if (completionSem != VK_NULL_HANDLE) {
                    int usedTextureIdx =
                        (g_VulkanCapture.writeIndex + CAPTURE_TEXTURE_COUNT - 1) % CAPTURE_TEXTURE_COUNT;
                    // PASS RAW QPC: MediaEngine converts to MS using trusted frequency
                    g_VulkanCapture.SignalFrameReady(g_IPC, usedTextureIdx, qpc.QuadPart, g_VulkanCapture.fenceValue);
...
PFN_vkVoidFunction VKAPI_CALL Detour_vkGetInstanceProcAddr(VkInstance instance, const char* pName)
{
    ...
    if (strcmp(pName, "vkCreateInstance") == 0) return (PFN_vkVoidFunction)Detour_vkCreateInstance;
    if (strcmp(pName, "vkCreateDevice") == 0) return (PFN_vkVoidFunction)Detour_vkCreateDevice;
    ...
    if (strcmp(pName, "vkCreateWin32SurfaceKHR") == 0) return (PFN_vkVoidFunction)Detour_vkCreateWin32SurfaceKHR;
    if (strcmp(pName, "vkGetInstanceProcAddr") == 0) return (PFN_vkVoidFunction)Detour_vkGetInstanceProcAddr;
    if (strcmp(pName, "vkGetDeviceProcAddr") == 0) return (PFN_vkVoidFunction)Detour_vkGetDeviceProcAddr;
...
void VulkanHook::Init()
{
    ...
    CreateHook("vkCreateInstance", (void*)&Detour_vkCreateInstance, (void**)&o_vkCreateInstance);
    CreateHook("vkCreateDevice", (void*)&Detour_vkCreateDevice, (void**)&o_vkCreateDevice);
    CreateHook("vkCreateWin32SurfaceKHR", (void*)&Detour_vkCreateWin32SurfaceKHR, (void**)&o_vkCreateWin32SurfaceKHR);
    ...
    CreateHook("vkQueuePresentKHR", (void*)&Detour_vkQueuePresentKHR, (void**)&o_vkQueuePresentKHR);
    CreateHook("vkQueueSubmit", (void*)&Detour_vkQueueSubmit, (void**)&o_vkQueueSubmit);
    CreateHook("vkQueueSubmit2", (void*)&Detour_vkQueueSubmit2, (void**)&o_vkQueueSubmit2);
    CreateHook("vkQueueSubmit2KHR", (void*)&Detour_vkQueueSubmit2KHR, (void**)&o_vkQueueSubmit2KHR);
    CreateHook("vkUpdateDescriptorSets", (void*)&Detour_vkUpdateDescriptorSets, (void**)&o_vkUpdateDescriptorSets);
    CreateHook("vkDestroySampler", (void*)&Detour_vkDestroySampler, (void**)&o_vkDestroySampler);
    CreateHook("vkCreateSwapchainKHR", (void*)&Detour_vkCreateSwapchainKHR, (void**)&o_vkCreateSwapchainKHR);

    // Critical for AF forcing - game may call these directly without ProcAddr lookup
    CreateHook("vkCreateSampler", (void*)&Detour_vkCreateSampler, (void**)&o_vkCreateSampler);
