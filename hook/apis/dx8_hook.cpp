...
typedef HRESULT(STDMETHODCALLTYPE *D3D8Present_t)(IDirect3DDevice8 *device,
                                                  const RECT *pSourceRect,
                                                  const RECT *pDestRect,
                                                  HWND hDestWindowOverride,
                                                  const RGNDATA *pDirtyRegion);

typedef HRESULT(STDMETHODCALLTYPE *D3D8Reset_t)(IDirect3DDevice8 *device,
                                                void *pPresentationParameters);

typedef HRESULT(STDMETHODCALLTYPE *D3D8GetBackBuffer_t)(
    IDirect3DDevice8 *device, UINT BackBuffer, UINT Type,
    IDirect3DSurface8 **ppBackBuffer);

typedef HRESULT(STDMETHODCALLTYPE *D3D8SetTextureStageState_t)(
    IDirect3DDevice8 *device, DWORD Stage, DWORD Type, DWORD Value);
...
typedef HRESULT(STDMETHODCALLTYPE *D3D8CreateDevice_t)(
    IDirect3D8 *d3d, UINT Adapter, UINT DeviceType, HWND hFocusWindow,
    DWORD BehaviorFlags, D3D8_PRESENT_PARAMETERS *pPresentationParameters,
...
static void ApplyDX8MSAAOverride(IDirect3D8 *d3d, UINT adapter, UINT deviceType,
                                 D3D8_PRESENT_PARAMETERS *pp) {
  ...
  if (msType != 0) {
    ...
    // 5)
    typedef HRESULT(STDMETHODCALLTYPE * CheckMS_t)(IDirect3D8 *, UINT, UINT,
                                                   UINT, BOOL, DWORD);
    void **vtable = *(void ***)d3d;
    CheckMS_t pCheckMS = (CheckMS_t)vtable[5];

    if (SUCCEEDED(pCheckMS(d3d, adapter, deviceType, pp->BackBufferFormat,
                           pp->Windowed, msType))) {
...
public:
  ...
  bool CreateD3D9ExSharedSurface() {
    ...
    HRESULT hr = d3d9DeviceEx->CreateOffscreenPlainSurfaceEx(
        width, height, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, &d3d9SharedSurface,
        &sharedHandle, 0);
  ...
  void CaptureFrame(IDirect3DDevice8 *device) {
    ...
    if (SUCCEEDED(
            d3d9SharedSurface->LockRect(&lockedRect, NULL, D3DLOCK_READONLY))) {
...
static void DrawDX8Overlay(HWND hwnd) {
  ...
  g_OverlayAdapter.SetDroppedFrames(
      g_DX8Capture.droppedFrames.load(std::memory_order_relaxed));
  g_OverlayAdapter.SetGraphicsAPI("DX8");

  // DX9Ex wrapper handles BeginScene/EndScene?
  // Original code called BeginScene/EndScene around
  // ImGui_ImplDX9_RenderDrawData. OverlayAdapter.RenderOverlay handles its own
  // render passes but assumes state is ready? DX9 backend of CustomOverlay
  // calls BeginScene/EndScene internally? Let's check
  // OverlayAdapter.RenderOverlay -> Backend->Render(). DX9Backend::Render calls
  // BeginScene/EndScene if not already? Usually OverlayAdapter expects to be
  // called inside a frame. BUT here we are wrapping. Original:
...
static HRESULT STDMETHODCALLTYPE DetourD3D8Present(
    IDirect3DDevice8 *device, const RECT *pSourceRect, const RECT *pDestRect,
  ...
  auto doOverlay = [&]() {
    if (shouldDrawOverlay) {
      HWND hwnd =
          hDestWindowOverride ? hDestWindowOverride : GetForegroundWindow();
      DrawDX8Overlay(hwnd);
    }
  };
  ...
  if (g_IPC && g_IPC->GetSharedMem()->graphicsConfig.prerenderLimit >= 0) {
    ApplyPrerenderLimitDX8(
        device, g_IPC->GetSharedMem()->graphicsConfig.prerenderLimit);
  }
  ...
  // Call original
  HRESULT hr = oD3D8Present(device, pSourceRect, pDestRect, hDestWindowOverride,
                            pDirtyRegion);
...
DetourD3D8Reset(IDirect3DDevice8 *device, void *pPresentationParameters) {
    ...
    if (mode != "default" && pPresentationParameters) {
      ...
      if (mode == "off")
        pp->FullScreen_PresentationInterval =
            0x80000000; // D3DPRESENT_INTERVAL_IMMEDIATE
      else if (mode == "fifo")
        pp->FullScreen_PresentationInterval =
            0x00000001; // D3DPRESENT_INTERVAL_ONE
...
static HRESULT STDMETHODCALLTYPE DetourD3D8CreateDevice(
  ...
  HRESULT hr =
      oD3D8CreateDevice(d3d, Adapter, DeviceType, hFocusWindow, BehaviorFlags,
                        pPresentationParameters, ppDevice);
...
void DX8Hook::Init() {
  ...
  Direct3DCreate8_t pDirect3DCreate8 =
      (Direct3DCreate8_t)GetProcAddress(d3d8Module, "Direct3DCreate8");
  if (!pDirect3DCreate8) {
  ...
  HWND dummyHwnd =
      CreateWindowExA(0, wc.lpszClassName, "DX8Dummy", WS_OVERLAPPEDWINDOW, 0,
  ...
  HRESULT hr =
      pCreateDevice(d3d8, 0, 1, dummyHwnd, 0x20, &d3d8pp, &dummyDevice);
  ...
  if (SUCCEEDED(hr) && dummyDevice) {
    ...
    if (VTableHook::Create(&deviceVTable[D3D8_VTABLE_PRESENT],
                           (LPVOID)&DetourD3D8Present,
                           (LPVOID *)&oD3D8Present) == VTableHook::Success) {
      HookLog("DX8: Present hook installed");
    }

    // Hook Reset (index 14)
    if (VTableHook::Create(&deviceVTable[D3D8_VTABLE_RESET],
                           (LPVOID)&DetourD3D8Reset,
    ...
    if (VTableHook::Create(
            &deviceVTable[63], (LPVOID)&DetourD3D8SetTextureStageState,
    ...
    if (VTableHook::Create(&d3d8VTable[15], (LPVOID)&DetourD3D8CreateDevice,
                           (LPVOID *)&oD3D8CreateDevice) ==
        VTableHook::Success) {
      HookLog("DX8: CreateDevice hook installed");
    }

    // Release dummy device
    ((void(STDMETHODCALLTYPE *)(void *))deviceVTable[2])(
        dummyDevice); // Release
  } else {
