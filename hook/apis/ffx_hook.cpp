...
typedef ffxReturnCode_t (*PfnFfxCreateContext)(
    ffxContext *context, ffxCreateContextDescHeader *desc,
...
ffxReturnCode_t Hooked_ffxCreateContext(ffxContext *context,
  ...
  if (result == FFX_API_RETURN_OK && desc) {
    ...
    if (effectId == FFX_API_EFFECT_ID_FRAMEGENERATION ||
      ...
      int prevCount =
          g_ActiveFGContextCount.fetch_add(1, std::memory_order_acq_rel);
      HookLog("FFX Hook: Frame Generation context CREATED (type=0x%llx, "
      ...
      if (prevCount == 0) {
        HookLog(
            "FFX Hook: FSR Frame Generation ACTIVATED (first context created)");
...
ffxReturnCode_t Hooked_ffxDestroyContext(ffxContext *context,
                                         const ffxAllocationCallbacks *memCb) {
  ...
  if (result == FFX_API_RETURN_OK && isFGContext) {
    ...
    int newCount =
        g_ActiveFGContextCount.fetch_sub(1, std::memory_order_acq_rel) - 1;
    if (newCount < 0) {
    ...
    if (newCount == 0) {
      HookLog("FFX Hook: FSR Frame Generation DEACTIVATED (all contexts "
              "destroyed)");
...
void InstallHooksForModule(HMODULE hModule, const char *moduleName) {
  ...
  PfnFfxCreateContext createCtx =
      (PfnFfxCreateContext)GetProcAddress(hModule, "ffxCreateContext");
  PfnFfxDestroyContext destroyCtx =
      (PfnFfxDestroyContext)GetProcAddress(hModule, "ffxDestroyContext");

  if (!createCtx && !destroyCtx) {
    HookLog(
        "FFX Hook: Neither ffxCreateContext nor ffxDestroyContext found in %s",
        moduleName);
    return;
  }
  ...
  void *dummy = nullptr;
  if (createCtx) {
    HookLog("FFX Hook: ffxCreateContext found at %p, hooking via IAT",
            createCtx);
    ...
    IATHook::RegisterDynamicHook("ffxCreateContext",
                                 (void *)Hooked_ffxCreateContext,
                                 (void **)&g_Original_ffxCreateContext);
  }

  if (destroyCtx) {
    HookLog("FFX Hook: ffxDestroyContext found at %p, hooking via IAT",
            destroyCtx);
    IATHook::PatchIATAllModules(moduleName, "ffxDestroyContext",
                                (void *)Hooked_ffxDestroyContext, &dummy);
    IATHook::RegisterDynamicHook("ffxDestroyContext",
                                 (void *)Hooked_ffxDestroyContext,
