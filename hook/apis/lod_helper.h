...
inline bool GetSGSSAABias(bool sgssaa, const char* msaa, float& outBias, int overrideSampleCount = 0)
{
    ...
    if (overrideSampleCount > 0) {
        ...
        if (msaa[0] == 'd' || strcmp(msaa, "off") == 0) {
            // No forced MSAA, so we can't do SGSSAA (unless the game natively uses MSAA, which we can't easily detect
            // globally here) But if overrideSampleCount is 0, we assume we are checking against the forced setting.
            return false;
