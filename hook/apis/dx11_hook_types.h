...
DXGIShared::APIType DetectSwapChainAPITypeForDX11Hook(IDXGISwapChain* swapChain);const char* GetDX11HookBaseAPIName(DXGIShared::APIType api);bool IsDeferredAFBootstrapped11(ID3D11DeviceContext* context);void MarkDeferredAFBootstrapped11(ID3D11DeviceContext* context);bool ApplyDX11BackbufferCountOverride(DXGI_SWAP_CHAIN_DESC& desc, const char* source);bool ApplyDX11BackbufferCountOverride(DXGI_SWAP_CHAIN_DESC1& desc, const char* source);bool IsDXVKD3D10OrD3D11Loaded();const char* GetDX11HookOverlayAPIName(DXGIShared::APIType api);UINT ResolveDX11BackBufferIndex(IDXGISwapChain* swapChain, const DXGI_SWAP_CHAIN_DESC* swapChainDesc = nullptr);bool IsUnityProcess();
