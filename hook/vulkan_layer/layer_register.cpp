 ...
 *
 * Registers/unregisters the capture overlay Vulkan layer in the Windows registry.
 * This allows the layer to be loaded automatically by the Vulkan loader.
 */
...
const wchar_t* VULKAN_IMPLICIT_LAYERS_32 = L"SOFTWARE\\Khronos\\Vulkan\\ImplicitLayers";
const wchar_t* VULKAN_IMPLICIT_LAYERS_64 = L"SOFTWARE\\Khronos\\Vulkan\\ImplicitLayers";

// For 32-bit on 64-bit Windows
const wchar_t* VULKAN_IMPLICIT_LAYERS_WOW64 = L"SOFTWARE\\WOW6432Node\\Khronos\\Vulkan\\ImplicitLayers";
...
bool RegisterVulkanLayer(const std::wstring& manifestPath, bool forAllUsers = false)
{
    ...
    LONG result = RegCreateKeyExW(hRootKey, VULKAN_IMPLICIT_LAYERS_64, 0, nullptr, REG_OPTION_NON_VOLATILE,
...
bool UnregisterVulkanLayer(const std::wstring& manifestPath, bool forAllUsers = false)
{
    ...
    LONG result = RegOpenKeyExW(hRootKey, VULKAN_IMPLICIT_LAYERS_64, 0, KEY_SET_VALUE | KEY_WOW64_64KEY, &hKey);
...
bool IsLayerRegistered(const std::wstring& manifestPath, bool forAllUsers = false)
{
    ...
    LONG result = RegOpenKeyExW(hRootKey, VULKAN_IMPLICIT_LAYERS_64, 0, KEY_QUERY_VALUE | KEY_WOW64_64KEY, &hKey);
...
std::wstring GetLayerManifestPath()
{
    ...
    std::filesystem::path manifestPath = exePath.parent_path() / L"VK_LAYER_CAPTURE_overlay.json";
