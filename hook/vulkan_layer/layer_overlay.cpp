...
void InitializeOverlay(VkDevice device, VkSwapchainKHR swapchain,
  ...
  // Initialize SystemMetricsCollector with GPU LUID for system stats (GPU load,
  // VRAM)
  InstanceDispatch *instDisp =
      VulkanLayerState::Get().GetInstanceDispatch(state.instance);
  if (instDisp && instDisp->fp_vkGetPhysicalDeviceProperties2) {
    ...
    if (idProps.deviceLUIDValid) {
      ...
      LayerLog("Vulkan Layer: SystemMetricsCollector initialized with LUID: "
               "%08X%08X",
               luidHigh, luidLow);
      ...
      if (maxHeapSize > 0) {
        ...
        LayerLog("Vulkan Layer: VRAM Total set to: %llu MB",
  ...
  pool_info.poolSizeCount =
      (uint32_t)(sizeof(pool_sizes) / sizeof(pool_sizes[0]));
  pool_info.pPoolSizes = pool_sizes;

  if (disp->fp_vkCreateDescriptorPool(device, &pool_info, nullptr,
...
void RenderOverlay(VkDevice device, VkQueue queue, uint32_t imageIndex,
  ...
  // Deferred window hook: if we didn't have a window at init time, try to get
  // it now
  if (it->second.needsWindowHook) {
    ...
    if (hwnd) {
      ...
      if (foregroundPid == GetCurrentProcessId()) {
        ...
        it->second.needsWindowHook = false;
  ...
  disp->fp_vkWaitForFences(device, 1, &fence, VK_TRUE,
