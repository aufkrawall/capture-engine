...
static void RefreshLayerProcessName() {
    ce::vulkan_layer_participation::GetCurrentProcessBaseName(g_ProcessName, sizeof(g_ProcessName));
}
...
bool LayerIPC_IsProcessEligibleByCurrentHost(DWORD* inheritedParentPid) {
    ...
    const bool eligible =
        ce::vulkan_layer_participation::IsProcessEligibleByDiscovery(info, g_ProcessName, inheritedParentPid);
    if (info)
