...
bool LayerIPC_Init() {
  ...
  if (g_IPCClient.Connect()) {
    LayerLog("Layer IPC: Connected to Host PID %d",
             g_IPCClient.GetSharedMem()->GetHostPID());
    ...
    if (!whitelisted) {
      LayerLog("Layer IPC: Process '%s' NOT whitelisted. Layer dormant.",
               g_ProcessName);
...
uint32_t LayerIPC_GetWriteIndex() {
  ...
  if (!mem)
    return 0;
  return mem->frameRing.writeIndex.load(
      std::memory_order_acquire); // Use frameRing
}
...
void LayerIPC_IncrementWriteIndex(uint64_t timestamp) {
  ...
  if (slot < FRAME_RING_SIZE) {
    ...
    ring.slots[slot].textureIndex =
        wIdx % g_PublishedTextureCount; // Use tracked count
...
void LayerIPC_SetShmemDimensions(uint32_t width, uint32_t height,
  ...
  mem->SetHeight(height);
  mem->SetFormat(format);
  LayerLog("Layer IPC: Set SHMEM dimensions %ux%u format=%u", width, height,
           format);
}
...
void LayerIPC_Log(const char *fmt, ...) {
  ...
  int len = snprintf(s_lineBuffer, sizeof(s_lineBuffer),
                     "[%02d:%02d:%02d.%03d] [T:%04lX] [%s] %s", st.wHour,
