...
static bool CreateD3D11InteropDevice(IDXGIAdapter *adapter,
  ...
  PFN_D3D11_CREATE_DEVICE createFn =
  ...
  HRESULT hr = createFn(
      ...
      nullptr, flags, levels, 1, D3D11_SDK_VERSION, ppDevice, &featureLevel,
      ppContext);
...
static uint64_t MakeLuidKey(const LUID &luid) {
  return (static_cast<uint64_t>(luid.HighPart) << 32) |
...
static bool CreateSharedTextures(D3D11InteropDevice *interopDev, VkDevice vkDev,
                                 DeviceDispatch *disp, VkPhysicalDevice physDev,
                                 const LUID &luid, uint32_t width,
                                 uint32_t height, uint32_t vkFormat,
...
GetOrCreateSharedTextures(VkDevice vkDev, DeviceDispatch *disp,
  ...
  for (auto it = g_TextureCache.begin(); it != g_TextureCache.end();) {
    if (it->luidKey == luidKey && it->width == width && it->height == height &&
  ...
  if (!CreateSharedTextures(interopDev, vkDev, disp, physDev, luid, width,
                            height, vkFormat, newEntry)) {
    return nullptr;
  }
...
void InitializeCapture(VkDevice device, VkSwapchainKHR swapchain,
                       VkFormat format, VkExtent2D extent,
                       uint32_t imageCount) {
  LayerLog("Vulkan Layer: InitializeCapture(device=%p, images=%d, size=%dx%d, "
           "vkFormat=%d)",
  ...
  SharedTextureEntry *sharedTextures =
      GetOrCreateSharedTextures(device, disp, disp->physicalDevice, luid,
  ...
  LayerIPC_SetTextures(sharedTextures->textureHandles.data(),
                       (uint32_t)sharedTextures->textureHandles.size(),
  ...
  if (disp->fp_vkCreateCommandPool(device, &poolInfo, nullptr,
  ...
  VkCommandBufferAllocateInfo cbInfo = {
      ...
      state.commandPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY, imageCount};
  ...
  VkSemaphoreCreateInfo semInfo = {VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO,
                                   nullptr, 0};
  for (uint32_t i = 0; i < imageCount; i++) {
    disp->fp_vkCreateSemaphore(device, &semInfo, nullptr,
                               &state.signalSemaphores[i]);
  }
  ...
  exportInfo.handleTypes =
      VK_EXTERNAL_SEMAPHORE_HANDLE_TYPE_D3D12_FENCE_BIT; // Use D3D12_FENCE for
                                                         // cross-API fence
  ...
  if (disp->fp_vkCreateSemaphore(device, &timelineSemInfo, nullptr,
                                 &state.timelineSemaphore) == VK_SUCCESS) {
    if (disp->fp_vkGetSemaphoreWin32HandleKHR) {
      ...
      if (disp->fp_vkGetSemaphoreWin32HandleKHR(
              device, &getHandleInfo, &state.sharedFenceHandle) == VK_SUCCESS) {
        LayerIPC_SetFence(state.sharedFenceHandle);
        LayerLog("Vulkan Layer: Created Shared Fence %p",
                 state.sharedFenceHandle);
      } else {
  ...
  LayerLog("Vulkan Layer: Zero-Copy Capture Initialized (%dx%d)", extent.width,
           extent.height);
  ...
  if (it != g_CaptureStates.end()) {
    ...
    if (disp) {
      ...
      if (it->second.timelineSemaphore)
        disp->fp_vkDestroySemaphore(device, it->second.timelineSemaphore,
                                    nullptr);
    ...
    for (auto &entry : g_TextureCache) {
      if (entry.luidKey == luidKey) {
        ...
        entry.valid = false;
        LayerLog("Vulkan Layer: Cleaned up texture cache entry for LUID %llx",
                 luidKey);
      }
    }
  }
...
void CaptureFrame(VkDevice device, VkQueue queue, VkImage srcImage,
  ...
  VkResult waitResult =
      disp->fp_vkWaitForFences(device, 1, &fence, VK_TRUE, FENCE_TIMEOUT_NS);
  ...
  srcBarrier.srcAccessMask =
      ...
      VK_ACCESS_MEMORY_WRITE_BIT; // Paranoid: Wait for everything
  srcBarrier.dstAccessMask = VK_ACCESS_TRANSFER_READ_BIT;
  srcBarrier.oldLayout =
      VK_IMAGE_LAYOUT_PRESENT_SRC_KHR; // Assume presentable layout from game
  ...
  // etc.)
  disp->fp_vkCmdPipelineBarrier(cmd, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
  ...
  region.extent = {(uint32_t)sharedTextures->width,
                   (uint32_t)sharedTextures->height, 1};

  disp->fp_vkCmdCopyImage(cmd, srcImage, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                          sharedTextures->vkImages[slotIndex],
  ...
  if (disp->fp_vkQueueSubmit(queue, 1, &submit, fence) ==
