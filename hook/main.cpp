...
enum class ProcessCategory { PotentialGame, Launcher, InternalTool, Blacklisted };
...
typedef BOOL(WINAPI* CreateProcessA_t)(LPCSTR, LPSTR, LPSECURITY_ATTRIBUTES, LPSECURITY_ATTRIBUTES, BOOL, DWORD, LPVOID,
                                       LPCSTR, LPSTARTUPINFOA, LPPROCESS_INFORMATION);
typedef BOOL(WINAPI* CreateProcessW_t)(LPCWSTR, LPWSTR, LPSECURITY_ATTRIBUTES, LPSECURITY_ATTRIBUTES, BOOL, DWORD,
...
typedef LSTATUS(WINAPI* RegQueryValueExW_t)(HKEY hKey, LPCWSTR lpValueName, LPDWORD lpReserved, LPDWORD lpType,
...
BOOL WINAPI HookedCreateProcessA(LPCSTR lpApp, LPSTR lpCmd, LPSECURITY_ATTRIBUTES lpPA, LPSECURITY_ATTRIBUTES lpTA,
                                 BOOL bInherit, DWORD dwFlags, LPVOID lpEnv, LPCSTR lpDir, LPSTARTUPINFOA lpSI,
...
BOOL WINAPI HookedCreateProcessW(LPCWSTR lpApp, LPWSTR lpCmd, LPSECURITY_ATTRIBUTES lpPA, LPSECURITY_ATTRIBUTES lpTA,
                                 BOOL bInherit, DWORD dwFlags, LPVOID lpEnv, LPCWSTR lpDir, LPSTARTUPINFOW lpSI,
                                 LPPROCESS_INFORMATION lpPI)
{
...
std::string GetRedirectedPath(const std::string& requestedPath)
{
    ...
    try {
        ...
        if (overridePath.empty() && g_pLocalConfig) {
            ...
            else if (filenameLower.find("sl.") == 0 || filenameLower == "nvngx_deepdvc.dll" ||
...
HMODULE WINAPI HookedLoadLibraryExA(LPCSTR lpLibFileName, HANDLE hFile, DWORD dwFlags)
{
    if (lpLibFileName) {
        std::string redirect = GetRedirectedPath(lpLibFileName);
        if (!redirect.empty()) {
            // Use OriginalLoadLibraryA for the redirect to simplify (Ex flags might conflict with absolute path?
            // usually ok) But let's stick to ExA to respect flags if possible, filtering flags that shouldn't apply to
            // absolute path? Actually, usually users just want to load the DLL. LOAD_LIBRARY_SEARCH_DLL_LOAD_DIR might
            // be an issue. Let's try to trust the user path is absolute.
        ...
        if (refStr) {
            ...
            for (int i = 0; i < 100; i++) {
                ...
                if (*(p - i) == 0xE8) {
                    ...
                    // 48 8B 05 ?? ?? ?? ?? C3
                    if (*(uint8_t*)funcAddr == 0x48 && *(uint8_t*)(funcAddr + 1) == 0x8B &&
                        *(uint8_t*)(funcAddr + 7) == 0xC3) {
                        ...
                        HookLog("UE5: Found ConsoleManager singleton at %p (via r.DumpingMovie)",
...
// Check if Steam overlay is present - if so, we need to be very careful about hook installation
// to avoid recursion crashes
static bool IsSteamOverlayPresent()
{
    ...
    if (!s_checked) {
        ...
        if (GetModuleHandleA("gameoverlayrenderer64.dll") || GetModuleHandleA("gameoverlayrenderer.dll")) {
            s_present = true;
            EarlyLog("STEAM OVERLAY DETECTED - Will use minimal hook mode to avoid recursion");
        }
    }
    return s_present;
}
...
DWORD WINAPI HookThread(LPVOID lpParam)
{
    ...
    if (g_ProcessCategory == ProcessCategory::Launcher) {
        ...
        void* dummy;
        IATHook::PatchIATAllModules("kernel32.dll", "CreateProcessA", (void*)&HookedCreateProcessA, &dummy);
        IATHook::PatchIATAllModules("kernel32.dll", "CreateProcessW", (void*)&HookedCreateProcessW, &dummy);
        ...
        while (true) {
            bool hasGraphicsAPI =
                (GetModuleHandleA("d3d12.dll") != NULL || GetModuleHandleA("d3d11.dll") != NULL ||
                 ...
                 GetModuleHandleA("opengl32.dll") != NULL || GetModuleHandleA("vulkan-1.dll") != NULL);
    ...
    IATHook::InitializeKernel32Hooks(
        ...
        (void**)&OriginalLoadLibraryW, (void*)&HookedLoadLibraryExA, (void**)&OriginalLoadLibraryExA,
        (void*)&HookedLoadLibraryExW, (void**)&OriginalLoadLibraryExW, (void*)&HookedCreateProcessA,
        (void**)&OriginalCreateProcessA, (void*)&HookedCreateProcessW, (void**)&OriginalCreateProcessW);
    ...
    if (GetModuleHandleA("advapi32.dll")) {
        ...
        IATHook::InitializeAdvapi32Hooks((void*)&HookedRegQueryValueExW, (void**)&OriginalRegQueryValueExW);
    } else {
...
static bool IsServiceProcess(const char* name)
{
    ...
    return (
        _stricmp(name, "svchost.exe") == 0 || _stricmp(name, "lsass.exe") == 0 || _stricmp(name, "services.exe") == 0 ||
        _stricmp(name, "smss.exe") == 0 || _stricmp(name, "wininit.exe") == 0 || _stricmp(name, "csrss.exe") == 0 ||
        _stricmp(name, "conhost.exe") == 0 || _stricmp(name, "dllhost.exe") == 0 ||  // COM Surrogate
        _stricmp(name, "sihost.exe") == 0 || _stricmp(name, "pwahelper.exe") == 0 ||
        _stricmp(name, "PerfWatson.exe") == 0 || _stricmp(name, "DataExchangeHost.exe") == 0 ||
        _stricmp(name, "GamebarFTServer.exe") == 0 || _stricmp(name, "WerFault.exe") == 0 ||  // Windows Error Reporting
...
// If we export CBTHookProc, Steam's hook can still find and call it even if we don't install it
// Removing the export breaks any lingering hook registrations
...
extern "C" BOOL WINAPI DllMain(HINSTANCE hinstDLL, DWORD ul_reason_for_call, LPVOID lpReserved)
{
    if (ul_reason_for_call == DLL_PROCESS_ATTACH) {
        ...
        bool isOurTool =
            (_stricmp(fileName, "captureengine.exe") == 0 || _stricmp(fileName, "captureengine_x86.exe") == 0);

        if (!isOurTool) {
            HMODULE hPin = NULL;
            GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_PIN, (LPCSTR)hinstDLL,
                               &hPin);
        }
        ...
        if (_stricmp(fileName, "explorer.exe") == 0 || _stricmp(fileName, "dwm.exe") == 0 ||
            _stricmp(fileName, "winlogon.exe") == 0 || _stricmp(fileName, "captureengine.exe") == 0 ||
            _stricmp(fileName, "captureengine_x86.exe") == 0 || _stricmp(fileName, "sihost.exe") == 0 ||
            _stricmp(fileName, "SearchUI.exe") == 0 || _stricmp(fileName, "ShellExperienceHost.exe") == 0 ||
        ...
        if (g_ProcessCategory == ProcessCategory::PotentialGame) {
            if (!(GetModuleHandleA("d3d12.dll") || GetModuleHandleA("d3d11.dll") || GetModuleHandleA("d3d9.dll") ||
                  GetModuleHandleA("vulkan-1.dll") || GetModuleHandleA("opengl32.dll") ||
