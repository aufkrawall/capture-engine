...
#  endif
  ...
  // Always install the DirectDrawCreate export hooks when ddraw.dll is present.
  // DDrawHook::Init independently decides whether synthetic bootstrap is safe;
  // conflating the two decisions leaves a mixed-module DirectDraw game no way
  // to produce the real interface evidence that identifies it.
  const bool ddrawDllLoaded = GetModuleHandleA("ddraw.dll") != nullptr;
  if (!s_vulkanActive && !g_DDrawHook && ddrawDllLoaded) {
  ...
  } else if (s_vulkanActive && !g_DDrawHook && ddrawDllLoaded) {
    HookLog("DDraw hooks suppressed because the Vulkan layer owns presentation");
  }
