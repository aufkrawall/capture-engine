...
void CaptureManager::RegisterCaptureTarget(const char *name,
                                           ISharedCaptureTarget *target) {
...
SharedCaptureD3D11::SharedCaptureD3D11()
    : m_SharedHandles{nullptr, nullptr}, m_WriteIndex(0), m_FrameCounter(0),
      m_Active(false) {
...
bool SharedCaptureD3D11::Initialize(ID3D11Device *pDevice,
  ...
  if (!CreateSharedTexture(desc.BufferDesc.Width, desc.BufferDesc.Height,
...
bool SharedCaptureD3D11::CreateSharedTexture(UINT width, UINT height,
                                             DXGI_FORMAT format) {
  ...
  for (int i = 0; i < 2; i++) {
    HRESULT hr =
        m_pDevice->CreateTexture2D(&desc, nullptr, &m_SharedTextures[i]);
    if (FAILED(hr)) {
      return false;
    }
...
SharedCaptureD3D12::SharedCaptureD3D12()
    : m_SharedHandles{nullptr, nullptr}, m_FenceShareHandle(nullptr),
      m_FenceEvent(nullptr), m_FenceValue(0), m_FenceValues{0, 0},
      m_WriteIndex(0), m_FrameCounter(0), m_Active(false) {
  ZeroMemory(&m_CurrentFrame, sizeof(m_CurrentFrame));
}
...
bool SharedCaptureD3D12::Initialize(ID3D12Device *pDevice,
                                    IDXGISwapChain *pSwapChain) {
  if (!pDevice || !pSwapChain)
    return false;
  ...
  if (FAILED(pDevice->CreateFence(0, D3D12_FENCE_FLAG_SHARED,
  ...
  if (FAILED(pDevice->CreateSharedHandle(m_Fence.Get(), nullptr, GENERIC_ALL,
                                         nullptr, &m_FenceShareHandle))) {
  ...
  for (int i = 0; i < 2; i++) {
    if (FAILED(pDevice->CreateCommandAllocator(
      ...
      EarlyLog("DX12: SharedCapture - Failed to Create Command Allocator %d",
               i);
      return false;
    }
  }
  ...
  // code)
  if (!CreateSharedResources(desc.BufferDesc.Width, desc.BufferDesc.Height,
                             desc.BufferDesc.Format)) {
...
bool SharedCaptureD3D12::CreateSharedResources(UINT width, UINT height,
                                               DXGI_FORMAT format) {
  EarlyLog("DX12: CreateSharedResources - w=%u h=%u fmt=%d", width, height,
           format);
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if (FAILED(hr)) {
      EarlyLog("DX12: CreateSharedResources - Failed to create D3D12 texture "
               "%d (hr=0x%08X)",
               i, hr);
      return false;
    }
    ...
    hr = m_pDevice->CreateSharedHandle(m_SharedResources[i].Get(), nullptr,
                                       GENERIC_ALL, nullptr,
                                       &m_SharedHandles[i]);

    if (FAILED(hr)) {
      EarlyLog("DX12: CreateSharedResources - Failed to create shared handle "
               "for texture %d (hr=0x%08X)",
               i, hr);
      return false;
    }

    EarlyLog("DX12: CreateSharedResources - Created D3D12 shared texture %d, "
             "handle=%p",
             i, m_SharedHandles[i]);
  }
...
bool SharedCaptureD3D12::CaptureFrame(ID3D12CommandQueue *pCommandQueue,
  ...
  if (FAILED(m_pSwapChain->GetBuffer(backBufferIndex,
  ...
  m_CommandList->CopyResource(m_SharedResources[writeIdx].Get(),
                              backBuffer.Get());
