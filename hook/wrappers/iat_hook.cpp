  ...
  if (af != "default") {
    if (af == "off") {
      ...
      if (pDesc->AddressU != D3D12_TEXTURE_ADDRESS_MODE_BORDER &&
        ...
        bool comparison =
            (pDesc->Filter >= D3D12_FILTER_COMPARISON_MIN_MAG_MIP_POINT);
        pDesc->Filter = comparison ? D3D12_FILTER_COMPARISON_ANISOTROPIC
...
static IMAGE_EXPORT_DIRECTORY *GetExportDirectory(HMODULE module,
                                                  DWORD *exportSize = nullptr) {
  // Validate module is still loaded before accessing memory
  if (!IsModuleValid(module))
    return nullptr;
...
bool PatchIAT(HMODULE targetModule, const char *sourceModule,
              const char *functionName, void *hookFunction,
              void **outOriginal) {
  if (!targetModule)
  ...
  while (importDesc->Name != 0) {
    ...
    if (_stricmp(moduleName, sourceModule) == 0) {
      ...
      auto thunkData = reinterpret_cast<IMAGE_THUNK_DATA *>(
          reinterpret_cast<BYTE *>(targetModule) +
        ...
        if (!(thunkData->u1.Ordinal & IMAGE_ORDINAL_FLAG)) {
          ...
          if (strcmp(importByName->Name, functionName) == 0) {
            ...
            if (VirtualProtect(&iatEntry->u1.Function, sizeof(void *),
              ...
              VirtualProtect(&iatEntry->u1.Function, sizeof(void *), oldProtect,
                             &oldProtect);

              WrapperLog("IAT: Successfully patched %s!%s in module %p",
              ...
              WrapperLog("IAT: Patched %s!%s in module %p", sourceModule,
...
bool PatchIATAllModules(const char *sourceModule, const char *functionName,
  ...
  if (EnumProcessModules(GetCurrentProcess(), modules, sizeof(modules),
    ...
    for (int i = 0; i < count; ++i) {
      ...
      if (GetModuleFileNameExW(GetCurrentProcess(), modules[i], szModName,
                               MAX_PATH)) {
      ...
      if (PatchIAT(modules[i], sourceModule, functionName, hookFunction,
                   &orig)) {
  ...
  // Also patch main exe
  if (!anyPatched) {
    anyPatched = PatchIAT(nullptr, sourceModule, functionName, hookFunction,
                          outOriginal);
  }
...
bool RestoreIAT(HMODULE targetModule, const char *sourceModule,
  ...
  for (auto it = g_PatchedEntries.begin(); it != g_PatchedEntries.end(); ++it) {
    if (it->targetModule == targetModule &&
      ...
      DWORD oldProtect;
      if (VirtualProtect(it->iatEntry, sizeof(void *), PAGE_READWRITE,
                         &oldProtect)) {
        *it->iatEntry =
            originalFunction ? originalFunction : it->originalFunction;
...
bool PatchEAT(HMODULE exportingModule, const char *functionName,
  ...
  auto ordinals =
      reinterpret_cast<WORD *>(reinterpret_cast<BYTE *>(exportingModule) +
                               exportDir->AddressOfNameOrdinals);
  auto functions =
      reinterpret_cast<DWORD *>(reinterpret_cast<BYTE *>(exportingModule) +
  ...
  for (DWORD i = 0; i < exportDir->NumberOfNames; ++i) {
    ...
    if (strcmp(name, functionName) == 0) {
      ...
      // Calculate new RVA
      DWORD newRVA =
          static_cast<DWORD>(reinterpret_cast<BYTE *>(hookFunction) -
                             reinterpret_cast<BYTE *>(exportingModule));

      DWORD oldProtect;
      if (VirtualProtect(&functions[ordinals[i]], sizeof(DWORD), PAGE_READWRITE,
                         &oldProtect)) {
        functions[ordinals[i]] = newRVA;
        VirtualProtect(&functions[ordinals[i]], sizeof(DWORD), oldProtect,
                       &oldProtect);
...
extern HRESULT WINAPI Wrapped_CreateDXGIFactory2(UINT Flags, REFIID riid,
                                                 void **ppFactory);

#ifdef ENABLE_D3D12_WRAPPER
extern HRESULT
    WINAPI Wrapped_D3D12CreateDevice(IUnknown *pAdapter,
...
extern PFN_D3D12_SERIALIZE_VERSIONED_ROOT_SIGNATURE
...
bool InitializeDXGIHooks() {
  ...
  if (hDXGI) {
    ...
    if (!PatchIATAllModules("dxgi.dll", "CreateDXGIFactory",
      ...
      WrapperLog(
          "IAT: CreateDXGIFactory not found in IAT (may not be imported)");
    }
    ...
    RegisterDynamicHook("CreateDXGIFactory", (void *)Wrapped_CreateDXGIFactory,
                        (void **)&oCreateDXGIFactory);
    RegisterDynamicHook("CreateDXGIFactory1",
                        (void *)Wrapped_CreateDXGIFactory1,
                        (void **)&oCreateDXGIFactory1);
    RegisterDynamicHook("CreateDXGIFactory2",
                        (void *)Wrapped_CreateDXGIFactory2,
                        (void **)&oCreateDXGIFactory2);
...
bool InitializeD3D10Hooks() {
  ...
  if (hD3D10) {
    ...
    ::oD3D10CreateDeviceAndSwapChain =
        (PFN_D3D10CreateDeviceAndSwapChain)GetProcAddress(
            hD3D10, "D3D10CreateDeviceAndSwapChain");

    void *dummy;
    PatchIATAllModules("d3d10.dll", "D3D10CreateDevice",
                       (void *)::Wrapped_D3D10CreateDevice, &dummy);
    ...
    if (hD3D10_1) {
      ::oD3D10CreateDevice1 = (PFN_D3D10CreateDevice1)GetProcAddress(
...
bool InitializeKernel32Hooks(
  ...
  if (pOriginalLoadLibraryExA) {
    *pOriginalLoadLibraryExA =
        (void *)GetProcAddress(hKernel32, "LoadLibraryExA");
  }
  if (pOriginalLoadLibraryExW) {
    *pOriginalLoadLibraryExW =
        (void *)GetProcAddress(hKernel32, "LoadLibraryExW");
  }
  if (pOriginalCreateProcessA) {
    *pOriginalCreateProcessA =
        (void *)GetProcAddress(hKernel32, "CreateProcessA");
  }
  if (pOriginalCreateProcessW) {
    *pOriginalCreateProcessW =
        (void *)GetProcAddress(hKernel32, "CreateProcessW");
  }

  // Patch LoadLibrary* in all modules
  void *dummy;
  if (LoadLibraryAHook) {
    PatchIATAllModules("kernel32.dll", "LoadLibraryA", LoadLibraryAHook,
                       &dummy);
  }
  if (LoadLibraryWHook) {
    PatchIATAllModules("kernel32.dll", "LoadLibraryW", LoadLibraryWHook,
                       &dummy);
  }
  if (LoadLibraryExAHook) {
    PatchIATAllModules("kernel32.dll", "LoadLibraryExA", LoadLibraryExAHook,
                       &dummy);
  }
  if (LoadLibraryExWHook) {
    PatchIATAllModules("kernel32.dll", "LoadLibraryExW", LoadLibraryExWHook,
                       &dummy);
  }

  // Patch CreateProcess* in all modules
  if (CreateProcessAHook) {
    PatchIATAllModules("kernel32.dll", "CreateProcessA", CreateProcessAHook,
                       &dummy);
  }
  if (CreateProcessWHook) {
    PatchIATAllModules("kernel32.dll", "CreateProcessW", CreateProcessWHook,
                       &dummy);
  }
...
bool InitializeAdvapi32Hooks(void *RegQueryValueExWHook,
  ...
  if (pOriginalRegQueryValueExW) {
    *pOriginalRegQueryValueExW =
        (void *)GetProcAddress(hAdvapi32, "RegQueryValueExW");
  }

  // Patch in all modules
  void *dummy;
  if (RegQueryValueExWHook) {
    PatchIATAllModules("advapi32.dll", "RegQueryValueExW", RegQueryValueExWHook,
                       &dummy);
  }
...
void ShutdownIATHooks() {
  ...
  for (auto &entry : g_PatchedEntries) {
    DWORD oldProtect;
    if (VirtualProtect(entry.iatEntry, sizeof(void *), PAGE_READWRITE,
                       &oldProtect)) {
...
void InitializeGetProcAddressHook() {
  WrapperLog(
      "IAT: Initializing GetProcAddress hook for dynamic interception...");

  HMODULE hKernel32 = GetModuleHandleA("kernel32.dll");
  if (!hKernel32)
    return;

  oGetProcAddress = (FARPROC(WINAPI *)(HMODULE, LPCSTR))GetProcAddress(
      hKernel32, "GetProcAddress");

  if (oGetProcAddress) {
    void *dummy;
    PatchIATAllModules("kernel32.dll", "GetProcAddress",
                       (void *)DetourGetProcAddress, &dummy);
