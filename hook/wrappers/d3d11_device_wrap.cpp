...
CWrapD3D11Device::CWrapD3D11Device(ID3D11Device* pReal)
    : m_pReal(pReal), m_pReal1(nullptr), m_pReal2(nullptr), m_pReal3(nullptr), m_pReal4(nullptr), m_pReal5(nullptr),
      m_RefCount(1), m_Version(0), m_pWrappedContext(nullptr)
{
    if (pReal) {
        pReal->AddRef();
        PromoteInterfaces();
...
ULONG STDMETHODCALLTYPE CWrapD3D11Device::AddRef() { return InterlockedIncrement(&m_RefCount); }
...
D3D_FEATURE_LEVEL STDMETHODCALLTYPE CWrapD3D11Device::GetFeatureLevel() { return m_pReal->GetFeatureLevel(); }

UINT STDMETHODCALLTYPE CWrapD3D11Device::GetCreationFlags() { return m_pReal->GetCreationFlags(); }

HRESULT STDMETHODCALLTYPE CWrapD3D11Device::GetDeviceRemovedReason() { return m_pReal->GetDeviceRemovedReason(); }
...
UINT STDMETHODCALLTYPE CWrapD3D11Device::GetExceptionMode() { return m_pReal->GetExceptionMode(); }
