...
CWrapD3D12CommandQueue::CWrapD3D12CommandQueue(ID3D12CommandQueue* pReal, CWrapD3D12Device* pDevice)
    : m_pReal(pReal), m_pDevice(pDevice), m_RefCount(1), m_Type(D3D12_COMMAND_LIST_TYPE_DIRECT), m_bRegistered(false)
{
    if (pReal) {
        pReal->AddRef();
...
ULONG STDMETHODCALLTYPE CWrapD3D12CommandQueue::AddRef() { return InterlockedIncrement(&m_RefCount); }
...
HRESULT STDMETHODCALLTYPE CWrapD3D12CommandQueue::SetName(LPCWSTR Name) { return m_pReal->SetName(Name); }
...
void STDMETHODCALLTYPE CWrapD3D12CommandQueue::EndEvent() { m_pReal->EndEvent(); }
...
D3D12_COMMAND_QUEUE_DESC STDMETHODCALLTYPE CWrapD3D12CommandQueue::GetDesc() { return m_pReal->GetDesc(); }
