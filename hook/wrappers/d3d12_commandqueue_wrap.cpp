...
CWrapD3D12CommandQueue::CWrapD3D12CommandQueue(ID3D12CommandQueue *pReal,
                                               CWrapD3D12Device *pDevice)
    : m_pReal(pReal), m_pDevice(pDevice), m_RefCount(1),
      m_Type(D3D12_COMMAND_LIST_TYPE_DIRECT), m_bRegistered(false) {
  if (pReal) {
    pReal->AddRef();
  ...
  WrapperLog("D3D12 CommandQueue Wrapper: Created (real=%p, type=%d)", pReal,
             m_Type);
}
...
CWrapD3D12CommandQueue::QueryInterface(REFIID riid, void **ppvObj) {
  if (!ppvObj)
    return E_POINTER;
...
HRESULT STDMETHODCALLTYPE CWrapD3D12CommandQueue::GetPrivateData(
    REFGUID guid, UINT *pDataSize, void *pData) {
  return m_pReal->GetPrivateData(guid, pDataSize, pData);
}

HRESULT STDMETHODCALLTYPE CWrapD3D12CommandQueue::SetPrivateData(
    REFGUID guid, UINT DataSize, const void *pData) {
  return m_pReal->SetPrivateData(guid, DataSize, pData);
}

HRESULT STDMETHODCALLTYPE CWrapD3D12CommandQueue::SetPrivateDataInterface(
...
HRESULT STDMETHODCALLTYPE CWrapD3D12CommandQueue::GetDevice(REFIID riid,
                                                            void **ppvDevice) {
  if (m_pDevice) {
...
void STDMETHODCALLTYPE CWrapD3D12CommandQueue::CopyTileMappings(
  ...
  m_pReal->CopyTileMappings(pDstResource, pDstRegionStartCoordinate,
...
void STDMETHODCALLTYPE CWrapD3D12CommandQueue::ExecuteCommandLists(
  ...
  std::call_once(s_lookupFlag, []() {
    ...
    if (hMod) {
      ...
      s_notifyFn =
          (void (*)(UINT))GetProcAddress(hMod, "DX12_NotifyCommandLists");
      s_setQueueFn = (void (*)(ID3D12CommandQueue *))GetProcAddress(
          hMod, "DX12_SetCommandQueue");
      WrapperLog(
          "D3D12 CQW: DX12_NotifyCommandLists=%p, DX12_SetCommandQueue=%p",
          s_notifyFn, s_setQueueFn);
    } else {
  ...
  if (!m_bRegistered && s_setQueueFn) {
    if (m_pReal) {
      WrapperLog("D3D12 CQW: Registering command queue %p (type=%d)", m_pReal,
                 m_Type);
...
void STDMETHODCALLTYPE CWrapD3D12CommandQueue::SetMarker(UINT Metadata,
                                                         const void *pData,
                                                         UINT Size) {
  m_pReal->SetMarker(Metadata, pData, Size);
}

void STDMETHODCALLTYPE CWrapD3D12CommandQueue::BeginEvent(UINT Metadata,
                                                          const void *pData,
                                                          UINT Size) {
...
HRESULT STDMETHODCALLTYPE CWrapD3D12CommandQueue::Signal(ID3D12Fence *pFence,
                                                         UINT64 Value) {
  return m_pReal->Signal(pFence, Value);
}

HRESULT STDMETHODCALLTYPE CWrapD3D12CommandQueue::Wait(ID3D12Fence *pFence,
                                                       UINT64 Value) {
  return m_pReal->Wait(pFence, Value);
}

HRESULT STDMETHODCALLTYPE
CWrapD3D12CommandQueue::GetTimestampFrequency(UINT64 *pFrequency) {
  return m_pReal->GetTimestampFrequency(pFrequency);
}

HRESULT STDMETHODCALLTYPE CWrapD3D12CommandQueue::GetClockCalibration(
