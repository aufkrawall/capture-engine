...
CWrapDXGIAdapter::CWrapDXGIAdapter(IDXGIAdapter *pReal,
                                   CWrapDXGIFactory2 *pFactoryWrapper)
    : m_pReal(pReal), m_pReal1(nullptr), m_pReal2(nullptr), m_pReal3(nullptr),
      m_pReal4(nullptr), m_pParentFactory(pFactoryWrapper), m_RefCount(1),
      m_Version(0) {
  if (pReal) {
    pReal->AddRef();
    PromoteInterfaces();
  ...
  WrapperLog("DXGI Adapter Wrapper: Created (real=%p, version=%d)", pReal,
             m_Version);
}

CWrapDXGIAdapter::CWrapDXGIAdapter(IDXGIAdapter1 *pReal,
                                   CWrapDXGIFactory2 *pFactoryWrapper)
...
HRESULT STDMETHODCALLTYPE CWrapDXGIAdapter::QueryInterface(REFIID riid,
                                                           void **ppvObj) {
  if (!ppvObj)
    return E_POINTER;
...
HRESULT STDMETHODCALLTYPE CWrapDXGIAdapter::SetPrivateData(REFGUID Name,
                                                           UINT DataSize,
                                                           const void *pData) {
  return m_pReal->SetPrivateData(Name, DataSize, pData);
}

HRESULT STDMETHODCALLTYPE CWrapDXGIAdapter::SetPrivateDataInterface(
    REFGUID Name, const IUnknown *pUnknown) {
  return m_pReal->SetPrivateDataInterface(Name, pUnknown);
}

HRESULT STDMETHODCALLTYPE CWrapDXGIAdapter::GetPrivateData(REFGUID Name,
                                                           UINT *pDataSize,
                                                           void *pData) {
  return m_pReal->GetPrivateData(Name, pDataSize, pData);
}

HRESULT STDMETHODCALLTYPE CWrapDXGIAdapter::GetParent(REFIID riid,
                                                      void **ppParent) {
...
CWrapDXGIAdapter::EnumOutputs(UINT Output, IDXGIOutput **ppOutput) {
  ...
  if (SUCCEEDED(hr) && pRealOutput) {
    *ppOutput = new CWrapDXGIOutput(pRealOutput, this);
    ...
    WrapperLog("DXGI Adapter: EnumOutputs(%u) -> Wrapped=%p, Real=%p", Output,
...
HRESULT STDMETHODCALLTYPE CWrapDXGIAdapter::GetDesc(DXGI_ADAPTER_DESC *pDesc) {
  ...
  if (SUCCEEDED(hr) && pDesc) {
    WrapperLog(
        "DXGI Adapter: GetDesc - VRAM: %llu MB, Shared: %llu MB, Name: %S",
        pDesc->DedicatedVideoMemory / (1024 * 1024),
        pDesc->SharedSystemMemory / (1024 * 1024), pDesc->Description);
  } else {
...
HRESULT STDMETHODCALLTYPE CWrapDXGIAdapter::CheckInterfaceSupport(
...
HRESULT STDMETHODCALLTYPE
CWrapDXGIAdapter::GetDesc1(DXGI_ADAPTER_DESC1 *pDesc) {
  if (!m_pReal1)
...
HRESULT STDMETHODCALLTYPE
CWrapDXGIAdapter::GetDesc2(DXGI_ADAPTER_DESC2 *pDesc) {
  if (!m_pReal2)
    return DXGI_ERROR_UNSUPPORTED;
  // FIX: Pass through to real adapter without VRAM override
...
HRESULT STDMETHODCALLTYPE
CWrapDXGIAdapter::RegisterHardwareContentProtectionTeardownStatusEvent(
    HANDLE hEvent, DWORD *pdwCookie) {
  if (!m_pReal3)
    return DXGI_ERROR_UNSUPPORTED;
  return m_pReal3->RegisterHardwareContentProtectionTeardownStatusEvent(
      hEvent, pdwCookie);
}

void STDMETHODCALLTYPE
CWrapDXGIAdapter::UnregisterHardwareContentProtectionTeardownStatus(
    DWORD dwCookie) {
  if (m_pReal3)
    m_pReal3->UnregisterHardwareContentProtectionTeardownStatus(dwCookie);
}

HRESULT STDMETHODCALLTYPE CWrapDXGIAdapter::QueryVideoMemoryInfo(
    UINT NodeIndex, DXGI_MEMORY_SEGMENT_GROUP MemorySegmentGroup,
    DXGI_QUERY_VIDEO_MEMORY_INFO *pVideoMemoryInfo) {
  if (!m_pReal3)
    return DXGI_ERROR_UNSUPPORTED;
  // FIX: Pass through to real adapter without VRAM override
  HRESULT hr = m_pReal3->QueryVideoMemoryInfo(NodeIndex, MemorySegmentGroup,
                                              pVideoMemoryInfo);
...
HRESULT STDMETHODCALLTYPE CWrapDXGIAdapter::SetVideoMemoryReservation(
    UINT NodeIndex, DXGI_MEMORY_SEGMENT_GROUP MemorySegmentGroup,
    UINT64 Reservation) {
  if (!m_pReal3)
    return DXGI_ERROR_UNSUPPORTED;
  return m_pReal3->SetVideoMemoryReservation(NodeIndex, MemorySegmentGroup,
                                             Reservation);
}

HRESULT STDMETHODCALLTYPE
CWrapDXGIAdapter::RegisterVideoMemoryBudgetChangeNotificationEvent(
    HANDLE hEvent, DWORD *pdwCookie) {
  if (!m_pReal3)
    return DXGI_ERROR_UNSUPPORTED;
  return m_pReal3->RegisterVideoMemoryBudgetChangeNotificationEvent(hEvent,
                                                                    pdwCookie);
}

void STDMETHODCALLTYPE
CWrapDXGIAdapter::UnregisterVideoMemoryBudgetChangeNotification(
    DWORD dwCookie) {
  if (m_pReal3)
...
HRESULT STDMETHODCALLTYPE
CWrapDXGIAdapter::GetDesc3(DXGI_ADAPTER_DESC3 *pDesc) {
  if (!m_pReal4)
    return DXGI_ERROR_UNSUPPORTED;
