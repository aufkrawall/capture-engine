 ...
 *
 * Wraps ID3D11Device to intercept CreateSamplerState for AF/mip overrides.
 */
...
/**
 * CWrapD3D11Device - Wraps ID3D11Device through ID3D11Device5
 *
 * Key interceptions:
 * - CreateSamplerState: Apply AF/mip LOD bias overrides
 */
class CWrapD3D11Device : public ID3D11Device5 {
public:
  CWrapD3D11Device(ID3D11Device *pReal);
  virtual ~CWrapD3D11Device();

  ID3D11Device *GetReal() const { return m_pReal; }
  ...
  HRESULT STDMETHODCALLTYPE CreateDeferredContext(
      UINT ContextFlags, ID3D11DeviceContext **ppDeferredContext) override;
  ...
  HRESULT STDMETHODCALLTYPE CreateDeferredContext1(
      UINT ContextFlags, ID3D11DeviceContext1 **ppDeferredContext) override;
  HRESULT STDMETHODCALLTYPE
  ...
  HRESULT STDMETHODCALLTYPE CreateDeferredContext2(
      UINT ContextFlags, ID3D11DeviceContext2 **ppDeferredContext) override;
  ...
  CheckMultisampleQualityLevels1(DXGI_FORMAT Format, UINT SampleCount,
                                 UINT Flags, UINT *pNumQualityLevels) override;
  ...
  HRESULT STDMETHODCALLTYPE CreateQuery1(const D3D11_QUERY_DESC1 *pQueryDesc1,
                                         ID3D11Query1 **ppQuery1) override;
  void STDMETHODCALLTYPE
  ...
  void STDMETHODCALLTYPE WriteToSubresource(ID3D11Resource *pDstResource,
                                            UINT DstSubresource,
                                            const D3D11_BOX *pDstBox,
                                            const void *pSrcData,
                                            UINT SrcRowPitch,
                                            UINT SrcDepthPitch) override;
  void STDMETHODCALLTYPE ReadFromSubresource(void *pDstData, UINT DstRowPitch,
                                             UINT DstDepthPitch,
  ...
  // ========================================================================
  HRESULT STDMETHODCALLTYPE
  RegisterDeviceRemovedEvent(HANDLE hEvent, DWORD *pdwCookie) override;
  void STDMETHODCALLTYPE UnregisterDeviceRemoved(DWORD dwCookie) override;

  // ========================================================================
  // ID3D11Device5
  // ========================================================================
  HRESULT STDMETHODCALLTYPE OpenSharedFence(HANDLE hFence,
                                            REFIID ReturnedInterface,
...
private:
  ...
  ID3D11Device2 *m_pReal2;
  ID3D11Device3 *m_pReal3;
  ID3D11Device4 *m_pReal4;
  ID3D11Device5 *m_pReal5;
  LONG m_RefCount;
  int m_Version;

  // Wrapped immediate context (created on demand)
  class CWrapD3D11DeviceContext *m_pWrappedContext;

  void PromoteInterfaces();

  // Apply AF/mip override to sampler desc
  void ApplySamplerOverrides(D3D11_SAMPLER_DESC *pDesc);
};
