...
CWrapD3D9Device::CWrapD3D9Device(IDirect3DDevice9 *pReal, bool isEx)
    : m_pReal(pReal), m_pRealEx(nullptr), m_RefCount(1), m_IsEx(isEx),
      m_MaxAnisotropy(16), m_ForceVSync(false) {
  if (pReal) {
    pReal->AddRef();
    if (isEx) {
...
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::QueryInterface(REFIID riid,
                                                          void **ppvObj) {
  if (!ppvObj)
    return E_POINTER;
...
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::Present(
  ...
  HRESULT hr = m_pReal->Present(pSourceRect, pDestRect, hDestWindowOverride,
                                pDirtyRegion);
...
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::PresentEx(
    const RECT *pSourceRect, const RECT *pDestRect, HWND hDestWindowOverride,
  ...
  HRESULT hr = m_pRealEx->PresentEx(pSourceRect, pDestRect, hDestWindowOverride,
                                    pDirtyRegion, flags);
...
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::SetSamplerState(
...
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::GetSamplerState(
...
CWrapD3D9Device::GetDisplayMode(UINT iSwapChain, D3DDISPLAYMODE *pMode) {
  return m_pReal->GetDisplayMode(iSwapChain, pMode);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::GetCreationParameters(
    D3DDEVICE_CREATION_PARAMETERS *pParameters) {
  return m_pReal->GetCreationParameters(pParameters);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::SetCursorProperties(
...
void STDMETHODCALLTYPE CWrapD3D9Device::SetCursorPosition(int X, int Y,
                                                          DWORD Flags) {
...
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::CreateAdditionalSwapChain(
    D3DPRESENT_PARAMETERS *pPresentationParameters,
    IDirect3DSwapChain9 **pSwapChain) {
  return m_pReal->CreateAdditionalSwapChain(pPresentationParameters,
                                            pSwapChain);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::GetSwapChain(
...
HRESULT STDMETHODCALLTYPE
CWrapD3D9Device::Reset(D3DPRESENT_PARAMETERS *pPresentationParameters) {
  return m_pReal->Reset(pPresentationParameters);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::GetBackBuffer(
...
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::CreateVolumeTexture(
  ...
  return m_pReal->CreateVolumeTexture(Width, Height, Depth, Levels, Usage,
                                      Format, Pool, ppVolumeTexture,
                                      pSharedHandle);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::CreateCubeTexture(
    UINT EdgeLength, UINT Levels, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool,
    IDirect3DCubeTexture9 **ppCubeTexture, HANDLE *pSharedHandle) {
  return m_pReal->CreateCubeTexture(EdgeLength, Levels, Usage, Format, Pool,
                                    ppCubeTexture, pSharedHandle);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::CreateVertexBuffer(
    UINT Length, DWORD Usage, DWORD FVF, D3DPOOL Pool,
    IDirect3DVertexBuffer9 **ppVertexBuffer, HANDLE *pSharedHandle) {
  return m_pReal->CreateVertexBuffer(Length, Usage, FVF, Pool, ppVertexBuffer,
                                     pSharedHandle);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::CreateIndexBuffer(
    UINT Length, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool,
    IDirect3DIndexBuffer9 **ppIndexBuffer, HANDLE *pSharedHandle) {
  return m_pReal->CreateIndexBuffer(Length, Usage, Format, Pool, ppIndexBuffer,
                                    pSharedHandle);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::CreateRenderTarget(
    ...
    HANDLE *pSharedHandle) {
  return m_pReal->CreateRenderTarget(Width, Height, Format, MultiSample,
                                     MultisampleQuality, Lockable, ppSurface,
                                     pSharedHandle);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::CreateDepthStencilSurface(
    UINT Width, UINT Height, D3DFORMAT Format, D3DMULTISAMPLE_TYPE MultiSample,
    DWORD MultisampleQuality, BOOL Discard, IDirect3DSurface9 **ppSurface,
    HANDLE *pSharedHandle) {
  return m_pReal->CreateDepthStencilSurface(Width, Height, Format, MultiSample,
                                            MultisampleQuality, Discard,
                                            ppSurface, pSharedHandle);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::UpdateSurface(
  ...
  return m_pReal->UpdateSurface(pSourceSurface, pSourceRect,
                                pDestinationSurface, pDestinationPoint);
}
HRESULT STDMETHODCALLTYPE
CWrapD3D9Device::UpdateTexture(IDirect3DBaseTexture9 *pSourceTexture,
                               IDirect3DBaseTexture9 *pDestinationTexture) {
  return m_pReal->UpdateTexture(pSourceTexture, pDestinationTexture);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::GetRenderTargetData(
...
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::GetFrontBufferData(
    UINT iSwapChain, IDirect3DSurface9 *pDestSurface) {
  return m_pReal->GetFrontBufferData(iSwapChain, pDestSurface);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::StretchRect(
    IDirect3DSurface9 *pSourceSurface, const RECT *pSourceRect,
    IDirect3DSurface9 *pDestSurface, const RECT *pDestRect,
    D3DTEXTUREFILTERTYPE Filter) {
  return m_pReal->StretchRect(pSourceSurface, pSourceRect, pDestSurface,
                              pDestRect, Filter);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::ColorFill(
    IDirect3DSurface9 *pSurface, const RECT *pRect, D3DCOLOR color) {
  return m_pReal->ColorFill(pSurface, pRect, color);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::CreateOffscreenPlainSurface(
    UINT Width, UINT Height, D3DFORMAT Format, D3DPOOL Pool,
    IDirect3DSurface9 **ppSurface, HANDLE *pSharedHandle) {
  return m_pReal->CreateOffscreenPlainSurface(Width, Height, Format, Pool,
                                              ppSurface, pSharedHandle);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::SetRenderTarget(
    DWORD RenderTargetIndex, IDirect3DSurface9 *pRenderTarget) {
  return m_pReal->SetRenderTarget(RenderTargetIndex, pRenderTarget);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::GetRenderTarget(
    DWORD RenderTargetIndex, IDirect3DSurface9 **ppRenderTarget) {
  return m_pReal->GetRenderTarget(RenderTargetIndex, ppRenderTarget);
}
HRESULT STDMETHODCALLTYPE
CWrapD3D9Device::SetDepthStencilSurface(IDirect3DSurface9 *pNewZStencil) {
  return m_pReal->SetDepthStencilSurface(pNewZStencil);
}
HRESULT STDMETHODCALLTYPE
CWrapD3D9Device::GetDepthStencilSurface(IDirect3DSurface9 **ppZStencilSurface) {
...
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::Clear(DWORD Count,
                                                 const D3DRECT *pRects,
                                                 DWORD Flags, D3DCOLOR Color,
                                                 float Z, DWORD Stencil) {
  return m_pReal->Clear(Count, pRects, Flags, Color, Z, Stencil);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::SetTransform(
    D3DTRANSFORMSTATETYPE State, const D3DMATRIX *pMatrix) {
  return m_pReal->SetTransform(State, pMatrix);
}
HRESULT STDMETHODCALLTYPE
CWrapD3D9Device::GetTransform(D3DTRANSFORMSTATETYPE State, D3DMATRIX *pMatrix) {
  return m_pReal->GetTransform(State, pMatrix);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::MultiplyTransform(
    D3DTRANSFORMSTATETYPE State, const D3DMATRIX *pMatrix) {
  return m_pReal->MultiplyTransform(State, pMatrix);
}
HRESULT STDMETHODCALLTYPE
CWrapD3D9Device::SetViewport(const D3DVIEWPORT9 *pViewport) {
  return m_pReal->SetViewport(pViewport);
}
HRESULT STDMETHODCALLTYPE
CWrapD3D9Device::GetViewport(D3DVIEWPORT9 *pViewport) {
  return m_pReal->GetViewport(pViewport);
}
HRESULT STDMETHODCALLTYPE
CWrapD3D9Device::SetMaterial(const D3DMATERIAL9 *pMaterial) {
  return m_pReal->SetMaterial(pMaterial);
}
HRESULT STDMETHODCALLTYPE
CWrapD3D9Device::GetMaterial(D3DMATERIAL9 *pMaterial) {
  return m_pReal->GetMaterial(pMaterial);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::SetLight(DWORD Index,
                                                    const D3DLIGHT9 *pLight) {
  return m_pReal->SetLight(Index, pLight);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::GetLight(DWORD Index,
                                                    D3DLIGHT9 *pLight) {
  return m_pReal->GetLight(Index, pLight);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::LightEnable(DWORD Index,
                                                       BOOL Enable) {
  return m_pReal->LightEnable(Index, Enable);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::GetLightEnable(DWORD Index,
                                                          BOOL *pEnable) {
  return m_pReal->GetLightEnable(Index, pEnable);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::SetClipPlane(DWORD Index,
                                                        const float *pPlane) {
  return m_pReal->SetClipPlane(Index, pPlane);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::GetClipPlane(DWORD Index,
                                                        float *pPlane) {
  return m_pReal->GetClipPlane(Index, pPlane);
}
HRESULT STDMETHODCALLTYPE
CWrapD3D9Device::SetRenderState(D3DRENDERSTATETYPE State, DWORD Value) {
  return m_pReal->SetRenderState(State, Value);
}
HRESULT STDMETHODCALLTYPE
CWrapD3D9Device::GetRenderState(D3DRENDERSTATETYPE State, DWORD *pValue) {
  return m_pReal->GetRenderState(State, pValue);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::CreateStateBlock(
...
HRESULT STDMETHODCALLTYPE
CWrapD3D9Device::EndStateBlock(IDirect3DStateBlock9 **ppSB) {
  return m_pReal->EndStateBlock(ppSB);
}
HRESULT STDMETHODCALLTYPE
CWrapD3D9Device::SetClipStatus(const D3DCLIPSTATUS9 *pClipStatus) {
  return m_pReal->SetClipStatus(pClipStatus);
}
HRESULT STDMETHODCALLTYPE
CWrapD3D9Device::GetClipStatus(D3DCLIPSTATUS9 *pClipStatus) {
  return m_pReal->GetClipStatus(pClipStatus);
}
HRESULT STDMETHODCALLTYPE
CWrapD3D9Device::GetTexture(DWORD Stage, IDirect3DBaseTexture9 **ppTexture) {
  return m_pReal->GetTexture(Stage, ppTexture);
}
HRESULT STDMETHODCALLTYPE
CWrapD3D9Device::SetTexture(DWORD Stage, IDirect3DBaseTexture9 *pTexture) {
  return m_pReal->SetTexture(Stage, pTexture);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::GetTextureStageState(
    DWORD Stage, D3DTEXTURESTAGESTATETYPE Type, DWORD *pValue) {
  return m_pReal->GetTextureStageState(Stage, Type, pValue);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::SetTextureStageState(
...
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::SetPaletteEntries(
    UINT PaletteNumber, const PALETTEENTRY *pEntries) {
  return m_pReal->SetPaletteEntries(PaletteNumber, pEntries);
}
HRESULT STDMETHODCALLTYPE
CWrapD3D9Device::GetPaletteEntries(UINT PaletteNumber, PALETTEENTRY *pEntries) {
  return m_pReal->GetPaletteEntries(PaletteNumber, pEntries);
}
HRESULT STDMETHODCALLTYPE
CWrapD3D9Device::SetCurrentTexturePalette(UINT PaletteNumber) {
  return m_pReal->SetCurrentTexturePalette(PaletteNumber);
}
HRESULT STDMETHODCALLTYPE
CWrapD3D9Device::GetCurrentTexturePalette(UINT *PaletteNumber) {
...
HRESULT STDMETHODCALLTYPE
CWrapD3D9Device::SetSoftwareVertexProcessing(BOOL bSoftware) {
...
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::DrawPrimitive(
...
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::DrawIndexedPrimitive(
    D3DPRIMITIVETYPE PrimitiveType, INT BaseVertexIndex, UINT MinVertexIndex,
    UINT NumVertices, UINT startIndex, UINT primCount) {
  return m_pReal->DrawIndexedPrimitive(PrimitiveType, BaseVertexIndex,
                                       MinVertexIndex, NumVertices, startIndex,
                                       primCount);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::DrawPrimitiveUP(
    D3DPRIMITIVETYPE PrimitiveType, UINT PrimitiveCount,
    const void *pVertexStreamZeroData, UINT VertexStreamZeroStride) {
  return m_pReal->DrawPrimitiveUP(PrimitiveType, PrimitiveCount,
                                  pVertexStreamZeroData,
                                  VertexStreamZeroStride);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::DrawIndexedPrimitiveUP(
    ...
    const void *pVertexStreamZeroData, UINT VertexStreamZeroStride) {
  return m_pReal->DrawIndexedPrimitiveUP(
      PrimitiveType, MinVertexIndex, NumVertices, PrimitiveCount, pIndexData,
      IndexDataFormat, pVertexStreamZeroData, VertexStreamZeroStride);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::ProcessVertices(
    UINT SrcStartIndex, UINT DestIndex, UINT VertexCount,
    IDirect3DVertexBuffer9 *pDestBuffer,
    IDirect3DVertexDeclaration9 *pVertexDecl, DWORD Flags) {
  return m_pReal->ProcessVertices(SrcStartIndex, DestIndex, VertexCount,
                                  pDestBuffer, pVertexDecl, Flags);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::CreateVertexDeclaration(
    const D3DVERTEXELEMENT9 *pVertexElements,
    IDirect3DVertexDeclaration9 **ppDecl) {
  return m_pReal->CreateVertexDeclaration(pVertexElements, ppDecl);
}
HRESULT STDMETHODCALLTYPE
CWrapD3D9Device::SetVertexDeclaration(IDirect3DVertexDeclaration9 *pDecl) {
  return m_pReal->SetVertexDeclaration(pDecl);
}
HRESULT STDMETHODCALLTYPE
CWrapD3D9Device::GetVertexDeclaration(IDirect3DVertexDeclaration9 **ppDecl) {
...
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::CreateVertexShader(
    const DWORD *pFunction, IDirect3DVertexShader9 **ppShader) {
  return m_pReal->CreateVertexShader(pFunction, ppShader);
}
HRESULT STDMETHODCALLTYPE
CWrapD3D9Device::SetVertexShader(IDirect3DVertexShader9 *pShader) {
  return m_pReal->SetVertexShader(pShader);
}
HRESULT STDMETHODCALLTYPE
CWrapD3D9Device::GetVertexShader(IDirect3DVertexShader9 **ppShader) {
  return m_pReal->GetVertexShader(ppShader);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::SetVertexShaderConstantF(
    UINT StartRegister, const float *pConstantData, UINT Vector4fCount) {
  return m_pReal->SetVertexShaderConstantF(StartRegister, pConstantData,
                                           Vector4fCount);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::GetVertexShaderConstantF(
    UINT StartRegister, float *pConstantData, UINT Vector4fCount) {
  return m_pReal->GetVertexShaderConstantF(StartRegister, pConstantData,
                                           Vector4fCount);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::SetVertexShaderConstantI(
    UINT StartRegister, const int *pConstantData, UINT Vector4iCount) {
  return m_pReal->SetVertexShaderConstantI(StartRegister, pConstantData,
                                           Vector4iCount);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::GetVertexShaderConstantI(
    UINT StartRegister, int *pConstantData, UINT Vector4iCount) {
  return m_pReal->GetVertexShaderConstantI(StartRegister, pConstantData,
                                           Vector4iCount);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::SetVertexShaderConstantB(
    UINT StartRegister, const BOOL *pConstantData, UINT BoolCount) {
  return m_pReal->SetVertexShaderConstantB(StartRegister, pConstantData,
                                           BoolCount);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::GetVertexShaderConstantB(
    UINT StartRegister, BOOL *pConstantData, UINT BoolCount) {
  return m_pReal->GetVertexShaderConstantB(StartRegister, pConstantData,
                                           BoolCount);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::SetStreamSource(
    UINT StreamNumber, IDirect3DVertexBuffer9 *pStreamData, UINT OffsetInBytes,
    UINT Stride) {
  return m_pReal->SetStreamSource(StreamNumber, pStreamData, OffsetInBytes,
                                  Stride);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::GetStreamSource(
    UINT StreamNumber, IDirect3DVertexBuffer9 **ppStreamData,
    UINT *OffsetInBytes, UINT *pStride) {
  return m_pReal->GetStreamSource(StreamNumber, ppStreamData, OffsetInBytes,
                                  pStride);
}
HRESULT STDMETHODCALLTYPE
CWrapD3D9Device::SetStreamSourceFreq(UINT StreamNumber, UINT Divider) {
  return m_pReal->SetStreamSourceFreq(StreamNumber, Divider);
}
HRESULT STDMETHODCALLTYPE
CWrapD3D9Device::GetStreamSourceFreq(UINT StreamNumber, UINT *Divider) {
  return m_pReal->GetStreamSourceFreq(StreamNumber, Divider);
}
HRESULT STDMETHODCALLTYPE
CWrapD3D9Device::SetIndices(IDirect3DIndexBuffer9 *pIndexData) {
  return m_pReal->SetIndices(pIndexData);
}
HRESULT STDMETHODCALLTYPE
CWrapD3D9Device::GetIndices(IDirect3DIndexBuffer9 **ppIndexData) {
  return m_pReal->GetIndices(ppIndexData);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::CreatePixelShader(
    const DWORD *pFunction, IDirect3DPixelShader9 **ppShader) {
  return m_pReal->CreatePixelShader(pFunction, ppShader);
}
HRESULT STDMETHODCALLTYPE
CWrapD3D9Device::SetPixelShader(IDirect3DPixelShader9 *pShader) {
  return m_pReal->SetPixelShader(pShader);
}
HRESULT STDMETHODCALLTYPE
CWrapD3D9Device::GetPixelShader(IDirect3DPixelShader9 **ppShader) {
  return m_pReal->GetPixelShader(ppShader);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::SetPixelShaderConstantF(
    UINT StartRegister, const float *pConstantData, UINT Vector4fCount) {
  return m_pReal->SetPixelShaderConstantF(StartRegister, pConstantData,
                                          Vector4fCount);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::GetPixelShaderConstantF(
    UINT StartRegister, float *pConstantData, UINT Vector4fCount) {
  return m_pReal->GetPixelShaderConstantF(StartRegister, pConstantData,
                                          Vector4fCount);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::SetPixelShaderConstantI(
    UINT StartRegister, const int *pConstantData, UINT Vector4iCount) {
  return m_pReal->SetPixelShaderConstantI(StartRegister, pConstantData,
                                          Vector4iCount);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::GetPixelShaderConstantI(
    UINT StartRegister, int *pConstantData, UINT Vector4iCount) {
  return m_pReal->GetPixelShaderConstantI(StartRegister, pConstantData,
                                          Vector4iCount);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::SetPixelShaderConstantB(
    UINT StartRegister, const BOOL *pConstantData, UINT BoolCount) {
  return m_pReal->SetPixelShaderConstantB(StartRegister, pConstantData,
                                          BoolCount);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::GetPixelShaderConstantB(
    UINT StartRegister, BOOL *pConstantData, UINT BoolCount) {
  return m_pReal->GetPixelShaderConstantB(StartRegister, pConstantData,
                                          BoolCount);
}
HRESULT STDMETHODCALLTYPE
CWrapD3D9Device::DrawRectPatch(UINT Handle, const float *pNumSegs,
                               const D3DRECTPATCH_INFO *pRectPatchInfo) {
  return m_pReal->DrawRectPatch(Handle, pNumSegs, pRectPatchInfo);
}
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::DrawTriPatch(
...
HRESULT STDMETHODCALLTYPE
CWrapD3D9Device::CreateQuery(D3DQUERYTYPE Type, IDirect3DQuery9 **ppQuery) {
...
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::SetConvolutionMonoKernel(
...
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::ComposeRects(
  ...
  if (!m_pRealEx)
    return E_NOTIMPL;
  return m_pRealEx->ComposeRects(pSrc, pDst, pSrcRectDescs, NumRects,
                                 pDstRectDescs, Operation, Xoffset, Yoffset);
}

HRESULT STDMETHODCALLTYPE
CWrapD3D9Device::GetGPUThreadPriority(INT *pPriority) {
  if (!m_pRealEx)
    return E_NOTIMPL;
...
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::CheckResourceResidency(
...
HRESULT STDMETHODCALLTYPE
CWrapD3D9Device::GetMaximumFrameLatency(UINT *pMaxLatency) {
  if (!m_pRealEx)
    return E_NOTIMPL;
...
HRESULT STDMETHODCALLTYPE
CWrapD3D9Device::CheckDeviceState(HWND hDestinationWindow) {
  if (!m_pRealEx)
    return E_NOTIMPL;
  return m_pRealEx->CheckDeviceState(hDestinationWindow);
}

HRESULT STDMETHODCALLTYPE CWrapD3D9Device::CreateRenderTargetEx(
    UINT Width, UINT Height, D3DFORMAT Format, D3DMULTISAMPLE_TYPE MultiSample,
    DWORD MultisampleQuality, BOOL Lockable, IDirect3DSurface9 **ppSurface,
    HANDLE *pSharedHandle, DWORD Usage) {
  if (!m_pRealEx)
    return E_NOTIMPL;
  return m_pRealEx->CreateRenderTargetEx(Width, Height, Format, MultiSample,
                                         MultisampleQuality, Lockable,
                                         ppSurface, pSharedHandle, Usage);
}

HRESULT STDMETHODCALLTYPE CWrapD3D9Device::CreateOffscreenPlainSurfaceEx(
    UINT Width, UINT Height, D3DFORMAT Format, D3DPOOL Pool,
    IDirect3DSurface9 **ppSurface, HANDLE *pSharedHandle, DWORD Usage) {
  if (!m_pRealEx)
    return E_NOTIMPL;
  return m_pRealEx->CreateOffscreenPlainSurfaceEx(
      Width, Height, Format, Pool, ppSurface, pSharedHandle, Usage);
}

HRESULT STDMETHODCALLTYPE CWrapD3D9Device::CreateDepthStencilSurfaceEx(
    UINT Width, UINT Height, D3DFORMAT Format, D3DMULTISAMPLE_TYPE MultiSample,
    DWORD MultisampleQuality, BOOL Discard, IDirect3DSurface9 **ppSurface,
    HANDLE *pSharedHandle, DWORD Usage) {
  if (!m_pRealEx)
    return E_NOTIMPL;
  return m_pRealEx->CreateDepthStencilSurfaceEx(
      Width, Height, Format, MultiSample, MultisampleQuality, Discard,
      ppSurface, pSharedHandle, Usage);
}

HRESULT STDMETHODCALLTYPE
CWrapD3D9Device::ResetEx(D3DPRESENT_PARAMETERS *pPresentationParameters,
...
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::GetDisplayModeEx(
