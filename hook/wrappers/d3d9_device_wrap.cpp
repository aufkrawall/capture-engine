...
CWrapD3D9Device::CWrapD3D9Device(IDirect3DDevice9* pReal, bool isEx)
    : m_pReal(pReal), m_pRealEx(nullptr), m_RefCount(1), m_IsEx(isEx), m_MaxAnisotropy(16), m_ForceVSync(false)
{
    if (pReal) {
        pReal->AddRef();
        if (isEx) {
...
ULONG STDMETHODCALLTYPE CWrapD3D9Device::AddRef() { return InterlockedIncrement(&m_RefCount); }
...
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::GetRenderTarget(DWORD RenderTargetIndex, IDirect3DSurface9** ppRenderTarget)
{
...
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::SetTextureStageState(DWORD Stage, D3DTEXTURESTAGESTATETYPE Type, DWORD Value)
{
...
HRESULT STDMETHODCALLTYPE CWrapD3D9Device::CreateVertexShader(const DWORD* pFunction, IDirect3DVertexShader9** ppShader)
{
