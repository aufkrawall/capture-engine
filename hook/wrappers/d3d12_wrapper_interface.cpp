...
// Helper to check config (basic implementation since we don't link full
// hook_common)
static bool IsDebugLoggingEnabled() {
  ...
  if (!s_Checked) {
    ...
    s_Enabled = GetPrivateProfileIntA("Capture", "DebugLogging", 0,
                                      ".\\config.ini") != 0;
    s_Checked = true;
  }
  return s_Enabled;
...
// Wrapper GUID for identification - use same GUID as wrapper_base.h for
// consistency {C3D4E5F6-7890-ABCD-EF12-345678901234}
...
D3D12_EXPORT HRESULT WINAPI D3D12Wrapper_CreateDevice(
  ...
  HRESULT hr = D3D12CreateDevice(pAdapter, MinimumFeatureLevel,
...
D3D12Wrapper_UnwrapDevice(ID3D12Device *pDevice) {
  ...
  if (SUCCEEDED(pDevice->QueryInterface(IID_CWrapD3D12Device, &pWrapper))) {
    CWrapD3D12Device *pReal =
        static_cast<CWrapD3D12Device *>((IUnknown *)pWrapper);
...
D3D12_EXPORT ID3D12CommandQueue *WINAPI D3D12Wrapper_WrapCommandQueue(
  ...
  // Note: We pass nullptr for device since the C interface doesn't have access
  // to CWrapD3D12Device The command queue wrapper will work but won't have a
  // back-reference to the wrapper device
  CWrapD3D12CommandQueue *pWrapper =
      new CWrapD3D12CommandQueue(pRealQueue, nullptr);
  return pWrapper;
}
