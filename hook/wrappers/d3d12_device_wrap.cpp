...
CWrapD3D12Device::CWrapD3D12Device(ID3D12Device *pReal)
    : m_pReal(pReal), m_pReal1(nullptr), m_pReal2(nullptr), m_pReal3(nullptr),
      m_pReal4(nullptr), m_pReal5(nullptr), m_pReal6(nullptr),
      m_pReal7(nullptr), m_RefCount(1), m_Version(0) {
  if (pReal) {
    pReal->AddRef();
    PromoteInterfaces();
  }
  WrapperLog("D3D12 Device Wrapper: Created (real=%p, version=%d)", pReal,
             m_Version);
}
...
HRESULT STDMETHODCALLTYPE CWrapD3D12Device::QueryInterface(REFIID riid,
                                                           void **ppvObj) {
  if (!ppvObj)
    return E_POINTER;
...
ULONG STDMETHODCALLTYPE CWrapD3D12Device::AddRef() {
...
HRESULT STDMETHODCALLTYPE CWrapD3D12Device::GetPrivateData(REFGUID guid,
                                                           UINT *pDataSize,
                                                           void *pData) {
  return m_pReal->GetPrivateData(guid, pDataSize, pData);
}

HRESULT STDMETHODCALLTYPE CWrapD3D12Device::SetPrivateData(REFGUID guid,
                                                           UINT DataSize,
                                                           const void *pData) {
...
CWrapD3D12Device::SetPrivateDataInterface(REFGUID guid, const IUnknown *pData) {
  return m_pReal->SetPrivateDataInterface(guid, pData);
}

HRESULT STDMETHODCALLTYPE CWrapD3D12Device::SetName(LPCWSTR Name) {
...
UINT STDMETHODCALLTYPE CWrapD3D12Device::GetNodeCount() {
  return m_pReal->GetNodeCount();
}

HRESULT STDMETHODCALLTYPE CWrapD3D12Device::CreateCommandQueue(
    const D3D12_COMMAND_QUEUE_DESC *pDesc, REFIID riid, void **ppCommandQueue) {
  if (!ppCommandQueue)
    return E_POINTER;

  WrapperLog("D3D12 Device: CreateCommandQueue (Type=%d)",
             pDesc ? pDesc->Type : -1);
  ...
  if (SUCCEEDED(hr) && pRealQueue) {
    ...
    if (SUCCEEDED(hr)) {
      WrapperLog("D3D12 Device: Created wrapped CommandQueue (wrapper=%p)",
                 *ppCommandQueue);
    } else {
      WrapperLog(
          "D3D12 Device: Failed to query interface for CommandQueue, hr=0x%08X",
          hr);
    }
  }
...
HRESULT STDMETHODCALLTYPE CWrapD3D12Device::CreateCommandAllocator(
    D3D12_COMMAND_LIST_TYPE type, REFIID riid, void **ppCmdAlloc) {
  return m_pReal->CreateCommandAllocator(type, riid, ppCmdAlloc);
}

HRESULT STDMETHODCALLTYPE CWrapD3D12Device::CreateGraphicsPipelineState(
    const D3D12_GRAPHICS_PIPELINE_STATE_DESC *pDesc, REFIID riid,
    void **ppPSO) {
  return m_pReal->CreateGraphicsPipelineState(pDesc, riid, ppPSO);
}

HRESULT STDMETHODCALLTYPE CWrapD3D12Device::CreateComputePipelineState(
...
HRESULT STDMETHODCALLTYPE CWrapD3D12Device::CreateCommandList(
    UINT nodeMask, D3D12_COMMAND_LIST_TYPE type, ID3D12CommandAllocator *pAlloc,
    ID3D12PipelineState *pInitial, REFIID riid, void **ppCmdList) {
  return m_pReal->CreateCommandList(nodeMask, type, pAlloc, pInitial, riid,
                                    ppCmdList);
}

HRESULT STDMETHODCALLTYPE CWrapD3D12Device::CheckFeatureSupport(
    D3D12_FEATURE Feature, void *pData, UINT DataSize) {
  ...
  WrapperLog(
      "D3D12 Device: CheckFeatureSupport CALLED - Feature=%s (%d), DataSize=%u",
  ...
  if (Feature == D3D12_FEATURE_ARCHITECTURE1 && SUCCEEDED(hr) && pData &&
    ...
    D3D12_FEATURE_DATA_ARCHITECTURE1 *pArch =
        (D3D12_FEATURE_DATA_ARCHITECTURE1 *)pData;
    WrapperLog("D3D12 Device: ARCHITECTURE1 - UMA=%d, CacheCoherentUMA=%d, "
               "IsolatedMMU=%d",
               pArch->UMA, pArch->CacheCoherentUMA, pArch->IsolatedMMU);
  }

  // Log GPU virtual address support which contains memory info
  if (Feature == D3D12_FEATURE_GPU_VIRTUAL_ADDRESS_SUPPORT && SUCCEEDED(hr) &&
      pData &&
      DataSize >= sizeof(D3D12_FEATURE_DATA_GPU_VIRTUAL_ADDRESS_SUPPORT)) {
    D3D12_FEATURE_DATA_GPU_VIRTUAL_ADDRESS_SUPPORT *pGpuVA =
        (D3D12_FEATURE_DATA_GPU_VIRTUAL_ADDRESS_SUPPORT *)pData;
    WrapperLog("D3D12 Device: GPU_VIRTUAL_ADDRESS_SUPPORT - "
               "MaxGPUVirtualAddressBitsPerResource=%u, "
               "MaxGPUVirtualAddressBitsPerProcess=%u",
               pGpuVA->MaxGPUVirtualAddressBitsPerResource,
               pGpuVA->MaxGPUVirtualAddressBitsPerProcess);
  }

  return hr;
}

HRESULT STDMETHODCALLTYPE CWrapD3D12Device::CreateDescriptorHeap(
    const D3D12_DESCRIPTOR_HEAP_DESC *pDesc, REFIID riid, void **ppHeap) {
  return m_pReal->CreateDescriptorHeap(pDesc, riid, ppHeap);
}

UINT STDMETHODCALLTYPE CWrapD3D12Device::GetDescriptorHandleIncrementSize(
    D3D12_DESCRIPTOR_HEAP_TYPE Type) {
  return m_pReal->GetDescriptorHandleIncrementSize(Type);
}

// Helper to modify static samplers in root signature blob
static std::vector<BYTE> ModifyRootSignatureStaticSamplers(const void *pBlob,
                                                           SIZE_T blobLen) {
  std::vector<BYTE> result;

  if (!pBlob || blobLen < sizeof(UINT))
    return result;
...
HRESULT STDMETHODCALLTYPE CWrapD3D12Device::CreateRootSignature(
    UINT nodeMask, const void *pBlob, SIZE_T blobLen, REFIID riid,
    void **ppRootSig) {
  ...
  if (!modifiedBlob.empty()) {
    return m_pReal->CreateRootSignature(nodeMask, modifiedBlob.data(),
                                        modifiedBlob.size(), riid, ppRootSig);
  }

  return m_pReal->CreateRootSignature(nodeMask, pBlob, blobLen, riid,
                                      ppRootSig);
}

void STDMETHODCALLTYPE CWrapD3D12Device::CreateConstantBufferView(
    const D3D12_CONSTANT_BUFFER_VIEW_DESC *pDesc,
    D3D12_CPU_DESCRIPTOR_HANDLE Dest) {
  m_pReal->CreateConstantBufferView(pDesc, Dest);
}

void STDMETHODCALLTYPE CWrapD3D12Device::CreateShaderResourceView(
    ID3D12Resource *pRes, const D3D12_SHADER_RESOURCE_VIEW_DESC *pDesc,
    D3D12_CPU_DESCRIPTOR_HANDLE Dest) {
  m_pReal->CreateShaderResourceView(pRes, pDesc, Dest);
}

void STDMETHODCALLTYPE CWrapD3D12Device::CreateUnorderedAccessView(
    ID3D12Resource *pRes, ID3D12Resource *pCounter,
    const D3D12_UNORDERED_ACCESS_VIEW_DESC *pDesc,
    D3D12_CPU_DESCRIPTOR_HANDLE Dest) {
  m_pReal->CreateUnorderedAccessView(pRes, pCounter, pDesc, Dest);
}

void STDMETHODCALLTYPE CWrapD3D12Device::CreateRenderTargetView(
    ID3D12Resource *pRes, const D3D12_RENDER_TARGET_VIEW_DESC *pDesc,
    D3D12_CPU_DESCRIPTOR_HANDLE Dest) {
  m_pReal->CreateRenderTargetView(pRes, pDesc, Dest);
}

void STDMETHODCALLTYPE CWrapD3D12Device::CreateDepthStencilView(
    ID3D12Resource *pRes, const D3D12_DEPTH_STENCIL_VIEW_DESC *pDesc,
    D3D12_CPU_DESCRIPTOR_HANDLE Dest) {
  m_pReal->CreateDepthStencilView(pRes, pDesc, Dest);
}

void STDMETHODCALLTYPE CWrapD3D12Device::CreateSampler(
    const D3D12_SAMPLER_DESC *pDesc, D3D12_CPU_DESCRIPTOR_HANDLE Dest) {
  if (pDesc) {
...
void STDMETHODCALLTYPE CWrapD3D12Device::CopyDescriptors(
    UINT NumDest, const D3D12_CPU_DESCRIPTOR_HANDLE *pDestStarts,
    const UINT *pDestSizes, UINT NumSrc,
    const D3D12_CPU_DESCRIPTOR_HANDLE *pSrcStarts, const UINT *pSrcSizes,
    D3D12_DESCRIPTOR_HEAP_TYPE Type) {
  m_pReal->CopyDescriptors(NumDest, pDestStarts, pDestSizes, NumSrc, pSrcStarts,
                           pSrcSizes, Type);
}

void STDMETHODCALLTYPE CWrapD3D12Device::CopyDescriptorsSimple(
    UINT Num, D3D12_CPU_DESCRIPTOR_HANDLE Dest, D3D12_CPU_DESCRIPTOR_HANDLE Src,
    D3D12_DESCRIPTOR_HEAP_TYPE Type) {
  m_pReal->CopyDescriptorsSimple(Num, Dest, Src, Type);
}

D3D12_RESOURCE_ALLOCATION_INFO STDMETHODCALLTYPE
CWrapD3D12Device::GetResourceAllocationInfo(UINT mask, UINT num,
                                            const D3D12_RESOURCE_DESC *pDescs) {
  return m_pReal->GetResourceAllocationInfo(mask, num, pDescs);
}

D3D12_HEAP_PROPERTIES STDMETHODCALLTYPE
CWrapD3D12Device::GetCustomHeapProperties(UINT nodeMask, D3D12_HEAP_TYPE type) {
  return m_pReal->GetCustomHeapProperties(nodeMask, type);
}

HRESULT STDMETHODCALLTYPE CWrapD3D12Device::CreateCommittedResource(
    const D3D12_HEAP_PROPERTIES *pHeap, D3D12_HEAP_FLAGS flags,
    const D3D12_RESOURCE_DESC *pDesc, D3D12_RESOURCE_STATES state,
    const D3D12_CLEAR_VALUE *pClear, REFIID riid, void **ppRes) {
  return m_pReal->CreateCommittedResource(pHeap, flags, pDesc, state, pClear,
                                          riid, ppRes);
}

HRESULT STDMETHODCALLTYPE CWrapD3D12Device::CreateHeap(
    const D3D12_HEAP_DESC *pDesc, REFIID riid, void **ppHeap) {
  return m_pReal->CreateHeap(pDesc, riid, ppHeap);
}

HRESULT STDMETHODCALLTYPE CWrapD3D12Device::CreatePlacedResource(
    ID3D12Heap *pHeap, UINT64 offset, const D3D12_RESOURCE_DESC *pDesc,
    D3D12_RESOURCE_STATES state, const D3D12_CLEAR_VALUE *pClear, REFIID riid,
    void **ppRes) {
  return m_pReal->CreatePlacedResource(pHeap, offset, pDesc, state, pClear,
                                       riid, ppRes);
}

HRESULT STDMETHODCALLTYPE CWrapD3D12Device::CreateReservedResource(
    const D3D12_RESOURCE_DESC *pDesc, D3D12_RESOURCE_STATES state,
    const D3D12_CLEAR_VALUE *pClear, REFIID riid, void **ppRes) {
  return m_pReal->CreateReservedResource(pDesc, state, pClear, riid, ppRes);
}

HRESULT STDMETHODCALLTYPE CWrapD3D12Device::CreateSharedHandle(
    ID3D12DeviceChild *pObj, const SECURITY_ATTRIBUTES *pAttr, DWORD access,
    LPCWSTR name, HANDLE *pHandle) {
  return m_pReal->CreateSharedHandle(pObj, pAttr, access, name, pHandle);
}

HRESULT STDMETHODCALLTYPE CWrapD3D12Device::OpenSharedHandle(HANDLE handle,
                                                             REFIID riid,
                                                             void **ppObj) {
  return m_pReal->OpenSharedHandle(handle, riid, ppObj);
}

HRESULT STDMETHODCALLTYPE CWrapD3D12Device::OpenSharedHandleByName(
    LPCWSTR name, DWORD access, HANDLE *pHandle) {
  return m_pReal->OpenSharedHandleByName(name, access, pHandle);
}

HRESULT STDMETHODCALLTYPE
CWrapD3D12Device::MakeResident(UINT num, ID3D12Pageable *const *ppObjs) {
  return m_pReal->MakeResident(num, ppObjs);
}

HRESULT STDMETHODCALLTYPE
CWrapD3D12Device::Evict(UINT num, ID3D12Pageable *const *ppObjs) {
  return m_pReal->Evict(num, ppObjs);
}

HRESULT STDMETHODCALLTYPE CWrapD3D12Device::CreateFence(UINT64 val,
...
HRESULT STDMETHODCALLTYPE CWrapD3D12Device::GetDeviceRemovedReason() {
  return m_pReal->GetDeviceRemovedReason();
}

void STDMETHODCALLTYPE CWrapD3D12Device::GetCopyableFootprints(
    const D3D12_RESOURCE_DESC *pDesc, UINT first, UINT num, UINT64 base,
    D3D12_PLACED_SUBRESOURCE_FOOTPRINT *pLayouts, UINT *pRows,
    UINT64 *pRowSizes, UINT64 *pTotal) {
  m_pReal->GetCopyableFootprints(pDesc, first, num, base, pLayouts, pRows,
                                 pRowSizes, pTotal);
}

HRESULT STDMETHODCALLTYPE CWrapD3D12Device::CreateQueryHeap(
    const D3D12_QUERY_HEAP_DESC *pDesc, REFIID riid, void **ppHeap) {
...
HRESULT STDMETHODCALLTYPE CWrapD3D12Device::CreateCommandSignature(
    const D3D12_COMMAND_SIGNATURE_DESC *pDesc, ID3D12RootSignature *pRoot,
    REFIID riid, void **ppSig) {
  return m_pReal->CreateCommandSignature(pDesc, pRoot, riid, ppSig);
}

void STDMETHODCALLTYPE CWrapD3D12Device::GetResourceTiling(
    ID3D12Resource *pRes, UINT *pTiles, D3D12_PACKED_MIP_INFO *pPacked,
    D3D12_TILE_SHAPE *pShape, UINT *pNum, UINT first,
    D3D12_SUBRESOURCE_TILING *pTilings) {
  m_pReal->GetResourceTiling(pRes, pTiles, pPacked, pShape, pNum, first,
                             pTilings);
}

LUID STDMETHODCALLTYPE CWrapD3D12Device::GetAdapterLuid() {
  LUID luid = m_pReal->GetAdapterLuid();
  WrapperLog("D3D12 Device: GetAdapterLuid called - LUID: %08X:%08X",
...
HRESULT STDMETHODCALLTYPE CWrapD3D12Device::CreatePipelineLibrary(
    const void *pBlob, SIZE_T len, REFIID riid, void **ppLib) {
  if (!m_pReal1)
    return E_NOINTERFACE;
  return m_pReal1->CreatePipelineLibrary(pBlob, len, riid, ppLib);
}

HRESULT STDMETHODCALLTYPE CWrapD3D12Device::SetEventOnMultipleFenceCompletion(
    ID3D12Fence *const *ppFences, const UINT64 *pVals, UINT num,
    D3D12_MULTIPLE_FENCE_WAIT_FLAGS flags, HANDLE hEvent) {
  if (!m_pReal1)
    return E_NOINTERFACE;
  return m_pReal1->SetEventOnMultipleFenceCompletion(ppFences, pVals, num,
                                                     flags, hEvent);
}

HRESULT STDMETHODCALLTYPE CWrapD3D12Device::SetResidencyPriority(
    UINT num, ID3D12Pageable *const *ppObjs,
...
HRESULT STDMETHODCALLTYPE CWrapD3D12Device::CreatePipelineState(
    const D3D12_PIPELINE_STATE_STREAM_DESC *pDesc, REFIID riid, void **ppPSO) {
  if (!m_pReal2)
    return E_NOINTERFACE;
  return m_pReal2->CreatePipelineState(pDesc, riid, ppPSO);
}

HRESULT STDMETHODCALLTYPE CWrapD3D12Device::OpenExistingHeapFromAddress(
    const void *pAddr, REFIID riid, void **ppHeap) {
  if (!m_pReal3)
    return E_NOINTERFACE;
  return m_pReal3->OpenExistingHeapFromAddress(pAddr, riid, ppHeap);
}

HRESULT STDMETHODCALLTYPE CWrapD3D12Device::OpenExistingHeapFromFileMapping(
    HANDLE hFile, REFIID riid, void **ppHeap) {
  if (!m_pReal3)
    return E_NOINTERFACE;
  return m_pReal3->OpenExistingHeapFromFileMapping(hFile, riid, ppHeap);
}

HRESULT STDMETHODCALLTYPE CWrapD3D12Device::EnqueueMakeResident(
    D3D12_RESIDENCY_FLAGS flags, UINT num, ID3D12Pageable *const *ppObjs,
    ID3D12Fence *pFence, UINT64 val) {
  if (!m_pReal3)
    return E_NOINTERFACE;
  return m_pReal3->EnqueueMakeResident(flags, num, ppObjs, pFence, val);
}

HRESULT STDMETHODCALLTYPE CWrapD3D12Device::CreateCommandList1(
    UINT nodeMask, D3D12_COMMAND_LIST_TYPE type, D3D12_COMMAND_LIST_FLAGS flags,
...
HRESULT STDMETHODCALLTYPE CWrapD3D12Device::CreateProtectedResourceSession(
    const D3D12_PROTECTED_RESOURCE_SESSION_DESC *pDesc, REFIID riid,
    void **ppSession) {
  if (!m_pReal4)
    return E_NOINTERFACE;
...
HRESULT STDMETHODCALLTYPE CWrapD3D12Device::CreateCommittedResource1(
    const D3D12_HEAP_PROPERTIES *pHeap, D3D12_HEAP_FLAGS flags,
    const D3D12_RESOURCE_DESC *pDesc, D3D12_RESOURCE_STATES state,
    const D3D12_CLEAR_VALUE *pClear, ID3D12ProtectedResourceSession *pSession,
    REFIID riid, void **ppRes) {
  if (!m_pReal4)
    return E_NOINTERFACE;
  return m_pReal4->CreateCommittedResource1(pHeap, flags, pDesc, state, pClear,
                                            pSession, riid, ppRes);
}

HRESULT STDMETHODCALLTYPE CWrapD3D12Device::CreateHeap1(
    const D3D12_HEAP_DESC *pDesc, ID3D12ProtectedResourceSession *pSession,
...
HRESULT STDMETHODCALLTYPE CWrapD3D12Device::CreateReservedResource1(
    const D3D12_RESOURCE_DESC *pDesc, D3D12_RESOURCE_STATES state,
    const D3D12_CLEAR_VALUE *pClear, ID3D12ProtectedResourceSession *pSession,
    REFIID riid, void **ppRes) {
  if (!m_pReal4)
    return E_NOINTERFACE;
  return m_pReal4->CreateReservedResource1(pDesc, state, pClear, pSession, riid,
                                           ppRes);
}

D3D12_RESOURCE_ALLOCATION_INFO STDMETHODCALLTYPE
CWrapD3D12Device::GetResourceAllocationInfo1(
    UINT mask, UINT num, const D3D12_RESOURCE_DESC *pDescs,
    D3D12_RESOURCE_ALLOCATION_INFO1 *pInfo1) {
  if (!m_pReal4)
    return {};
...
// ID3D12Device5
HRESULT STDMETHODCALLTYPE CWrapD3D12Device::CreateLifetimeTracker(
    ID3D12LifetimeOwner *pOwner, REFIID riid, void **ppTracker) {
  if (!m_pReal5)
    return E_NOINTERFACE;
...
void STDMETHODCALLTYPE CWrapD3D12Device::RemoveDevice() {
  if (m_pReal5)
    m_pReal5->RemoveDevice();
}

HRESULT STDMETHODCALLTYPE CWrapD3D12Device::EnumerateMetaCommands(
    UINT *pNum, D3D12_META_COMMAND_DESC *pDescs) {
  if (!m_pReal5)
    return E_NOINTERFACE;
  return m_pReal5->EnumerateMetaCommands(pNum, pDescs);
}

HRESULT STDMETHODCALLTYPE CWrapD3D12Device::EnumerateMetaCommandParameters(
    REFGUID id, D3D12_META_COMMAND_PARAMETER_STAGE stage, UINT *pSize,
    UINT *pCount, D3D12_META_COMMAND_PARAMETER_DESC *pDescs) {
  if (!m_pReal5)
    return E_NOINTERFACE;
  return m_pReal5->EnumerateMetaCommandParameters(id, stage, pSize, pCount,
                                                  pDescs);
}

HRESULT STDMETHODCALLTYPE CWrapD3D12Device::CreateMetaCommand(
    REFGUID id, UINT nodeMask, const void *pData, SIZE_T size, REFIID riid,
    void **ppCmd) {
  if (!m_pReal5)
    return E_NOINTERFACE;
...
HRESULT STDMETHODCALLTYPE CWrapD3D12Device::CreateStateObject(
    const D3D12_STATE_OBJECT_DESC *pDesc, REFIID riid, void **ppState) {
  if (!m_pReal5)
    return E_NOINTERFACE;
...
CWrapD3D12Device::GetRaytracingAccelerationStructurePrebuildInfo(
    const D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_INPUTS *pDesc,
    D3D12_RAYTRACING_ACCELERATION_STRUCTURE_PREBUILD_INFO *pInfo) {
  if (m_pReal5)
    m_pReal5->GetRaytracingAccelerationStructurePrebuildInfo(pDesc, pInfo);
}

D3D12_DRIVER_MATCHING_IDENTIFIER_STATUS STDMETHODCALLTYPE
CWrapD3D12Device::CheckDriverMatchingIdentifier(
    D3D12_SERIALIZED_DATA_TYPE type,
    const D3D12_SERIALIZED_DATA_DRIVER_MATCHING_IDENTIFIER *pId) {
  if (!m_pReal5)
...
// ID3D12Device6
HRESULT STDMETHODCALLTYPE CWrapD3D12Device::SetBackgroundProcessingMode(
    D3D12_BACKGROUND_PROCESSING_MODE mode, D3D12_MEASUREMENTS_ACTION action,
    HANDLE hEvent, BOOL *pFurther) {
  if (!m_pReal6)
    return E_NOINTERFACE;
...
// ID3D12Device7
HRESULT STDMETHODCALLTYPE CWrapD3D12Device::AddToStateObject(
    const D3D12_STATE_OBJECT_DESC *pAdd, ID3D12StateObject *pState, REFIID riid,
    void **ppNew) {
  if (!m_pReal7)
    return E_NOINTERFACE;
...
HRESULT STDMETHODCALLTYPE CWrapD3D12Device::CreateProtectedResourceSession1(
    const D3D12_PROTECTED_RESOURCE_SESSION_DESC1 *pDesc, REFIID riid,
    void **ppSession) {
  if (!m_pReal7)
    return E_NOINTERFACE;
...
// Note: ID3D12Device8-10 methods omitted due to ABI incompatibility with
// MinGW/MSYS2
