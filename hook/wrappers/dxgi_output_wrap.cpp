...
CWrapDXGIOutput::CWrapDXGIOutput(IDXGIOutput *pReal,
                                 CWrapDXGIAdapter *pAdapterWrapper)
    : m_pReal(pReal), m_pReal1(nullptr), m_pReal2(nullptr), m_pReal3(nullptr),
      m_pReal4(nullptr), m_pReal5(nullptr), m_pReal6(nullptr),
      m_pParentAdapter(pAdapterWrapper), m_RefCount(1), m_Version(0) {
  if (pReal) {
    pReal->AddRef();
    PromoteInterfaces();
  }
...
HRESULT STDMETHODCALLTYPE CWrapDXGIOutput::QueryInterface(REFIID riid,
                                                          void **ppvObj) {
  if (!ppvObj)
    return E_POINTER;
...
HRESULT STDMETHODCALLTYPE CWrapDXGIOutput::SetPrivateData(REFGUID Name,
                                                          UINT DataSize,
                                                          const void *pData) {
  return m_pReal->SetPrivateData(Name, DataSize, pData);
}

HRESULT STDMETHODCALLTYPE CWrapDXGIOutput::SetPrivateDataInterface(
    REFGUID Name, const IUnknown *pUnknown) {
  return m_pReal->SetPrivateDataInterface(Name, pUnknown);
}

HRESULT STDMETHODCALLTYPE CWrapDXGIOutput::GetPrivateData(REFGUID Name,
                                                          UINT *pDataSize,
                                                          void *pData) {
  return m_pReal->GetPrivateData(Name, pDataSize, pData);
}

HRESULT STDMETHODCALLTYPE CWrapDXGIOutput::GetParent(REFIID riid,
                                                     void **ppParent) {
...
CWrapDXGIOutput::GetDisplayModeList(DXGI_FORMAT EnumFormat, UINT Flags,
                                    UINT *pNumModes, DXGI_MODE_DESC *pDesc) {
  return m_pReal->GetDisplayModeList(EnumFormat, Flags, pNumModes, pDesc);
}

HRESULT STDMETHODCALLTYPE CWrapDXGIOutput::FindClosestMatchingMode(
...
HRESULT STDMETHODCALLTYPE CWrapDXGIOutput::TakeOwnership(IUnknown *pDevice,
                                                         BOOL Exclusive) {
...
HRESULT STDMETHODCALLTYPE CWrapDXGIOutput::GetGammaControlCapabilities(
    DXGI_GAMMA_CONTROL_CAPABILITIES *pGammaCaps) {
  return m_pReal->GetGammaControlCapabilities(pGammaCaps);
}

HRESULT STDMETHODCALLTYPE
CWrapDXGIOutput::SetGammaControl(const DXGI_GAMMA_CONTROL *pArray) {
  return m_pReal->SetGammaControl(pArray);
}

HRESULT STDMETHODCALLTYPE
CWrapDXGIOutput::GetGammaControl(DXGI_GAMMA_CONTROL *pArray) {
  return m_pReal->GetGammaControl(pArray);
}

HRESULT STDMETHODCALLTYPE
CWrapDXGIOutput::SetDisplaySurface(IDXGISurface *pScanoutSurface) {
  return m_pReal->SetDisplaySurface(pScanoutSurface);
}

HRESULT STDMETHODCALLTYPE
CWrapDXGIOutput::GetDisplaySurfaceData(IDXGISurface *pDestination) {
  return m_pReal->GetDisplaySurfaceData(pDestination);
}

HRESULT STDMETHODCALLTYPE
CWrapDXGIOutput::GetFrameStatistics(DXGI_FRAME_STATISTICS *pStats) {
...
HRESULT STDMETHODCALLTYPE
CWrapDXGIOutput::GetDisplayModeList1(DXGI_FORMAT EnumFormat, UINT Flags,
...
HRESULT STDMETHODCALLTYPE CWrapDXGIOutput::FindClosestMatchingMode1(
...
HRESULT STDMETHODCALLTYPE
CWrapDXGIOutput::GetDisplaySurfaceData1(IDXGIResource *pDestination) {
  if (!m_pReal1)
    return DXGI_ERROR_UNSUPPORTED;
  return m_pReal1->GetDisplaySurfaceData1(pDestination);
}

HRESULT STDMETHODCALLTYPE CWrapDXGIOutput::DuplicateOutput(
...
HRESULT STDMETHODCALLTYPE CWrapDXGIOutput::CheckOverlaySupport(
    DXGI_FORMAT EnumFormat, IUnknown *pConcernedDevice, UINT *pFlags) {
  if (!m_pReal3)
    return DXGI_ERROR_UNSUPPORTED;
...
HRESULT STDMETHODCALLTYPE CWrapDXGIOutput::CheckOverlayColorSpaceSupport(
    DXGI_FORMAT Format, DXGI_COLOR_SPACE_TYPE ColorSpace,
    IUnknown *pConcernedDevice, UINT *pFlags) {
  if (!m_pReal4)
    return DXGI_ERROR_UNSUPPORTED;
  return m_pReal4->CheckOverlayColorSpaceSupport(Format, ColorSpace,
                                                 pConcernedDevice, pFlags);
}
...
HRESULT STDMETHODCALLTYPE CWrapDXGIOutput::DuplicateOutput1(
    IUnknown *pDevice, UINT Flags, UINT SupportedFormatsCount,
    const DXGI_FORMAT *pSupportedFormats,
    IDXGIOutputDuplication **ppOutputDuplication) {
  if (!m_pReal5)
    return DXGI_ERROR_UNSUPPORTED;
  return m_pReal5->DuplicateOutput1(pDevice, Flags, SupportedFormatsCount,
...
HRESULT STDMETHODCALLTYPE
CWrapDXGIOutput::CheckHardwareCompositionSupport(UINT *pFlags) {
  if (!m_pReal6)
    return DXGI_ERROR_UNSUPPORTED;
