...
CWrapDXGIOutput::CWrapDXGIOutput(IDXGIOutput* pReal, CWrapDXGIAdapter* pAdapterWrapper)
    : m_pReal(pReal), m_pReal1(nullptr), m_pReal2(nullptr), m_pReal3(nullptr), m_pReal4(nullptr), m_pReal5(nullptr),
      m_pReal6(nullptr), m_pParentAdapter(pAdapterWrapper), m_RefCount(1), m_Version(0)
{
    if (pReal) {
        pReal->AddRef();
        PromoteInterfaces();
...
ULONG STDMETHODCALLTYPE CWrapDXGIOutput::AddRef() { return InterlockedIncrement(&m_RefCount); }
...
HRESULT STDMETHODCALLTYPE CWrapDXGIOutput::GetDesc(DXGI_OUTPUT_DESC* pDesc) { return m_pReal->GetDesc(pDesc); }
...
HRESULT STDMETHODCALLTYPE CWrapDXGIOutput::WaitForVBlank() { return m_pReal->WaitForVBlank(); }
...
void STDMETHODCALLTYPE CWrapDXGIOutput::ReleaseOwnership() { m_pReal->ReleaseOwnership(); }
