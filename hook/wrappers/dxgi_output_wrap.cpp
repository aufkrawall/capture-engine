...
CWrapDXGIOutput::~CWrapDXGIOutput() {
    ...
    if (m_pParentAdapter) m_pParentAdapter->Release();
...
HRESULT STDMETHODCALLTYPE CWrapDXGIOutput::QueryInterface(REFIID riid, void** ppvObj) {
    ...
    // WrapperLog("DXGI Output Wrapper: QueryInterface for unknown IID (First DWORD: %08X)", riid.Data1);
...
HRESULT STDMETHODCALLTYPE CWrapDXGIOutput::GetDisplayModeList(DXGI_FORMAT EnumFormat, UINT Flags, UINT* pNumModes, DXGI_MODE_DESC* pDesc) {
    return m_pReal->GetDisplayModeList(EnumFormat, Flags, pNumModes, pDesc);
}

HRESULT STDMETHODCALLTYPE CWrapDXGIOutput::FindClosestMatchingMode(const DXGI_MODE_DESC* pModeToMatch, DXGI_MODE_DESC* pClosestMatch, IUnknown* pConcernedDevice) {
    // NOTE: pConcernedDevice might need wrapping/unwrapping if it's our device wrapper, but usually null or real.
    // If the game passes our wrapped device, m_pReal->FindClosestMatchingMode might fail if it expects a real device.
...
HRESULT STDMETHODCALLTYPE CWrapDXGIOutput::GetDisplayModeList1(DXGI_FORMAT EnumFormat, UINT Flags, UINT* pNumModes, DXGI_MODE_DESC1* pDesc) {
    if (!m_pReal1) return DXGI_ERROR_UNSUPPORTED;
    return m_pReal1->GetDisplayModeList1(EnumFormat, Flags, pNumModes, pDesc);
}

HRESULT STDMETHODCALLTYPE CWrapDXGIOutput::FindClosestMatchingMode1(const DXGI_MODE_DESC1* pModeToMatch, DXGI_MODE_DESC1* pClosestMatch, IUnknown* pConcernedDevice) {
    if (!m_pReal1) return DXGI_ERROR_UNSUPPORTED;
...
HRESULT STDMETHODCALLTYPE CWrapDXGIOutput::DuplicateOutput(IUnknown* pDevice, IDXGIOutputDuplication** ppOutputDuplication) {
    if (!m_pReal1) return DXGI_ERROR_UNSUPPORTED;
...
HRESULT STDMETHODCALLTYPE CWrapDXGIOutput::CheckOverlaySupport(DXGI_FORMAT EnumFormat, IUnknown* pConcernedDevice, UINT* pFlags) {
...
HRESULT STDMETHODCALLTYPE CWrapDXGIOutput::CheckOverlayColorSpaceSupport(DXGI_FORMAT Format, DXGI_COLOR_SPACE_TYPE ColorSpace, IUnknown* pConcernedDevice, UINT* pFlags) {
...
HRESULT STDMETHODCALLTYPE CWrapDXGIOutput::DuplicateOutput1(IUnknown* pDevice, UINT Flags, UINT SupportedFormatsCount, const DXGI_FORMAT* pSupportedFormats, IDXGIOutputDuplication** ppOutputDuplication) {
