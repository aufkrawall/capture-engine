...
Status HookVTableEntry(void **vtableEntry, void *detour, void **original) {
  ...
  VTableHook::Status vStatus =
      VTableHook::Create(vtableEntry, detour, original);
...
Status UnhookVTableEntry(void **vtableEntry, void *original) {
  ...
  if (original) {
    DWORD oldProtect;
    if (VirtualProtect(vtableEntry, sizeof(void *), PAGE_READWRITE,
                       &oldProtect)) {
...
Status HookExport(const char *moduleName, const char *functionName,
  ...
  HookLog("CustomHook: IAT patching failed for %s!%s (not imported?)",
...
Status HookExportW(const wchar_t *moduleName, const char *functionName,
  ...
  WideCharToMultiByte(CP_ACP, 0, moduleName, -1, narrowName, MAX_PATH, nullptr,
                      nullptr);
...
Status UnhookExport(const char *moduleName, const char *functionName,
  ...
  if (IATHook::RestoreIAT(nullptr, moduleName, functionName, original)) {
    ...
    if (hMod) {
      void *procAddr =
          reinterpret_cast<void *>(GetProcAddress(hMod, functionName));
      std::unique_lock<std::shared_mutex> lock(g_HookMutex);
