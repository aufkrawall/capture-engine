...
Status Create(void *pVTableEntry, void *pDetour, void **ppOriginal) {
  ...
  if (currentValue == pDetour) {
    HookLog("VTableHook: Create - Target %p ALREADY contains Detour %p. "
            "Skipping write.",
            ppEntry, pDetour);
  ...
  if (isSelfHook && currentValue != pDetour) {
    ...
    HookLog("VTableHook: WARNING - Target %p contains %p which is ANOTHER hook "
  ...
  if (VirtualQuery(ppEntry, &mbi, sizeof(mbi))) {
    HookLog("VTableHook: DEBUG - Target %p in region: Base=%p, Size=%zu, "
            "Protect=0x%X",
  ...
  HookLog(
      "VTableHook: Create - Patching %p (Original=%p, Detour=%p, SelfHook=%d)",
  ...
  if (!VirtualProtect(ppEntry, sizeof(void *), PAGE_READWRITE, &oldProtect)) {
    HookLog("VTableHook: Create - VirtualProtect FAILED for %p (Error=%lu)",
  ...
  *ppEntry = pDetour;
  ...
  if (!VirtualProtect(ppEntry, sizeof(void *), oldProtect, &newProtect)) {
    HookLog(
        "VTableHook: Create - VirtualProtect RESTORE FAILED for %p (Error=%lu)",
        ppEntry, GetLastError());
  }
  ...
  if (verifyValue != pDetour) {
    HookLog("VTableHook: Create - VERIFY FAILED! Expected=%p, Got=%p", pDetour,
            verifyValue);
