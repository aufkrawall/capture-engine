...
Status Create(void* pVTableEntry, void* pDetour, void** ppOriginal)
{
    ...
    if (isSelfHook && currentValue != pDetour) {
        ...
        HookLog("VTableHook: WARNING - Target %p contains %p which is ANOTHER hook from our DLL! SKIPPING.", ppEntry,
                currentValue);
    ...
    if (VirtualQuery(ppEntry, &mbi, sizeof(mbi))) {
        HookLog("VTableHook: DEBUG - Target %p in region: Base=%p, Size=%zu, Protect=0x%X", ppEntry, mbi.BaseAddress,
