...
CWrapDXGIFactory2::~CWrapDXGIFactory2()
{
    ...
    // Some games (Strange Brigade / Asura Engine) crash if the factory is destroyed
    // while the D3D12 device is still alive, even if refcounts suggest it should be safe.
    // This leaks the factory object, but ensures stability.
...
BOOL STDMETHODCALLTYPE CWrapDXGIFactory2::IsCurrent() { return m_pReal->IsCurrent(); }
