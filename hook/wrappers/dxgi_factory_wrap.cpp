...
void SetSwapchainWrapperDisabled(bool disabled) {
  ...
  if (disabled) {
    WrapperLog(
        "DXGI Factory: Swapchain wrapper DISABLED for FSR FG compatibility");
  }
}
...
CWrapDXGIFactory2::CWrapDXGIFactory2(IDXGIFactory2 *pReal)
    : m_pReal(pReal), m_pReal3(nullptr), m_pReal4(nullptr), m_pReal5(nullptr),
      m_pReal6(nullptr), m_pReal7(nullptr), m_RefCount(1), m_Version(2) {
  if (pReal) {
    pReal->AddRef();
    PromoteInterfaces();
  }
  WrapperLog("DXGI Factory Wrapper: Created (real=%p, version=%d)", pReal,
             m_Version);
}
...
HRESULT STDMETHODCALLTYPE CWrapDXGIFactory2::QueryInterface(REFIID riid,
                                                            void **ppvObj) {
  if (!ppvObj)
    return E_POINTER;
  ...
  if (riid == IID_IUnknown || riid == IID_IDXGIObject ||
      riid == IID_IDXGIFactory || riid == IID_IDXGIFactory1 ||
...
HRESULT STDMETHODCALLTYPE CWrapDXGIFactory2::SetPrivateData(REFGUID Name,
                                                            UINT DataSize,
                                                            const void *pData) {
  return m_pReal->SetPrivateData(Name, DataSize, pData);
}
HRESULT STDMETHODCALLTYPE CWrapDXGIFactory2::SetPrivateDataInterface(
    REFGUID Name, const IUnknown *pUnknown) {
  return m_pReal->SetPrivateDataInterface(Name, pUnknown);
}
HRESULT STDMETHODCALLTYPE CWrapDXGIFactory2::GetPrivateData(REFGUID Name,
                                                            UINT *pDataSize,
                                                            void *pData) {
  return m_pReal->GetPrivateData(Name, pDataSize, pData);
}
HRESULT STDMETHODCALLTYPE CWrapDXGIFactory2::GetParent(REFIID riid,
                                                       void **ppParent) {
...
CWrapDXGIFactory2::EnumAdapters(UINT Adapter, IDXGIAdapter **ppAdapter) {
  ...
  if (SUCCEEDED(hr) && pRealAdapter) {
    *ppAdapter = new CWrapDXGIAdapter(pRealAdapter, this);
    ...
    WrapperLog("DXGI Factory: EnumAdapters(%u) -> Wrapped adapter returned",
               Adapter);
  } else {
    *ppAdapter = nullptr;
    if (FAILED(hr)) {
      WrapperLog("DXGI Factory: EnumAdapters(%u) -> FAILED hr=0x%08X", Adapter,
                 hr);
    }
  }
  return hr;
...
HRESULT STDMETHODCALLTYPE
CWrapDXGIFactory2::MakeWindowAssociation(HWND WindowHandle, UINT Flags) {
  return m_pReal->MakeWindowAssociation(WindowHandle, Flags);
}
HRESULT STDMETHODCALLTYPE
CWrapDXGIFactory2::GetWindowAssociation(HWND *pWindowHandle) {
  return m_pReal->GetWindowAssociation(pWindowHandle);
}

HRESULT STDMETHODCALLTYPE CWrapDXGIFactory2::CreateSwapChain(
    IUnknown *pDevice, DXGI_SWAP_CHAIN_DESC *pDesc,
    IDXGISwapChain **ppSwapChain) {
  WrapperLog("CreateSwapChain: CALLED (device=%p, hwnd=%p)", pDevice,
...
HRESULT STDMETHODCALLTYPE CWrapDXGIFactory2::CreateSoftwareAdapter(
...
HRESULT STDMETHODCALLTYPE
CWrapDXGIFactory2::EnumAdapters1(UINT Adapter, IDXGIAdapter1 **ppAdapter) {
  ...
  if (SUCCEEDED(hr) && pRealAdapter) {
    *ppAdapter = (IDXGIAdapter1 *)new CWrapDXGIAdapter(pRealAdapter, this);
    pRealAdapter->Release();
    WrapperLog("DXGI Factory: EnumAdapters1(%u) -> Wrapped adapter returned",
               Adapter);
  } else {
    *ppAdapter = nullptr;
    if (FAILED(hr)) {
...
HRESULT STDMETHODCALLTYPE CWrapDXGIFactory2::CreateSwapChainForHwnd(
  ...
  WrapperLog("CreateSwapChainForHwnd: CALLED (device=%p, hwnd=%p)", pDevice,
             hWnd);
...
HRESULT STDMETHODCALLTYPE
CWrapDXGIFactory2::GetSharedResourceAdapterLuid(HANDLE hResource, LUID *pLuid) {
  return m_pReal->GetSharedResourceAdapterLuid(hResource, pLuid);
}
HRESULT STDMETHODCALLTYPE CWrapDXGIFactory2::RegisterStereoStatusWindow(
    HWND WindowHandle, UINT wMsg, DWORD *pdwCookie) {
  return m_pReal->RegisterStereoStatusWindow(WindowHandle, wMsg, pdwCookie);
}
HRESULT STDMETHODCALLTYPE
CWrapDXGIFactory2::RegisterStereoStatusEvent(HANDLE hEvent, DWORD *pdwCookie) {
  return m_pReal->RegisterStereoStatusEvent(hEvent, pdwCookie);
}
void STDMETHODCALLTYPE
CWrapDXGIFactory2::UnregisterStereoStatus(DWORD dwCookie) {
  m_pReal->UnregisterStereoStatus(dwCookie);
}
HRESULT STDMETHODCALLTYPE CWrapDXGIFactory2::RegisterOcclusionStatusWindow(
    HWND WindowHandle, UINT wMsg, DWORD *pdwCookie) {
  return m_pReal->RegisterOcclusionStatusWindow(WindowHandle, wMsg, pdwCookie);
}
HRESULT STDMETHODCALLTYPE CWrapDXGIFactory2::RegisterOcclusionStatusEvent(
    HANDLE hEvent, DWORD *pdwCookie) {
  return m_pReal->RegisterOcclusionStatusEvent(hEvent, pdwCookie);
}
void STDMETHODCALLTYPE
CWrapDXGIFactory2::UnregisterOcclusionStatus(DWORD dwCookie) {
...
// IDXGIFactory4
// ============================================================================
HRESULT STDMETHODCALLTYPE CWrapDXGIFactory2::EnumAdapterByLuid(
    LUID AdapterLuid, REFIID riid, void **ppvAdapter) {
  if (!m_pReal4)
    return DXGI_ERROR_UNSUPPORTED;
  WrapperLog("DXGI Factory: EnumAdapterByLuid(Luid=%08X:%08X)",
             AdapterLuid.HighPart, AdapterLuid.LowPart);
  IUnknown *pRealUnk = nullptr;
  HRESULT hr = m_pReal4->EnumAdapterByLuid(AdapterLuid, IID_IDXGIAdapter,
                                           (void **)&pRealUnk);
...
HRESULT STDMETHODCALLTYPE
CWrapDXGIFactory2::EnumWarpAdapter(REFIID riid, void **ppvAdapter) {
  if (!m_pReal4)
    return DXGI_ERROR_UNSUPPORTED;
...
// IDXGIFactory5
// ============================================================================
HRESULT STDMETHODCALLTYPE CWrapDXGIFactory2::CheckFeatureSupport(
    DXGI_FEATURE Feature, void *pFeatureSupportData,
    UINT FeatureSupportDataSize) {
  if (!m_pReal5)
    return DXGI_ERROR_UNSUPPORTED;
  return m_pReal5->CheckFeatureSupport(Feature, pFeatureSupportData,
...
// IDXGIFactory6
// ============================================================================
HRESULT STDMETHODCALLTYPE CWrapDXGIFactory2::EnumAdapterByGpuPreference(
    UINT Adapter, DXGI_GPU_PREFERENCE GpuPreference, REFIID riid,
    void **ppvAdapter) {
  if (!m_pReal6)
    return DXGI_ERROR_UNSUPPORTED;
  WrapperLog(
      "DXGI Factory: EnumAdapterByGpuPreference(Adapter=%u, Preference=%d)",
      Adapter, (int)GpuPreference);
  IUnknown *pRealUnk = nullptr;
  HRESULT hr = m_pReal6->EnumAdapterByGpuPreference(
      Adapter, GpuPreference, IID_IDXGIAdapter, (void **)&pRealUnk);
  if (SUCCEEDED(hr) && pRealUnk) {
    IDXGIAdapter *pRealAdapter = (IDXGIAdapter *)pRealUnk;
    CWrapDXGIAdapter *pWrapper = new CWrapDXGIAdapter(pRealAdapter, this);
    hr = pWrapper->QueryInterface(riid, ppvAdapter);
    ...
    pRealAdapter->Release();
    WrapperLog(
        "DXGI Factory: EnumAdapterByGpuPreference -> Wrapped adapter returned");
  } else {
    WrapperLog("DXGI Factory: EnumAdapterByGpuPreference -> FAILED hr=0x%08X",
               hr);
  }
  return hr;
}
...
// IDXGIFactory7
// ============================================================================
HRESULT STDMETHODCALLTYPE CWrapDXGIFactory2::RegisterAdaptersChangedEvent(
    HANDLE hEvent, DWORD *pdwCookie) {
  if (!m_pReal7)
    return DXGI_ERROR_UNSUPPORTED;
...
HRESULT STDMETHODCALLTYPE
CWrapDXGIFactory2::UnregisterAdaptersChangedEvent(DWORD dwCookie) {
  if (!m_pReal7)
    return DXGI_ERROR_UNSUPPORTED;
