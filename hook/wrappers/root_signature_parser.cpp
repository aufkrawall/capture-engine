...
bool ApplyStaticSamplerOverrides(D3D12_STATIC_SAMPLER_DESC& sampler) {
    ...
    HookLog("ApplyStaticSamplerOverrides: AF=%s, MipBias=%s, Mode=%s, Filter=0x%X, Aniso=%d, Bias=%.2f",
            gfx.anisotropicFiltering.c_str(), gfx.mipBias.c_str(), gfx.mipBiasMode.c_str(),
            sampler.Filter, sampler.MaxAnisotropy, sampler.MipLODBias);
    ...
    if (af != "default" && !hasBorderAddress) {
        if (af == "off") {
            ...
            if (sampler.Filter == D3D12_FILTER_ANISOTROPIC ||
                ...
                sampler.Filter = isComparison ? D3D12_FILTER_COMPARISON_MIN_MAG_MIP_LINEAR
    ...
    if (biasStr != "default") {
        try {
            ...
            if (sampler.MipLODBias < -16.0f) sampler.MipLODBias = -16.0f;
            if (sampler.MipLODBias > 15.99f) sampler.MipLODBias = 15.99f;
    ...
    if (modified) {
        HookLog("ApplyStaticSamplerOverrides: MODIFIED, Filter=0x%X->0x%X, Aniso=%d->%d, Bias=%.2f->%.2f",
