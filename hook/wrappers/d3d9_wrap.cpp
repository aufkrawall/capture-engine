...
static const GUID IID_CWrapDirect3D9 = {
    0xaabbccdd,
    0x1122,
    0x3344,
...
HRESULT STDMETHODCALLTYPE CWrapDirect3D9::QueryInterface(REFIID riid,
                                                         void **ppvObj) {
  if (!ppvObj)
    return E_POINTER;
...
HRESULT STDMETHODCALLTYPE CWrapDirect3D9::CreateDevice(
    UINT Adapter, D3DDEVTYPE DeviceType, HWND hFocusWindow, DWORD BehaviorFlags,
    D3DPRESENT_PARAMETERS *pPresentationParameters,
    IDirect3DDevice9 **ppReturnedDeviceInterface) {

  WrapperLog("D3D9: CreateDevice called");

  IDirect3DDevice9 *pRealDevice = nullptr;
  HRESULT hr =
      m_pReal->CreateDevice(Adapter, DeviceType, hFocusWindow, BehaviorFlags,
                            pPresentationParameters, &pRealDevice);
  ...
  if (SUCCEEDED(hr) && pRealDevice) {
    ...
    *ppReturnedDeviceInterface = pWrapper;
    WrapperLog("D3D9: Created wrapped device");
    return S_OK;
  }
...
HRESULT STDMETHODCALLTYPE CWrapDirect3D9::CreateDeviceEx(
    UINT Adapter, D3DDEVTYPE DeviceType, HWND hFocusWindow, DWORD BehaviorFlags,
    D3DPRESENT_PARAMETERS *pPresentationParameters,
  ...
  HRESULT hr = m_pRealEx->CreateDeviceEx(Adapter, DeviceType, hFocusWindow,
...
CWrapDirect3D9::RegisterSoftwareDevice(void *pInitializeFunction) {
...
HRESULT STDMETHODCALLTYPE CWrapDirect3D9::GetAdapterIdentifier(
    UINT Adapter, DWORD Flags, D3DADAPTER_IDENTIFIER9 *pIdentifier) {
  return m_pReal->GetAdapterIdentifier(Adapter, Flags, pIdentifier);
}

UINT STDMETHODCALLTYPE CWrapDirect3D9::GetAdapterModeCount(UINT Adapter,
                                                           D3DFORMAT Format) {
  return m_pReal->GetAdapterModeCount(Adapter, Format);
}

HRESULT STDMETHODCALLTYPE CWrapDirect3D9::EnumAdapterModes(
    UINT Adapter, D3DFORMAT Format, UINT Mode, D3DDISPLAYMODE *pMode) {
  return m_pReal->EnumAdapterModes(Adapter, Format, Mode, pMode);
}

HRESULT STDMETHODCALLTYPE
CWrapDirect3D9::GetAdapterDisplayMode(UINT Adapter, D3DDISPLAYMODE *pMode) {
  return m_pReal->GetAdapterDisplayMode(Adapter, pMode);
}

HRESULT STDMETHODCALLTYPE CWrapDirect3D9::CheckDeviceType(
    UINT Adapter, D3DDEVTYPE DevType, D3DFORMAT AdapterFormat,
    D3DFORMAT BackBufferFormat, BOOL bWindowed) {
  return m_pReal->CheckDeviceType(Adapter, DevType, AdapterFormat,
                                  BackBufferFormat, bWindowed);
}

HRESULT STDMETHODCALLTYPE CWrapDirect3D9::CheckDeviceFormat(
    UINT Adapter, D3DDEVTYPE DeviceType, D3DFORMAT AdapterFormat, DWORD Usage,
    D3DRESOURCETYPE RType, D3DFORMAT CheckFormat) {
  return m_pReal->CheckDeviceFormat(Adapter, DeviceType, AdapterFormat, Usage,
                                    RType, CheckFormat);
}

HRESULT STDMETHODCALLTYPE CWrapDirect3D9::CheckDeviceMultiSampleType(
  ...
  return m_pReal->CheckDeviceMultiSampleType(Adapter, DeviceType, SurfaceFormat,
                                             Windowed, MultiSampleType,
                                             pQualityLevels);
}

HRESULT STDMETHODCALLTYPE CWrapDirect3D9::CheckDepthStencilMatch(
    UINT Adapter, D3DDEVTYPE DeviceType, D3DFORMAT AdapterFormat,
    D3DFORMAT RenderTargetFormat, D3DFORMAT DepthStencilFormat) {
  return m_pReal->CheckDepthStencilMatch(Adapter, DeviceType, AdapterFormat,
                                         RenderTargetFormat,
                                         DepthStencilFormat);
}

HRESULT STDMETHODCALLTYPE CWrapDirect3D9::CheckDeviceFormatConversion(
    UINT Adapter, D3DDEVTYPE DeviceType, D3DFORMAT SourceFormat,
    D3DFORMAT TargetFormat) {
  return m_pReal->CheckDeviceFormatConversion(Adapter, DeviceType, SourceFormat,
                                              TargetFormat);
}

HRESULT STDMETHODCALLTYPE CWrapDirect3D9::GetDeviceCaps(UINT Adapter,
...
UINT STDMETHODCALLTYPE CWrapDirect3D9::GetAdapterModeCountEx(
...
HRESULT STDMETHODCALLTYPE CWrapDirect3D9::EnumAdapterModesEx(
...
HRESULT STDMETHODCALLTYPE CWrapDirect3D9::GetAdapterDisplayModeEx(
...
HRESULT STDMETHODCALLTYPE CWrapDirect3D9::GetAdapterLUID(UINT Adapter,
                                                         LUID *pLUID) {
  if (!m_pRealEx)
    return E_NOTIMPL;
