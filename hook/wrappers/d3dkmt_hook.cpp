...
// Function prototypes
typedef NTSTATUS(WINAPI *PFN_D3DKMTQueryVideoMemoryInfo)(
    const D3DKMT_QUERYVIDEOMEMORYINFO *);
...
void InitializeConfig() {
  ...
  if (explicitOverrideMB > 0) {
    ...
    g_VramConfig.dedicatedVramBytes =
        static_cast<UINT64>(explicitOverrideMB) * 1024 * 1024;
    HookLog("D3DKMT: VRAM override ENABLED (user requested) - Dedicated: %d MB",
            explicitOverrideMB);
  } else {
    HookLog("D3DKMT: VRAM override disabled - passing through real values "
            "(SpecialK-style)");
  }
}
...
Hook_D3DKMTQueryVideoMemoryInfo(const D3DKMT_QUERYVIDEOMEMORYINFO *pInfo) {
  ...
  HookLog("D3DKMT: QueryVideoMemoryInfo - Process=%u, Segment=%s",
          ...
          pInfo->MemorySegmentGroup == KMT_MEMORY_SEGMENT_GROUP_LOCAL
              ? "LOCAL"
              : "NON_LOCAL");

  // Cast away const to modify the output struct (if needed)
  D3DKMT_QUERYVIDEOMEMORYINFO *pMutableInfo =
      const_cast<D3DKMT_QUERYVIDEOMEMORYINFO *>(pInfo);
  ...
  if (pInfo->MemorySegmentGroup == KMT_MEMORY_SEGMENT_GROUP_LOCAL) {
    ...
    if (g_VramConfig.enabled) {
      ...
      pMutableInfo->CurrentUsage =
          std::min(pInfo->CurrentUsage,
                   g_VramConfig.dedicatedVramBytes - (512 * 1024 * 1024));
      pMutableInfo->AvailableForReservation =
          g_VramConfig.dedicatedVramBytes / 2;
      pMutableInfo->CurrentReservation = std::min(
          pInfo->CurrentReservation, pMutableInfo->AvailableForReservation);
      ...
      if (++queryCount <= 5) { // Only log first 5 queries to avoid spam
        HookLog(
            "D3DKMT: Local VRAM PASSTHROUGH - Budget: %llu MB, Usage: %llu MB",
    ...
    if (g_VramConfig.enabled) {
      ...
      pMutableInfo->CurrentUsage =
          std::min(pInfo->CurrentUsage,
                   g_VramConfig.sharedVramBytes - (256 * 1024 * 1024));
      ...
      if (++sharedQueryCount <= 3) { // Only log first 3 queries to avoid spam
        HookLog(
            "D3DKMT: Shared VRAM PASSTHROUGH - Budget: %llu MB, Usage: %llu MB",
            pInfo->Budget / (1024 * 1024), pInfo->CurrentUsage / (1024 * 1024));
      }
    }
  }
...
static NTSTATUS WINAPI
Hook_D3DKMTQueryAdapterInfo(const D3DKMT_QUERYADAPTERINFO *pInfo) {
  ...
  HookLog("D3DKMT: QueryAdapterInfo - Type=%u, Size=%u", pInfo->Type,
...
static NTSTATUS WINAPI
Hook_D3DKMTEnumAdapters(const D3DKMT_ENUMADAPTERS *pEnumAdapters) {
...
static NTSTATUS WINAPI
Hook_D3DKMTEnumAdapters2(const D3DKMT_ENUMADAPTERS2 *pEnumAdapters) {
...
bool Install() {
  ...
  if (hWin32U) {
    o_D3DKMTQueryVideoMemoryInfo =
        (decltype(o_D3DKMTQueryVideoMemoryInfo))GetProcAddress(
            hWin32U, "D3DKMTQueryVideoMemoryInfo");
    o_D3DKMTQueryAdapterInfo =
        (decltype(o_D3DKMTQueryAdapterInfo))GetProcAddress(
            hWin32U, "D3DKMTQueryAdapterInfo");
    o_D3DKMTEnumAdapters = (decltype(o_D3DKMTEnumAdapters))GetProcAddress(
        hWin32U, "D3DKMTEnumAdapters");
    o_D3DKMTEnumAdapters2 = (decltype(o_D3DKMTEnumAdapters2))GetProcAddress(
        hWin32U, "D3DKMTEnumAdapters2");
  ...
  if (!o_D3DKMTQueryVideoMemoryInfo && hGdi32) {
    o_D3DKMTQueryVideoMemoryInfo =
        (decltype(o_D3DKMTQueryVideoMemoryInfo))GetProcAddress(
            hGdi32, "D3DKMTQueryVideoMemoryInfo");
    o_D3DKMTQueryAdapterInfo =
        (decltype(o_D3DKMTQueryAdapterInfo))GetProcAddress(
            hGdi32, "D3DKMTQueryAdapterInfo");
    o_D3DKMTEnumAdapters = (decltype(o_D3DKMTEnumAdapters))GetProcAddress(
        hGdi32, "D3DKMTEnumAdapters");
    o_D3DKMTEnumAdapters2 = (decltype(o_D3DKMTEnumAdapters2))GetProcAddress(
        hGdi32, "D3DKMTEnumAdapters2");
...
void SetVramOverride(UINT64 dedicatedBytes, UINT64 sharedBytes) {
  ...
  HookLog("D3DKMT: VRAM override updated - Dedicated: %llu MB, Shared: %llu MB",
          dedicatedBytes / (1024 * 1024), sharedBytes / (1024 * 1024));
}
