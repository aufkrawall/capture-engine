    ...
    if (!ipc || !ipc->GetSharedMem()) {
        ...
        if (initialized && context) {
            ...
            if (!hwnd || (IsWindow((HWND)hwnd) && IsWindowVisible((HWND)hwnd))) {
                ...
                if (ImGui::Begin("OverlayFallback", nullptr,
    ...
    if (ImGui::Begin("Overlay", nullptr,
        ...
        if (beginTable) {
            ...
            auto RenderRow = [&](const char* label, const char* value, ImU32 labelColor, ImU32 valColor) {
                ...
                RenderTextWithOutline(label, ScaleColor(labelColor, hdrScale), cfg.textOutline, cfg.textOutlineColor,
                                      cfg.textOutlineThickness);
                ImGui::TableSetColumnIndex(1);
                RenderTextWithOutline(value, ScaleColor(valColor, hdrScale), cfg.textOutline, cfg.textOutlineColor,
                                      cfg.textOutlineThickness);
            };

            auto RenderRowBytes = [&](const char* label, uint64_t used, uint64_t total, ImU32 labelColor,
                                      ImU32 valColor) {
                ImGui::TableNextRow();
                ImGui::TableSetColumnIndex(0);
                RenderTextWithOutline(label, ScaleColor(labelColor, hdrScale), cfg.textOutline, cfg.textOutlineColor,
                ...
                RenderTextWithOutline(buf, ScaleColor(valColor, hdrScale), cfg.textOutline, cfg.textOutlineColor,
                                      cfg.textOutlineThickness);
                ...
                if (total > 0) {
                    ...
                    RenderTextWithOutline(buf, ScaleColor(cfg.textColor, hdrScale), cfg.textOutline,
                                          cfg.textOutlineColor, cfg.textOutlineThickness);
            ...
            if (cfg.showFPS) {
                ...
                if (dlss.srActive) {
                    ...
                    if (preset != '?') {
                        if (qStr[0])
                            snprintf(srBuf, 64, "DLSS SR %c %s", preset, qStr);
                        else
                            snprintf(srBuf, 64, "DLSS SR %c %.1fx", preset, scale);  // Fallback to scale if Q unknown
                    } else {
                        // If preset unknown (common without NvApi), show Quality/Scale if possible or just "DLSS SR"
                        if (qStr[0])
                            snprintf(srBuf, 64, "DLSS SR %s", qStr);
                        else
                            snprintf(srBuf, 64, "DLSS SR %.1fx", scale);
                    }
                    dlssText += srBuf;
                    showDlssLine = true;
                }
            ...
            if (cfg.showRecording) {
                if (cachedIsRecording) {
                    snprintf(buf, 64, "REC %02d:%02d:%02d", cachedRecordingSeconds / 3600,
                             (cachedRecordingSeconds % 3600) / 60, cachedRecordingSeconds % 60);
                    ImGui::TableNextRow();
                    ImGui::TableSetColumnIndex(0);
                    // Span 2 columns
                    RenderTextWithOutline(buf, IM_COL32(255, 50, 50, 255), cfg.textOutline, cfg.textOutlineColor,
                                          cfg.textOutlineThickness);
                    ...
                    if (overloadFlags != 0) {
                        ...
                        ImGui::TableNextRow();
                        ImGui::TableSetColumnIndex(0);
                        RenderTextWithOutline(buf, IM_COL32(255, 140, 0, 255), cfg.textOutline, cfg.textOutlineColor,
                                              cfg.textOutlineThickness);
                        ...
                        if (muxOver) {
                            ...
                            ImGui::TableNextRow();
                            ImGui::TableSetColumnIndex(0);
                            RenderTextWithOutline(buf, IM_COL32(255, 140, 0, 255), cfg.textOutline,
                                                  cfg.textOutlineColor, cfg.textOutlineThickness);
                        }
                    }
                }
        ...
        if (cfg.showFrameTime && metrics && cfg.showFPS) {
            ...
            ImGui::PlotLines("##FrameTime", graphBuf, PerformanceMetrics::GRAPH_HISTORY_SIZE, 0, nullptr, minScale,
                             maxScale, graphSize);
            ...
            drawList->AddLine(ImVec2(graphPos.x, graphPos.y), ImVec2(graphPos.x + 6.0f * dpiScale, graphPos.y),
                              scaleColor, 1.5f);

            // Draw text smaller (0.6x scale), offset to the right of the bar
            // Adjust Y slightly to align center of text with the line looks odd, usually just top alignment is fine for
            // max marker But since it's "Max", text below the line makes sense? Or just next to it. Let's put it next
            // to it.
            metrics->GetSmartScale(minScale, maxScale);  // Call again? No, we have it.

            drawList->AddText(ImGui::GetFont(), ImGui::GetFontSize() * 0.6f,
                              ImVec2(graphPos.x + 8.0f * dpiScale, graphPos.y), scaleColor, scaleBuf);
            ...
            drawList->AddText(ImGui::GetFont(), ImGui::GetFontSize() * 0.75f,
                              ImVec2(graphPos.x + graphSize.x - (txtSize.x * 0.75f) - 2, graphPos.y), latColor, latBuf);
