...
void Renderer::AddQuad(float x, float y, float w, float h,
...
void Renderer::FlushBatch(bool useTexture) {
    ...
    if (!commands.empty() && commands.back().useTexture == useTexture) {
        ...
        commands.back().vertexCount = (uint32_t)vertices.size() - commands.back().vertexOffset;
        commands.back().indexCount = (uint32_t)indices.size() - commands.back().indexOffset;
    } else {
        // New command
        DrawCommand cmd = {};
        cmd.vertexOffset = 0;
...
void Renderer::DrawTextWithShadow(float x, float y, const char* text,
                                   uint32_t color, uint32_t shadowColor, float shadowOffset) {
    if (!initialized || !text) return;
...
void Renderer::EndWindow() {
    if (!initialized || !inWindow) return;
    inWindow = false;
}
