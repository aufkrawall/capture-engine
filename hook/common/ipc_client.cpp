...
IPCClient::IPCClient()
    : hMapFile(NULL), pSharedMem(nullptr), hMapShmem(NULL), pShmem(nullptr) {}
...
bool IPCClient::Connect() {
  ...
  if (hDiscovery) {
    ...
    if (pDiscovery) {
      ...
      if (magic == DISCOVERY_MAGIC && pid != 0) {
        ...
        if (hMapFile) {
          ...
          hMapFile = NULL;
          EarlyLog("IPC: Failed to map view of main shared memory. Error=%d",
                   GetLastError());
        } else {
          EarlyLog("IPC: Failed to open main shared memory '%ls'. Error=%d",
                   sharedMemName, GetLastError());
        }

        UnmapViewOfFile(pDiscovery);
      } else {
        EarlyLog("IPC: Discovery found but invalid magic/pid. Magic=%X, PID=%d",
                 pDiscovery->magic, pDiscovery->injectPid);
        UnmapViewOfFile(pDiscovery);
      }
    } else {
    ...
    EarlyLog("IPC: Failed to open discovery mapping '%ls'. Error=%d",
             SHARED_MEM_DISCOVERY, GetLastError());
  }
