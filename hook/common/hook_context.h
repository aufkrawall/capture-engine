...
// Forward declaration for unique_ptr - we use void* instead to avoid incomplete type issues
// The actual PerformanceMetrics is not owned by HookContext during this migration phase

// Which graphics API is currently active (only one per process)
enum class ActiveGraphicsAPI : uint8_t {
    None = 0,
    DX9,
    DX10,
    DX11,
...
inline const char* GraphicsAPIName(ActiveGraphicsAPI api) {
    static const char* names[] = {
        "None", "DX9", "DX10", "DX11", "DX12", "Vulkan", "OpenGL"
    };
...
union GraphicsContextData {
    ...
    GraphicsContextData() { memset(this, 0, sizeof(*this)); }
};
...
struct HookContext {
    ...
    bool CanCapture() const {
        return hookLifecycle.IsActive() && 
               captureLifecycle.CanCapture() &&
...
inline HookContext* CreateHookContext() {
    bool expected = false;
    if (detail::g_HookContextCreated.compare_exchange_strong(expected, true, 
...
inline GraphicsConfig HookContext::GetActiveGraphicsConfig() const {
    ...
    // Return from shared mem graphics config - convert SharedGraphicsConfig to GraphicsConfig
    if (sharedMem) {
