...
inline const char *GraphicsAPIName(ActiveGraphicsAPI api) {
  static const char *names[] = {"None", "DX9",    "DX10",  "DX11",
                                "DX12", "Vulkan", "OpenGL"};
...
#define HOOK_SHUTTING_DOWN                                                     \
  (HOOK_CTX && HOOK_CTX->shuttingDown.load(std::memory_order_acquire))
...
inline HookContext *GetHookContext() { return detail::g_HookContext; }
...
inline HookContext *CreateHookContext() {
  bool expected = false;
  if (detail::g_HookContextCreated.compare_exchange_strong(
...
inline GraphicsConfig HookContext::GetActiveGraphicsConfig() const {
  ...
  if (sharedMem) {
    ...
    result.anisotropicFiltering =
        sharedMem->graphicsConfig.anisotropicFiltering;
    ...
    result.nvidiaSmoothMotionCompat = (smCompat == 1)   ? "on"
                                      : (smCompat == 2) ? "off"
                                                        : "auto";
    return result;
  }
