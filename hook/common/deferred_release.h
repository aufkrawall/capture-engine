...
// Prevents micro-stutters on the render thread caused by heavy resource
// destruction
...
public:
  ...
  // Ensure you called AddRef() before queuing if you want to keep using it
  // elsewhere, or simply pass the pointer you want to release.
  ...
  // Process all pending releases. Call this from a background thread (e.g.
  // HookThread).
  void Process() {
  ...
  void Clear() {
    std::lock_guard<std::mutex> lock(mutex);
    queue.clear();
    // Note: Leaks references, but avoids crashing if D3D device is already
    // destroyed
  }
};
