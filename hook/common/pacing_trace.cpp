...
void Save(const std::vector<Event>& events, const char* reason, int64_t now) {
    ...
    fprintf(file, "# display:a=present_start_us,b=stream_generation,c=graph_time_us (refresh-bounded overlay graph time; equals qpc_us unless the kernel reported a synchronized flip sooner than one refresh),flags=resolved; callback:id=FSR_frame_id,object=list,flags=generated; callback_end:a=total_us,b=wrapped_us; work:a=overlay_bit1_selfcompose_bit2\n");
