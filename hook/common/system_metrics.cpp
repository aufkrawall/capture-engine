...
SystemMetricsCollector::SystemMetricsCollector()
    : threadRunning(false), stopThread(false), pdhInitialized(false),
...
void SystemMetricsCollector::InitPDH() {
  if (strstr(g_ProcessName, "SonsOfTheForest") != nullptr) {
    EarlyLog("SystemMetricsCollector: PDH disabled for %s to prevent crash.",
             g_ProcessName);
    return;
  }
  ...
  if (PdhOpenQueryA(NULL, 0, &query) == ERROR_SUCCESS) {
    ...
    if (PdhAddEnglishCounterA(query, "\\Processor(_Total)\\% Processor Time", 0,
      ...
      if (PdhAddCounterA(query, "\\Processor(_Total)\\% Processor Time", 0,
                         &counter) == ERROR_SUCCESS) {
        cpuCounter = (void *)counter;
        PdhCollectQueryData(query);
        pdhInitialized = true;
...
void SystemMetricsCollector::Initialize(int32_t luidLow, int32_t luidHigh) {
  {
    ...
    if (adapterLuid.LowPart == (DWORD)luidLow &&
        adapterLuid.HighPart == (LONG)luidHigh) {
      if (!threadRunning) {
        EarlyLog("SystemMetricsCollector: LUID matches but thread not running. "
                 "Starting...");
        stopThread = false;
        updateThread =
            std::thread(&SystemMetricsCollector::BackgroundUpdateLoop, this);
        threadRunning = true;
      }
      return;
    }
    ...
    snprintf(cachedLuidPart, sizeof(cachedLuidPart), "luid_0x%08X_0x%08X",
             (unsigned int)luidHigh, (unsigned int)luidLow);
  ...
  EarlyLog("SystemMetricsCollector: Initialized with LUID %s. ThreadRunning=%d",
           cachedLuidPart, threadRunning.load());
  DetectHardwareNames();
  ...
  if (!threadRunning) {
    ...
    try {
      ...
      threadRunning = true;
      EarlyLog(
          "SystemMetricsCollector: Background Thread started successfully.");
    } catch (...) {
...
void SystemMetricsCollector::BackgroundUpdateLoop() {
  ...
  while (!stopThread && !g_ShuttingDown.load()) {
    ...
    if (g_IPC && g_IPC->GetSharedMem()) {
      ...
      {
        std::lock_guard<std::mutex> lock(mutex);
        if (g_IPC->GetSharedMem()->GetLuidLowPart() !=
            (int32_t)adapterLuid.LowPart) {
      ...
      if (cpu > 0.0f) {
        ...
        current.ramUsed =
            (uint64_t)(shm.ramUsage.load(std::memory_order_relaxed) * 1024.0f *
                       1024.0f * 1024.0f);
        ...
        current.cpuMaxCoreUsage =
            (float)shm.maxCoreLoad.load(std::memory_order_relaxed);
      }
    }
    ...
    if (!usedIPC) {
      ...
      if (!loggedFallback) {
        EarlyLog("SystemMetricsCollector: IPC stats not available yet. Waiting "
                 "for Host...");
        loggedFallback = true;
      }
...
void SystemMetricsCollector::Update() {
  ...
  if (g_IPC && g_IPC->GetSharedMem()) {
    ...
    if (cpu > 0.0f) {
      ...
      current.cpuUsage = cpu;
      current.ramUsed =
          (uint64_t)(shmMetrics.ramUsage.load(std::memory_order_relaxed) *
      ...
      current.cpuMaxCoreUsage =
          (float)shmMetrics.maxCoreLoad.load(std::memory_order_relaxed);
      return;
    }
  }
}
...
void SystemMetricsCollector::SetVRAMTotal(uint64_t totalBytes) {
  ...
  EarlyLog("SystemMetricsCollector: VRAM Total set explicitly: %llu MB",
...
void SystemMetricsCollector::UpdateCPU() {
  ...
  if (!initialized) {
    ...
    if (hNtDll) {
      NtQuerySystemInformation = (NtQuerySystemInformationPtr)GetProcAddress(
  ...
  NTSTATUS status = NtQuerySystemInformation(
      8, currInfo.data(),
      sizeof(SYSTEM_PROCESSOR_PERFORMANCE_INFORMATION) * numProcs, &len);
  ...
  if (status >= 0) { // success
    ...
    for (int i = 0; i < numProcs; i++) {
      uint64_t idle =
          currInfo[i].IdleTime.QuadPart - prevInfo[i].IdleTime.QuadPart;
      uint64_t kernel =
          currInfo[i].KernelTime.QuadPart - prevInfo[i].KernelTime.QuadPart;
      uint64_t user =
          currInfo[i].UserTime.QuadPart - prevInfo[i].UserTime.QuadPart;
...
void SystemMetricsCollector::UpdateGPU() {
  ...
  if (!gpuPdhInitialized && gpuQuery == nullptr) {
    ...
    if (PdhOpenQueryA(NULL, 0, &query) == ERROR_SUCCESS) {
      gpuQuery = (void *)query;
      PDH_HCOUNTER counter = nullptr;
      PDH_STATUS status = PdhAddEnglishCounterA(
          query, "\\GPU Engine(*)\\Utilization Percentage", 0, &counter);
  ...
  if (gpuPdhInitialized && gpuQuery && gpuCounter) {
    if (PdhCollectQueryData((PDH_HQUERY)gpuQuery) == ERROR_SUCCESS) {
      ...
      PdhGetFormattedCounterArrayA((PDH_HCOUNTER)gpuCounter, PDH_FMT_DOUBLE,
      ...
      if (bufSize > 0) {
        ...
        if (pdhBuffer) {
          PDH_FMT_COUNTERVALUE_ITEM_A *items =
              (PDH_FMT_COUNTERVALUE_ITEM_A *)pdhBuffer;
          if (PdhGetFormattedCounterArrayA(
                  (PDH_HCOUNTER)gpuCounter, PDH_FMT_DOUBLE, &pdhBufferSize,
                  &itemCount, items) == ERROR_SUCCESS) {
            double totalLoad = 0;
            for (DWORD i = 0; i < itemCount; i++) {
              const char *instance = items[i].szName;
              if (instance && items[i].FmtValue.CStatus == ERROR_SUCCESS) {
                if (strstr(instance, cachedLuidPart) ||
                    strstr(instance, "luid_") == nullptr) {
                  // Robust Aggregation: Sum EVERYTHING for this GPU
                  // excluding only video recording/decode noise
                  bool isVideo = strstr(instance, "VideoDecode") ||
                                 strstr(instance, "VideoEncode") ||
                                 strstr(instance, "OFA_0") ||
                                 strstr(instance, "JPEG_Decode");
            ...
            const float smoothingWeight =
                (peakLoad > smoothedGpuUsage) ? 0.65f : 0.4f;
            smoothedGpuUsage = smoothedGpuUsage * (1.0f - smoothingWeight) +
                               peakLoad * smoothingWeight;
  ...
  if (!vramPdhInitialized && vramQuery == nullptr) {
    ...
    if (PdhOpenQueryA(NULL, 0, &query) == ERROR_SUCCESS) {
      ...
      PDH_HCOUNTER counter = nullptr;
      PDH_STATUS status = PdhAddEnglishCounterA(
          query, "\\GPU Adapter Memory(*)\\Dedicated Usage", 0, &counter);
      if (status != ERROR_SUCCESS) {
        status = PdhAddCounterA(
            query, "\\GPU Adapter Memory(*)\\Dedicated Usage", 0, &counter);
      }
  ...
  if (vramPdhInitialized && vramQuery && vramCounter) {
    if (PdhCollectQueryData((PDH_HQUERY)vramQuery) == ERROR_SUCCESS) {
      DWORD bufSize = 0, itemCount = 0;
      PdhGetFormattedCounterArrayA((PDH_HCOUNTER)vramCounter, PDH_FMT_DOUBLE,
                                   &bufSize, &itemCount, NULL);
      if (bufSize > 0) {
        ...
        if (vramBuffer) {
          PDH_FMT_COUNTERVALUE_ITEM_A *items =
              (PDH_FMT_COUNTERVALUE_ITEM_A *)vramBuffer;
          if (PdhGetFormattedCounterArrayA(
                  (PDH_HCOUNTER)vramCounter, PDH_FMT_DOUBLE, &vramBufferSize,
                  &itemCount, items) == ERROR_SUCCESS) {
            for (DWORD i = 0; i < itemCount; i++) {
              const char *instance = items[i].szName;
...
void SystemMetricsCollector::UpdateVRAMTotal() {
  ...
  if (cachedFactory && !cachedAdapter) {
    ...
    if (adapterLuid.LowPart != 0 || adapterLuid.HighPart != 0) {
      if (!s_LoggedEnum)
        EarlyLog("UpdateGPU: Searching for LUID %08x:%08x",
                 adapterLuid.HighPart, adapterLuid.LowPart);
    }

    for (UINT i = 0;
         pFactory->EnumAdapters1(i, &tempAdapter) != DXGI_ERROR_NOT_FOUND;
         ++i) {
      DXGI_ADAPTER_DESC1 desc;
      tempAdapter->GetDesc1(&desc);

      if (!s_LoggedEnum) {
        EarlyLog("UpdateGPU: Checking Adapter %d: LUID %08x:%08x", i,
                 desc.AdapterLuid.HighPart, desc.AdapterLuid.LowPart);
      }

      if (desc.AdapterLuid.LowPart == adapterLuid.LowPart &&
          desc.AdapterLuid.HighPart == adapterLuid.HighPart) {
        tempAdapter->QueryInterface(__uuidof(IDXGIAdapter3),
                                    (void **)&cachedAdapter);
        {
          std::lock_guard<std::mutex> lock(mutex);
...
static std::string CleanHardwareName(std::string raw, bool isGpu) {
  ...
  if (isGpu) {
    ...
    for (const auto &t : tokens) {
      ...
      // Blocklist
      const char *blocklist[] = {
          "AMD",       "INTEL", "RYZEN", "CORE",   "PROCESSOR",
          "CPU",       "APU",   "SOC",   "GEN",    "COMPUTE",
          "MICROSOFT", "DUAL",  "QUAD",  "HEXA",   "OCTA",
          "EIGHT",     "SIX",   "TEN",   "TWELVE", "SIXTEEN",
          "TWENTY",    "R",     "TM",    "CO.",    "LTD."};
      for (const char *block : blocklist) {
        if (upperT == block)
          skip = true;
...
void SystemMetricsCollector::DetectHardwareNames() {
  ...
  if (RegOpenKeyExA(HKEY_LOCAL_MACHINE,
    ...
    if (RegQueryValueExA(hKey, "ProcessorNameString", 0, 0, (LPBYTE)buf,
    ...
    if (pFactory) {
      ...
      for (UINT i = 0;
        ...
        if (desc.AdapterLuid.LowPart == adapterLuid.LowPart &&
            desc.AdapterLuid.HighPart == adapterLuid.HighPart) {
          char mbBuf[128];
          WideCharToMultiByte(CP_UTF8, 0, desc.Description, -1, mbBuf,
                              sizeof(mbBuf), NULL, NULL);
  ...
  EarlyLog("Hardware Detected: CPU='%s', GPU='%s'", cachedCpuName.c_str(),
           cachedGpuName.c_str());
}
