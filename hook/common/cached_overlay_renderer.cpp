...
void FrameTimeGraphData::InitGpuBuffer(ID3D12Device* device) {
    ...
    device->CreateCommittedResource(&heapProps, D3D12_HEAP_FLAG_NONE, &resourceDesc, D3D12_RESOURCE_STATE_GENERIC_READ,
                                    nullptr, IID_PPV_ARGS(&gpuHistoryBuffer));
...
bool CachedOverlayRenderer::Initialize(ID3D12Device* device, ID3D12CommandQueue* queue, uint32_t bufferCount) {
...
bool CachedOverlayRenderer::CreateFrameResources(uint32_t bufferCount) {
    ...
    for (uint32_t i = 0; i < bufferCount; i++) {
        ...
        HRESULT hr =
            device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&frame.commandAllocator));
        ...
        // Create command list
        hr = device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, frame.commandAllocator, nullptr,
        ...
        if (!CreateUploadBuffer(device, CachedFrameResources::VB_SIZE, &frame.vertexBuffer,
                                &frame.vertexBufferCpuPtr)) {
            return false;
        }

        // Create index buffer
        if (!CreateUploadBuffer(device, CachedFrameResources::IB_SIZE, &frame.indexBuffer, &frame.indexBufferCpuPtr)) {
            return false;
        }
...
bool CachedOverlayRenderer::WaitForFrameResources(uint32_t frameIndex, uint32_t timeoutMs) {
    if (frameIndex >= perFrameResources.size()) {
        return false;
    }
...
void CachedOverlayRenderer::UpdateContent(PerformanceMetrics* metrics, float displayWidth, float displayHeight,
                                          float deltaTime) {
...
void CachedOverlayRenderer::RebuildCommandList(CachedFrameResources& frame, const ImDrawData* drawData) {
    ...
    for (int cmdListIdx = 0; cmdListIdx < drawData->CmdListsCount; cmdListIdx++) {
        ...
        // Bounds check
        if (vbOffset + vbSize > CachedFrameResources::VB_SIZE || ibOffset + ibSize > CachedFrameResources::IB_SIZE) {
            break;  // Buffer full
        }
    ...
    // This is a simplified version - full implementation would use ImGui's
    // renderer
...
void CachedOverlayRenderer::SubmitCachedCommandList(CachedFrameResources& frame,
    ...
    // CRITICAL FIX: Bundles cannot change render targets, so we can't use
    // ExecuteBundle for overlay rendering. Instead, we record the ImGui commands
    // directly to the target command list. The cached renderer approach needs to
    // be reworked for Frame Generation support without bundle limitations.

    // For now, just execute the cached command list directly
    // The command list was recorded with all necessary state (including render
    // target) in RebuildCommandList, so we just need to execute it.

    // Note: In D3D12, we can't execute a command list that was recorded for a
    // different render target. The proper fix is to either:
    // 1. Use secondary command lists with inheritance
    // 2. Record overlay commands per-frame (current approach in standard
    // renderer)
...
void CachedOverlayRenderer::Render(ID3D12GraphicsCommandList* targetCommandList, uint32_t backBufferIndex,
                                   bool isRealFrame, const ImVec2& displaySize) {
    ...
    // This happens on real frames, OR on any frame if we don't have valid cached
    // content yet
...
void ImGuiRenderCache::CacheDrawData(const ImDrawData* drawData) {
    ...
    for (int i = 0; i < drawData->CmdListsCount; i++) {
        ...
        // Copy vertices
        cachedData.vertices.insert(cachedData.vertices.end(), cmdList->VtxBuffer.Data,
