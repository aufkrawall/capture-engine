...
static bool CreateUploadBuffer(ID3D12Device *device, size_t size,
                               ID3D12Resource **outResource, void **outCpuPtr) {
...
CachedOverlayRenderer::~CachedOverlayRenderer() { Shutdown(); }
...
bool CachedOverlayRenderer::CreateFrameResources(uint32_t bufferCount) {
  ...
  for (uint32_t i = 0; i < bufferCount; i++) {
    ...
    if (!CreateUploadBuffer(device, CachedFrameResources::VB_SIZE,
                            &frame.vertexBuffer, &frame.vertexBufferCpuPtr)) {
      return false;
    }

    // Create index buffer
    if (!CreateUploadBuffer(device, CachedFrameResources::IB_SIZE,
                            &frame.indexBuffer, &frame.indexBufferCpuPtr)) {
      return false;
    }
...
void CachedOverlayRenderer::RebuildCommandList(CachedFrameResources &frame,
  ...
  // renderer
  ImGui_ImplDX12_RenderDrawData(const_cast<ImDrawData *>(drawData),
                                frame.commandList);
...
void CachedOverlayRenderer::SubmitCachedCommandList(
    CachedFrameResources &frame, ID3D12GraphicsCommandList *targetList) {
...
void CachedOverlayRenderer::Render(ID3D12GraphicsCommandList *targetCommandList,
                                   uint32_t backBufferIndex, bool isRealFrame,
...
void ImGuiRenderCache::CacheDrawData(const ImDrawData *drawData) {
  ...
  for (int i = 0; i < drawData->CmdListsCount; i++) {
    ...
    // Copy vertices
    cachedData.vertices.insert(
        cachedData.vertices.end(), cmdList->VtxBuffer.Data,
        cmdList->VtxBuffer.Data + cmdList->VtxBuffer.Size);

    // Copy indices (with offset adjustment)
    for (int j = 0; j < cmdList->IdxBuffer.Size; j++) {
      cachedData.indices.push_back(cmdList->IdxBuffer.Data[j] +
                                   (ImDrawIdx)vbStart);
    }
