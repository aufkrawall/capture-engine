...
void ReportHookCpuCostIfDue() {
    ...
    HookLogImportant(
        ...
        "cyclesPerMs=%llu) executeCommandLists(calls=%llu avgCycles=%llu maxCycles=%llu cyclesPerMs=%llu) "
        "— divide cyclesPerMs by the core's MHz to read it as a share of one core",
