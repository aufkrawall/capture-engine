...
typedef void (*OverlayDrawCallback)(IDXGISwapChain3 *pSwapChain,
...
static const GUID IID_OverlaySwapChainWrapper = {
    ...
    0x1405,
    0x424c,
    {0xae, 0x88, 0x0d, 0x3e, 0x9d, 0x46, 0xc9, 0x14}};

// Streamline Native Interface GUID - BLOCK this to prevent Streamline from
// unwrapping our wrapper {ADEC44E2-61F0-45C3-AD9F-1B37379284FF} Per SpecialK
// research: When FG runtimes (DLSS-G, FSR-G) query this GUID, they're trying to
// detect and unwrap any proxy wrappers. By returning E_NOINTERFACE, we prevent
// them from seeing our wrapper, which avoids crashes during FG processing.
...
// Swapchain wrapper that intercepts Present to draw overlay BEFORE FG
// processing
...
public:
  OverlaySwapChainWrapper(IDXGISwapChain *pReal, ID3D12CommandQueue *pQueue)
    ...
    EarlyLog("OverlaySwapChainWrapper: Created wrapper for %p (queue=%p)",
             pReal, pQueue);
  }
  ...
  HRESULT STDMETHODCALLTYPE QueryInterface(REFIID riid,
    ...
    // Per SpecialK research, returning E_NOINTERFACE prevents FG runtimes
    // (DLSS-G, FSR-G) from detecting our wrapper as a proxy, avoiding crashes
    // during FG processing. This is THE key fix for the FSR FG overlay crash.
    if (riid == IID_StreamlineNativeInterfaceBlock) {
      static bool logOnce = true;
      if (logOnce) {
        EarlyLog("OverlaySwapChainWrapper: Blocking Streamline native "
                 "interface query (FG compat)");
        logOnce = false;
      }
      *ppvObj = nullptr;
    ...
    if (riid == IID_OverlaySwapChainWrapper || riid == __uuidof(IUnknown) ||
  ...
  HRESULT STDMETHODCALLTYPE ResizeBuffers(UINT BufferCount, UINT Width,
    ...
    HRESULT hr = m_pReal->ResizeBuffers(BufferCount, Width, Height, NewFormat,
                                        SwapChainFlags);
  ...
  Present1(UINT SyncInterval, UINT PresentFlags,
    ...
    return m_pReal1 ? m_pReal1->Present1(SyncInterval, PresentFlags,
                                         pPresentParameters)
                    : E_NOTIMPL;
  }
  ...
  GetRestrictToOutput(IDXGIOutput **ppRestrictToOutput) override {
    return m_pReal1 ? m_pReal1->GetRestrictToOutput(ppRestrictToOutput)
                    : E_NOTIMPL;
  }
  HRESULT STDMETHODCALLTYPE
  ...
  HRESULT STDMETHODCALLTYPE SetSourceSize(UINT Width, UINT Height) override {
    return m_pReal3 ? static_cast<IDXGISwapChain2 *>(m_pReal3)->SetSourceSize(
                          Width, Height)
                    : E_NOTIMPL;
  }
  HRESULT STDMETHODCALLTYPE GetSourceSize(UINT *pWidth,
                                          UINT *pHeight) override {
    return m_pReal3 ? static_cast<IDXGISwapChain2 *>(m_pReal3)->GetSourceSize(
                          pWidth, pHeight)
                    : E_NOTIMPL;
  }
  HRESULT STDMETHODCALLTYPE SetMaximumFrameLatency(UINT MaxLatency) override {
    return m_pReal3 ? static_cast<IDXGISwapChain2 *>(m_pReal3)
  ...
  SetMatrixTransform(const DXGI_MATRIX_3X2_F *pMatrix) override {
    return m_pReal3
               ? static_cast<IDXGISwapChain2 *>(m_pReal3)->SetMatrixTransform(
                     pMatrix)
               : E_NOTIMPL;
  }
  HRESULT STDMETHODCALLTYPE
  GetMatrixTransform(DXGI_MATRIX_3X2_F *pMatrix) override {
    return m_pReal3
               ? static_cast<IDXGISwapChain2 *>(m_pReal3)->GetMatrixTransform(
                     pMatrix)
               : E_NOTIMPL;
  }
  ...
  HRESULT STDMETHODCALLTYPE CheckColorSpaceSupport(
    ...
    return m_pReal3 ? m_pReal3->CheckColorSpaceSupport(ColorSpace,
                                                       pColorSpaceSupport)
                    : E_NOTIMPL;
  }
  HRESULT STDMETHODCALLTYPE
  ...
  ResizeBuffers1(UINT BufferCount, UINT Width, UINT Height, DXGI_FORMAT Format,
    ...
    DX12_OnSwapchainResizeBegin();
    HRESULT hr =
        m_pReal3 ? m_pReal3->ResizeBuffers1(BufferCount, Width, Height, Format,
  ...
  HRESULT STDMETHODCALLTYPE SetHDRMetaData(DXGI_HDR_METADATA_TYPE Type,
    ...
    return m_pReal4 ? m_pReal4->SetHDRMetaData(Type, Size, pMetaData)
                    : E_NOTIMPL;
  }
