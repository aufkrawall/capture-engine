...
public:
    ...
    void SetUpdateRate(float targetHz) {
        updateIntervalMs = 1000.0f / targetHz;
    }
...
public:
    ...
    const CachedDrawData* GetCachedData() const {
        return &cachedData;
    }
    bool HasData() const {
        return !cachedData.vertices.empty();
    }
    void Clear() {
        cachedData = CachedDrawData();
    }
