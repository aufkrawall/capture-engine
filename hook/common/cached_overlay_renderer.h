...
inline void
ImGui_ImplDX12_RenderDrawData(ImDrawData *drawData,
...
struct FrameTimeGraphData {
  ...
  void AddSample(float frameTimeMs) {
    uint32_t idx =
        writeIndex.fetch_add(1, std::memory_order_relaxed) % HISTORY_SIZE;
...
public:
  ...
  // Main render entry - called EVERY Present (both real and interpolated
  // frames) cpuFrameType: True if this is a "real" frame (game submitted work)
  //               False if interpolated (FG generated frame)
  void Render(ID3D12GraphicsCommandList *targetCommandList,
              uint32_t backBufferIndex, bool isRealFrame,
              const ImVec2 &displaySize);

  // Update overlay content - only call on real frames
  void UpdateContent(PerformanceMetrics *metrics, float displayWidth,
                     float displayHeight, float deltaTime);
...
private:
  ...
  void RebuildCommandList(CachedFrameResources &frame,
                          const ImDrawData *drawData);

  // Submit cached command list (zero CPU work)
  void SubmitCachedCommandList(CachedFrameResources &frame,
