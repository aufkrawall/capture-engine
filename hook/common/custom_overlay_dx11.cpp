...
bool DX11Backend::Initialize(int fontTextureWidth, int fontTextureHeight, const uint8_t* fontTextureData) {
    ...
    hr = device->CreateShaderResourceView(fontTexture.Get(), nullptr, &fontTextureSRV);
    if (FAILED(hr))
        return false;
...
bool DX11Backend::CreateShaders() {
    HRESULT hr = device->CreateVertexShader(g_VS_4_0, sizeof(g_VS_4_0), nullptr, &vertexShader);
    if (FAILED(hr))
        return false;

    hr = device->CreatePixelShader(g_PS_Textured_4_0, sizeof(g_PS_Textured_4_0), nullptr, &pixelShader);
    if (FAILED(hr))
        return false;

    hr = device->CreatePixelShader(g_PS_Solid_4_0, sizeof(g_PS_Solid_4_0), nullptr, &pixelShaderSolid);
    if (FAILED(hr))
        return false;
...
void DX11Backend::Render(const std::vector<DrawVertex>& vertices, const std::vector<uint16_t>& indices,
    ...
    for (const auto& cmd : commands) {
        ID3D11PixelShader* targetPS = cmd.useTexture ? pixelShader.Get() : pixelShaderSolid.Get();
