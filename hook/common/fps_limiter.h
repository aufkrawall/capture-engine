...
public:
  ...
  void Apply() {
    ...
    // Only try to open events if we are not in test mode (implied by dbgShm presence usually, but let's just check name)
    ...
    uint32_t myRequest =
        shm->fpsLimiter.requestCount.fetch_add(1, std::memory_order_acq_rel) + 1;
    if (requestEvent)
    ...
    if (releaseEvent || dbgShm) { // Allow test mode to use SmartWait if manual targetTime is set
      // For real usage: check releaseEvent. For test: if dbgShm is set, we might skip the WaitForSingleObject or mocking it.
      // But typically tests won't have the external limiter process running.
      // So let's make the test set targetTimeTicks manually, and we skp WaitForSingleObject if it's a test?
      // Better: In a test, we can CreateEvent ourselves.
