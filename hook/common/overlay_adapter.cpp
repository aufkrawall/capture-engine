...
void OverlayAdapter::RenderContent(int viewportWidth, int viewportHeight) {
    ...
    if (cfg.showVRAM) {
        ...
        renderer->DrawTextWithShadow(x, cursorY, "VRAM", cfg.vramColor, shadowColor);
    ...
    if (cfg.showFPS) {
        ...
        renderer->DrawTextWithShadow(x + 100, cursorY, buf, cfg.fpsColor, shadowColor);
        cursorY += lineHeight;

        // FPS stats
        if (cachedAvgFPS > 0 && cached1PercentLow > 0) {
            snprintf(buf, 64, "%.0f/%.0f/%.0f", cachedAvgFPS, cached1PercentLow, cached01PercentLow);
            renderer->DrawTextWithShadow(x, cursorY, "Avg/1%/0.1%", textColor, shadowColor);
            renderer->DrawTextWithShadow(x + 100, cursorY, buf, cfg.fpsColor, shadowColor);
            cursorY += lineHeight;
        }
    }
