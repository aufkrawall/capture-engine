...
void TryEnableFrameTimeCSVLogging(SharedMemoryLayout *shm, const void *address,
  ...
  if (BuildLogFilePathForModuleAddress(address, "frame_times.csv", csvPath,
                                       sizeof(csvPath))) {
    metrics.EnableCSVLogging(csvPath);
    HookLog("%s: Frame time CSV logging enabled (%s)",
            apiName ? apiName : "API", csvPath);
  }
...
static void LogToFileAtomic(const char *baseFilename, const char *fmt,
  ...
  if (s_logDir[0] == '\0') {
    ...
    if (BuildLogFilePathForModuleAddress((const void *)&EarlyLog, baseFilename,
...
GraphicsConfig GetActiveGraphicsConfig() {
  ...
  if (g_IPC && g_IPC->GetSharedMem()) {
    currentVersion =
        g_IPC->GetSharedMem()->configVersion.load(std::memory_order_acquire);
  }
  ...
  if (g_IPC && g_IPC->GetSharedMem()) {
    ...
    mergedConfig.parsed.presetUltraPerformance =
        shmGfx.dlssPresetUltraPerformance;
    ...
    mergedConfig.parsed.rrPresetUltraPerformance =
        shmGfx.dlssRRPresetUltraPerformance;
  ...
  if (g_pLocalConfig) {
    if (g_pLocalConfig->graphics.parsed.srPreset > 0) {
      ...
      if (g_pLocalConfig->graphics.parsed.srPreset != lastLoggedLocal) {
        HookLog("Config: Local srPreset is %u",
                g_pLocalConfig->graphics.parsed.srPreset);
        lastLoggedLocal = g_pLocalConfig->graphics.parsed.srPreset;
      }
    }
    if (g_pLocalConfig->graphics.cpuPrerenderLimit > -0.5f) {
      mergedConfig.cpuPrerenderLimit =
          g_pLocalConfig->graphics.cpuPrerenderLimit;
    }
    if (g_pLocalConfig->graphics.vsyncMode != "default" &&
        !g_pLocalConfig->graphics.vsyncMode.empty()) {
    ...
    if (g_pLocalConfig->graphics.disableAutoMipBias) {
      mergedConfig.disableAutoMipBias =
          g_pLocalConfig->graphics.disableAutoMipBias;
    }
    ...
    if (g_pLocalConfig->graphics.dlssExposureNormalization != "default" &&
      ...
      mergedConfig.dlssExposureNormalization =
          g_pLocalConfig->graphics.dlssExposureNormalization;
    }
    ...
    if (g_pLocalConfig->graphics.anisotropicFiltering != "default" &&
      ...
      mergedConfig.anisotropicFiltering =
          g_pLocalConfig->graphics.anisotropicFiltering;
    }
    if (g_pLocalConfig->graphics.mipMapping != "default" &&
        !g_pLocalConfig->graphics.mipMapping.empty()) {
      mergedConfig.mipMapping = g_pLocalConfig->graphics.mipMapping;
    }
    if (g_pLocalConfig->graphics.mipBias != "default" &&
        !g_pLocalConfig->graphics.mipBias.empty()) {
      mergedConfig.mipBias = g_pLocalConfig->graphics.mipBias;
    }
    if (g_pLocalConfig->graphics.msaaSamples != "default" &&
        !g_pLocalConfig->graphics.msaaSamples.empty()) {
    ...
    if (g_pLocalConfig->graphics.parsed.presetDLAA > 0)
      mergedConfig.parsed.presetDLAA =
          g_pLocalConfig->graphics.parsed.presetDLAA;
    if (g_pLocalConfig->graphics.parsed.presetQuality > 0)
      mergedConfig.parsed.presetQuality =
          g_pLocalConfig->graphics.parsed.presetQuality;
    if (g_pLocalConfig->graphics.parsed.presetBalanced > 0)
      mergedConfig.parsed.presetBalanced =
          g_pLocalConfig->graphics.parsed.presetBalanced;
    if (g_pLocalConfig->graphics.parsed.presetPerformance > 0)
      mergedConfig.parsed.presetPerformance =
          g_pLocalConfig->graphics.parsed.presetPerformance;
    if (g_pLocalConfig->graphics.parsed.presetUltraPerformance > 0)
      mergedConfig.parsed.presetUltraPerformance =
          g_pLocalConfig->graphics.parsed.presetUltraPerformance;
    if (g_pLocalConfig->graphics.parsed.presetUltraQuality > 0)
      mergedConfig.parsed.presetUltraQuality =
          g_pLocalConfig->graphics.parsed.presetUltraQuality;

    if (g_pLocalConfig->graphics.parsed.rrPresetDLAA > 0)
      mergedConfig.parsed.rrPresetDLAA =
          g_pLocalConfig->graphics.parsed.rrPresetDLAA;
    if (g_pLocalConfig->graphics.parsed.rrPresetQuality > 0)
      mergedConfig.parsed.rrPresetQuality =
          g_pLocalConfig->graphics.parsed.rrPresetQuality;
    if (g_pLocalConfig->graphics.parsed.rrPresetBalanced > 0)
      mergedConfig.parsed.rrPresetBalanced =
          g_pLocalConfig->graphics.parsed.rrPresetBalanced;
    if (g_pLocalConfig->graphics.parsed.rrPresetPerformance > 0)
      mergedConfig.parsed.rrPresetPerformance =
          g_pLocalConfig->graphics.parsed.rrPresetPerformance;
    if (g_pLocalConfig->graphics.parsed.rrPresetUltraPerformance > 0)
      mergedConfig.parsed.rrPresetUltraPerformance =
          g_pLocalConfig->graphics.parsed.rrPresetUltraPerformance;
    if (g_pLocalConfig->graphics.parsed.rrPresetUltraQuality > 0)
      mergedConfig.parsed.rrPresetUltraQuality =
          g_pLocalConfig->graphics.parsed.rrPresetUltraQuality;
    ...
    if (g_pLocalConfig->graphics.parsed.dlssSharpening > -1.5f) {
      mergedConfig.parsed.dlssSharpening =
          g_pLocalConfig->graphics.parsed.dlssSharpening;
    }
  }
  ...
  if (mergedConfig.vsyncMode != "default" && !mergedConfig.vsyncMode.empty())
    anyActive = true;
  else if (mergedConfig.anisotropicFiltering != "default" &&
           !mergedConfig.anisotropicFiltering.empty())
    anyActive = true;
  else if (mergedConfig.mipMapping != "default" &&
           !mergedConfig.mipMapping.empty())
    anyActive = true;
  else if (mergedConfig.mipBias != "default" && !mergedConfig.mipBias.empty())
    anyActive = true;
  else if (mergedConfig.msaaSamples != "default" &&
           !mergedConfig.msaaSamples.empty())
    anyActive = true;
    ...
    anyActive = true;
  else if (mergedConfig.dlssAutoExposure != "default" &&
           !mergedConfig.dlssAutoExposure.empty())
    anyActive = true;
  else if (mergedConfig.dlssExposureNormalization != "default" &&
           !mergedConfig.dlssExposureNormalization.empty())
    anyActive = true;
  else if (mergedConfig.parsed.presetDLAA > 0 ||
           mergedConfig.parsed.presetQuality > 0)
    anyActive = true;
  else if (mergedConfig.parsed.rrPresetDLAA > 0 ||
           mergedConfig.parsed.rrPresetQuality > 0)
    anyActive = true;
...
VSyncOverride GetVSyncOverride() {
  ...
  if (cfg.vsyncMode == "off") {
    result.presentInterval =
        0; // DX9: D3DPRESENT_INTERVAL_IMMEDIATE, DX11/12: sync interval 0
    result.useMailbox = false;
  } else if (cfg.vsyncMode == "fifo") {
    result.presentInterval =
        1; // DX9: D3DPRESENT_INTERVAL_ONE, DX11/12: sync interval 1
    result.useMailbox = false;
  } else if (cfg.vsyncMode == "mailbox") {
    result.presentInterval =
        0; // DX9: immediate (no true mailbox), DX11/12: sync 0 + flip_discard
    result.useMailbox = true;
  } else if (cfg.vsyncMode == "adaptive") {
    result.presentInterval =
        1; // DX9: no adaptive (use fifo), DX11/12: sync interval 1
    result.useMailbox = false;
  } else {
