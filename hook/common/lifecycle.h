...
public:
    ...
    bool TransitionTo(HookState newState)
    {
        ...
        if (!IsValidTransition(current, newState)) {
            CE_LOG_ERROR("HookLC", "invalid %s->%s", HookStateName(current), HookStateName(newState));
            CE_ASSERT(false);
            return false;
        }
    ...
    HookState GetState() const { return state_.load(std::memory_order_acquire); }
...
public:
    ...
    bool TransitionTo(CaptureState newState)
    {
        ...
        if (!IsValidTransition(current, newState)) {
            CE_LOG_ERROR("CapLC", "invalid %s->%s", CaptureStateName(current), CaptureStateName(newState));
            CE_ASSERT(false);
            return false;
        }
...
public:
    ...
    bool TransitionTo(OverlayState newState)
    {
        ...
        if (!IsValidTransition(current, newState)) {
            CE_LOG_ERROR("OvlLC", "invalid %s->%s", OverlayStateName(current), OverlayStateName(newState));
            CE_ASSERT(false);
            return false;
        }
...
public:
    ...
    bool TransitionTo(EncoderState newState)
    {
        ...
        if (!IsValidTransition(current, newState)) {
            CE_LOG_ERROR("EncLC", "invalid %s->%s", EncoderStateName(current), EncoderStateName(newState));
            CE_ASSERT(false);
            return false;
        }
