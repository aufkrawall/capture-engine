...
template <typename StateEnum, size_t N, const char *LogTag,
          const char *(*NameFn)(StateEnum)>
class Lifecycle {
public:
  using TransitionTable = bool[N][N];
  using StateChangeCallback =
      std::function<void(StateEnum oldState, StateEnum newState)>;
  ...
  bool IsInState(StateEnum s) const { return GetState() == s; }
...
inline const char *HookStateName(HookState s) {
  static const char *names[] = {"Uninit",        "Connecting", "Connected",
...
class HookLifecycle
    : public Lifecycle<HookState, 8, kHookLCTag, HookStateName> {
public:
  bool IsActive() const { return IsInState(HookState::Active); }
  bool IsShuttingDown() const {
    auto s = GetState();
    return s == HookState::Detaching || s == HookState::Disconnecting ||
           s == HookState::Detached;
  }
};

template <>
inline const Lifecycle<HookState, 8, kHookLCTag,
                       HookStateName>::TransitionTable &
...
inline const char *CaptureStateName(CaptureState s) {
  static const char *names[] = {"Uninit", "Init'ing", "Ready",  "Capturing",
...
class CaptureLifecycle
    : public Lifecycle<CaptureState, 7, kCapLCTag, CaptureStateName> {
public:
...
template <>
inline const Lifecycle<CaptureState, 7, kCapLCTag,
                       CaptureStateName>::TransitionTable &
...
inline const char *OverlayStateName(OverlayState s) {
  static const char *names[] = {"Uninit", "Init'ing", "Ready",  "Rendering",
                                "Hidden", "Resizing", "Cleanup"};
  auto idx = static_cast<size_t>(s);
...
class OverlayLifecycle
    : public Lifecycle<OverlayState, 7, kOvlLCTag, OverlayStateName> {
public:
...
template <>
inline const Lifecycle<OverlayState, 7, kOvlLCTag,
                       OverlayStateName>::TransitionTable &
...
inline const char *EncoderStateName(EncoderState s) {
  static const char *names[] = {"Uninit",   "Init'ing", "Ready",  "Recording",
                                "Stopping", "Error",    "Cleanup"};
  auto idx = static_cast<size_t>(s);
...
class EncoderLifecycle
    : public Lifecycle<EncoderState, 7, kEncLCTag, EncoderStateName> {
public:
...
template <>
inline const Lifecycle<EncoderState, 7, kEncLCTag,
                       EncoderStateName>::TransitionTable &
